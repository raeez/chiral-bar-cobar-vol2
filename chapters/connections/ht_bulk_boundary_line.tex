...
% AP1: F_2 is LINEAR in kappa (not quadratic). lambda_2^FP = (2^3-1)/2^3 * |B_4|/4! = 7/8 * (1/30)/24 = 7/5760.
...
$\kappa=1$,
% AP1: kappa(betagamma_lambda) = 2(6*lambda^2 - 6*lambda + 1)/(something); at lambda=1: c_bg(1)=2, kappa=c_bg/2=1. Corrected from -2 (AP5 propagation from frontier file).
$\nabla^{\mathrm{hol}}_{0,n}
=d+\sum_{i<j}
...
% AP1: kappa(betagamma_1) = 1 (corrected from -2; see frontier file). F_1 = kappa/24.
