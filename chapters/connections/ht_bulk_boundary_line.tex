...
\begin{theorem}[Line operators as Koszul-dual modules; spectral
...
\index{line operators!Koszul-dual modules|textbf}%
...
\noindent\textbf{Status.}\enspace \ClaimStatusProvedHere on the
chirally Koszul locus $\ChirAlg_{X}^{E_{1},\mathrm{Kosz}}$, with
Verdier-duality effectiveness $\effKoszul$ (Vol~I Theorem~B on the
positive-energy / Koszul class).

\noindent\textbf{Source.}\enspace augmented complete chirally Koszul
$A \in \ChirAlg_{X}^{E_{1},\mathrm{Kosz}}$ with universal twisting
morphism
$\tau : \BarTwc{A} \to A$ of
\cref{thm:typed-boundary-holographic-realisation}\,(ii).

\noindent\textbf{Target.}\enspace the perturbative line category
$\cC_{\mathrm{line}}$ on $A$ and the spectral $R$-matrix package on
the open-colour Koszul-dual algebra $A^{!}_{\mathrm{line}}$.
...
\begin{proof}
\textup{(a)} The equivalence is the line/Koszul-dual face of bar--cobar
...
$\overline{\FM}_{3}(C)$
(\cref{thm:typed-boundary-holographic-realisation}, Step~3).
...
(Belavin--Drinfeld~\cite{BD83}; Etingof--Schiffmann~\cite{ES98}); the
...
carrier \textup{(C1)} dualises to the Koszul-dual algebra
...
$\sum_{g\ge 1}F_g\,x^{2g}=\kappaChHodge(\hat A(ix)-1)$
...
\begin{equation}\label{eq:htbb-heegner-identity}
 h_n^{\mathrm{Br}} \;=\; c_{\phi_{-2,1}}(n)\cdot [H_n], \qquad n\geq 1,
\end{equation}
...
\[
 h_3^{\mathrm{Br}} = -8\cdot[H_3], \quad
 h_4^{\mathrm{Br}} = +12\cdot[H_4], \quad
 h_7^{\mathrm{Br}} = -39\cdot[H_7], \quad
 h_8^{\mathrm{Br}} = +56\cdot[H_8],
\]
...
c_{\phi_{-2,1}}(n)\cdot[H_n]$ is compared, after the recognition gates,
...
$h_n^{\mathrm{Br}} = c_{\phi_{-2,1}}(n)\cdot[H_n]$ at every
