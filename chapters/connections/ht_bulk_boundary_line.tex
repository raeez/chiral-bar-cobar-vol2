...
\[
 c^{\mathrm{Br}}_3
 =[\log(\Phi_{10}^{\mathrm{un}}/\eta^{24})]_{p^3,\mathrm{Br}}
 =c_{\phi_{-2,1}}(3)[H_3]
 =-8[H_3]
\]
...
\item the $\hbar^3$-order correction to the line-operator pairing
 is the Bruinier reduced class $c^{\mathrm{Br}}_3=-8[H_3]$ by
...
$c^{\mathrm{Br}}_3
=[\log(\Phi_{10}^{\mathrm{un}}/\eta^{24})]_{p^3,\mathrm{Br}}
=-8[H_3]$, trivialised on the generic Koszul locus by BKM-module
...
\end{enumerate}
All four give the same Bruinier reduced class $-8[H_3]$ after
...
of the pullback of $\phi_{-2,1}$ to the Wilson-line worldvolume; at the
$p^3$-level geodesic simplex, the reduced phase evaluates to
$-8 \bmod 8 \equiv 0$ (consistent with $\hbar^3$-order
...
The three-loop obstruction $c^{\mathrm{Br}}_3 = -8\cdot[H_3]$ inscribed in
...
Admissibility gives $c_n^{\mathrm{triangle}} = 0$ for $n\not\equiv 0, 3
...
single Borcherds logarithm. The loop variable $\hbar$ substitutes for
the Fourier--Jacobi variable $p$ after the $p^n$ coefficient has been
sent to the divisor class $[H_n]$; it is not an exponent multiplying
the whole logarithm.
...
presentation of the perturbative tower. The bulk--boundary
...
$\Phi_{10}^{\mathrm{un}}/\eta^{24}$ has $p^n$ Fourier--Jacobi
coefficient $c_{\phi_{-2,1}}(n)$ in the Bruinier reduced Humbert
normalisation. Applying $\mathsf H_{\mathrm{Br}}$ coefficientwise and
then substituting $p=\hbar$ gives~\eqref{eq:htbb-allloop-master}. The
...
corresponding $c_n^{\mathrm{triangle}}$. The perturbative expansion is
the formal $\hbar$-series obtained by substituting $p=\hbar$ after this
...
$\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ has weight~$10$, while the
$\eta^{24}$ factor is the separate elliptic one-loop normalization.
\end{remark}
