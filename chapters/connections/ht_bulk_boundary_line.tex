...
=\frac{4(k+3)+4(-k-3)}{3}
=0$,
so the affine KM complementarity $\kappa(\cA)+\kappa(\cA^!)=0$ holds
(cf.\ Remark~\ref*{rem:kappa-shift-convention}
of Vol~I; for Virasoro, $\kappa+\kappa^!=13\neq 0$). The central charges likewise satisfy
...
\begin{theorem}[HT bulk-boundary-line $\hbar^3$-triangle obstruction;
 \ClaimStatusProvedHere]
\label{thm:htbb-3loop-triangle-general}
\index{HT bulk-boundary-line triangle!3-loop general}
The $\hbar^3$-order coupling of the HT bulk-boundary-line triangle
...
$\kappa + \kappa^! = 0$ (on the Heisenberg-family Koszul pair)
