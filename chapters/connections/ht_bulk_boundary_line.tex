...
The novelty lies elsewhere: we strengthen the microlocal algebraic core. Inside an exact local sector, the boundary, line, and bulk vertices admit explicit constructions and theorem-level comparisons.
...
The operations above define a complete filtered $A_\infty$ algebra structure on $K_{F,p}^{\gr}$, denoted $K^{\mathrm{exp}}_{F,p}$, and there is an $A_\infty$ quasi-isomorphism
\[
K^{\mathrm{exp}}_{F,p}\simeq K^{\mathrm{ex}}_{F,p}\simeq \RHom_{B_{F,p}}(\kk,\kk).
\]
\end{corollary}
...
\label{rem:htbb-K3}
The HT bulk-boundary line at $K3 \times E$ has boundary = Vol~III K3 chiral Hall--Drinfeld double; bulk = twisted 11D-SUGRA on $K3 \times T^2$ Omega-deformed with $24$ M5-branes on $I_1$ Kodaira fibres. $\Delta_5$ is the $1$-loop-forced output of paramodular anomaly cancellation; weight $5 = \chi(\mathcal{O}_{K3}) + \sum_{24}\mathrm{Res}(I_1) = 2 + 3$. Four duality frames (het Harvey--Moore; IIB DVV; IIA DMVV; M-theory Witten) produce the same form.
\end{remark}
...
$c_3 = [\log(\Phi_{10}/\eta^{24})]_{p^3}$, with leading numerical
...
[\Phi_{10}/\eta^{24}]$ defines a $\mu_8$-gerbe $\mathcal{G}$ on
...
$\Phi_{10} = pqy^{1/2}\prod(1-p^mq^ny^r)^{-c_{\phi_{-2,1}}(4mn-r^2)}$;
...
partition function on $K3\times\bC^2$ is $\Phi_{10}$; its Feynman
...
of $\log(\Phi_{10}/\eta^{24})$ via the Gritsenko--Nikulin denominator
...
The Wave-convergent perturbative HT bulk--boundary--line coupling
...
$\log$-expansion of $\Phi_{10}$; D$3$-instanton tail (Vol~I
Theorem~\ref{thm:bvbrst-nonperturbative-completion}) matches the
$\Phi_{10}$ Borcherds-product factors; Schwinger tail (present
...
$\Phi_{10}$ raised to a formal $\hbar$-power, with the HT triangle
...
\begin{equation}\label{eq:htbb-allloop-master}
 ...
 \;=\; \bigl(\Phi_{10}(Z)/\eta(q)^{24}\bigr)^{\hbar},
\end{equation}
...
e^{S_{\mathrm{cl}}/\hbar}\cdot(\Phi_{10}/\eta^{24})$ and
$Z_{\mathrm{boundary}}(Z) = 1/\Phi_{10}(Z)$ the character of the
...
\[
 ...
 \;=\; \hbar\cdot\log\bigl(\Phi_{10}/\eta^{24}\bigr)
\]
...
$\Phi_{10}/\eta^{24}$ lifted to the all-loop effective action by
...
1/\Phi_{10}$, by the Gritsenko--Nikulin denominator
...
The bulk factor $\Phi_{10}/\eta^{24}$ is the holomorphic
...
The boundary factor $1/\Phi_{10}$ is the topological
...
$\int d\tau\,\mathrm{Tr}\,\log\Phi_{10}\sim\log\Phi_{10}$,
...
analytic object $\Phi_{10}(Z)$ against three distinct operational
...
$\log(\Phi_{10}/\eta^{24})$ relative to the classical normalisation,
equal to the $c_\Lambda(0)/2$ of the Kodaira--M$5$ residue
contribution.
\end{remark}
