...
\section{The all-filtration Dynkin rigidity theorem for path sectors}

The computations at filtrations $2$ and $3$ revealed a pattern:
universal coefficients $\frac12$ and $\frac13$, each uniquely forced by
rigidity, each transported by explicit shift-character ratios.  We now
prove that this pattern extends to all filtrations for root-system sectors
of \emph{path type}.  The key is a structural theorem in the free Lie
algebra under path commutation relations, combined with the
Dynkin--Specht--Wever projection and a classical integral identity for
binomial-harmonic sums.
...
\begin{remark}
At $n=2$ these are the triangle vectors of the adjacent-root
computation.  At $n=3$ they are the quadrilateral vectors of
Definition~\ref{def:path-sector}~above (relabeling
...
For $[L,R]\neq 0$, both $S$ and $T$ must be unions of connected subpaths
of the path $1\text{---}2\text{---}\cdots\text{---}n$.  Indeed, if $S$
contains a gap---say $a,c\in S$ with $a<b<c$ and $b\notin S$---then $L$
is a Lie monomial on variables $\{X_s:s\in S\}$ where $X_a$ and $X_c$
must eventually be bracketed, but every intermediate variable $X_b$ lies
in~$T$.  Since $[X_a,X_c]=0$ (as $|a-c|\ge 2$), and any bracket
involving $X_a$ and $X_c$ in $L$ factors through $[X_a,X_c]$ by
multilinearity, we get $L=0$.  Similarly for~$R$.  Furthermore, each of
$S$ and $T$ must be a single connected subpath (not a disjoint union of
subpaths) since a Lie monomial on a disconnected subset of the path
generators vanishes by the same argument.
...
\;P_{\alpha_1\cdots\alpha_n}^{L}.
\end{equation}
Similarly, the projection of the opposite ordered product to the right
$n$-gon sector is
\begin{equation}\label{eq:ngon-right}
\frac{(-1)^{n-1}}{n}\,
\rho_{\alpha_1}(u_n')\,\rho_{\alpha_2}(u_{n-1}')\cdots
\rho_{\alpha_n}(u_1')
\;P_{\alpha_1\cdots\alpha_n}^{R},
\end{equation}
where $u_k'$ denotes the dual spectral parameter (reading the
coproduct from the opposite end).
\end{theorem}

\begin{proof}
By Theorem~\ref{thm:dynkin-bch-path}, the multilinear part is
$\frac{1}{n}[X_1^{1,2},\,[X_2^{1,3},\ldots,
[X_{n-1}^{1,n},X_n^{1,n+1}]\cdots]]$.

The nested bracket evaluates as follows.  The innermost bracket
$[X_{n-1},X_n]$ lives in the $(\alpha_{n-1},\alpha_n)$-root space;
the next bracket $[X_{n-2},[X_{n-1},X_n]]$ lives in the
$(\alpha_{n-2},\alpha_{n-1},\alpha_n)$-root space; at the top,
$[X_1,\ldots,[X_{n-1},X_n]\cdots]$ lives in the
$(\alpha_1,\ldots,\alpha_n)$-root space.

Since the first tensor factor carries the raising operator and the
$(i+1)$-st factor carries $F_{\alpha_i}$, the projection onto the
left $n$-gon vector $P^L$ extracts exactly the product
$\prod_{i=1}^{n}\rho_{\alpha_i}(u_i)$ of one-body coefficients.  The
right-sector computation is identical up to orientation reversal, which
introduces the sign $(-1)^{n-1}$ from the reversal of the nesting
order.
\end{proof}
...
\begin{remark}[The sequence $\frac12,\frac13,\frac14,\ldots$ as Dynkin
coefficients]
The universal coefficient $1/n$ at filtration $n$ is the
\emph{Dynkin coefficient}: the eigenvalue of the Dynkin projection
$D_n\colon T^n(V)\to\Lie^n(V)$, $D_n=\frac{1}{n}\ell_n$, evaluated
on the ordered concatenation $X_1\otimes\cdots\otimes X_n$.  The
$1/n$ arises from the beta integral $\int_0^1(1-t)^{n-1}\,dt$,
which encodes the combinatorics of all compositions contributing to the
BCH logarithm.
The pattern is not numerological: it reflects the fact that the
ordered exponentiation $e^{X_1}\cdots e^{X_n}$ projects onto the Lie
algebra through the Dynkin operator with eigenvalue $1/n$, and the
path relations ensure only the right-normed bracket survives.
\end{remark}
...
\subsection{Non-path sectors: the genuine remaining frontier}

\begin{remark}[Classification of non-path sectors]
\label{rem:non-path-sectors}
At filtrations $2$ and $3$, every connected subdiagram of a Dynkin
diagram on $\le 3$ nodes is a path (type $A_k$).  At
filtration~$4$, a new possibility appears: the \emph{star sector}
(type~$D_4$), where a central node $c$ is adjacent to three outer
nodes $1,2,3$ with $[X_1,X_2]=[X_1,X_3]=[X_2,X_3]=0$.

The multilinear degree-$4$ space of the star Lie algebra (generators
$X_1,X_c,X_2,X_3$ with $[X_i,X_j]=0$ for $i,j\in\{1,2,3\}$) has
dimension \emph{two}, not one.  A basis is
\[
A = [[X_2,[X_1,X_c]],X_3],
\qquad
B = [[X_3,[X_1,X_c]],X_2].
\]
These two elements are exchanged by the $S_3$ symmetry permuting the
outer nodes.  The multilinear part of the ordered BCH product
$\log(e^{X_1}e^{X_c}e^{X_2}e^{X_3})$ is a specific linear combination
$aA + bB$ with $a\neq b$ (the ordering breaks the $S_3$ symmetry).
\end{remark}

\begin{proposition}[Star-sector obstruction space; \ClaimStatusProvedHere]
\label{prop:star-sector}
In the $D_4$-star sector at filtration~$4$, the relevant multilinear Lie
space has $\dim=2$.  The spectral Drinfeld class restricted to this
sector therefore lives in a potentially nontrivial one-dimensional
quotient (after modding out by the transport-controlled subspace).
\end{proposition}

\begin{proof}
The two-dimensionality is verified by direct computation in the proof of
Remark~\ref{rem:non-path-sectors}: $A$ and $B$ are nonzero and
linearly independent in the star Lie algebra (they are related by
transposing $X_2\leftrightarrow X_3$, which is not a Lie algebra
automorphism of the ordered product).  The coproduct rigidity provides
one constraint (analogous to the one-body transport at each
filtration), leaving at most a one-dimensional obstruction quotient.
\end{proof}

\begin{remark}[Star sectors as higher-rank corrections]
Star sectors at filtration~$4$ appear only in Lie algebras of type
$D_n$ ($n\ge 4$), $E_6$, $E_7$, $E_8$.  For type-$A$ Lie algebras
(i.e., $\mathfrak{sl}_N$), the Dynkin diagram is a path, and
\emph{every} sector at every filtration is path-type.
Corollary~\ref{cor:all-filtration-no-obstruction} therefore gives:
\end{remark}

\begin{theorem}[Complete strictification for type $A$; \ClaimStatusProvedHere]
\label{thm:type-A-strictification}
For $\fg=\mathfrak{sl}_N$, the spectral Drinfeld class vanishes at
every filtration.  Therefore, every residue-bounded complete dg-shifted
Yangian of type~$A$ admits a strict spectral factorization realization
(subject to the standing hypotheses of the conditional strictification
criterion, Theorem~\ref{thm:conditional-strictification-criterion}).
\end{theorem}

\begin{proof}
The Dynkin diagram of type~$A_{N-1}$ is a path.  Every connected
subdiagram is again a path.  By
Corollary~\ref{cor:all-filtration-no-obstruction}, the spectral
Drinfeld class vanishes in every path-type sector, which exhausts all
sectors.  The conditional strictification criterion then applies.
\end{proof}

\begin{remark}[Sharpened frontier]
The filtration-$\ge 4$ frontier, previously open in full generality,
now decomposes as:
\begin{enumerate}
\item \textbf{Path sectors (all types, all filtrations): CLOSED.}
  Universal coefficient $1/n$, no independent obstruction.
\item \textbf{Type $A$ (all sectors, all filtrations): CLOSED.}
  Complete strictification.
\item \textbf{Star sectors (types $D$, $E$, filtration $\ge 4$):
  OPEN.}
  A one-dimensional obstruction quotient survives in the $D_4$-star
  sector.  Whether this class vanishes for the dg-shifted Yangians
  arising from holomorphic-topological field theory is the
  genuine remaining question.
\end{enumerate}
\end{remark}
