...
Theorem~\ref{thm:yangian-recognition} shows that the open-colour
line-side algebra $\cA^!_{\mathrm{line}}$ on the chirally Koszul locus
carries the dg-shifted Yangian package, whose axioms are the
$(\SCchtop)^{!}$-operadic identities (closed $=$ Lie, open $=$ Ass)
decomposed by colour. The spectral Drinfeld problem for this package is
not an identification of dg-shifted Yangians with factorization quantum
groups. It is a filtered strictification problem for the regularized
holonomy of a bar-horizontal spectral connection.
...
The mechanism that makes the computed sectors rigid is
\emph{root multiplicity one}: every root of a simple Lie algebra has
multiplicity one, so each nonzero root-sector obstruction is a scalar.
This does not by itself kill the obstruction. It identifies the exact
test: the product-side scalar must equal the coproduct-side scalar
...
term. When these operators are chain closed and satisfy the disjointness
and spectral Kohno identities above, they form a bar-horizontal
strictification. Theorem~\ref{thm:derived-additive-kz} proves flatness
from those identities; it is not an existence theorem for the identities.
\end{remark}
...
genuinely new and independent obstruction could live is filtration
$\ge 4$, where new Hall words and higher Magnus/BCH terms enter: the
$D_4$ star, for instance, gives a two-dimensional free multilinear Lie
space. Theorem~\ref{thm:root-space-one-dim} collapses the corresponding
finite root sector to a scalar, and
Theorem~\ref{thm:complete-strictification} identifies the remaining
scalar obstruction. The low-order coefficients $\frac12$ and $\frac13$
are the first instances of the universal $\frac1n$ in path sectors.
\end{remark}
...
The theorem does not say that the obstruction vanishes. It says that
the obstruction has only one scalar coordinate in each nonzero finite
root sector.
...
function by a nonzero constant. Its vanishing is therefore intrinsic,
but it is an additional scalar equation, not a formal consequence of
one-dimensionality.
...
The logical architecture is: root multiplicity one
...
coefficient $1/n$ in path sectors; and the beta integral
$\int_0^1(1-t)^{n-1}\,dt = 1/n$ evaluates that coefficient. Vanishing
is proved exactly in the sectors where the scalar rigidity equation is
proved.
\end{remark}
...
The scalar criterion separates two facts that are easily conflated.
Root multiplicity one is representation-theoretic: it turns a finite
root-sector obstruction into one scalar class. The BCH and Dynkin
computations are combinatorial: in path sectors they compute that
scalar as $1/n$, and the rigidity equation kills it. A one-dimensional
space can still carry a nonzero cohomology class; the obstruction
vanishes only after the scalar class is shown to be a coboundary.
...
$\SCchtop$~datum. The strictification criterion says that, once the
spectral quasi-factorization package has been constructed, the passage
from quasi to strict is governed by the scalar classes
$[\epsilon_{\alpha,n}]$. In the path sectors these classes vanish by
explicit calculation; outside those sectors the criterion names the
remaining equation.

For Kac--Moody algebras with root multiplicities~$>1$, the finite-type
scalar reduction fails in higher-dimensional root spaces. Real affine
root sectors retain the scalar reduction; Cartan-diagonal imaginary
sectors are exact under the commutation hypothesis of
Theorem~\ref{thm:cartan-gauge-twist}. Mixed real--imaginary sectors
require the separate analysis stated in
...
Theorem~\ref{thm:complete-strictification} therefore identifies the
real-root sector with its scalar obstruction class. Nothing further is
forced by one-dimensionality alone.
...
Drinfeld obstruction vanishes on the Cartan-diagonal imaginary sector.
\end{proof}
...
\end{gather*}
The last arrow is a filtered twist equivalence exactly on the vanishing
locus of the spectral Drinfeld class. For finite simple Lie algebras,
...
and in general $\frac1n$ in path filtration $n$) are the exact scalar
...
This chapter constructs the open-colour spectral strictification
criterion for the Swiss-cheese operad. The companion mixed-colour
statement is the affine half-space theorem of
...
$\widehat{\fg}_k$, on the scalar-vanishing locus isolated by
...
factorization; the bridge between (2) and (4) is the central content of
the chapter.
\end{enumerate}
\end{convention}
...
construction of this chapter outputs a spectral quasi-factorization
quantum group and a scalar obstruction criterion for strict spectral
factorization on $\mathbb{A}^1_u$ (type 4). Its lift to a chiral
factorization on a curve $X$ requires the modular-Yangian comparison
theorem (frontier; see Vol~I \cref{V1-thm:modular-yangian-pro}). The
dg-shifted package $Y^{\mathrm{dg}}_\hbar(\fg)$ produced here is
sufficient input for both definitions; the equivalence at chain level is
conditional on the curved-Dunn $H^2 = 0$ closure of Vol~II
(non-logarithmic standard landscape: closed; logarithmic $W(p)$: open).
\end{remark}
...
normalisation. Thus the scalar obstruction criterion of
...
strict realisation of the resulting bar-side data exactly when the
root-sector scalar obstruction classes vanish. Root multiplicity one is
the cohomological reduction to those scalar classes, while the
...
Theorem~\ref{thm:complete-strictification}, the strict spectral
factorization quantum group output of this chapter --
$Y^{\mathrm{dg,strict}}_\hbar(\fg)$ together with its strict spectral
$R(z)$ produced by
...
$R$-matrix. The open-colour proof input is not dimension alone, but the
vanishing of the scalar obstruction classes
$[\epsilon_{\alpha,n}]$.
\end{remark}
