...
\label{sec:dgsfb-wave15-DNA}

\begin{remark}[Two-$\hbar$ bridge: Drinfeld $\hbar$ vs BV $\hbar$ at $\hbar^2 = -1/8$ (AP151 closure)]
\label{rem:dgsfb-DNA-wave15-two-hbar-bridge}
Cache AP$151$ (two-$\hbar$ without bridge) flags the conflation of two
distinct formal parameters that both appear in the Wave $13$/$14$
K$3$ chiral Hall--Drinfeld double $\mathbf H_{\Delta_5}$:
\begin{itemize}
...
The bridge, at the Wave $13$/$14$ Koszul locus, is constructed in two
stages.

\noindent\emph{Stage 1 (Drinfeld side).} With $\ell = 8$ (the Mukai
doubling $K^{\kappa_{\mathrm{ch}}} = 2c_+ = 8$ restated; Wave $13$
D.$7$, D.$8$), $\hbar^{\mathrm{Drinfeld}} = 2\pi i / 8 = \pi i / 4$, so
...
on $I_1$ Kodaira fibres (Wave $13$ Costello-voice) is proportional to
the Bruinier reciprocity coefficient on $H_1(\overline{\mathcal A}_2,
\mathbb Q)$ evaluated at the class $[\Delta_5]$. By
...
\begin{equation}
\label{eq:dgsfb-DNA-wave15-two-hbar-bridge-eq}
 (\hbar^{\mathrm{Drinfeld}})^2_{\mathrm{BKM-normalised}}
 \;=\; (\hbar^{\mathrm{BV}})^2
 \;=\; -\,\tfrac{1}{8}
 \;=\; -\,\tfrac{1}{K^{\kappa_{\mathrm{ch}}}},
\end{equation}
with the right-most identification the Wave $13$ universal identity
...
doubling $K^{\kappa_{\mathrm{ch}}} = 8$. AP$151$ closes on this
locus: the two $\hbar$'s are bridged, not conflated.
...
$(\infty,1)$-quantisation of the Drinfeld double (stage $1$). Pattern
$269$ adjunction-strictness applies: the $(\infty,1)$-side provides
the universal property of the deformation, the chain-level side
provides the explicit numerical value.
\end{remark}

\begin{proposition}[$\kappa_{\mathrm{ch}}$-Bruinier reciprocity as the forcing mechanism of $\hbar^2 = -1/8$]
\label{prop:dgsfb-DNA-wave15-kappa-bruinier-reciprocity}
Let $\mathbf H_{\Delta_5}$ be the K$3$ chiral Hall--Drinfeld double of
Wave $13$ and let $\Delta_5 \in M_{5/2}(\widehat{\mathrm{Sp}_4(\mathbb Z)}^{v_{\Delta_5}})$ be
...
\begin{equation}
\label{eq:dgsfb-DNA-wave15-bruinier-forcing}
 \hbar^{\mathrm{BV},\,2}
 \;=\; -\,\tfrac{1}{8}
...
(cf.~Remark~\ref{rem:cclimax-DNA-wave15-koszul-locus} in Vol~I), and
...
\begin{remark}[Dictionary: $\hbar^{\mathrm{Drinfeld}}$ $\leftrightarrow$ $\hbar^{\mathrm{BV}}$ on the full Wave--$13$/$15$ parameter space]
\label{rem:dgsfb-DNA-wave15-two-hbar-dictionary}
The Wave $15$ bridge extends point-by-point on
