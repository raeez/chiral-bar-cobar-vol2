...
\begin{proof}
We verify that $A$ and $B$ are nonzero and linearly independent.

\emph{Nonvanishing of $A$.}  We have
$A=[[X_2,[X_1,X_c]],X_3]$.  The inner bracket $[X_1,X_c]\neq 0$
(adjacent in the Dynkin diagram).  Then $[X_2,[X_1,X_c]]\neq 0$ since
$X_2$ is adjacent to $X_c$ and the bracket extends to a nontrivial
element in the path subalgebra on $\{X_1,X_c,X_2\}$.  Finally
$[[\cdot],X_3]\neq 0$ since $X_3$ is adjacent to $X_c$ and the
element $[X_2,[X_1,X_c]]$ involves $X_c$.  (Formally: in the
universal enveloping algebra, $A$ has a nonzero component in the
monomial $X_1X_cX_2X_3$, verified by expanding the nested
commutators.)

\emph{Linear independence.}  $B$ is obtained from $A$ by transposing
$X_2\leftrightarrow X_3$.  Since the star Lie algebra is \emph{not}
symmetric under $X_2\leftrightarrow X_3$ relative to the ordered
product (the $\mathbb R$-ordering distinguishes $X_2$ from $X_3$),
$A$ and $B$ differ as elements of the Lie algebra.  To confirm
independence, expand both in the universal enveloping algebra: $A$
has a nonzero $X_1X_cX_2X_3$ coefficient while $B$ has a nonzero
$X_1X_cX_3X_2$ coefficient, and these monomials are linearly
independent in the associative algebra.

\emph{Spanning.}  Every other multilinear degree-$4$ Lie monomial in
$\{X_1,X_c,X_2,X_3\}$ (with $[X_i,X_j]=0$ for $i,j\in\{1,2,3\}$)
either vanishes or reduces to a linear combination of $A$ and $B$,
as verified by systematic application of the Jacobi identity and the
three vanishing relations.
\end{proof}
