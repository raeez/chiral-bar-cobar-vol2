...
false.  The strongest corrected triangle, derived from locality and
Hochschild theory, is available under the compact-generation /
derived-center hypotheses relevant to the boundary-linear exact
sector and on the chirally Koszul locus:
...
\cA^!_{\mathrm{line}}\text{-}\mathbf{mod}$ is the
line-operator category (Theorem~\ref{thm:lines_as_modules}).  The
...
line-module descriptions retain the extra scope just stated.
In that scoped setting, the bulk is the \emph{derived center} of the
boundary, not the boundary itself; line operators are modules for
the open-colour Koszul dual~$\cA^!_{\mathrm{line}}$, not for the
...
statement that $\cA^!_{\mathrm{line}}$ is a dg-shifted
Yangian (Theorem~\ref{thm:yangian-recognition}), while the full
...
results organize the scoped corrected triangle: the Yangian and
line-module package controls the line corner, while the
...
self-intersection algebra are modules for the open-colour Koszul dual.
