...
produces three classes of operators: bulk local operators (supported
in the interior), boundary local operators (supported on a wall),
and line operators (extended along $\mathbb{R}_t$).  The naive
...
(Volume~I, Theorem~A identifies $\cA^!$ as the Koszul dual).
...
$\Theta_\cA^{E_1}$ specialises at arity $2$ to the spectral
kernel $r(z)$, at arity $3$ to the Drinfeld associator, and its
...
The slab geometry of the preceding subsection is the concrete
geometric realisation of the $\SCchtop$ open disk \cite{Dimofte25}.
The five structural components (i)--(v) listed there define the
Drinfeld double $\cA \bowtie \cA^!$ as the universal algebra of
line operators.  This remark records the translation to the
monograph's algebraic framework.
