...
\begin{remark}[$E_1$ primacy and the triangle]
...
the line-operator vertex~$\cC_{\mathrm{line}}$ is the
\emph{primitive} datum: it carries the full ordered/braided
content ($r$-matrix, KZ associator, higher Yangian coherences).
The bulk vertex $\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ is
its derived center (a Morita-invariant averaging), and the
boundary vertex $\cA = \mathrm{End}(b)$ is a chart.
The triangle descends from the three-sector decomposition of the
$\mathsf{SC}^{\mathrm{ch,top}}$ convolution algebra
(Volume~I, Corollary~\ref*{V1-cor:convolution-factorization}):
closed factor~$\mathfrak{g}^{\mathrm{mod}}$ (bulk/modular),
open factor~$\mathfrak{g}^{\mathbb{R}}$ (line/$r$-matrix),
mixed sector (bulk-to-boundary brace evaluation).
\end{remark}
...
as a \emph{slab} with transverse boundary conditions
\cite{Dimofte25}.
For a 4d holomorphic--topological theory~$T$ on
$\mathbb{C}_z \times \mathbb{R}^2_{x,t}$, choose a pair of
transverse boundary conditions~$D$ (Dirichlet) and~$N$
(Neumann).  The slab $D \,|\, T \,|\, N$ with a line
operator~$\ell$ threaded through defines a vector space
$F(\ell) := \Hom_T(D,\, \ell,\, N)$.  The monoidal structure
$F(\ell \otimes \ell') \simeq F(\ell) \otimes F(\ell')$
follows from slab factorisation in the topological
$\mathbb{R}$-direction.

The universal algebra $U = \End(F)$ is identified as the
\emph{Drinfeld double}
$H_D \bowtie H_N$ of the two boundary algebras:
\begin{itemize}[leftmargin=1.8em]
\item $H_D = \cA$: the boundary chiral algebra (the
  monograph's open-colour algebra);
\item $H_N = \cA^!$: the Koszul dual boundary condition
  (Volume~I, Theorem~A);
\item the Hopf pairing
  $\langle a, b \rangle$ for $a \in H_D$, $b \in H_N$ is the
  hemisphere partition function;
\item the antipode $S(a)$ comes from orientation reversal (the
  cup geometry);
\item the coproduct
...
\begin{itemize}[leftmargin=1.8em]
\item the slab $D \,|\, T \,|\, N$ is a \emph{bimodule}
  geometry with two boundary components ($D$-wall and
  $N$-wall), not a Swiss-cheese disk; the $\SCchtop$ open
  disk has one closed interior boundary and one open boundary,
  which is a different categorical structure;
\item complementarity (Theorem~C of Volume~I) is the algebraic
  manifestation of transversality:
  $Q_g(\cA) \oplus Q_g(\cA^!) = H^*(\Mbar_g,\, Z(\cA))$;
\item the deconcatenation coproduct on $B^{\mathrm{ord}}(\cA)$
  is the algebraic avatar of the line-cutting geometry;
...
The slab picture of
Remark~\ref{rem:slab-fiber-functor} presupposes that the bulk
theory~$T$ admits transverse boundary conditions~$D$ and~$N$.
Many $3$d holomorphic--topological theories of interest are
rigid in this respect: they carry no topological boundary
condition that the line sector can end on
\cite{Dimofte25}.  In that situation one replaces the walls by
\emph{interfaces} from~$T$ to a simpler reference theory~$\cF$,
taken to be a free theory (Heisenberg $\Heis$, or a
supersymmetric refinement thereof), for which the category
$\cC_\cF$ of line operators is understood in closed form.
Let $I_D, I_N \colon T \leftrightsquigarrow \cF$ be two
transverse $T$--$\cF$ interfaces.  The slab
$I_D \,|\, T \,|\, I_N$ then defines, not a vector space, but a
module over~$\cC_\cF$: the fiber functor refines to
\[
...
$\cF = \text{trivial theory}$ recovers
$\cC_\cF = \Vect$ and the absolute fiber functor of
Remark~\ref{rem:slab-fiber-functor}.

Consequently the Hopf algebras $H_D = \cA$ and
$H_N = \cA^!$ of the slab picture become Hopf algebras
\emph{internal to} $\cC_\cF$: they are \emph{relative} Hopf
algebras over the base Hopf algebra of~$\cF$.  On the chiral
algebra side this is the statement that the natural object is
not the absolute bar coalgebra $B^{\mathrm{ord}}(\cA)$ but the
\emph{relative bar complex}
$B^{\mathrm{ord}}(\cA)/B^{\mathrm{ord}}(\Heis)$, computed by
tensoring the Heisenberg bar over its own deconcatenation
coaction.  Relative bar-cobar duality
(Volume~I, Theorem~A, applied in the category
$\Heis\text{-}\mathrm{comod}$) then produces a relative Koszul
dual $\cA^!_{/\Heis}$, and complementarity
(Theorem~C) holds in $\cC_\cF$ rather than in graded vector
spaces.  The degeneration $\cF \to \text{trivial}$,
$\Heis \to \mathbf{k}$ collapses
$B^{\mathrm{ord}}(\cA)/B^{\mathrm{ord}}(\mathbf{k})
\simeq B^{\mathrm{ord}}(\cA)$ and recovers the absolute
framework of
Remark~\ref{rem:slab-fiber-functor}.  The interface
generalization is thus the unique extension of the slab
picture to bulk theories whose line sector cannot be cut by a
wall but only by an interface to a free model.
\end{remark}
...
of $\widehat{\mathfrak{sl}}_2$ (AP126: at $k=0$ the
$r$-matrix vanishes, consistent with the abelian limit).  One checks directly
...
% AP1: kappa(V_k(sl_2)) = dim(sl_2)(k+h^v)/(2h^v) = 3(k+2)/4; NOT k
...
For $G = U(1)$: $\Bbound = \cH_k$, $R(z) = e^{k\hbar/z}$, $\Abulk = \cH_k$ (bulk $\simeq$ boundary), $\alpha_T = m_2 + \hbar k\,\eta \otimes \Lambda$.  The classical $r$-matrix $r(z) = k/z$ satisfies the $k = 0$ vanishing check trivially (AP126).

\smallskip
\noindent\emph{Non-abelian Chern--Simons.}
For $G = SL_2$: $\Bbound = \widehat{\mathfrak{sl}}_2{}_k$, $\cA^!_{\mathrm{ch}} = \widehat{\mathfrak{sl}}_2{}_{-k-4}$, $r(z) = k\,\Omega/z$ (vanishing at $k = 0$, AP126 satisfied).  The KZ connection is the arity-$(2,0)$ projection of $\alpha_T$; on evaluation modules, the reduced HT spectral $R$-matrix agrees with the quantum-group $R$-matrix of $U_q(\mathfrak{sl}_2)$ at $q = e^{i\pi/(k+2)}$ (Theorem~\ref{thm:affine-monodromy-identification}).  Line category:
\[
