...
\label{sec:htbb-core-wave13-DNA}

\begin{remark}[HT bulk-boundary line at K3: K3 chiral Hall-Drinfeld double]
\label{rem:htbb-core-DNA-hall-drinfeld}
The HT bulk-boundary line specialises at $K3 \times E$ to the Vol~III Wave~13 K3 chiral Hall--Drinfeld double $\mathcal{D}_\hbar(\mathcal{Y}^{\mathrm{Hall}}_\hbar(\mathrm{CoHA}_{K3 \times E}))$ on the boundary side, with bulk twisted 11D-SUGRA on $K3 \times T^2$ and $24$ M5-brane line defects on the Kodaira $I_1$ fibres. Classification invariant $H^2(\mathfrak{g}_{\Delta_5})^{\mathbb{Z}/2, K(1)} \cong \mathbb{C} \cdot \Delta_5$: the Igusa cusp form IS the bulk-boundary gauge-class invariant.
\end{remark}
