...
Before constructing anything new, we fix the foundational inputs.
...
classification invariant is the line
$H^2(\mathfrak{g}_{\Delta_5})^{\mathbb{Z}/2, K(1)}
\cong \mathbb{C}\cdot\Delta_5$; the Igusa cusp form is the
bulk--boundary gauge-class invariant on this compact Hall locus.
\end{remark}
...
of Vol~III~\cite[Ch.~\ref{V3-ch:k3-chiral-bialgebra-platonic}]{Lorgat3},
...
\end{array}
\]
is controlled by the Bruinier reduced BV obstruction class
\[
 c^{\mathrm{Br}}_3
 = c_{\phi_{-2,1}}(3)\,[H_3]
 = -8\,[H_3]
 \in H^2(\mathfrak{g}_{\Delta_5});
\]
...
\]
with $c_3^{\lambda}$ the restriction of $c^{\mathrm{Br}}_3$ to the BKM module
$\lambda$, and $A$ the gauge-field pullback from the bulk. The
trivialisation is possible precisely when
$\mathrm{tr}_\lambda(c^{\mathrm{Br}}_3) = 0$ in $H^2$ of the
...
\[
 ...
 \;=\;
 c_{\phi_{-2,1}}(3)\cdot\mathrm{tr}_\lambda([H_3])
 = -8\,\mathrm{tr}_\lambda([H_3]),
\]
and vanishes iff $\lambda$ is perpendicular (in the BKM-module inner
product) to $[H_3]\in H^2(\mathfrak{g}_{\Delta_5})$. On the open
...
the $24$-fold decomposition supplies the root-stalk labels for
Wilson-line insertions in the graph calculus of
...
\[
 ...
 \qquad
 o^{(3)}_{24}=c_{\phi_{-2,1}}(3)[H_3]=-8[H_3]
\]
...
The $\hbar^2$-order BV obstruction on $K3\times\mathbb{C}^2$ is
\[
 c^{\mathrm{triangle}}_2
 = c_{\phi_{-2,1}}(2)[H_2]
 =0
 \quad\text{in }H^2(\mathfrak{g}_{\Delta_5}),
\]
because discriminant~$2$ is not in the Eichler--Zagier admissible
support $0,3\pmod 4$. The same vanishing is seen diagrammatically as
the $24-24=0$ two-loop Feynman cancellation. In the HT
bulk-boundary-line triangle, this
...
$\mathcal{W}^{\mathrm{BKM}}_\lambda$ is required. The
$\hbar^3$ obstruction $c^{\mathrm{Br}}_3=-8[H_3]\neq 0$ marks the first non-trivial
...
\item The $3$-loop BV obstruction class is
 $c_3 = [\log(\Phi_{10}^{\mathrm{un}}/\eta^{24})]_{p^3}$, with
 Bruinier reduced value
 $c^{\mathrm{Br}}_3=c_{\phi_{-2,1}}(3)[H_3]=-8[H_3]$;
...
\item consistency with the pentagon-$\hbar^3$ cocycle
 ...
 + (\Phi_{10}^{\mathrm{un}}/\eta^{24}) c_{\Phi_{10}}$: the non-trivial
 $(\Phi_{10}^{\mathrm{un}}/\eta^{24}) c_{\Phi_{10}}$ summand is precisely $c_3$.
\end{enumerate}
The progression $c_0 = 0$ (classical) $\to c_1 = [\Delta_5]$
(paramodular anomaly) $\to c_2 = 0$ (two-loop vanishing)
$\to c^{\mathrm{Br}}_3 = -8\cdot[H_3]$ (first non-trivial Heegner
...
object: the weak Jacobi form lifting to $\Phi_{10}^{\mathrm{un}}$ under the
Borcherds~$1998$ singular-theta
lift~\cite[Theorem 10.1]{Borcherds1998}. The Bruinier reduced
three-loop value is $c^{\mathrm{Br}}_3=-8[H_3]$. The integer
...
it is not a Fourier coefficient of $\phi_{-2,1}$ and not the reduced
BV obstruction.
\end{remark}
