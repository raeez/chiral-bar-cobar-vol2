...
Part~I applies to $\cA_{\cB}$ without modification, yielding a chain of
...
The mathematically controlled input from
holomorphic-topological Yang--Mills theory entering the bar-cobar framework is the following.
...
the same boundary package, with precise algebraic content and conjectural input separated.
\end{enumerate}
\end{summary}
...
and the construction of Theorem~\ref{thm:main-koszul-hoch} is functorial, since each step (bar/cobar, derived $\mathrm{Hom}$, and Verdier duality) is natural.  Restricting to
...
\]
The assumed vanishing of $HH^{-1}_{\mathrm{chiral}}(\cA_{\cB}^{!})$ forces $HH^{3}_{\mathrm{chiral}}(\cA_{\cB})=0$.
\end{proof}
