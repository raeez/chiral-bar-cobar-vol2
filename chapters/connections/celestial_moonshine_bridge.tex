...
$\MtwFour$ acts on the matter factor, and Mathieu moonshine is
the $\MtwFour$-twined decomposition of
$\cA^{\mathcal{N}{=}4}_{K3}$ inside this tensor product. Moonshine
is an output of the universal celestial
...
Strominger's celestial $\wtc$ and the Gaberdiel--Hohenegger--Volpato
$\MtwFour$-action assemble into a single celestial-side construction whose
...
\emph{(ii).}
The $\MtwFour$-action on $T_4^{K3}$ is the
...
the gravity factor $T_{\mathrm{sd grav}}$. The
celestial dictionary is functorial in $Q$-equivariant maps of
4d HT theories (Theorem~\ref{thm:uch-main}(i)); under it the
$\MtwFour$-action descends to the trivial action on $\wtc$ and to the
Gaberdiel--Hohenegger--Volpato twining on
$\cA^{\mathcal{N}{=}4}_{K3}$. The twined celestial partition function
is the product. The $21$ K3-realized twining characters of
Eguchi--Ooguri--Tachikawa~\cite{EguchiOoguriTachikawa10} are
$Z_{K3}^{(g)}$ for the $21$ conjugacy classes acting by physical K3
symmetries; the remaining $5$ classes of Vol~III
Remark~\ref{rem:twining-genera} are Frame shapes whose action on
the K3 lattice has no geometric sigma-model lift,
and their celestial
realization in the extended
Cheng--Duncan--Harvey / umbral setting appears in
\S\ref{subsec:cmb-umbral}.
...
factorization homology). Each identity traces to its arrow.
\end{remark}
...
$\cAcel(T_4^{K3})$ into $\wtc$-module summands
labelled by $\MtwFour$-conjugacy classes. The action takes the
...
(Vol~III equation~\eqref{V3-eq:mathieu-decomposition}) twined by $g$. By
...
\S\ref{V3-subsec:mathieu-yangian} verification data).
...
\begin{remark}[$\MtwFour$-action on $\wtc$-modules: permutation]
\label{rem:cmb-permutation-not-deformation}
The $\MtwFour$-action of Theorem~\ref{thm:mathieu-celestial-correspondence}(iv)
permutes the $\wtc$-module summands of $\cAcel(T_4^{K3})$: trivial on the
$\wtc$-factor, by permutation of the labelled
$\cA^{\mathcal{N}{=}4}_{K3}$-summands. The $\wtc$-algebra is fixed
pointwise by $\MtwFour$, consistent with
Remark~\ref{rem:cmb-gravity-m24-invariant}; the
$\MtwFour$-representation structure lives entirely in the combinatorics
of the $\wtc$-module labels, separate from any deformation of the
stress tensor or of the $\wtc$-algebra itself.
\end{remark}
...
of the umbral mock modular forms, securing the connection
...
equation~\eqref{V3-eq:mock-modular-h} (generalized to each conjugacy
...
Vol~III equation~\eqref{V3-eq:mock-modular-h}.
...
equation~\eqref{V3-eq:mock-modular-decomposition}) and
...
$h(\tau)$ of Vol~III equation~\eqref{V3-eq:mock-modular-h}.
...
is the depth-$2$ shadow invariant of the $\mathcal{N}{=}4$ superconformal
algebra at $c = 6$. Here $\kapch(\cA^{\mathcal{N}{=}4}_{K3}) =
\chi(\cO_{K3}) = 2$ is the chiral-Hodge characteristic of the K3 algebra
(the $\Phi$-functor value $\Xi(K3) = 2$), consistent with
...
Remark~\ref{V3-rem:mock-modular-k3} is the $g = 1A$ specialization of a
...
$-2 = -\kapch$ (Vol~III equation~\eqref{V3-eq:mock-modular-decomposition}),
...
flagged as an aspiration. The theorem above realizes the connection
in the K3 lane: for $\cA^{\mathcal{N}{=}4}_{K3}$, the Rademacher
expansion of the mock modular form $h$ is the shadow-tower
structure of the bar complex. The class~M aspirational link
becomes a theorem on restriction to K3; the general class~M case
remains open.
\end{remark}
...
Remark~\ref{V3-rem:umbral-yangian}). Then the combined 4d theory
...
% =====================================
\subsection{Mathieu moonshine as a theorem of the celestial framework}
...
\]
The truth of the moonshine decomposition does not change; its
in-programme derivation does. Where Vol~III imported moonshine from
the external literature, the celestial bridge derives it from the universal
celestial holography theorem composed with the Costello--Paquette K3
chiral algebra assignment. Moonshine is an algebraic
consequence of the celestial framework.
\end{remark}

\begin{remark}[Sources]
\label{rem:cmb-novelty}
\emph{Internal.} The derivation: Mathieu moonshine is an output of
...
self-dual gravity (Strominger, DFPT, BHS). The bridge places all
of these as consequences of a single tensor decomposition of a single
celestial chiral algebra.

\emph{Cross-referenced.}
...
distinct specializations of the shadow-tower machinery; the two
identifications stand side by side.
\end{remark}
...
produces $\wtc \otimes \cA^{\mathcal{N}{=}4}_{K3}$. $\MtwFour$ acts
trivially on the first factor and by the Gaberdiel--Hohenegger--Volpato
...
\item \emph{Resolves.}
(a) The in-programme derivation of the moonshine connection.
(b) The aspirational shadow/$L$-function link from Volume~I in the
K3 lane: shadow-tower coefficients are the Rademacher expansion
...
assignment, the moonshine structure of K3 is forced in the
celestial sector.
\end{enumerate}
