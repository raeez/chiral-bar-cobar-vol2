% DEPRECATED: This file is not compiled. The canonical version is ht_physical_origins.tex.
...
\begin{remark}[Scope]
The $R$-twisted OPE structure is consistent with Definition~\ref*{V1-def:chiral-ass-operad}; deriving it from non-commutative CS requires Seiberg--Witten deformation quantization~\cite{SW99}, which lies outside this monograph.
\end{remark}
...
\begin{remark}[Scope]
Rieffel Morita equivalence and Connes' $\mathrm{HC}_*(A_\theta)$ computation are established; the missing step is constructing the topological $\Eone$ bar complex for $\mathcal{A}_\theta$ and verifying that its cohomology recovers the Koszul dual at $1/\theta$.
\end{remark}
