...
canonical $(-2)$-shifted symplectic structure: the Liouville form
on $T^*[-1]$ has degree $(-1) + (-1) = (-2)$.  This defines
$\Mvac(\cA)$ together with its shifted symplectic form.

The zero section $\cL_\cA \hookrightarrow T^*[-1]\cL_\cA = \Mvac(\cA)$
is Lagrangian: the Liouville form restricts to zero on the zero
section, and the conormal sequence for the zero-section embedding is
$\mathbb{L}_{\cL/T^*[-1]\cL} \simeq \mathbb{T}_\cL[-1]$---the
Lagrangian condition.

The self-intersection of the zero section in the shifted cotangent is
the derived tensor product
$\cO(\cL) \otimes^{\mathbb{L}}_{\cO(T^*[-1]\cL)} \cO(\cL)$,
which by the Koszul resolution of the zero section is
$T^c(s^{-1}\overline{\cO(\cL)}) = \barB(\cA)$.

Uniqueness: any $(-2)$-shifted symplectic $\cM$ with Lagrangian
$\cL$ satisfying $\cO(\cL \times_\cM \cL) \simeq \barB(\cA)$ is
formally equivalent to $T^*[-1]\cL_\cA$ by the formal Darboux
theorem for $(-2)$-shifted symplectic stacks
(Bussi--Brav--Joyce~\cite{BBJ19}).
\end{proof}
...
\item Koszul duality is Serre duality in the derived symplectic
  category: $\cA^!$ is the Serre-dual point, and the bar-cobar
  adjunction is the Serre functor on the endomorphism category.
...
symplectic structure that is the bar-cobar pairing.  In the derived
symplectic category, complementary Lagrangians are Serre-dual
objects.
...
Serre duality.  The genus tower is the deformation of the point
over the moduli of curves.  The holographic principle is the theorem
that a point determines its ambient.
