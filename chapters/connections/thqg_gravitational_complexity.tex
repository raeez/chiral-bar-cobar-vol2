...
H^*(\Defcyc(\cA))_{r,0}$ is the degree-$r$, genus-$0$
...
\index{shadow depth!equals $A_\infty$-depth equals $L_\infty$-formality level}%
For any chirally Koszul algebra~$\cA$,
...
$m_k^H = 0 \Leftrightarrow S_k = 0$, giving
$d_\infty = r_{\max}$.
...
has interaction vertices of degree at most $r_{\max}(\cA)$.
...
By the all-degree shadow--formality identification of Vol~I
...
$s > r_{\max}(\cA)$. Hence the tower stabilises at degree
$r_{\max}(\cA)$, and the bulk HT theory has no interaction vertices
above that degree. Conversely, a non-vanishing $r$-ary bulk bracket
...
$r_{\max} = 2$. The bracket term at degree~$3$ vanishes
($\ell_3^{(0),\mathrm{tr}} = 0$: abelian OPE).
With $\mathfrak{C} = 0$ and $\mathfrak{Q} = 0$, both
sources vanish at all higher degrees by induction.
Class~$\mathbf{G}$.
...
does not arise: with $\mathfrak{C} = 0$, the only degree-$5$
source is $[\mathfrak{Q}, \mathfrak{Q}]_5$, which requires
two degree-$4$ vertices and three internal edges. For
...
\begin{proof}
The degree-$r$ component $\Theta_\cA|_r$ determines the $r$-ary
...
bulk theory has non-vanishing higher brackets at infinitely many degrees.
\end{proof}
...
% RECTIFICATION-FLAG: [Gate 1 TRUTH, normalisation conflict] The
% prefactor $1/r$ stated here does NOT reproduce the explicit Virasoro
% table in \eqref{eq:thqg-virasoro-potential-explicit}: direct
% computation gives table-coeff$(x^r) = \mathrm{Sh}_r/r!$ exactly
% (verified $r=2..7$). Three sites (this Def line 874, the proof
% "$\mathrm{Sh}_r/r$" below, affine archetype $\tfrac13\mathfrak C_0$
% at item (b)) use the $1/r$ reading; the explicit septic table and
% the $\tfrac{1}{3!}\mathfrak C_0$ sites (cubic potential in
% \S6/\S26 remark, line 583/2080) use the $1/r!$ reading. The
% MC-action principle (Thm "MC = Euler--Lagrange") structurally needs
% $1/r$ for $\partial_x S = \kappa x + \sum \mathrm{Sh}_r x^{r-1}$.
% Same conflict mirrored in Vol I w_algebras.tex:5572 (states
% $1/2r$, table again equals $\mathrm{Sh}_r/r!$). Resolution requires
% the author to fix one convention globally; not invented here.
...
\begin{proof}
% RECTIFICATION-FLAG: [Gate 1 TRUTH] "$\mathrm{Sh}_r/r$" contradicts
% the table above, whose coefficients equal $\mathrm{Sh}_r/r!$
% (verified $r=2..7$). See the normalisation flag at
% Definition (Gravitational complementarity potential).
...
\item $V_k(\mathfrak{sl}_2)$ ($\mathbf{L}$):
 % RECTIFICATION-FLAG: [Gate 1 TRUTH] cubic term $\tfrac13\mathfrak C_0$
 % ($1/r$ reading) conflicts with $\tfrac{1}{3!}\mathfrak C_0$ at
 % line 583/2080 ($1/r!$ reading); see Definition normalisation flag.
...
discriminant is $\Delta = 8\kappa S_4$; at $\Delta = 0$ the
quadratic $Q_L = (2\kappa + 3\alpha t)^2$ is a perfect square and
the tower is finite.
...
\item \textbf{Lie} ($r_{\max} = 3$):
 ...
 Jacobi identity kills all higher degrees.
...
$S''_\cA(0) = \kappaChHodge(\cA) = H_\cA$. The genus-$g$
