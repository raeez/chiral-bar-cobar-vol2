...
\label{subsec:thqg-obstruction-tower}
...
arity~$r{+}1$ reads
\begin{equation}\label{eq:thqg-obstruction-recursion}
...
\begin{equation}\label{eq:thqg-obstruction-cohomology}
o_{r+1}(\cA)
\;:=\;
[\mathfrak{o}^{(r+1)}_{\mathrm{grav}}(\cA)]
\;\in\;
...
\label{subsec:thqg-shadow-depth-grav}

\begin{definition}[Gravitational complexity]
\label{def:thqg-gravitational-complexity}
...
\begin{equation}\label{eq:thqg-grav-complexity}
\rho_{\mathrm{grav}}(\cA)
\;:=\;
r_{\max}(\cA)
\;=\;
...
\begin{equation}\label{eq:thqg-three-invariants}
\rho_{\mathrm{grav}}(\cA)
\;=\;
d_\infty(\cA)
\;=\;
...
\label{thm:thqg-complexity-controls-bulk}
\ClaimStatusProvedHere
...
\begin{equation}\label{eq:thqg-stabilization}
\Theta_\cA^{\leq r}
\;=\;
...
\label{prop:thqg-depth-qi-invariant}
\ClaimStatusProvedHere
...
\label{subsec:thqg-four-classes}
...
\begin{equation}\label{eq:thqg-cubic-source-contribution}
...
\begin{equation}\label{eq:thqg-cubic-source-explicit}
...
\label{subsec:thqg-grav-interpretation}

\begin{theorem}[Gravitational content of the four classes]
\label{thm:thqg-grav-content}
\ClaimStatusProvedHere
...
\label{cor:thqg-gaussian-potential}
\ClaimStatusProvedHere
...
\begin{equation}\label{eq:thqg-virasoro-quintic}
...
\label{thm:thqg-virasoro-potential}
\ClaimStatusProvedHere
...
\item $\beta\gamma$ ($\mathbf{C}$):
  $S_{\beta\gamma}(x) = -\frac{1}{2}\,x^2
  + \frac{1}{4!}\mathfrak{Q}_0\,x^4$
  (with $\kappa(\beta\gamma) = -1$; the Hessian is nonzero).
...
\label{cor:thqg-virasoro-genus1}
\ClaimStatusProvedHere
\index{Virasoro!genus-1 Hessian correction}
\begin{equation}\label{eq:thqg-virasoro-genus1-hessian}
...
\label{subsec:thqg-additivity}

\begin{theorem}[Independent sum factorization]
\label{thm:thqg-independent-sum}
\ClaimStatusProvedHere
...
\label{prop:thqg-tropical-profiles}
\ClaimStatusProvedHere
\begin{center}
\small
\renewcommand{\arraystretch}{1.15}
...
\label{subsec:thqg-complexity-summary}

\begin{maintheorem}[Gravitational complexity; result (G2)]
\label{thm:thqg-g2-main}
\ClaimStatusProvedHere
...
\begin{equation}\label{eq:thqg-chienv}
\chienv(R)
\;:=\;
...
satisfy
\begin{equation}\label{eq:thqg-coefficient-bound}
|S_r|
\;\leq\;
...
\label{ex:thqg-virasoro-class-jumps}
...
\label{ex:thqg-affine-class-jumps}
...
$S_\cA(x)$:
\begin{equation}\label{eq:thqg-euler-lagrange}
...
\label{rem:thqg-four-gravity-theories}
...
\label{subsec:thqg-holographic-ss}
...
\label{constr:thqg-collision-filtration}
...
\begin{equation}\label{eq:thqg-graded-convolution}
...
\begin{equation}\label{eq:thqg-e2}
E_2^{r,q}
\;=\;
...
\label{prop:thqg-collapse-criteria}
\ClaimStatusProvedHere
...
\begin{equation}\label{eq:thqg-e2-virasoro}
E_2^{r,0} = 0
\quad\text{for all } r \geq 2.
\end{equation}
...
\label{subsec:thqg-ss-collapse}

\begin{proposition}[Collapse for class~$\mathbf{L}$]
\label{prop:thqg-collapse-L}
\ClaimStatusProvedHere
...
\begin{equation}\label{eq:thqg-collapse-L}
E_2^{2,0} \cong \C,
\qquad
E_2^{3,0} \cong \C,
\qquad
...
\label{subsec:thqg-open-problems}
...
\label{subsec:shadow-steinberg-stratification}
...
\label{def:stratification-depth}
...
\label{prop:shadow-equals-stratification}
...
\label{subsec:t-structure-interpretation}
...
\label{rmk:t-structure-intrinsic}
