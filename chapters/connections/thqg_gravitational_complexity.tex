...
\item $\mathrm{Vir}_c$ ($\mathbf{M}$):
 Non-polynomial; see~\eqref{V1-eq:thqg-virasoro-potential-explicit}.
\end{enumerate}
\end{example}
...
\label{sec:gravcomplex-wave13-DNA}

\begin{remark}[Gravitational complexity of $\mathbf H_{\Delta_5}$]
\label{rem:gravcomplex-DNA-1}
The gravitational-complexity analysis applied to the Wave~13 K3
background yields a Schiffmann--Vasserot-type complexity measure
\[
\mathcal C_{\mathrm{grav}}(\mathbf H_{\Delta_5}) \;=\; \dim\, \mathrm{Ext}^\bullet_{\mathrm{CoHA}_{K3\times E}}\bigl(\mathbf H_{\Delta_5}, \mathbf H_{\Delta_5}\bigr)^{\mathbb Z/2,\, K(1)},
\]
the $\mathbb Z/2$-equivariant Morava-$K(1)$-local Ext-complexity in the
Hall category. By the classification
$H^2(\mathfrak g_{\Delta_5})^{\mathbb Z/2,K(1)} \cong \mathbb C\cdot \Delta_5$
(\S\ref{sec:schtop-heptagon-wave13-DNA}), the leading complexity is
one-dimensional at $\hbar = 0$ and grows as $\hbar\to\hbar_{\mathrm{Lusztig}} = \sqrt{-1/8}$
through a unique Gritsenko-additive-lift direction. This is the
Etingof--Kazhdan Part~V super-category content of gravitational
complexity on $K3\times T^2$ twisted 11D SUGRA. Primary-lit:
Etingof--Kazhdan 2007 Part~V, Borcherds 1992/1998, Gritsenko--Nikulin
1996--98, Susskind 2014 (complexity conjecture).
\end{remark}
