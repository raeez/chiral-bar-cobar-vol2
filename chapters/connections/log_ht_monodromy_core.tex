...
\begin{enumerate}[label=\textup{(\alph*)}]
  ...
  \]
  (AP126: here $\Omega$ is an abstract element of $Y\otimes Y$, not the
  affine Casimir specifically; in the affine specialization
  $Y\mapsto U(\widehat{\mathfrak g}_k)$, the residue element absorbs
...
residue tensor $k\,\Omega_{\mathrm{Cas}}/(k+h^\vee)$ absorbing the level prefix;
cf.\ AP126 and the level-explicit form at~\eqref{eq:R-level-explicit} below)
...
$\wp$-function. (AP126: the $k$ prefix on $r_1$ is inherited from
the collision residue of the affine $r$-matrix
$r_{\widehat{\fg}_k}(z) = k\,\Omega/((k+h^\vee)z)$; the intrinsic
...
Theorem~\ref{rem:affine-half-space-bv} proves one-loop
...
in the proof of Theorem~\ref{rem:affine-half-space-bv}).  Therefore the
...
$r(z) = k\,\Omega/z$ at level~$k$ \textup{(}AP126\textup{)} up to scalars.
