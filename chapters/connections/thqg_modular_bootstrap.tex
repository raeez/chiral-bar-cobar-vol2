...
Verification: $d_{\Theta^{(0)}}^2 = 0$. The genus-$0$ MC equation gives
...
recursion for $\cH_k$ is entirely scalar. The computation gives
...
$Z_1 = \exp(F_1)$ where $F_1 = \kappaChHodge(\cA) \cdot
...
its output is conditional on identifying $\Theta_\cA$ with a bulk
saddle expansion.
\end{remark}
...
Write
\[
 \phi_{0,1}(\tau,z)=\sum_{a\geq0,\ r\in\mathbb Z}f(a,r)q^a y^r,
 \qquad
...
$\phi_{0,1}$ coefficient extracted from the singular theta lift
...
$\phi_{0,1}$; the K3 elliptic genus is $2\phi_{0,1}$ and therefore
feeds the doubled scalar-square exponent lane.  Since
...
(ii) Eichler--Zagier 1985 \S 9 Jacobi-form coefficients;
(iii) Borcherds 1998 Thm.~10.1 singular theta-lift;
(iv) DMVV 1997 \S 4 Table 1 second-quantised elliptic genus;
(v) the normalisation identities
$\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ and $D_5=64^{-1}\Delta_5$.
\end{proof}
...
\begin{align}
 ...
 &\;=\;\phi_{0,1}(\tau,0)\mathcal R_{-2,1}(\tau)\cdot\eta^{-8},
...
$\phi_{0,1}(\tau,0)\mathcal R_{-2,1}(\tau)$ is the corresponding
...
$\mathcal R_{-2,1}(\tau)$ from their theta-function factorisations
and multiply through; the resulting scalar and residue expansions match the
