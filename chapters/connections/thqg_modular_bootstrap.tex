...
\end{itemize}
All four converge to $\Phi_{10}^{\mathrm{un}}$ because they all restrict to the same
K3 bar--cobar adjunction on $\mathbf H_{\Delta_5}$; the
...
The modular bootstrap closes onto $\Phi_{10}^{\mathrm{un}}$ (Remark~\ref{rem:modbootstrap-1})
...
The Fourier expansion of $(\Phi_{10}^{\mathrm{un}}(Z))^{-1}$ around the standard cusp
...
reading off the Fourier coefficients of $(\Phi_{10}^{\mathrm{un}})^{-1}$ via the
second-quantised elliptic genus):
