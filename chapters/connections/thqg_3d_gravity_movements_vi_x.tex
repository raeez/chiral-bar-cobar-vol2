...
\label{sec:3dgravvix-wave13-DNA}

\begin{remark}[$3$d-gravity movements VI--X at the Wave~13 K3 base]
\label{rem:3dgravvix-DNA-1}
Movements VI--X of the $3$d quantum gravity climax collectively
implement the Wave~13 K3 structure as their universal target: Movement~VI
delivers the twisted 11D-SUGRA on $K3\times T^2$ reduction to $3$d
gravity on $\mathrm{AdS}_3\times S^2$; Movement~VII constructs the
$\mathbf H_{\Delta_5}$-module category as the boundary Hilbert space;
Movement~VIII computes the $\mathrm{AdS}_3$-partition function as
$\Phi_{10}(Z)$ via the Siegel-modular ensemble; Movement~IX exhibits
the gravitational $S$-duality (Vol.~II, \S\ref{sec:grav-sdual-wave13-DNA})
and the $24$-fold soft-graviton theorem
(\S\ref{sec:softgrav-wave13-DNA}); Movement~X closes the heptagon
coherence at order $\hbar^3$ with the pentagon coboundary
$\phi^{(3)} = \zeta(3)\, c_{\mathrm{symm}} + \tfrac{25}{3}\, c_{\mathrm{timelike}} + (\Phi_{10}/\eta^{24})\, c_{\Phi_{10}}$
installed as the gravitational $E_8$ anomaly cancellation. Primary-lit:
Witten 2007 ($3$d gravity partition function), Maloney--Witten 2009,
Maxfield--Turiaci 2020 (Wormhole contributions), Dijkgraaf--Moore--Verlinde--Verlinde
1997. Cross-ref Vol.~III,
