...
theorem (Volume~I, Theorem~\ref{V1-thm:quantum-complementarity-main}). In the
...
curvature, but not a proved trivial higher-degree tower. What
...
with nonvanishing obstructions at every degree. This might suggest
...
short-distance singularities. The degree cutoff
...
\begin{remark}[The ten movements at the K3 synthesis base: mathematical content and physical scope]
\label{rem:3dgravvix-1}
The ten movements realise the following mathematical constructions on
the K3 synthesis base, with each physical phrase translated into the
...
\item \textbf{(Movement~VI, geometric.)} The Costello--Li
  ...
  the level of the holomorphic twist; no dynamical-metric path
  integral on $\mathrm{AdS}_3 \times S^2$ is constructed.
  \ClaimStatusConditional for the factorisation-algebra
  specialisation; the dynamical-metric reading is
  \ClaimStatusHeuristic and sits at the scope of Costello--Gaiotto
  2018 twisted holography.
\item \textbf{(Movement~VII, algebraic.)} The
  ...
  $\mathrm{ch}(\mathbf{H}_{\Delta_5}\text{-Fock}) = 1/\Phi_{10}(Z)$
...
\item \textbf{(Movement~VIII, automorphic; scope-qualified.)} The
  ...
  ``$\mathrm{AdS}_3$-partition function equals $\Phi_{10}(Z)$''
  ...
  elliptic genera): this is \emph{also} $1/\Phi_{10}(Z)$ by the
  ...
  $\Phi_{10}$ and diverge at the level of what integrates to produce
  it. The twisted-holography identification is
  \ClaimStatusProvedHere; the Maloney--Witten identification is
  \ClaimStatusConjectured and requires the full off-shell path
  integral.
\item \textbf{(Movement~IX, cross-volume.)} The gravitational
  $S$-duality $c \mapsto 26 - c$ on the Virasoro family and the
  $24$-fold soft-graviton structure on the K3 synthesis pair are
  inscribed in Vol.~II
  ...
  algebra) and \ClaimStatusHeuristic at the untwisted scope.
\item \textbf{(Movement~X, algebraic.)} Heptagon coherence at order
  ...
  scope. \ClaimStatusProvedHere for the chain-level heptagon
  coherence; the Green--Schwarz reading is
  \ClaimStatusHeuristic with the scope fixed by
  Costello--Gaiotto--Paquette~\cite{CostelloGaiottoPaquette2018}.
\end{enumerate}
...
$\mathrm{AdS}_3$ saddles, not equal to $\Phi_{10}$ on the nose);
...
partition function but sharing the automorphic $1/\Phi_{10}$ via
