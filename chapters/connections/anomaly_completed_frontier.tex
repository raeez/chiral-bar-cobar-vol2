...
\section{Anomaly-completed holography: extensions and comparison theorems}
...
\(\Theta=\kappa(\mathrm{Vir}_c)\cdot\omega_1\).  The
...
\qquad
d\eta=\kappa(\mathrm{Vir}_c)\cdot\omega_1,
\]
Through
...
\[
\kappa(\mathcal W)=\kappa(\mathcal A)-\kappa_{\mathrm{ghost}}(f,k);
\]
...
For KM the self-dual and $\kappa(\cA)=0$ loci coincide;
...
\[
\widetilde\kappa(\mathcal W_\lambda^k)
:=\kappa(\mathcal W_\lambda^k)-\frac{1}{2}K^\kappa_\lambda,
\qquad
K^\kappa_\lambda=
\kappa(\mathcal W_\lambda^k)+
\kappa(\mathcal W_{\lambda^t}^{k^\vee}),
\]
vanishes at a self-dual point.  The intrinsic curvature
\(\kappa(\mathcal W_\lambda^k)\) need not vanish there.  For generic
$k$,
...
c(\mathcal B^k)+c(\mathcal B^{-k-6})=196,
\qquad
\kappa(\mathcal B^k)+\kappa(\mathcal B^{-k-6})=98/3.
\]
...
\kappa_0^{(2,2)}
:=
\kappa(\mathcal W^k(\mathfrak{sl}_4,f_{(2,2)}))
+\kappa(\mathcal W^{-k-8}(\mathfrak{sl}_4,f_{(2,2)})).
\]
...
For anomaly completion, the Mukai-doubling Koszul conductor $K^{\kappa_{\mathrm{ch}}}(\mathcal{H}_{\mathrm{Muk}}(K3)) = 2\,c_+(\mathrm{Mukai}(K3)) = 8$ identifies (via Bruinier $2002$ Proposition~$5.1$ Heegner Chern-class reciprocity) with: (a) order of monodromy of $\mathcal{L}^{\Delta_5}$ around Humbert divisor $H_1 \subset \overline{\mathcal{A}_2}$; (b) Lusztig specialisation $\ell = 8$ at $\zeta^8 = 1$, $\hbar^2 = -1/8$. Universal identity $\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$ on the Lorentzian-lattice-parametric $\mathcal{B}$-family. Theorem~C enlarges to $\{0, 8, 13, 250/3, 98/3\}$.
\end{remark}
