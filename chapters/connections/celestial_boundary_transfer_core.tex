...
\begin{proof}
Work in the holomorphic-topological twist of $4$d $\mathcal{N} = 2$ Yang--Mills. The boundary theory on $\mathbb{C} \times \{0\}$ is $\widehat{\mathfrak{g}}_k$, and the bulk theory on $\mathbb{C} \times \mathbb{R}_{> 0}$ is holomorphic Chern--Simons fibred over~$\mathbb{R}_+$.
...
For general $n$, the bar-complex integral over $\overline{\mathrm{FM}}_n(\mathbb{C})$ localises on the $(2n-5)!!$ top-dimensional boundary strata, indexed by trivalent (unrooted) trees with $n$ labelled leaves; the claim is that the associated residue sum reproduces the Parke--Taylor denominator $\prod_{i=1}^{n}\langle i,i{+}1\rangle^{-1}$. This follows from the general theory of Koba--Nielsen integrals on $\overline{\mathrm{FM}}_n(\mathbb{C})$. The full proof for general $n$ requires the recursive (BCFW) structure of the bar differential.
\end{remark}
