...
forces one holographic extension: the holomorphic-topological
factorisation algebra on $X \times \R$ whose boundary observable
algebra is $\bdyAlg$ and whose bulk observable algebra is the
level-$\mathsf{Z}$ derived chiral centre
...
conformal vector on a smooth projective curve $X$ to the category of
holomorphic-topological gauge theories on $X \times \R$. By
\cref{cor:universal-holography-via-bicoloured-universality}, $\Phihol$
factors as
$\Phi^{\mathrm{HT}}_{\mathrm{bic}} \circ \mathsf{Prim}^{\mathrm{bic}}$:
Universal Holography is the universal stage chain made physical. The boundary
...
\item \textbf{Boundary restriction.} For every
 $(\cA, \omega, T_\omega) \in \ChirAlg^{\omega,\mathrm{BL,adm}}_X$ the
...
\item \textbf{DS-functoriality.} For every good $\Z$-grading
 ...
 \[
 \begin{array}{c}
 \ChirAlg^{\omega,\mathrm{BL,adm}}_X(V_k(\fg))
 \;\xrightarrow{\;\DS_f\;}\;
 \ChirAlg^{\omega,\mathrm{BL,adm}}_X(\cW_k(\fg, f))
 \\
...
\ChirAlg^{\omega,\mathrm{BL,adm}}_X$. The Costello-Gwilliam
...
2021~\cite{CostelloGaiotto2021}) via the boundary-linear class-L bulk
identification of Theorem~\ref*{thm:global-triangle-boundary-linear}.
DS applied to the
...
\label{rem:uhf-scope}
The functor is defined on $\ChirAlg^{\omega,\mathrm{BL,adm}}_X$: the
\emph{conformal vector} and \emph{boundary-linear} hypotheses are
essential, and the admissible/generic exactness hypotheses supply the
Koszul comparison surface. Without conformal vector the bulk stays
...
conditional image of $\Phi_{\mathrm{hol}}$; under these hypotheses
...
$\Phi_{\mathrm{hol}}$ after the local BV anomaly datum and
...
Let $\cA \in \ChirAlg^{\omega,\mathrm{BL,adm}}_X$ be modular Koszul
...
Theorem~\ref*{thm:gravity-koszul-triangle}(iii) is the
...
(Proposition~\ref*{prop:uhf-shadow-vs-holographic}) and is
...
\item \textbf{thm:uhf-monster-orbifold-bv-anomaly-vanishes}
 ...
 Test: \texttt{test\_monster\_chain\_level\_e3\_top.py} verifies the
 normalized $\Z/2$ cohomology convention and the determinant-sign
 insufficiency.
...
\item \emph{Non-boundary-linear sector.}
 $\Phi_{\mathrm{hol}}$ is defined on
 $\ChirAlg^{\omega,\mathrm{BL,adm}}_X$. The non-boundary-linear
