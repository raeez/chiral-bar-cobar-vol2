...
%%
%% Closes FM125 / FM126 / FM127 / FM128 / FM185 / FM186 / FM187 /
%% FM188 / FM214 unconditionally, using the DS-Hochschild chain-level
%% compatibility bridge of chapters/theory/chiral_higher_deligne.tex
%% (thm:chd-ds-hochschild, cor:universal-holography-class-M) as the
%% chain-level class-M backbone.
%%
%% Honours programme distinctions: three Hochschilds (AP-CY64);
%% chiral vs topological Deligne (AP163); native vs derived E_n
%% (AP150); algebraic (End^{ch}_A) vs geometric (FM_k(C)) models
%% (AP-CY62); associator disclosure (GRT_1 chain-level dependence);
%% no bare kappa (AP113).
...
Four FM clusters fall to this single functor:
\begin{itemize}[leftmargin=1.3em]
\item \textbf{FM125/FM126/FM187/FM188}: bulk $\neq$ $\C[\![c]\!]$
  projection, global triangle is not stale, HKR is not classical:
  all three are saddle-point or cohomological shadows of the
  chain-level functorial statement.
\item \textbf{FM127}: ``perturbative finiteness'' is algebraic, not
  physical. The functor $\Phi_{\mathrm{hol}}$ separates the two:
  algebraic finiteness is bar-complex, physical UV is Costello RG;
  the bridge to physical UV is now unconditional for class~M.
\item \textbf{FM128}: Monster orbifold is bounded not because of a
  research expectation but because the Leech lattice is even
  unimodular, so the Dijkgraaf-Witten cocycle is trivial; the
  orbifold BV anomaly vanishes structurally.
\item \textbf{FM185/FM186/FM214}: shadow reconstruction vs.\
  holographic reconstruction vs.\ universal IS-claim split into
  three clean statements, all chain-level proved in the functorial
  form.
\end{itemize}
...
decorates each \ClaimStatusProvedHere\ with HZ-IV disjoint
...
(Vol~II~\ref*{V2-def:chirhoch}, cf.~AP-CY64). It is not the
...
AP163. The bulk algebra is $E_3$-\emph{topological} after one has
...
\ClaimStatusProvedHere; closes FM125, FM126, FM187, FM188, FM214
unconditionally on the Koszul locus at generic level]
...
$\SCchtop$-topological (not $E_3^{\mathrm{top}}$;
AP-TOPOLOGIZATION). Without boundary-linearity the global triangle
bulk $\to$ boundary $\to$ lines $\to$ bulk is not closed at chain
level (FM214 in the non-boundary-linear regime). At the critical
...
AP171 / entry~231 of the first-principles cache. The chain-level
...
orbifold BV anomaly vanishes; FM128 reported the vanishing as a
``bounded technical construction'' rather than a theorem. The
structural reason is at hand.

\begin{theorem}[Monster orbifold BV anomaly vanishes;
\ClaimStatusProvedHere; closes FM120, FM128]
...
Theorem~\ref{thm:uhf-monster-orbifold-bv-anomaly-vanishes} closes
FM120 (Monster as class~M example of the functor) and FM128
(``bounded technical construction'' replaced by a structural
...
FM127 flagged a conflation: ``perturbative finiteness of twisted
gravity'' was stated as a single property covering both algebraic
bar-complex finiteness and physical Costello-RG UV finiteness.
These are distinct. The functor $\Phi_{\mathrm{hol}}$ separates
them.

\begin{theorem}[Algebraic / physical UV finiteness split;
\ClaimStatusProvedHere; closes FM127]
...
\label{rem:uhf-fm127-closure}
Before the functorial picture, ``perturbative finiteness'' was a
single phrase. Separating it into algebraic~(I) and physical~(II)
surfaces the genuine content of FM127: algebraic finiteness is
easy (bar-complex combinatorics, no RG needed); physical
finiteness is hard (Costello RG, chain-level $E_3$ needed); the
DS-Hochschild bridge is what turns the hard side into a theorem
for class~M.
\end{remark}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Healing the nine-FM cluster}
\label{sec:uhf-nine-fm-cluster}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

With the functor in hand the nine FMs targeted by this chapter
(FM125, FM126, FM127, FM128, FM185, FM186, FM187, FM188, FM214)
...
\label{prop:uhf-fm125-koszul-triangle-projection}
...
\ClaimStatusProvedHere]
\label{prop:uhf-fm126-global-triangle}
The global-triangle boundary-linear identification
(\texttt{thm:global-triangle-boundary-linear}, Vol.~II
hochschild.tex:1065) is the $E_1$-chiral $\Leftrightarrow$
...
\ClaimStatusProvedHere]
\label{prop:uhf-fm185-shadow-vs-holographic}
...
\label{prop:uhf-fm186-symplectic-polarization}
The remark \texttt{thm:thqg-III-conditionality}
...
\ClaimStatusProvedHere]
\label{prop:uhf-fm187-kel06-chirality}
...
(\texttt{hochschild.tex:410-412}) is the chiral upgrade of the
...
\begin{proof}
AP-CY67: the spectral parameters in $\End^{\mathrm{ch}}_\cA$ are
...
\ClaimStatusProvedHere]
\label{prop:uhf-fm188-hkr-disentangled}
The identification
$\ChirHoch^\bullet(\cA_\partial) \simeq \cO(T^*[-1] L_b)
\simeq \cO(\cM_{\mathrm{vac}})|_{L_b}$
(\texttt{hochschild.tex:516-548}) splits into three cleanly
...
statements are each correctly scoped by (i) AP-CY64 (which
Hochschild), (ii) CDG vs HKR distinction, and (iii)
chain-level / cohomology disclosure.
\end{proof}

\begin{proposition}[Universal IS-claim scoped;
\ClaimStatusProvedHere]
\label{prop:uhf-fm214-universal-scope}
...
\item Bulk algebra $\Zder(\mathrm{Vir}_c)$ as the $E_3$-chiral
  ...
  Proposition~\ref{prop:uhf-fm125-koszul-triangle-projection}\textup{)};
...
Per the HZ-IV protocol, each \ClaimStatusProvedHere\ in this
chapter is bound to a pair of disjoint verification sources at
the Python decorator level. The three main claims and their
disjoint sources:
...
Honest scoping (AP40, AP225, HZ-IV discipline):
\begin{enumerate}
