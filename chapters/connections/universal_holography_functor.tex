...
%% carrying the conditionality stated in the chiral Higher Deligne chapter.
%%
...
chain-level, while the strict $E_3$ chain lift carries the
conditionality stated in the chiral Higher Deligne chapter.
Three-dimensional quantum gravity is the value of this functor at
Virasoro. Monster moonshine is the value at the Leech orbifold.
...
\ClaimStatusProvedHere, with class-M strict $E_3$ chain lift
\ClaimStatusConditional; ambient-qualified on the Koszul locus at
generic level]
...
chain lift inherits the conditionality of
...
\item \textbf{Bulk identification.}
  ...
  \]
  as HT factorization algebras on $X \times \R$. After the Sugawara
  ...
  strict class-M chain-level $E_3$ lift has the conditional status
  stated above.
...
Theorem~\ref*{thm:chiral-higher-deligne}, with that theorem's
conditional chain-level status for the strict $E_3$ lift). The
...
exactly the conditional clause recorded in the theorem statement.
...
has the conditional chain-level status of
...
is the conditional lift recorded in
...
\item \textbf{Holographic reconstruction} \textup{(}chain-level,
  ...
  restriction. On the Koszul locus at generic level this is
...
quantum gravity of Part~\ref*{part:gravity} is the Virasoro value of
the universal holography functor.

\begin{corollary}[3d quantum gravity as the Virasoro value;
\ClaimStatusProvedHere]
...
\end{enumerate}
Thus the Part~\ref*{part:gravity} Virasoro theory is the object
...
$\Phi_{\mathrm{hol}}$, and Virasoro gives 3d gravity as one value.
...
The functor $\Phi_{\mathrm{hol}}$ is the structural object: 3d
gravity is its value at Virasoro, Monster moonshine is its value at
the Leech orbifold, non-abelian gauge gravity is its value at
