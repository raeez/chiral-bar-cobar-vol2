...
%% (iii) conditional comparisons with the Khan--Zeng HT Poisson sigma model.
...
\item \textbf{Conditional comparisons:} link the formal algebra to the Poisson sigma model; depend on the existence of a resolved chiral/factorization datum and half-space perturbative quantization.
\end{enumerate}
\end{convention}
...
\begin{remark}[Wave-14 anchor: modular PVA quantization as classical
limit of the universal-conductor master]%
\label{rem:wave14-anchor-modular-pva-quantization}%
\index{Wave 14!modular PVA quantization}%
\index{Wave 14!classical limit of universal holography}%
\index{Khan--Zeng!classical-limit lift to modular bar}%
The modular PVA quantization above is the \emph{classical limit}
($\hbar \to 0$ with the modular grading retained) of the wave-14
...
3D HT Poisson sigma model on $\R \times \C$; the master theorem
quantises the boundary algebra to $\cA$ and identifies the bulk with
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ as $E_3$-topological. The
quantisation is conducted at every rung of the operadic ladder:
\begin{itemize}[nosep]
