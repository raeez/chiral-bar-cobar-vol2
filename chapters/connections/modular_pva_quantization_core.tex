...
\begin{abstract}
Three layers: (i)~external input from the literature, (ii)~formal algebraic results proved here given a modular bar datum, (iii)~conditional comparisons with the Khan--Zeng holomorphic-topological Poisson sigma model.  Trees govern chirality; loops govern modularity.  We construct the stable-graph modular bar coalgebra, define
\[
\Defmod(A_{\cl}(V)) := \Coder(\Bmod(\A))[-1],
\]
and prove the universal obstruction recursion for lifting a genus-zero Maurer--Cartan class to all genera.  In variational coordinates, the genus-raising operator is the reduced odd Laplacian $D_1=\Delta_{\mathrm{cyc}}$.  For classical $W_3$: the first modular obstruction vanishes, the relevant shifted $H^1$ is one-dimensional (the central-parameter direction $\partial_c P_c$), and every genus-one lift is gauge-equivalent to a renormalization of the central parameter.
\end{abstract}
