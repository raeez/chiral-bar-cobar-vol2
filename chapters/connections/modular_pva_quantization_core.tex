...
\begin{theorem}[Genus completion (conditional on Assumption~\ref{assum:resolved-classical-datum})]
...
under Assumption~\ref{assum:resolved-classical-datum}]
...
\begin{theorem}[Formal structure theorem (conditional on Assumption~\ref{assum:resolved-classical-datum})]\label{thm:formal-structure}
Let $C$ be a modular bar datum. Then:
\begin{enumerate}[leftmargin=2.4em]
...
\begin{theorem}[KZ-locus genus-zero boundary quantization]\label{thm:conditional-tree}
...
In particular:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{[]} At genus $0$: $(D^{(0)})^2 = 0$
 \textup{(}the tree-level bar differential squares to zero\textup{)}.
\item \textup{[]} At genus $1$:
 $D^{(0)} D^{(1)} + D^{(1)} D^{(0)} = 0$
 \textup{(}the one-loop correction is a $D^{(0)}$-derivation\textup{)}.
\item \textup{[]} The full expansion is the Feynman transform of the modular operad
