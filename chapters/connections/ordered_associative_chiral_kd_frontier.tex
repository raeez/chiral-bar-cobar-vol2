...
$\times E$ frontier: strand S4]
\label{rem:oackd-front-DNA-bi-based-ran}
...
strand-S1 Hall--Drinfeld double structure (Remark
\ref{rem:oackd-front-DNA-hall-drinfeld-double} below) reduces
at the Koszul locus.
\end{remark}
...
\begin{remark}[Abelian-at-Lie / non-abelian-at-vertex: strand S7]
\label{rem:oackd-front-DNA-abelian-lie-nonabelian-vertex}
...
weight-$1$ sector (class $\mathbf{L}$, Heisenberg-abelian)
can support a much richer closed vertex algebra (classes
...
$\times E$ BKM frontier: strand S1]
\label{rem:oackd-front-DNA-hall-drinfeld-double}
...
\Ran_E)$ of Remark~\ref{rem:oackd-front-DNA-bi-based-ran}.
\end{remark}
...
strand S11]
\label{rem:oackd-front-DNA-universal-phi-trace}
...
\index{chiral Hochschild trinity!supertrace}%
...
$\mathsf{SC}^{\mathrm{ch,top}}$ / chiral Hochschild trinity
formalism to the Vol~III BKM landscape via the CY-to-chiral
...
extracted from the leading Igusa--Fourier coefficient
$c_\Lambda(0)/2$ of $\Delta_5$. The numerical value
$\kappa_{\mathrm{BKM}} = 5$ is the Vol~III reading of the
strand-S8 invariant $K^{\kappa_{\mathrm{ch}}} = 2 c_+(\mu(\mathrm{K3})) = 8$
after the Mukai $\to$ Humbert $\to$ Lusztig translation on the K3
narrow lattice; the three faces of $K^{\kappa_{\mathrm{ch}}}$
(Mukai / Humbert / Lusztig) are three independent verification
paths for the same number on the B-family
side, and the universal specialisation
$\hbar^2 K^{\kappa_{\mathrm{ch}}} = -1$ fixes the
$\hbar$-scale in which the $\Phi_*$-intertwining is an
equality rather than a proportionality. The $\Phi$-trace
identity is therefore the Vol~II $\leftrightarrow$ Vol~III
bridge at the $E_1$-chiral frontier of this chapter; it is
the frontier-level analogue of the Volume-I/II five-theorem
bridge and one of the four generating identities of the
the K3 synthesis reconstituted architecture.
\end{remark}
...
\begin{remark}[Classification $H^2(\mathfrak{g}_{\Delta_5})^{\Z/2,K(1)}
\cong \C\cdot\Delta_5$ and the three faces of
$K^{\kappa_{\mathrm{ch}}} = 8$: strands S2 and S8]
\label{rem:oackd-front-DNA-classification-K-kappa}
\index{H2 classification!g Delta5@$\mathfrak{g}_{\Delta_5}$}%
\index{Igusa!gauge invariant!Delta5}%
\index{K kappa ch@$K^{\kappa_{\mathrm{ch}}}$!three faces}%
...
the Borcherds BKM setting, where a classification theorem
pins down the deformation parameter uniquely. Concretely:
for the BKM $\mathfrak{g}_{\Delta_5}$ of Remark
\ref{rem:oackd-front-DNA-hall-drinfeld-double}, the $\Z/2
...
\[
  ...
  \C \cdot \Delta_5,
\]
a one-dimensional space canonically spanned by the Igusa
cusp form $\Delta_5$. That is: the Igusa cusp form
\emph{is} the gauge invariant of the Drinfeld double
deformation in this setting, not merely a book-keeping
denominator for the root multiplicities. The level parameter
in this presentation is the strand-S8 invariant
\[
  K^{\kappa_{\mathrm{ch}}}
  \;=\;
...
admits three equivalent presentations by three independent
cross-family paths:
\begin{enumerate}[label=\textup{(F\arabic*)}, leftmargin=*]
\item \emph{Mukai face}: $K^{\kappa_{\mathrm{ch}}} =
...
\end{enumerate}
The three faces coincide numerically ($K^{\kappa_{\mathrm{ch}}}
= 8$) and furnish three independent verification paths for
the same B-family invariant. The Vol~II universal
specialisation $\hbar^2 K^{\kappa_{\mathrm{ch}}} = -1$
...
$\Z/2 \times K(1)$-equivariant $H^2$ classification
above, determines the Drinfeld double of
$F_{\mathrm{K3}}$ up to a canonical Igusa scalar.
Together strands S1, S2, and S8 therefore exhibit the
K3-frontier quantum-group deformation as \emph{uniquely
pinned}: a single Hall-double structure, a one-dimensional
deformation space spanned by $\Delta_5$, and a single
B-family level $K^{\kappa_{\mathrm{ch}}} = 8$ with three
...
\section{Synthesis}
\label{sec:oackdf-wave13-DNA}

\begin{remark}[Ordered-associative chiral $K_d$ frontier at K3]
\label{rem:oackdf-DNA-1}
The ordered-associative chiral $K_d$ frontier at the K3 synthesis base
...
\label{sec:oackdf-wave15-DNA}

\begin{remark}[Refined $\Omega$ two-parameter family off self-dual; \ClaimStatusConjectured]
\label{rem:oackdf-W15-refined-omega}
the K3 synthesis fixed the K3 ordered-associative chain at $\hbar^2 = -1/8$ on
the self-dual $\Omega$-locus $\epsilon_1 + \epsilon_2 = 0$. the K3 synthesis
probes the pushoff: the ordered-associative frontier extends as a
...
Cross-ref Vol.~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic} and
core-companion Remark~\ref{rem:v2-oackd-DNA-W15-two-param}.
\end{remark}

\begin{remark}[$E_1$/$E_2$ native/derived scoping on the frontier; \ClaimStatusProvedHere]
\label{rem:oackdf-W15-native-derived}
The the K3 synthesis ordered-associative frontier at K3 is precisely an
$E_1$-chiral algebra at its native level, with the following
ladder of operadic lifts (cache guard: \emph{native/derived $E_n$}).
The assignments below are from the cached-confusion-pattern discipline
on $E_1$/$E_2$ for Phi-output K3 bialgebras.

\begin{itemize}
\item \textbf{Native $E_1$-chiral on $E$.} $\mathbf H_{\Delta_5}$ is
  strongly admissible in the sense of \S2 of the core companion, with
  bar coalgebra $\Barch(\mathbf H_{\Delta_5})$ computed on the ordered
  Ran space $\mathrm{Ran}^{\mathrm{ord}}(E^{\mathrm{nod,sm}}_{24})$ of
  the $24$-nodal elliptic base. The ordered-associative Koszul duality
  of this chapter operates at this level.
\item \textbf{Derived $E_2$ via K3-factorisation.} The lift to a K3
  factorisation algebra (Vol.~III construction) exposes the $E_2$
  structure on the \emph{derived centre}
  $Z^{\mathrm{der}}(\mathbf H_{\Delta_5}) = \mathrm{ChirHoch}^\bullet
  (\mathbf H_{\Delta_5})$ via chiral braces; the $E_2$ lives on the
  centre, not on $\mathbf H_{\Delta_5}$ itself (cache confusion~\#3,
  AP~CY-$*$).
\item \textbf{Vol.~III CY-2 $\Phi_2$ output.} The CY-to-chiral functor
  $\Phi_2$ at $d = 2$ outputs $\Phi_2(D^b\mathrm{Coh}(\mathrm{K3})) =
  \mathbf H_{\Delta_5}$ as an $E_2$-chiral algebra on K3; the factorisation
  over the elliptic base curve $E \subset \mathrm{K3}$ (e.g.\ smooth
  fibre of an elliptic K3) reverts the level to $E_1$.
\end{itemize}

Nothing on the frontier conflates $E_1$ and $E_2$: the ordered
associative Koszul chain lives natively at $E_1$, the derived centre
lifts to $E_2$, and the Vol.~III CY-$2$ output tracks the K3 surface
geometry. Cross-ref core companion
Remark~\ref{rem:v2-oackd-DNA-W15-e1-e2} and
...
$\phi_{0,1}^{\mathrm K3}$ (verified compute-side in
\texttt{compute/tests/test\_k3\_yangian\_wave14\_twisted\_11dsugra\_1loop.py},
16 tests, 5/5 Heegner labels matching EOT 2011 Table~1). Off
...
preserved on the curve $E$, derived $E_2$ level preserved on the K3
surface. The self-dual slice $\epsilon_1 + \epsilon_2 \to 0$
