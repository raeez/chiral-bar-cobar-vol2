...
\[
c_{\mathrm{BP}}(u) = 2-\frac{24(u+1)^2}{u+3}.
\]
Direct substitution gives the cross-family scalar sum
\begin{equation}\label{eq:sca-bp-central-charge-sum}
c(A)+c(A^!)
\;=\;
\frac{24k^2+248k+609}{k+3},
\end{equation}
which is not level-independent. Thus central charge does not
certify the conjectural SCA--BP Koszul duality. The evidence for
that comparison must come from the generator spectrum, pole
structure, and explicit ordered-bar differential below, while the
same-family BP conductor remains the separate identity
\[
c_{\mathrm{BP}}(u)+c_{\mathrm{BP}}(-u-6)=196.
\]
...
\]
so the sum is
$4+24((k'+5)^2-(k'+1)^2)/(k'+3)=4+192=196$.
...
rational model, not a construction of $U(\cA)$. We sketch the
definition, identify the obstructions, and verify the base cases
where a classical answer is already available.
...
Cross-ref Vol.~III Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic} and
