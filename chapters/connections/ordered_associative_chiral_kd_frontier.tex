...
of $Y_z^\hbar(\mathfrak{sl}_2)$, confirming that the classical
...
$R$-matrix provenance & derived from OPE (tier~(ii))
 ...
 & \textbf{changes} \\
Koszul dual & $Y_z^\hbar(\mathfrak{sl}_2)$
...
$V_{\sqrt{2}\Z}^! = Y_z^\hbar(\mathfrak{sl}_2)$
...
$Y_z^\hbar(\mathfrak{sl}_2)$ is deformed to the quantum
...
$V_\Lambda^! = Y_z^\hbar(\fg)$ to the trigonometric regime.
...
structure is determined by $r(z)$. All higher-arity
...
assembled from the arity-two classical $r$-matrix $r(z)$
of $\cA$ together with a tower of higher-arity corrections.
At tier~(b) (Heisenberg, Kac--Moody, $\beta\gamma$) the
higher-arity corrections are determined recursively from
...
\;+\;\cdots,
\end{equation}
with $\tau_{(n)}$ the arity-$n$ component of the
...
\paragraph{Obstruction 2: higher-arity structure from the
MC tower.} At tier~(b), the binary $r$-matrix $r(z)$
determines the mixed product at arity two, but not at
higher arities. The higher components of the mixed
product correspond to the higher Maurer--Cartan
components $\Theta_A^{(n)}$ of the bar-intrinsic MC
element (Theorem, Vol.~I). At arity $n\ge 3$ the
...
mixed product has no higher-arity corrections, the
...
associative structure at arity $\ge 3$ involves the
