...
(at $k=0$ the $r$-matrix vanishes, per AP126).  For the $\mathcal N=2$ SCA, the
...
(the $k$ prefactor enforces AP126: at $k=0$ the
...
(AP132).  The ordered bar coalgebra is
...
dual are distinct functors (AP34).  The Drinfeld double uses
...
AP126 check passes.  The mixed product of
...
$\mathrm{Sym}^{\mathrm{ch}}(V^*)$ (AP33), see the base-case analysis below).
...
sector collapses (AP126 and AP141 verified).  The mixed
...
(class~$\mathbf{M}$) the tower never terminates
(AP131: generating depth finite, algebraic depth
infinite), so the Drinfeld double $U(\cA)$ at tier~(b)
...
The descent is $r$-matrix-twisted (V2-AP4): the naive
...
algebra on the dual generator space; AP33).  The Drinfeld double partner is $\cH_{-k}$ (this is not the ``signed
reflection'' $\kappa\mapsto-\kappa$ of AP29 but the
...
and $U(\cH_0) = \mathbb{C}$, consistent with AP126 and
...
formula is consistent with AP8 scoping.)  The classical
$r$-matrix is
$r_{V_k(\mathfrak{sl}_2)}(z) = k\,\Omega/z$, which at
$k=0$ vanishes identically (AP126 check passes by
equation~\eqref{eq:drinfeld-r-kmc}) and at $k=1$
