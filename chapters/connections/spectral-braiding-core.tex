...
Theorem~\ref{thm:lines_as_modules} identifies line operators
with $\cA^!_{\mathrm{line}}$-modules, so the monoidal
...
\item Construction of a ``Yangian vertex algebra''---a vertex
  ...
  resolves the \emph{structural} question---the open-colour dual
  $\cA^!_{\mathrm{line}}$ carries a canonical
  $\SCchtop$-algebra structure encoding both the Sklyanin
  $\lambda$-bracket (closed color) and the Yangian product
  (open color)---while the explicit ``vertex algebra''
