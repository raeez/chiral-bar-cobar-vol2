...
\item[Bridge on the overlap.] Let $\cA$ admit both presentations (e.g.\ $V_k(\fg)$ with its affine Yangian $Y_\hbar(\widehat{\fg})$ completion at generic level), and restrict to the evaluation-module core of Theorem~\ref{thm:affine-monodromy}. There exist gauge elements $F(z) \in (\cA^!\widehat\otimes\cA^!)[\![z^{-1}]\!]^\times$ and an associator $\Phi \in \GRT_1(\bQ)$ such that
\[
...
\section{Wave 13 DNA perspective: Siegel-elliptic dynamical R-matrix}
\label{sec:sbc-wave13-DNA}

\begin{remark}[Fourth class of $R$-matrix: Siegel-elliptic dynamical]
\label{rem:sbc-DNA-R-sieg-fourth-class}
...
\label{sec:sbc-wave13-DNA-deep}

\begin{remark}[Spectral braiding on $\mathbf H_{\Delta_5}^{\otimes 2}$]
\label{rem:sbc-DNA-deep-1}
The spectral braiding core for the Wave~13 K3 bialgebra acts as
\[
