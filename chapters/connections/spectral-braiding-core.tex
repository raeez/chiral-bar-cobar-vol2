...
Let $A$ be a logarithmic $\SCchtop$-algebra.
Let $\mathcal C_{\partial}$ be the derived category of boundary line operators with the tensor structure induced by the $W$-module structure of~$M$. Then $R(z)$ defines a meromorphic braiding on $\mathcal C_{\partial}$, and the classical limit $r(z)$ integrates to a filtered quasi-triangular structure on~$\mathcal H$.
\end{theorem}
...
\begin{proposition}[Spectral-to-categorical bridge; \ClaimStatusProvedHere]
...
\beta_{L_1,L_2} \colon L_1 \otimes L_2 \xrightarrow{\ \sim\ }
  L_2 \otimes L_1
\]
of the reduced braided monoidal category
...
\begin{proof}
By the resolution hypothesis, the unconditionality theorem
...
The spectral $R$-matrix $R(z)$ arises by two logically distinct routes, each the canonical output of a different locality class of chiral algebra. Conflation of the two is the error pattern V2-AP2/FM54/AP-CY65; this remark records the right/wrong/correct triple explicitly.
\begin{description}
\item[Provenance (a), $E_\infty$-chiral \emph{(derived from OPE)}.] For a vertex algebra $\cA$ (KM at non-critical level, Virasoro, Heisenberg, $W_N$, lattice VOAs, all of which are $E_\infty$-chiral per V2-AP1), the classical $r$-matrix is extracted as the binary collision residue
\[
r(z) \;=\; \Res^{\mathrm{coll}}_{0,2}\bigl(\Theta_\cA\bigr) \;=\; \mathrm{dlog}\text{-part of the OPE},
\]
its quantum lift $R(z) = \exp(r(z))$ is then \emph{canonical} (determined up to $\GRT^{\mathrm{fin}}$ Casimir rescaling), and Kazhdan--Lusztig~\cite{KL94} identifies the evaluation-module category with the quantum-group tensor category at the matched level. No independent input is needed.
\item[Provenance (b), genuinely $E_1$-chiral \emph{(independent input)}.] For a genuinely $E_1$-chiral algebra $\cA$ (Yangian $Y_\hbar(\fg)$, Etingof--Kazhdan quantum VA; V2-AP6), the OPE data of $\cA$ do \emph{not} determine $R(z)$: the bar-intrinsic collision residue yields only the classical shadow $r(z)$ via Laplace transform, and the quantum lift is supplied by the quasi-triangularity axiom (H5) as \emph{additional} structure on $\cA$. Two distinct quantizations of the same classical $r(z)$ are generically inequivalent; the choice is part of the datum.
\item[Bridge on the overlap.] Let $\cA$ admit both presentations (e.g.\ $V_k(\fg)$ with its affine Yangian $Y_\hbar(\widehat{\fg})$ completion at generic level), and restrict to the evaluation-module core of Theorem~\ref{thm:affine-monodromy}~(AP47). There exist gauge elements $F(z) \in (\cA^!\widehat\otimes\cA^!)[\![z^{-1}]\!]^\times$ and an associator $\Phi \in \GRT_1(\bQ)$ such that
\[
...
\noindent\textbf{Spectral vs.\ categorical (the FM54 separation).} The spectral $R(z)$ is a \emph{family} of isomorphisms parametrised by $z\in\C^\times$ (equivalently by the complex modulus of $\FM_2(\C)$); the categorical $E_2$ braiding produced by Dunn on $\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ is a \emph{single} natural transformation $\beta_{X,Y}\colon X\otimes Y\to Y\otimes X$. The spectral parameter enters through the evaluation module functor $\mathsf{ev}_z\colon \mathrm{Mod}_\cA \to \mathrm{Mod}_\cA$ (translation by~$z$ on the curve), \emph{not} through the $E_2$ structure itself. The relationship
\[
\beta_{\mathsf{ev}_{z_1}(M),\,\mathsf{ev}_{z_2}(N)} \;=\; R(z_1 - z_2)\circ\tau
\]
is a \emph{theorem} requiring the $D$-module-enriched $E_2$ structure on $\cZ^{\mathrm{der}}_{\mathrm{ch}}$ (not a definition): the family $R(\cdot)$ is the image of the single braiding under the $\Z$-indexed family of translation automorphisms $\tau_z$, and conflating the two is V2-AP2.
...
\begin{theorem}[$\SCchtop$ Heptagon;
\ClaimStatusProvedHere]
...
sub-case) and let $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA) :=
C^\bullet_{\mathrm{ch}}(\cA,\cA)$ be its chiral derived centre. The
$\SCchtop$-algebra structure on the bulk-boundary pair
$(Z^{\mathrm{der}}_{\mathrm{ch}}(\cA), \cA)$ admits seven equivalent
presentations, any two of which are connected by a quasi-isomorphism
of two-coloured $\SCchtop$-algebras:
...
\Delta_z(a)_{(1)}$, cf.\ AP-CY25. P1 $\Leftrightarrow$ P7 is the
