...
object in chiral algebras on~$X$.  This structure lives on the
...
\operatorname{End}(V) \otimes Y_z^\hbar(\fg)[\![u^{-1}]\!]$.
...
The $E_1$-chiral $A_\infty$ structure on $Y_z^\hbar(\fg)$ is
the central open question. We state precisely what is known.

The Yangian $Y_z^\hbar(\fg)$ is a \emph{filtered} algebra:
the PBW filtration by total mode number $|t_{ij}^{(r)}| = r$
gives $\gr Y_z^\hbar(\fg) \simeq U(\fg[t])$.
...
\begin{question}[$E_1$-chiral $A_\infty$ of $Y_z^\hbar(\fg)$;
\ClaimStatusOpen]
\label{q:yangian-Ainfty}
\index{Yangian!$A_\infty$ structure}
Is the full $Y_z^\hbar(\fg)$ (not just its associated graded)
...
\item \emph{Filtered formality:}
 ...
 $Y_z^\hbar(\fg) \xrightarrow{\sim} (U(\fg[t]), m_2)$
 that is the identity on the associated graded. This would
 mean $Y_z^\hbar(\fg)$ is ``formal up to filtration'': the
...
t_{11}(u) & t_{12}(u) \\ t_{21}(u) & t_{22}(u)
\end{smallmatrix}\bigr)
\in \End(V) \otimes Y_z^\hbar(\mathfrak{sl}_2)[\![u^{-1}]\!]$.
...
Y_z^\hbar(\mathfrak{sl}_2)^{\otimes 2} \to
Y_z^\hbar(\mathfrak{sl}_2)
\]
...
$Y_z^\hbar(\mathfrak{sl}_2)$ is a purely topological
($E_1$-colour) datum.
\end{computation}
...
Y_z^\hbar(\mathfrak{sl}_2)^{\otimes 3} \to
Y_z^\hbar(\mathfrak{sl}_2).
\]
...
\end{equation}
The vanishing encodes the defining relation of $Y_z^\hbar(\fg)$:
...
$Y_z^\hbar(\mathfrak{sl}_2)$. For the affine lineage,
...
$[Y_z^\hbar(\fg)] \in \HH^2\bigl(U(\fg[t]),\, U(\fg[t])\bigr)$
...
$Y_z^\hbar(\fg) \xrightarrow{\sim} (U(\fg[t]), m_2)$ lies in
...
Yangian $Y_z^\hbar(\fg)$ is the rational degeneration of the
...
The Yangian $Y_z^\hbar(\fg)$ acts on $\cO$-modules on the
...
Filtered formality of $Y_z^\hbar(\fg) \xrightarrow{\sim}
...
$Y_z^\hbar(\fg)$, and the cobar recovers $\widehat{\fg}_k$.
...
\[
\sum_{g \ge 1} F_g(Y_z^\hbar(\fg))\,x^{2g}
\;=\;
...
\item The $A_\infty$ formality question
 ...
 reduce the filtered Yangian $Y_z^\hbar(\fg)$ to its associated
...
\item Construction of a ``Yangian vertex algebra'': a vertex
 algebra-like object on $Y_z^\hbar(\fg)$ encoding the spectral
