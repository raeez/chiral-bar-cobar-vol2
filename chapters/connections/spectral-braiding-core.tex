...
The braiding of two lines produces $R(z)$. Three lines $L_1,L_2,L_3$ at positions $z_1,z_2,z_3$ produce a consistency condition: the two orderings of pairwise braidings $R_{12}(z_{12})$, $R_{13}(z_{13})$, $R_{23}(z_{23})$ correspond to two maximal degenerations of $\FM_3(\C)$. Stokes' theorem on $\FM_3(\C)$ forces them to agree.
...
\label{subsec:Hopf}
The $r$-matrix determines the braiding; Koszul duality determines the algebra that the braiding acts on. Let $\mathbf B$ be the bar cooperad of $\mathsf{SC}^{\mathrm{ch,top}}$ and let $A$ be a $W(\mathsf{SC}^{\mathrm{ch,top}})$-algebra.
The bar construction $\mathsf{Bar}^{\mathrm{ch,top}}(A)$ is a coloured coalgebra over $\mathbf B$. On the boundary (open) colour, the Koszul-dual package determines a filtered Hopf-like object $\mathcal H$: the \emph{factorisation quantum group} of the theory, encoding the line-operator OPE and all higher products.
...
$R(z)$ defines a meromorphic braiding on
$\mathcal C_{\partial}^{\mathrm{red}}$, and the classical limit
$r(z)$ gives the filtered quasi-triangular local shadow of the
boundary Hopf-like object~$\mathcal H$. A global factorization quantum
group or Hopf object is not produced from this local datum alone.
\end{theorem}
...
(Theorem~\ref{thm:unconditionality}) applies: the full quantum line
complex admits a canonical $\h$-adic strong deformation retract
$(p, i, h)$, and the reduced superconnection collapses to an ordinary
logarithmic flat connection
...
The classical $r$-matrix $r(z) = k\,\Omega/z$ extracted from collision
...
along~$\R_{\ge 0}$.  The spectral $R$-matrix $R(z)$ of
...
The passage from $E_1$ to $E_2$ requires a quantum $R$-matrix $R(z)$ as
...
A \emph{meromorphic braided tensor category} is a category $\cC$ equipped with a tensor product $\Theta_z\colon \cC \otimes \cC \to \cC[\![z]\!]$ depending meromorphically on a complex parameter $z$, together with a braiding $R(z)\colon \Theta_z \to \Theta_z^{\mathrm{op}}$ that is meromorphic in $z$ and satisfies the hexagon axioms up to meromorphic correction along the diagonal. In the Vol~II framework, the ordered bar coalgebra $B^{\mathrm{ord}}(A) = T^c(s^{-1}\bar A)$ (with $\bar A = \ker \varepsilon$ the augmentation ideal) supplies a local source for such structures under the resolved hypotheses: the holomorphic colour $\C_z$ of $\SCchtop$ supplies the spectral parameter, while the topological colour $\R_{\geq 0}$ supplies the strict associativity and the ordering that generates the braid group action. This is the categorical reading of Dimofte's slab-and-fibre functor picture~\cite{Dimofte25} (PIRSA 25110067); it is not a classification theorem for all meromorphic braided tensor categories.
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item[Bridge on the overlap.] Let $\cA$ admit both presentations (e.g.\ $V_k(\fg)$ with its affine Yangian $Y_\hbar(\widehat{\fg})$ completion at generic level), and restrict to the evaluation-module core of Theorem~\ref{thm:affine-monodromy}. Conditional on the chosen quantization and Kazhdan--Lusztig comparison data, there exist gauge elements $F(z) \in (\cA^!\widehat\otimes\cA^!)[\![z^{-1}]\!]^\times$ and an associator $\Phi \in \GRT_1(\bQ)$ such that
\[
...
\noindent\textbf{Self-check.} At $k=0$: provenance (a) gives $r(z)=0$ hence $R(z)=\id$ canonically; provenance (b) admits nontrivial $R(z)$ (Yangian at $\hbar\neq 0$ is unaffected by the KM level). This confirms that (a) and (b) are genuinely distinct inputs that coincide only on the non-degenerate overlap.
\end{remark}
...
$E_3$-algebra over framed $3$-manifolds, while the present
volume computes the same data via the annular trace on the
annular bar complex $B^{\mathrm{ann}}(\cA)$
...
Section~\ref{subsec:benchmark-global-sl2}). The agreement of the
two traces on the perturbative locus (factorisation homology
...
The distinguishing feature of a factorisation quantum group is that
the braiding depends holomorphically on $z$ and is compatible with
factorisation over disjoint disks. In Latyntsev's framework
\cite{Lat23}, the full statement is a constructible cosheaf on the Ran
space $\Ran(\C)$. The present theorem proves only the local shadow:
for disks inside the chosen product-formal rectangle model, the
prefactorization algebra $\Obs^\partial$ supplies the local
factorisation maps, and the chosen $R(z)$ varies holomorphically away
from the collision divisor. The global Ran-cosheaf and representation
category comparison are not proved here.
\end{proof}
...
pole order: the Casimir pole $k\,\Omega/z$ becomes
...
The following gives a first-principles attack on
...
which is exactly the classical $r$-matrix $k\,\Omega/z = r_0/z$
...
C_\ast(\FM_\bullet(\C))$ via Kontsevich--Tamarkin formality
(with a fixed Drinfeld associator~$\Phi$, as per
Fresse--Vallette~\cite{FV20}) preserves Koszulity through the
quasi-isomorphism, giving the stated decomposition.
\end{proof}
...
universal chiral brace $B^{\mathrm{ch}}$ (Theorem~\ref{thm:chiral-brace}
...
Section~\ref{sec:spectral-R-matrix}; the existence of the half-braiding
...
chiral Higher Deligne Conjecture (Section~\ref{sec:chiral-higher-deligne});
...
operadic convolution for Koszul operads~\cite{LV12,FV20}. P1
...
$\kappa(\cA)+\kappa(\cA^!)=0$ in the Kac--Moody trace-form case; Vol~I, Theorem~C
...
$\Delta_5$. This is the spectral-braiding datum of the Vol~III K3 chiral Hall--Drinfeld
double (Vol~III the K3 synthesis).
\end{remark}
...
$\widetilde{\Phi}^{\mathrm{Sieg\text{-}Bor}}_{\mathrm{Sp}_4}[\Phi_{10}/\eta^{24}]$ satisfies the
...
\begin{remark}[Spectral braiding on $\mathbf H_{\Delta_5}^{\otimes 2}$]
\label{rem:sbc-deep-1}
The spectral braiding core for the K3 bialgebra acts as
\[
B_{\mathbf H_{\Delta_5}}(z_1,z_2;\tau,w) \;=\; (\mathrm{flip})\circ R_{\mathrm{Sieg,dyn}}(z_1,z_2;\tau,w)\ :\ \mathbf H_{\Delta_5}^{\otimes 2}\to\mathbf H_{\Delta_5}^{\otimes 2},
\]
...
(Etingof-Varchenko 1998). The spectral parameters $(z_1,z_2)$ live on
$E^{\mathrm{nod,sm}}_{24}$, the elliptic parameter $\tau$ on
$\mathfrak H_1$, and the dynamical parameter $w$ on $\mathbb C^*$.
\end{remark}

\begin{remark}[Humbert-$H_1$ monodromy of order $8$]
\label{rem:sbc-deep-2}
At the Lusztig specialisation $\hbar^2 = -1/8$ the spectral braiding
has monodromy of order $8$ around the Humbert divisor
$H_1\subset\bar{\mathcal A}_2$:
\[
B_{\mathbf H_{\Delta_5}}^{\otimes 8}\big|_{\gamma_{H_1}}\ \simeq\ \mathrm{id}_{\mathbf H_{\Delta_5}^{\otimes 2}},
\]
where $\gamma_{H_1}$ is a loop encircling $H_1$ once. This is the
braiding-level face of the Bruinier 2002 Prop.~5.1 Heegner Chern-class
reciprocity: the $\mathbb Z/8$-class in $\mathrm{CH}^1(H_1)$ manifests
as spectral braiding monodromy order. Primary-lit: Bruinier 2002
(arXiv:math/0108079), Etingof-Varchenko 1998, Pasol-Zagier 2013.
\end{remark}
