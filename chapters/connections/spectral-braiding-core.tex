...
(Theorem~\ref{thm:YBE}).  The complete
...
classification: $\beta\gamma$ and $bc$ systems are Koszul
with $c_0 = 0$ \textup{(}full decoupling\textup{)}; affine
Kac--Moody $\hat{\fg}_k$ is Koszul but has $c_0 \ne 0$
\textup{(}bar-complex decoupling only, propagator
entanglement at genus~$\ge 1$\textup{)}.
\end{theorem}
