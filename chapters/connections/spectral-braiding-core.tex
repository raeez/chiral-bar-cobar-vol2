...
A vector-valued curvature does not commute with the braiding:
different line operators $L_i$ couple to different components
$\kappa^{(j)}$ via the action of $H^2(\bar{B}(\cA))$ on
...
$\textup{(ii)} \Leftrightarrow \textup{(iii)}$,\quad
...
(ii)~$\Leftrightarrow$~(iii): Koszulness forces
$H^*(\bar{B}(\cA))$ to be concentrated in bar degree~$1$.
In particular $H^2(\bar{B}(\cA))$ lies entirely in bar
degree~$1$.  Conversely, if $H^2(\bar{B}(\cA))$ is concentrated
in bar degree~$1$, then the bar complex has no higher-degree
generators contributing to cohomological degree~$2$, which
(combined with the bar-cobar adjunction) gives Koszulness.
