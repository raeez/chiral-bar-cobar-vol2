...
$\SCchtop^{!}$-algebra structure (equivalently, a $(\mathrm{Lie},\mathrm{Ass})$-algebra structure, since $\SCchtop^{!}$ has closed sector $\operatorname{Com}^! = \operatorname{Lie}$ and open sector $\operatorname{Ass}^! = \operatorname{Ass}$; Proposition~\ref*{prop:sc-koszul-dual-three-sectors}):
\begin{enumerate}[label=\textup{(\roman*)}]
