...
When $r(z)=k\,\Omega/z$ (the affine collision residue at level~$k$; AP126), this is exactly the classical rational KZ connection.
...
If $r(z)=k\,\Omega/z+O(1)$ at level~$k$ near $z=0$ \textup{(}AP126\textup{)}, and $\Omega_{ij}$ denotes the action of $\Omega$ in factors $i,j$, then
\[
...
\[
  z_i-z_j = \Big(\prod_{e\in P(i,j)}t_e\Big)u_{ij},
\]
with $u_{ij}$ a unit. Since $r(z)=k\,\Omega/z+O(1)$ at level~$k$ \textup{(}AP126\textup{)}, one has
\[
...
identifies $k\,\Omega$ as the collision residue, consistent with
