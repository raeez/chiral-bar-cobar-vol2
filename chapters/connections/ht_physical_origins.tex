...
frontiers of this volume.  We now prove a structural criterion that
...
by which Koszulness fails.  The remaining question is: for
\emph{non-affine} HT theories (where the bulk quantization is not
one-loop exact), what is the relationship between the loop complexity
$\mathfrak{L}(\cA)$ and the bar cohomological width?
Theorem~\ref{thm:one-loop-koszul}(4) gives a partial answer: the
width is bounded by $\mathfrak{L}(\cA)+1$.  The full characterization
requires the jet-order-$3$ reflected weight identity
(Conjecture~\ref{conj:cubic-rwi} of
Chapter~\ref{chap:affine-half-space-bv}) to determine whether
non-affine bulk theories can be one-loop exact.
\end{remark}
