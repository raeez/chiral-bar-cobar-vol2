...
% RECTIFICATION-FLAG: [gate 0/1, numerical] The literal product $24\cdot 2\cdot\chi(\cO_{K3})$ with $\chi(\cO_{K3})=2$ evaluates to $96$, not $24$; and the parenthetical gloss says "$12$ nodes" while this file (and rem:htpo-1, rem:htpo-2) elsewhere uses $24$ Kodaira $I_1$ fibres. The source bv_brst.tex:3346 carries the same surface form. The graph-weight bookkeeping that produces the cancellation $=\chi(K3)=24$ needs to be made explicit and arithmetically consistent (node count + per-node residue weight). Not repaired here: cannot reconstruct the intended factorisation without inventing it.
