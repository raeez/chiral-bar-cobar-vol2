...
this programme and must be distinguished whenever the recognition
Theorem~\ref{thm:yangian-recognition} is invoked. The weaker
notion (Definition~\ref{def:e1-chiral-yangian}) asks only for an
...
The 3d HT theories enumerated in the present
...
algebras), in the part inscribing ``The CY Landscape''. The correspondence
...
the present chapter inscribes the HT side, the Volume~III CY
Landscape part inscribes the CY side, and the two sides meet through $\Phi$ and
...
\begin{remark}[Evidence via AGT correspondence]
...
\begin{remark}[Scope]
\ClaimStatusConjectured{} because the 4d gauge theory origin of W-algebras (Costello--Gaiotto) requires the holomorphic twist and infrared localization, which are physics inputs beyond the scope of this monograph. The bar complex computing W-algebra conformal blocks follows from Volume~I; the live task is the gauge-theoretic comparison.
...
beyond the algebraic constructions of Volume~I.
...
\subsection{Honest frontier: HT origins not yet inscribed}

\begin{remark}[HT landscape silent non-coverage, mirroring Volume~I]
\label{rem:ht-landscape-silent-non-coverage}
\index{HT landscape!silent non-coverage}
Volume~I's master landscape table carries a Remark
(``Silent non-coverage of the master table'') listing five
chiral-algebra families that are inscribed in the shadow-obstruction
tower census but do not yet receive a complete row in the master
invariant table. The HT landscape inherits the same honest
frontier: each such family has a shadow-level fingerprint but a
3d HT origin that is either conjectural, restricted to a sub-lane,
or presently absent. They are inscribed here as honest frontier,
not silently skipped.
\begin{enumerate}[leftmargin=*]
\item $\cN{=}2$ superconformal $\mathrm{SCA}_c$ (class~$M$,
self-dual $c^{*}=9$, Ito--Tanaka involution $c\mapsto 6-c$): the
...
the 3d HT origin of the full $\cN=2$ SCA on its own Ito--Tanaka
involution (not as a sublane of small $\cN=4$) is not inscribed
here and awaits a direct holomorphic-twist construction.
...
programme but is not inscribed in this chapter; the boundary
...
the indefinite case corresponds physically to 3d HT theories
with non-compact target and is presently open.
...
admissible levels are not inscribed separately; they sit under
the affine Kac--Moody row with a level restriction.
\end{enumerate}
The Schellekens $71$ holomorphic VOAs at $c=24$ are covered
through their Monster / Leech row (see the dedicated
\texttt{schellekens\_71\_alpha\_classification\_platonic.tex}
chapter); they are not re-inscribed in
Part~\ref{part:ht-landscape}. This remark is the Volume~II
companion to Volume~I's Rem~``Silent non-coverage of the master
table''.
\end{remark}
...
\section{Synthesis}
\label{sec:htpo-wave13-DNA}

\begin{remark}[M-theory physical origins of K3 synthesis]
\label{rem:htpo-DNA-1}
The the K3 synthesis twisted $11$D SUGRA on $K3\times T^2$ with $24$ M$5$-branes
on Kodaira $I_1$ fibres is the M-theoretic origin of the K3 chiral
bialgebra $\mathbf H_{\Delta_5}$: the $24$ M$5$-branes at the $24$
Kodaira $I_1$ fibres of $E^{\mathrm{nod,sm}}_{24}$ correspond, under
compactification to $4d$ class-$\mathcal S$ $A_1$ on $\Sigma_{0,24}$,
to the $24$ flavour $\mathfrak{su}(2)$ punctures. Four duality frames
(heterotic, IIA, IIB, M-theory) all produce $\Delta_5$ as the K3-BKM
classifying automorphic form \cite{EguchiOoguriTachikawa2010,Gaiotto2009}.
Cross-ref Vol.~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Heterotic / IIB / IIA / M-theory universal weight-$5$]
\label{rem:htpo-DNA-2}
...
$\chi(\cO_{K3}) = 2$ \cite{KugaSatake1967,Borcherds1992}. Cross-ref
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\section{physical fibre functor and Mukai grading}
\label{sec:htpo-wave15-DNA}

\begin{remark}[Physical realisation of the Tannakian fibre functor
$\omega$]
\label{rem:htpo-wave15-physical-omega}
the K3 synthesis Gelfand identifies the Tannakian fibre functor on
...
origin makes the Tannakian reconstruction
...
direction]
\label{rem:htpo-wave15-mukai-direction}
...
not categorical. Cross-ref Vol~III
Remark~\ref{rem:cydkappa-wave15-mukai-grading-direction} and the
cache entry W15-Mukai-grading-direction.  Primary:
...
\label{rem:htpo-wave15-gerbe-physical}
The $\mu_8$-gerbe obstructing neutrality of the Tannakian fibre on
$H_1 \cup H_4 \subset \overline{\mathcal A}_2$ is the physical
anomaly of the twisted 11D SUGRA partition function when the
parameter $Z\in\overline{\mathcal A}_2$ crosses a Humbert wall.
Concretely, at $H_1$ (discriminant $1$) the Siegel period $Z$
acquires a rank drop in $\mathrm{Im}(Z)$ -- the two genus-1 moduli
collide -- and the 1-loop-anomaly cancellation that forced
$\Delta_5$ (the K3 synthesis C.9) requires a gauge redefinition of the
...
Cross-ref Vol~III Remark~\ref{rem:cydkappa-wave15-gerbe}.  Primary:
...
\section{the K3 synthesis Costello DNA: CoHA-to-chiral discipline and
two-loop BV origin}
\label{sec:htpo-wave15-costello-DNA}

\begin{remark}[CoHA is $E_1$-associative, not $E_1$-chiral; $\Phi$
mediates]
\label{rem:htpo-wave15-coha-not-chiral}
\index{CoHA!not chiral|textbf}
\index{Phi functor!CoHA to chiral|textbf}
Cache-pattern AP-CY$7$ requires explicit distinction between the
...
Vol~III. The correct statement of the Wave~$13$ Hall--Drinfeld
double identification, as displayed on the right-hand side of the
Wave~$13$ synthesis formula, is
\[
 \mathbf{H}_{\Delta_5}
 \;=\;
...
Chapter~\ref{ch:cy-to-chiral}; without it, ``CoHA $=$ chiral algebra''
is a type error conflating two distinct $E_1$-structures. The Hall
convolution lives on $\mathrm{D}^b_{\mathrm{Coh}}(K3\times E)$ and its
Drinfeld double $\mathcal{D}_\hbar$; the chiral algebra lives on
$\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24})$ after $\Phi$-transport.
\end{remark}

\begin{remark}[Construction arrow requires $\Phi$; Wave~$15$
cache-append AP-CY$57$]
\label{rem:htpo-wave15-construction-arrow}

Beyond the CoHA vs chiral discipline of
Remark~\ref{rem:htpo-wave15-coha-not-chiral}, the deeper cache pattern
AP-CY$57$ (construction arrow explicit, not narration-implicit) requires
...
usage throughout Wave~$13$--$15$ inscription:
\begin{itemize}
...
\item \emph{never} ``$\mathrm{CoHA}_{K3\times E}$ is a chiral algebra''
(conflation; type error).
\end{itemize}
The analogue for $W$-algebras (AP-CY$7$ variant): apply the
Feigin--Frenkel Drinfeld--Sokolov reduction arrow explicitly;
$W_N$-algebras are $E_1$-chiral outputs of DS reduction applied to
affine Kac--Moody, not raw CoHA outputs. Wave~$15$ inscribes this as
standing discipline for cross-volume inscription; every cross-volume
label bearing ``CoHA'' or ``Hall algebra'' in Vol~II/III should match the
arrow-explicit form.
\end{remark}

\begin{remark}[Two-loop BV origin; holomorphic-anomaly cancellation]
\label{rem:htpo-wave15-two-loop}
\index{2-loop BV!holomorphic anomaly}
Wave~$14$ inscribed the $1$-loop origin of $\Delta_5$ as BV quantum
master-equation datum; Wave~$15$ pushes the inscription to $\hbar^2$
with obstruction-free verification. See Vol~I
Proposition~\ref{prop:bvbrst-wave15-2loop-obstruction} in
$bv\_brst.tex$: the two-loop obstruction class
$\mathrm{obs}^{(2)} \in H^2(\mathfrak{m}_{K3}) \cong \bC\cdot[\Delta_5]$
vanishes, with the $\chi(K3) = 24$ theta-graph contribution cancelling
the $24\cdot 2\cdot\chi(\cO_{K3}) = 24$ dumbbell-plus-figure-eight
contributions (twice $\chi(\cO_{K3}) = 2$ per Kodaira $I_1$ residue
summed over $12$ nodes). The physical origin of this cancellation:
the M$5$-brane count $24$ exactly matches the K3 Euler characteristic
$\chi(K3) = 24$, so the $2$-loop holomorphic anomaly is trivial, and
the partition-function match to $\log\Phi_{10}$ at order $p^2$
reproduces the Costello $2015$ ``Holography for $\Omega$-deformed
M$5$-brane'' \cite{Costello2015omega} result through the Harvey--Moore
$1996$ heterotic dual. At $\hbar^3$ the anomaly becomes non-trivial,
producing the $\Phi_{10}/\eta^{24}$ associator twist (Wave~$13$
D.$4$--$5$).
\end{remark}
