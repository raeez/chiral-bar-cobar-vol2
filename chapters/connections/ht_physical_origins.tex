...
\begin{theorem}[Localization to holomorphic data]\label{thm:ht-localization}
...
\begin{remark}[Measure-theoretic input]
The identification of the BV path integral with the bar-cobar pairing
requires a measure-theoretic framework for configuration-space
integrals with BV gauge-fixing data. The comparison uses a gauge-fixing
Lagrangian to choose the regularization of the FM compactification and
pairs $\bar{B}^{\mathrm{ch}}(\cA)$ with
$\Omega^{\mathrm{ch}}(\mathcal{C})$ by the resulting configuration-space
integral.
\end{remark}
...
\cite{EguchiOoguriTachikawa2010,Gaiotto2009}. Cross-ref Vol.~III
...
$24\cdot 2\cdot\chi(\cO_{K3}) = 24$ dumbbell-plus-figure-eight
contributions (twice $\chi(\cO_{K3}) = 2$ per Kodaira $I_1$ residue
summed over $12$ nodes). The scalar reason is the equality of the
...
$\Phi_{10}^{\mathrm{un}}/\eta^{24}$ associator twist.
\end{remark}
