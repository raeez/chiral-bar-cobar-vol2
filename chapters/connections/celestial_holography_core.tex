...
The bulk side is the $4$d HT-twist programme of Costello--Li--Paquette~\cite{CostelloLiSugraTwist, CostelloPaquetteTwistedSugraStates}; the boundary side is the celestial OPE formalism of Pasterski--Shao--Strominger~\cite{PasterskiShaoStromingerFlatSpaceHolog, StromingerLectures2018}; the observable hierarchy is Hamada--Shiu--Li--Strominger~\cite{HamadaShiuSubsubSoft,LiStromingerHigherSoft}.
...
Weinberg's soft graviton theorem at $r=2$ and Cachazo--Strominger's subleading theorem at $r=3$ occupy the first two shadow depths. Hamada--Shiu~\cite{HamadaShiuSubsubSoft} and Li--Strominger~\cite{LiStromingerHigherSoft} extend the soft-graviton residue hierarchy; the shadow tower supplies the associated graded and the comparison cone whose vanishing produces the exact filtered identity at each depth.
