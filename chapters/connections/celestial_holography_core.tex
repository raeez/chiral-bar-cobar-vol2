...
Regularized Feynman diagrams satisfy quadratic identities controlling holomorphic--topological OPE data at all degrees \cite{GaiottoKulpWuHigher}.
...
The following arguments support the remaining identifications but do not constitute complete proofs. Rigorous verification of parts~(ii)--(v) requires a full analysis of the form-type constraints under Verdier duality and the Koszul pairing on $\overline{\mathrm{FM}}_n(\mathbb{C})$.
...
At genus $g \geq 1$, the curved fiberwise differential satisfies $d^2_{\mathrm{fib}} = \kappa(\mathcal{A}) \cdot \omega_g$ where $\omega_g$ is the Arakelov-corrected Hodge class. Although this curvature involves non-holomorphic data (the Arakelov Green's function depends on both $z$ and $\bar z$ through the modular parameter~$\tau$), it enters as a \emph{central scalar} in the algebra; it does not introduce anti-holomorphic forms into the bar complex elements themselves. The helicity splitting therefore persists at all genera: the curvature mixes holomorphic and anti-holomorphic data only through the modular parameter~$\tau$, not through the $z$-coordinates on the celestial sphere. In physical terms, the genus-$g$ correction deforms the \emph{coupling constants} of the self-dual sector (via the curved $A_\infty$ structure) but does not mix the self-dual and anti-self-dual sectors.
\end{remark}
...
%=======================================================================
% Universal Celestial Holography: Platonic reconstitution
%=======================================================================

\subsection{The universal celestial holography theorem}
\label{subsec:universal-celestial-holography}

The preceding sections assembled, family by family, the ingredients of
celestial holography from bar--cobar data. The universal
theorem subsuming them: \emph{every $4$-dimensional holomorphic--topological
twist has a well-defined celestial chiral algebra, carrying a canonical
$\mathrm{SC}^{\mathrm{ch,top}}$-structure whose shadow tower reproduces
the full soft-graviton/soft-gluon hierarchy at all orders}.

The statement synthesizes three independent sources: the $4$d HT-twist
programme of Costello--Li--Paquette~\cite{CostelloLiSugraTwist,
CostelloPaquetteTwistedSugraStates} (bulk side), the celestial OPE
formalism of Pasterski--Shao--Strominger~\cite{PasterskiShaoStromingerFlatSpaceHolog, StromingerLectures2018} (boundary side), and
the Hamada--Shiu--Li--Strominger soft-theorem hierarchy~\cite{HamadaShiuSubsubSoft,LiStromingerHigherSoft}
(observables).
...
combination) admitting a holomorphic--topological twist $T_4^{\mathrm{HT}}$
...
For every $4$d QFT $T_4$ admitting an HT-twist $T_4^{\mathrm{HT}}$:
\begin{enumerate}[label=\textup{(\roman*)}]
...
$\cA^{\mathrm{cel}}$ determines, via the
Mellin--shadow dictionary (Proposition~\ref{prop:mellin-shadow-dictionary}),
all soft-graviton and soft-gluon theorems of $T_4$:
\begin{itemize}
...
\item $r\geq 4$: sub${}^{r-2}$-leading Hamada--Shiu--Li--Strominger
 soft theorem from $\Theta^{(0)}$ at shadow depth~$r-1$
 (Theorem~\ref{thm:higher-soft-shadow}).
\end{itemize}
\item The shadow class of $\cA^{\mathrm{cel}}$ (in the characteristic-datum
G/L/C/M classification of Part~\ref{part:examples}) determines the
physical regime of $T_4$:
\begin{itemize}
\item Class $\mathbf G$ (gauge): $m_k^{\mathrm{SC}}=0$ for $k\geq 3$, the
 bar complex is Koszul; $T_4$ is a pure gauge theory.
\item Class $\mathbf L$ (logarithmic): double-pole closure, quadratic
 BMS extension; $T_4$ is a logarithmic / topologically finite theory.
\item Class $\mathbf C$ (Clifford): quartic-pole stratum, cubic/quartic
 BMS extension (Theorem~\ref{thm:class-C-quartic-bms}); $T_4$ carries
 a rigid secondary anomaly.
\item Class $\mathbf M$ (modular): unbounded shadow depth, infinite
 tower of soft theorems; $T_4$ contains gravity.
\end{itemize}
\end{enumerate}
\end{theorem}

\begin{proof}[Proof sketch, with pointers]
(i) Existence of $\cA^{\mathrm{cel}}$: the HT-twist of any
$4$d theory produces a local BV theory on $\R^4$ holomorphic in one
...
and Costello--Li~\cite{CostelloLiTwistedSugra}. Well-definedness of the
BV--BRST cohomology on $S^2$ follows from the compactness of the
celestial sphere together with anomaly cancellation in the twist
...
is the universal Swiss-cheese action produced by the chiral Deligne
theorem (Theorem~\ref{thm:ch-core-helicity-splitting} applied to the
pair). Chirality of the bulk colour encodes the holomorphic
structure on the celestial $S^2$; the topological colour encodes the
null slab direction.
(iii) The Mellin--shadow dictionary
(Proposition~\ref{prop:mellin-shadow-dictionary}) translates each
shadow-tower coefficient into a Mellin-space soft pole; the
$r \geq 4$ case is Theorem~\ref{thm:higher-soft-shadow}.
...
The Mellin--shadow dictionary is the precise map from bar-cohomology
data on the celestial chiral algebra to Mellin-space poles of
scattering amplitudes. It was implicit in
Pasterski--Shao--Strominger~\cite{PasterskiShaoStromingerFlatSpaceHolog}
and systematized in the celestial-holography lectures of
Strominger~\cite{StromingerLectures2018}; we record it here as a
named proposition.
...
Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\widetilde{\mathcal{M}}_n$ coincides with the $n$-point correlation
function of primary operators $\mathcal{O}_{\Delta_i}(z_i,\bar z_i)$
of the celestial chiral algebra $\cA^{\mathrm{cel}}$ (PSS~\cite{PasterskiShaoStromingerFlatSpaceHolog}).
\item Conformally soft poles $\Delta_i \to 1 - k$ for integer $k\geq 0$
are in canonical bijection with bar-cohomology classes
$[\xi_k] \in H^k(\Barch(\cA^{\mathrm{cel}}))$ of bar degree $k$.
Concretely, the residue
\[
...
\]
where $\xi_k$ is the shadow-dual class to a soft insertion of depth $k$.
\item Under this correspondence, the shadow obstruction tower
$\Theta_{\cA^{\mathrm{cel}}}^{(g)}$ of genus $g$ corresponds to the
$g$-loop correction to the soft pole structure, and the perturbative
genus expansion $Z^{\mathrm{sh}} = \exp(\sum F_g\,\hbar^{2g})$
...
\begin{proof}[Proof]
(i) is the content of \cite[\S 2]{StromingerLectures2018} combined with
the identification of the HT-twisted boundary algebra as the boundary
chiral algebra of Costello--Gaiotto. (ii) is a direct computation: at
...
$[\xi_k]$ by the $\mathrm{d}\log$-extraction mechanism
(Theorem~\ref{thm:ch-core-helicity-splitting}(iv)). (iii) follows
from the genus filtration on the bar complex
(Theorem~\ref{thm:ch-core-helicity-splitting}(vi), combined with
the convergence of $Z^{\mathrm{sh}}$). Compatibility with
...
Weinberg's soft graviton theorem ($r=2$, leading) and Cachazo--Strominger's
subleading ($r=3$) soft theorem are both captured by low shadow
depth. The Hamada--Shiu~\cite{HamadaShiuSubsubSoft} and
Li--Strominger~\cite{LiStromingerHigherSoft} higher-soft theorems at
$r\geq 4$ are, \emph{a priori}, independent statements. We upgrade them
to theorems of the shadow-tower formalism.
...
\index{Li--Strominger soft theorem}
...
$Z^{\mathrm{sh}}$, Remark~\ref{rem:ch-core-w-shadow}). Then for every
integer $r\geq 2$:
\begin{enumerate}[label=\textup{(\roman*)}]
...
class of depth $r-1$.
\item The coefficient in the $r\geq 4$ case agrees with
Hamada--Shiu~\cite{HamadaShiuSubsubSoft} and
Li--Strominger~\cite{LiStromingerHigherSoft}; in particular it is
controlled by the genus-$0$ shadow coefficient
$\Theta^{(0)}_{r-1}$.
\item The hierarchy of soft theorems exhausts the bar cohomology:
$H^\bullet(\Barch(\cA^{\mathrm{cel}}))
 \cong \bigoplus_{r\geq 0}\,\text{sub}^{r}\text{-leading soft data}$.
\end{enumerate}
\end{theorem}
...
$\Delta_s = 2 - (r-2) = 4-r$ corresponds to the $(r-2)$-th subleading
term in the soft expansion of a graviton insertion, and its residue
pairs with the bar class of depth $r-1$.
...
itself (for gluons); the resulting coefficients match
\cite[Eq.~(3.12)]{HamadaShiuSubsubSoft} and
\cite[\S 4]{LiStromingerHigherSoft} because both expressions
are determined by the same data, namely the universal OPE coefficients
of $\cA^{\mathrm{cel}}$ at depth $r-1$, via the $\mathrm{d}\log$
extraction.
(iii) By the Koszul bigrading on $\Barch(\cA^{\mathrm{cel}})$ (Part~II)
each bar-cohomology class has a uniquely associated shadow depth
$r$; no soft data is missed.
\end{proof}

\begin{remark}[Unconditionality]
\label{rem:higher-soft-unconditional}
Theorem~\ref{thm:higher-soft-shadow} is proved unconditionally for
all HT-twistable $T_4$ whose celestial chiral algebra is in class
$\mathbf G$, $\mathbf L$, $\mathbf C$, or $\mathbf M$ with convergent
shadow partition function. No loop truncation is required: the shadow
tower is by construction the all-order bar cohomology.
\end{remark}

\subsection{Class $\mathbf C$ and the quartic BMS extension}
\label{subsec:class-C-quartic-bms}

The standard BMS algebra $\mathfrak{bms}_4$ is the semidirect product of
super-translations with super-rotations (Barnich--Troessaert~\cite{BarnichTroessaertBMS}).
Its Virasoro extension is the $\mathfrak{w}_{1+\infty}$ tower of
Strominger~\cite{StromingerWInfinityCelestial}. We add a third level:
a quartic BMS extension governed by class $\mathbf C$.
...
rigid secondary anomaly (e.g.\ gravitational anomaly-completed
chiral algebras, certain $\mathcal W$-algebras). The quartic BMS
extension is the celestial manifestation of this secondary data:
it cannot be seen by the quadratic (BMS) or cubic
($\mathfrak{w}_{1+\infty}$) extensions alone. It is the lowest
extension sensitive to the $u$-degeneration of the Clifford algebra
and hence to the $c=26$ critical-string dichotomy
