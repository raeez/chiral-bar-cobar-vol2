...
gravity) encode the associated-graded celestial OPE data through their bar complexes. The
collision residues of the bar differential $d_{\barBch}$ extract these OPE coefficients via
the $d\log$ absorption mechanism: the bar kernel $d\log(z_1 - z_2)$ absorbs one pole order,
so an OPE singularity $\sim z^{-N}$ produces an $r$-matrix pole $\sim z^{-(N-1)}$. Loop and
quantum corrections require the genus-filtered comparison maps of
...
\begin{theorem}[Tree-level gluon OPE transfer from the bar residue]
\label{thm:ch-core-celestial-gluon-ope}
Let $\mathcal{O}^{+,a}_{\Delta}(z)$ denote the celestial primary of a positive-helicity gluon
of colour~$a$ and conformal dimension~$\Delta$. Assume the tree-level Mellin comparison map
\[
\Phi^{\mathrm{tree}}_{\mathrm{Mell}}\colon
\gr^1\barBch(\widehat{\fg}_k)\longrightarrow
\gr^1\mathrm{Soft}^{(0)}_{\mathrm{HT}}
\]
diagonalizes the affine collision residue in this primary basis and is normalized so that the
colour projection of $k\Omega$ is $f^{ab}{}_{c}$. Then the tree-level associated-graded
celestial OPE is
\[
...
\begin{proof}
The collision $r$-matrix $r^{\mathrm{KM}}(z)=k\Omega/z$ is the simple-pole bar residue after
$d\log$ absorption. Applying $\Phi^{\mathrm{tree}}_{\mathrm{Mell}}$ and the stated colour
normalization sends $k\Omega$ to $f^{ab}{}_{c}$. The Mellin convolution of two positive-energy
external legs shifts conformal dimension by
\(\Delta_1+\Delta_2-1\). This gives the displayed associated-graded tree-level OPE. Positive
genus corrections are separate classes
\(\Obs^{\mathrm{Mell}}_{g,1}\in H^2(\Cone(\mu_{g,1}))\).
\end{proof}

\begin{remark}[Comparison with Fan--Fotopoulos--Taylor]
\label{rem:ch-core-fft}
This is the bar-complex form of the celestial gluon OPE obtained by direct Mellin transform
of the Parke--Taylor amplitude. Here $f^{ab}{}_c$ is extracted from the collision residue of
the bar complex via the $d\log$ absorption mechanism.
\end{remark}
...
\begin{theorem}[Tree-level graviton OPE transfer from the bar residue]
\label{thm:ch-core-celestial-graviton-ope}
Assume the tree-level Mellin comparison for $\barBch(\mathrm{Vir}_c)$ sends the residue
$2T$ to the Virasoro zero-mode action on a positive-helicity graviton primary and sends the
central residue $c/2$ to the central triple-pole transfer. Then the tree-level
associated-graded celestial OPE is
\[
...
\begin{proof}
The pole $r_1 = 2T$ acts on a celestial primary of dimension $\Delta_2$ via the Virasoro zero
mode, yielding the coefficient $(\Delta_2 - 1)$ after the spin-$2$ conformal weight shift.
The pole $r_3 = c/2$ is central and produces the triple-pole term with output dimension
\(\Delta_1+\Delta_2-3\). The absence of a $z^{-2}$ pole is the bar-complex manifestation of
the $d\log$ absorption of the $\partial T$ term. The Virasoro $\lambda$-bracket is cubic in
\(\lambda\), so no higher tree-level bar residue appears. Positive-genus corrections are
controlled by the classes
...
The bar complex sees the holomorphic helicity sector. The anti-holomorphic sector enters only
after a Verdier/Koszul comparison datum is supplied.
...
\[
d^2_{\mathrm{fib}}=\kappa(\mathcal A)\cdot\omega_g
\]
...
The preceding sections assemble the bar--cobar ingredients of celestial transfer. The
$d\log$ part of the Mellin transform identifies the associated graded of the HT-twisted soft
expansion with the shadow tower of the celestial chiral algebra. Exact equality of filtered
...
poles of HT-twisted scattering amplitudes. Its associated graded is the $d\log$ extraction
map; the filtered statement requires the comparison cone.
...
expressions are determined by the vertex-algebra OPE coefficients of
...
Strominger~\cite{StromingerWInfinityCelestial}. Class~$\mathbf C$ supplies the third
operation: a quartic BMS extension.
