...
celestial holography from bar--cobar data. The universal statement is a
filtered theorem. The $d\log$ part of the Mellin transform identifies
the associated graded of the HT-twisted soft expansion with the shadow
tower of the celestial chiral algebra; exact equality of the filtered
objects is the vanishing theorem for the corresponding comparison cone.
...
QME anomaly vanishes. Assume the celestial gluing class
\[
...
affine celestial chart to $\PP^1_{\mathrm{cel}}$. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
...
map of Proposition~\ref{prop:mellin-shadow-dictionary}:
\begin{itemize}
...
\end{itemize}
\item For each genus $g$ and shadow depth $r$ let
\[
\mu_{g,r}\colon
...
\]
be the $d\log$ Mellin comparison map. The exact filtered soft theorem
...
shadow-tower coefficient into the associated graded of a Mellin-space
soft pole. Exact filtered equality is standard obstruction theory for
the cone of $\mu_{g,r}$, giving the displayed
...
The Mellin--shadow dictionary is the filtered comparison from
bar-cohomology data on the celestial chiral algebra to Mellin-space
poles of HT-twisted scattering amplitudes. Its associated graded is
the $d\log$ extraction map.
...
holds exactly when
\[
\Obs^{\mathrm{Mell}}_{0,k}(T_4)
\in H^2\!\left(
\Cone\!\left(
\mu_{0,k}\colon
\gr^kH^\bullet(\Barch(\cA^{\mathrm{cel}}))
\to
\gr^k\mathrm{Soft}^{(0)}_{\mathrm{HT}}(T_4)
\right)\right)
\]
...
filtered comparison is the lifting problem for $\mu_{0,k}$, with
obstruction and ambiguity in the displayed cone cohomology. (iii)
follows from the genus filtration on the bar complex together with
...
Weinberg's soft graviton theorem ($r=2$, leading) and
Cachazo--Strominger's subleading ($r=3$) soft theorem are the first
two shadow depths. Hamada--Shiu~\cite{HamadaShiuSubsubSoft} and
Li--Strominger~\cite{LiStromingerHigherSoft} give the higher
soft-graviton residues; the shadow tower supplies the associated
graded and the cone criterion for exact filtered equality.
...
\cite[\S 4]{LiStromingerHigherSoft} because both associated-graded
expressions are determined by the universal OPE coefficients of
$\cA^{\mathrm{cel}}$ at depth $r-1$, via the $\mathrm{d}\log$
extraction. The filtered lift is governed by
...
Theorem~\ref{thm:higher-soft-shadow} gives the exact all-order
soft hierarchy in the HT-twisted perturbative sector. For classes
