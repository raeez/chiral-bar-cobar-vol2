...
\label{sec:bvht-ext-wave13-DNA}

\begin{remark}[BV--HT extension and the Wave~13 one-loop exactness]
\label{rem:bvht-ext-DNA-1}
The BV--HT extension technology applied to twisted 11D SUGRA on
$K3\times T^2$ yields, at the Wave~13 base point, an exact one-loop
result: the BV partition function
\[
Z^{\mathrm{BV}}_{\mathrm{Wave13}} \;=\; \int_{\mathcal L_{\mathrm{BV}}} e^{S_{\mathrm{HT}}^{\mathrm{Wave13}}/\hbar}
\]
evaluated on the HT-Lagrangian $\mathcal L_{\mathrm{BV}}\subset\mathbf H_{\Delta_5}$
truncates at one loop because the Wave~13 anomaly extension classification
$H^2(\mathfrak g_{\Delta_5})^{\mathbb Z/2,K(1)}\cong\mathbb C\cdot\Delta_5$
(Vol.~II, \S\ref{sec:anom-ext-wave13-DNA}) gives only a one-dimensional
...
path integral, and realises the Wave~13 refinement of the Gritsenko
