...
DS boundary condition~\cite{costello-gaiotto}. For any nilpotent
...
\begin{proposition}[BV partition function as bar-complex integral]
...
measure. Then the BV partition function $Z_g(\cA)$ equals the
bar-cobar pairing
\[
Z_g(\cA) = \langle \barBch(\cA)^{(g)},
 \Omegach(\cA^i)^{(g)} \rangle,
\]
evaluated at genus $g$.
\end{proposition}
...
\begin{corollary}[Genus-1 Heisenberg from bar-cobar]
...
The shadow obstruction tower contributions at degree $r \geq 3$ are the
...
$K3\times T^2$ yields, at the base point, a one-dimensional anomaly
line. The corresponding BV functional is
\[
Z^{\mathrm{BV}}_{K3} \;=\;
\int_{\mathcal L_{\mathrm{BV}}} e^{S_{\mathrm{HT}}^{K3}/\hbar}
\]
evaluated on the HT-Lagrangian $\mathcal L_{\mathrm{BV}}\subset\mathbf H_{\Delta_5}$
and its first quantum obstruction lies in the line
$H^2(\mathfrak g_{\Delta_5})^{\mathbb Z/2,K(1)}\cong\mathbb C\cdot\Delta_5$
(Vol.~II, \S\ref{sec:anom-ext-supplement}). The one-loop exact form is
therefore the sharp BV statement: after fixing the $\Delta_5$ anomaly
counterterm, higher loop terms are absent precisely when the completed
HT graph-complex obstruction classes vanish in the complement of this
line. This is the BV--HT form of the elliptic-genus one-loop mechanism
for M-theory on $K3\times T^2$, refined by the additive Borcherds lift
normalisation of Gritsenko. Primary-lit: Witten 1995,
Costello--Gwilliam 2017, Gwilliam--Williams 2020, Gritsenko 1995,
