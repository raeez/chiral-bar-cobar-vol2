...
\begin{proposition}[BV partition function as bar-complex integral; \ClaimStatusConditional]
...
\begin{remark}[BV--HT extension and the one-loop exactness]
\label{rem:bvht-ext-1}
The BV--HT extension technology applied to twisted 11D SUGRA on
$K3\times T^2$ yields, at the base point, an exact one-loop
result: the BV partition function
\[
Z^{\mathrm{BV}}_{\mathrm{the K3 synthesis}} \;=\; \int_{\mathcal L_{\mathrm{BV}}} e^{S_{\mathrm{HT}}^{\mathrm{the K3 synthesis}}/\hbar}
\]
evaluated on the HT-Lagrangian $\mathcal L_{\mathrm{BV}}\subset\mathbf H_{\Delta_5}$
truncates at one loop because the anomaly extension classification
$H^2(\mathfrak g_{\Delta_5})^{\mathbb Z/2,K(1)}\cong\mathbb C\cdot\Delta_5$
(Vol.~II, \S\ref{sec:anom-ext-supplement}) gives only a one-dimensional
space of independent loop contributions. This matches the Witten 1995
M-theory-on-$K3\times T^2$ one-loop exactness of the elliptic-genus-type
path integral, and realises the refinement of the Gritsenko
1995 additive lift in the BV--HT framework. Primary-lit: Witten 1995,
Costello--Gwilliam 2017, Gwilliam--Williams 2020 (HT-BV quantisation),
Gritsenko 1995, Etingof--Kazhdan 2007 Part~V.
\end{remark}
