...
pattern \(F_g = \kappa_{\mathrm{ch}}(\cA)\cdot
\lambda_g^{\mathrm{FP}}\) of Theorem~D.
...
$\overline{\mathcal{A}_2}$ forces the primitive BKM denominator
$\Delta_5$ as the unique Deligne-$H^2$ trivialiser, and
\[
...
$\log(\Phi_{10}^{\mathrm{un}}/\eta^{24})$:
\begin{equation}\label{eq:bv-3loop-obstruction-class}
 c_3
 \;=\;
 \bigl[\log(\Phi_{10}^{\mathrm{un}}/\eta^{24})\bigr]\big|_{p^3}
 \;=\;
...
$\Phi_{10}^{\mathrm{un}}(\tau,z,\sigma) = \sum_{m\geq 1}\phi_{10,m}(\tau,z)\,p^m$,
...
\emph{Step 2. Fourier-Jacobi expansion of $\log(\Phi_{10}^{\mathrm{un}}/\eta^{24})$.}
The non-OP Igusa square $\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ admits the Fourier-Jacobi expansion
\[
...
Kodaira \(I_1\) genus-\(1\) anomaly by
Proposition~\ref{V1-prop:bvbrst-2loop-obstruction}; thus
\[
 \log\bigl(\Phi_{10}^{\mathrm{un}}/\eta^{24}\bigr)
 \;=\;
...
$\log(\Phi_{10}^{\mathrm{un}}/\eta^{24})$ defines a Chern class on the Heegner
...
$\Phi_{10}^{\mathrm{un}}\cdot\prod(1-p^n)^{\sigma_1(n)} = p\,\phi_{10,1}$ at third
...
$\log\Phi_{10}^{\mathrm{un}}$:
\[
 Z^{\mathrm{bulk},(3)}_{K3\times\mathbb{C}^2}
 \;=\;
 \bigl[\log\Phi_{10}^{\mathrm{un}}\bigr]\big|_{p^3}
 \;=\;
 c_{\Phi_{10}^{\mathrm{un}}}^{\mathrm{tot}}(3)
 \;=\;
 176256
...
the $p^3$-coefficient of $\log\Phi_{10}^{\mathrm{un}}$.
\end{proof}
...
\item \emph{Harvey-Moore heterotic-dual path.}
 ...
 $\Phi_{10}^{\mathrm{un}}/\eta^{24}$ via the Harvey-Moore product formula
...
$\phi^{(3)} = \zeta(3) c_{\mathrm{symm}} + (25/3) c_{\mathrm{timelike}}
+ (\Phi_{10}^{\mathrm{un}}/\eta^{24}) c_{\Phi_{10}^{\mathrm{un}}}$ on the $A_\infty$-quasi-Hopf-super
decoration. The BV obstruction class $c_3$ of
Theorem~\ref{thm:bvbrst-3loop-obstruction} is the \emph{automorphic}
summand $(\Phi_{10}^{\mathrm{un}}/\eta^{24}) c_{\Phi_{10}^{\mathrm{un}}}$ of that pentagon expression,
...
$(\Phi_{10}^{\mathrm{un}}/\eta^{24}) c_{\Phi_{10}^{\mathrm{un}}}$ summand represents a non-trivial
...
to $\Phi_{10}^{\mathrm{un}}$~\cite[Theorem 10.1]{Borcherds1998} (gives the
...
with the Gritsenko--Nikulin denominator
identity~\cite[Thm.~2.1]{GritsenkoNikulin1998} (identifies the
characteristic projection of the twisted-holographic coupling with the
\(p^n\)-Fourier coefficient of
\(\log(\Phi_{10}^{\mathrm{un}}/\eta^{24})\)). The three inputs compose to the
...
The classes $c_n$ of
Theorem~\ref{thm:bvbrst-heegner-all-order-vol2} name the characteristic
Heegner projection of the obstruction at each order in $\hbar$; they do
not by themselves produce a strict all-order quantum master-equation
solution. The content of this section is the Costello renormalisation
trace~\cite[Ch.~2--5]{Cos11}
...
QME obstruction classes vanish to the order under discussion. In the
quotient by these obstruction classes, the BV characteristic series is
$(\Phi_{10}^{\mathrm{un}}/\eta^{24})^{\hbar}$.
...
For the $\Delta_5$ root line the one-loop trace-sector counterterm
vanishes on the $K3$-integrated level:
\begin{equation}\label{eq:costello-trace-counterterm-zero}
 [W_1^{\mathrm{ct}}[\epsilon]]_{\Delta_5}
 \;=\;
 \chi(K3)\cdot\langle\Delta_5,\Delta_5\rangle_{\mathrm{BKM}}
 \cdot\int_{K3}\Omega_{K3}\wedge\bar\Omega_{K3} \;=\; 0,
\end{equation}
because $\Delta_5$ is isotropic for the Borcherds--Cartan form
\cite[\S 5]{Borcherds1998}. This proves the one-loop trace-sector
claim only. At $\hbar^n$ for $n\geq 2$, the counterterm
...
\begin{proposition}[Counterterm obstruction classes for the
$\Delta_5$ trace sector;
\ClaimStatusProvedHere]
\label{prop:costello-trace-counterterms-vanish}
\index{counterterm!obstruction classes for $\mathfrak{g}_{\Delta_5}$}
...
\(\mathfrak{o}^{\mathrm{ct}}_n=0\). The isotropic-root calculation
\eqref{eq:costello-trace-counterterm-zero} proves
\(\mathfrak{o}^{\mathrm{ct}}_1=0\) in the \(\Delta_5\) trace sector.
For \(n\geq2\), vanishing is an additional finite graph statement; it
does not follow from \(\langle\Delta_5,\Delta_5\rangle_{\mathrm{BKM}}=0\)
alone.
\end{proposition}
...
removes it. The one-loop assertion is exactly
...
\begin{remark}[One-loop trivialisation is specific to $\Delta_5$]
...
Higher-order counterterm vanishing is governed by the classes
...
\begin{theorem}[Scale-$L$ QME obstruction tower on
...
\(\mathfrak{o}^{\mathrm{QME}}_n=0\). Under the
twisted-holographic comparison map to the BKM Heegner line, the
characteristic projection of the obstruction is
\begin{equation}\label{eq:costello-trace-obstruction}
 \chi_{\mathrm{BKM}}\bigl(\mathfrak{o}^{\mathrm{QME}}_n\bigr)
 \;=\;
...
requires the corresponding counterterm obstruction to vanish. The
projection \(\chi_{\mathrm{BKM}}\) is the Heegner characteristic map of
Theorem~\ref{thm:bvbrst-heegner-all-order-vol2}; applying it gives
\eqref{eq:costello-trace-obstruction}. The flow equation differentiates
the representative by a BRST-exact term, so the class is independent of
\(L\).
\end{proof}
...
\begin{theorem}[Borcherds characteristic series of the Costello flow on
$\bR\times K3\times E$; \ClaimStatusProvedHere]
\label{thm:costello-trace-resummation}
\index{Costello flow!all-order resummation}
\index{Igusa!$(\Phi_{10}^{\mathrm{un}}/\eta^{24})^\hbar$ partition function}
Let \(I_{\mathrm{BV}}\) be the ideal generated by
\(\{\mathfrak{o}^{\mathrm{ct}}_n,\mathfrak{o}^{\mathrm{QME}}_n\}_{n\ge1}\).
The BV characteristic class of the scale-$L$ partition function
...
Humbert divisors $\{[H_n]\}_{n\geq 1}$ and by \(I_{\mathrm{BV}}\), equals
\begin{equation}\label{eq:costello-trace-resummation}
 [Z[L]]_{\mathrm{BV\mbox{-}coh}}
 \;=\;
 \Bigl(\Phi_{10}^{\mathrm{un}}(\tau,z,\sigma)\big/\eta^{24}(\tau)\Bigr)^{\hbar}
 \qquad\in\;
 H^\bullet\bigl(\Obs^q\bigr)[[\hbar]]/I_{\mathrm{BV}},
\end{equation}
...
\begin{proof}
Combine the characteristic projection
...
counterterm and QME obstructions killed in the quotient
\(I_{\mathrm{BV}}\).
Exponentiation gives the right-hand side of
\eqref{eq:costello-trace-resummation}.
Independence of $L$ holds at the level of the characteristic class by
Theorem~\ref{thm:costello-trace-qme-solvable}.
\end{proof}

\begin{corollary}[Factorisation cohomology and BV cohomology modulo
...
on $\bR\times K3\times E$ computes the same characteristic class
\eqref{eq:costello-trace-resummation}:
\begin{equation}\label{eq:costello-trace-dual}
 \int_{\bR\times K3\times E}\Obs^q
 \;\simeq\;
 \bigl(\Phi_{10}^{\mathrm{un}}/\eta^{24}\bigr)^\hbar\cdot\mathbf{1}
 \quad
 \in\;
...
factorisation-algebra side; both sides have the same image
$(\Phi_{10}^{\mathrm{un}}/\eta^{24})^\hbar$ times the unit class after
quotienting by \(I_{\mathrm{BV}}\).
\end{corollary}
...
\(\Obs^q\). When the obstruction ideal is retained, the same argument
applies to the characteristic quotient. The common value is
\eqref{eq:costello-trace-resummation}.
\end{proof}
...
$\Phi_{10}^{\mathrm{un}}$ is the theta lift of $\phi_{-2,1}$
...
heat-kernel Bernoulli denominator \(1/24=-B_2/2\) and the Euler number
\(\chi(K3)=24\). The Jacobi-form convention used elsewhere in this
chapter is separate:
\[
 \phi_{-2,1}(\tau,z)
 =
 -(y-2+y^{-1})\prod_{n\ge1}
 \frac{(1-q^ny)^2(1-q^n/y)^2}{(1-q^n)^4},
 \qquad
 c_{\phi_{-2,1}}(-1)=-1 .
\]
The K3 elliptic-genus normalisation supplies the Euler number by
\(\phi_{0,1}^{K3}(\tau,0)=\chi(K3)=24\), and the BKM weight by
...
\item \emph{Arithmetic / automorphic}: the weak Jacobi form
 \(\phi_{-2,1}\) is normalised by \(c_{\phi_{-2,1}}(-1)=-1\);
 the \(24\) in the K3 elliptic genus is
 \(\phi_{0,1}^{K3}(\tau,0)\), not a polar coefficient of
 \(\phi_{-2,1}\)~\cite{EguchiOoguriTachikawa2011}.
\end{enumerate}
\end{proposition}
...
\phi_{-2,1}(\tau,z)$ has \(Z_{\mathrm{ell}}(K3;\tau,0)=24\). In the
Eichler--Zagier weak Jacobi basis, however,
\(c_{\phi_{-2,1}}(-1)=-1\). These are different coefficients.

The one-loop Feynman trace and the Euler characteristic multiply to the
unit in \eqref{eq:6dhcs-K3-integration}. The relevant index-theoretic
link is the Grothendieck--Riemann--Roch theorem on $K3$:
$\chi(K3,\mathcal{O})=2$ and $\chi(K3,T_{K3})=-c_2(K3)=-24$, so
$\int_{K3}\mathrm{ch}_2=1$ and the cancellation
...
\begin{theorem}[Borcherds characteristic series of the BV anomaly tower;
 \ClaimStatusProvedHere]
...
\begin{equation}\label{eq:6dhcs-borcherds-product}
 \mathcal Z_{\hCS}^{\mathrm{char}}(X,\hbar)
 \;=\;
 \Bigl(\frac{\Phi_{10}^{\mathrm{un}}(Z)}{\eta(\tau)^{24}}\Bigr)^{\hbar\cdot c_{K3}(Z)},
\end{equation}
...
\begin{equation}\label{eq:6dhcs-cK3}
 ...
 c_{\phi_{-2,1}}(g)\cdot H_g
\end{equation}
encoding the Heegner projection of the $g$-loop obstruction at each
genus, where
\(c_{\phi_{-2,1}}(g)\) is the discriminant-\(g\) coefficient of
\(\phi_{-2,1}\) in the convention \(c_{\phi_{-2,1}}(-1)=-1\), and
\(H_g\) is the \(g\)-th Humbert class in
$\mathrm{CH}^1(\mathcal{A}_2)$.
\end{theorem}

\begin{proof}
The Heegner projection of the $g$-loop BV obstruction on
$Y=\R^3\times K3\times\C^2$ is, by
Proposition~\ref{prop:6dhcs-chi-K3-24}, a Fourier coefficient of
$\phi_{-2,1}$; the characteristic exponentiation
\[
 \mathcal Z_{\hCS}^{\mathrm{char}}(X,\hbar)
 \;=\;
 \exp\!\Bigl(\sum_{g\geq 1}\hbar^g\,\mathrm{Obs}_g\,
 \log\frac{\Phi_{10}^{\mathrm{un}}(Z)}{\eta(\tau)^{24}}\Bigr)
\]
is the Borcherds formula
$-\log\Phi_{10}^{\mathrm{un}}=\sum_{n,r,m>0}c_{\phi_{-2,1}}(4nm-r^2)
...
\(\eta^{-24}\) and comes from \(\chi(K3)\). The same \(24\) cancels the
heat-kernel denominator \(1/24\) in
Proposition~\ref{prop:6dhcs-chi-K3-24}. It is not the polar
coefficient of \(\phi_{-2,1}\): the convention used in the Borcherds
product is \(c_{\phi_{-2,1}}(-1)=-1\). The BKM weight instead uses the
K3 elliptic-genus coefficient
\(c_{\phi_{0,1}^{K3}}(0)/2=10/2=5\), while the specialisation
\(\phi_{0,1}^{K3}(\tau,0)=24\) records the Euler characteristic.
\end{remark}
