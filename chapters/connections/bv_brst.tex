...
onto $H^2(\overline{\mathcal{M}}_{g,n})$ gives $F_g = \kappaChHodge(\cA) \cdot \lambda_g$
...
$\kappaChHodge(\cA) + \kappa(\cA^!) \in \{0, 13, \tfrac{250}{3}, \tfrac{98}{3},
...
Theorem~D gives $\mathrm{obs}_g(\cA) = \kappaChHodge(\cA)\cdot\lambda_g$
...
\(\Phi_{10}^{\mathrm{un}}=\Delta_5^2\), the \(\eta^{24}\) subtraction,
and the sign of \(c_{\phi_{-2,1}}(-1)\);
...
\begin{equation}\label{eq:bv-3loop-obstruction-class}
 ...
 -8\,[H_3],
\end{equation}
where $\phi_{-2,1}(\tau,z)=\vartheta_1(\tau,z)^2/\eta(\tau)^6$ is
normalised by $c_{\phi_{-2,1}}(-1)=-1$ and $[H_3]$ is the Humbert
divisor class of discriminant~$3$. Equivalently, the $p^3$-order
...
\emph{Step 1. Source BV obstruction cohomology at $\hbar^3$.}
...
\emph{Step 2. The primitive weak-Jacobi multiplier.}
The non-OP Igusa square $\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ is obtained
from the weak Jacobi form
\[
 \phi_{-2,1}(\tau,z)=\vartheta_1(\tau,z)^2/\eta(\tau)^6
\]
by the Gritsenko--Nikulin--Borcherds product
\cite[Thm.~2.1]{GritsenkoNikulin1998}. With the convention
$c_{\phi_{-2,1}}(-1)=-1$, Eichler--Zagier's Fourier table gives
$c_{\phi_{-2,1}}(3)=-8$~\cite[Theorem~9.3, Table~1]{EichlerZagier1985}.
...
\[
 ...
 \chi_{\mathrm{Br}}\bigl(c_3^{\mathrm{BV}}\bigr)
 =
 c_{\phi_{-2,1}}(3)[H_3]=-8[H_3].
\]

\emph{Step 4. Separation from the Fock/Hilbert scalar.}
...
$c_{\phi_{-2,1}}(3)=-8$, giving~\eqref{eq:bv-3loop-obstruction-class}.
...
$c_{\phi_{-2,1}}(3)=-8$. The cancellation statement used at the scalar
...
\(\operatorname{pr}_{H_3}^{\mathrm{Br}}
\chi_{\mathrm{Br}}(c_3^{\mathrm{BV}})=-8[H_3]\) admits three
...
\item \emph{Bruinier-Heegner path.} Theorem~\ref{V3-thm:bruinier-prop-5-1}
 ...
 $c_{\phi_{-2,1}}(3)=-8$ after reduction.
\item \emph{Harvey-Moore heterotic-dual path.}
 ...
 $c_{\phi_{-2,1}}(3)=-8$ via the Harvey-Moore product formula
 \cite[\S 4]{HarveyMoore1996}.
\end{enumerate}
The three paths agree on the Bruinier-reduced coefficient $-8$; the
...
$(\Phi_{10}^{\mathrm{un}}/\eta^{24}) c_{\Phi_{10}^{\mathrm{un}}}$ summand
...
\(\Phi_{10}^{\mathrm{un}}=\Delta_5^2\) with the \(\eta^{24}\) factor,
...
$c_{\phi_{-2,1}}(8) = 56$, and thus the scalar characteristics are
\[
\chi_{\mathrm{Heg}}(c_3^{\mathrm{BV}})=-8[H_3],\quad
\chi_{\mathrm{Heg}}(c_4^{\mathrm{BV}})=12[H_4],\quad
\chi_{\mathrm{Heg}}(c_7^{\mathrm{BV}})=-39[H_7],\quad
\chi_{\mathrm{Heg}}(c_8^{\mathrm{BV}})=56[H_8].
\]
\end{theorem}
...
lift of \(\phi_{-2,1}\) to \(\Phi_{10}^{\mathrm{un}}\)
\cite[Theorem 10.1]{Borcherds1998} supplies the multiplier
\(c_{\phi_{-2,1}}(n)\) through the product expansion, with the
Gritsenko--Nikulin denominator identity fixing the
...
the displayed numerical values follow from the Eichler--Zagier residue
condition $4n-r^2\equiv0,-1\pmod4$ at index \(m=1\)
...
form \(\phi_{-2,1}\) remains the generating function for the residual
...
The content of this section is the Costello renormalisation
...
scalar characteristic quotient, and only after the comparison package,
the target series is $(\Phi_{10}^{\mathrm{un}}/\eta^{24})^{\hbar}$.
...
\(c_{\phi_{-2,1}}(n)\cdot[H_n]\), conditional on the comparison map.
By the Borcherds product
...
Independent verification of the scalar automorphic target in
...
\begin{equation}\label{eq:6dhcs-cK3}
 c_{K3}(Z)
 \;=\; \sum_{g\geq 1}\chi_{\mathrm{ab}}(\mathrm{Obs}_g)
...
vacuum amplitude. This proves the scalar automorphic target, not an
...
\(\Phi_{10}^{\mathrm{un}}=\Delta_5^2\), the \(\eta^{24}\) factor, and
