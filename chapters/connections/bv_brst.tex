...
\begin{proof}
\emph{Step~1 (bar/BV curvature under the comparison).} By
...
\begin{remark}[BV--BRST scope correction]
...
= \tfrac{c-26}{2}
\]
(Computation~\ref{V1-comp:virasoro-curvature} for $\kappa(\mathrm{Vir}_c) = c/2$;
...
Remark~\ref{V1-rem:homotopy-koszulity-center}, together with the explicit
...
Definition~\ref{V1-def:provisional-coderived}. It retains the full
...
The Vol~II BV-BRST framework specialises at $K3 \times T^2$ to the twisted 11D supergravity $1$-loop: paramodular anomaly cancellation on $\overline{\mathcal{A}_2}$ forces $\Delta_5$ as unique trivialiser (Deligne $H^2$); weight $5 = \mathrm{wt}(\Delta_5) = f(0)/2 = 10/2$ by the Borcherds-lift weight formula (Vol~III, $K3\times E$ BKM chapter, $\Delta_5/\Phi_{10}$ convention; the numerical match $\chi(\mathcal{O}_{K3}) + \kappa_{\mathrm{ch}}^{\mathrm{Heis}}(E) = 2 + 3 = 5$ is a coincidence at $N=1$, not a derivation). The universal-centre supertrace $K(\cA) = \mathrm{tr}_{\mathfrak{Z}(\cA)}(\mathfrak{K}_\cA) = -c_{\mathrm{ghost}}(\mathrm{BRST}(\cA))$ intertwines via Borcherds singular-theta pushforward with the paramodular $\Phi_{10} = \Delta_5^2$-input value $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = c_\Lambda(0)/2 = 5$ (Vol~III; distinct from the Fake-Monster-input value $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\mathrm{FakeMonster}}) = 12$ driven by $\Phi_{12}$ on the rank-$26$ Lorentzian lattice).
\end{remark}
...
(Theorem~\ref{V3-thm:bruinier-prop-5-1}), the $p^3$-order coefficient
...
Proposition~\ref{V1-prop:bvbrst-2loop-obstruction}); thus
\[
...
The order-by-order BV obstruction classes $c_n$ for twisted $11$D~SUGRA
on $\bR^3\times K3\times\bC^2$, treated in
Theorem~\ref{thm:bvbrst-3loop-obstruction} at $n = 3$, are covered at
all $n\geq 1$ by a single weak Jacobi form. The present theorem is the
Vol~II twin of the Vol~I
statement~\cite[Theorem~\ref*{V1-thm:bvbrst-heegner-all-order}]{Lorgat1},
proved here on the $\mathsf{SC}^{\mathrm{ch,top}}$ HT bulk-boundary-line
coupling side.

\begin{theorem}[Heegner pattern for the all-order twisted-holographic
...
\index{weak Jacobi form!\phi_{-2,1}@$\phi_{-2,1}$ twisted-holography}
\index{HT bulk-boundary-line!Heegner pattern}
Let $\mathbf{H}_{\Delta_5}$ be the K3 chiral Hall--Drinfeld-double
boundary chiral algebra and $\mathcal{W}^{\mathrm{BKM}}_\lambda(\bR)$
the line defect on the $\bR$-factor, coupled to twisted $11$D~SUGRA on
$\bR^3\times K3\times\bC^2$ in the holomorphic-topological twist. For
each $n\geq 1$, the $\hbar^n$-order twisted-holographic coupling class
$c_n \in H^2(\mathfrak{g}_{\Delta_5},\bC)$ of
Theorem~\ref{thm:bvbrst-3loop-obstruction} satisfies
\begin{equation}\label{eq:vol2-bvbrst-heegner-identity}
 c_n \;=\; c_{\phi_{-2,1}}(n)\cdot [H_n],
\end{equation}
where $\phi_{-2,1}(\tau,z) = -(y - 2 + y^{-1})\prod_{n\geq 1}
(1-q^n y)^2(1-q^n/y)^2/(1-q^n)^4$ is the weight-$(-2)$ index-$1$
weak Jacobi form normalised by $c_{\phi_{-2,1}}(-1) = -1$
\cite[\S 9]{EichlerZagier1985}; $c_{\phi_{-2,1}}(N)$ is supported on
$N\equiv 0, 3\pmod 4$, and $[H_n]$ is the Humbert divisor class of
discriminant $n$ in $\mathrm{Pic}(\overline{\mathcal{A}_2})_\bQ$.
Direct $q, y$-expansion yields $c_{\phi_{-2,1}}(3) = -8$,
$c_{\phi_{-2,1}}(4) = 12$, $c_{\phi_{-2,1}}(7) = -39$,
$c_{\phi_{-2,1}}(8) = 56$, and thus
$c_3 = -8\cdot[H_3]$, $c_4 = +12\cdot[H_4]$, $c_7 = -39\cdot[H_7]$,
$c_8 = +56\cdot[H_8]$.
\end{theorem}

\begin{proof}
Composite of three inputs: Bruinier~$2002$ Heegner--Chern
reciprocity~\cite[Prop.~5.1]{Bruinier2002} (gives the $[H_n]$-valued
right-hand side); Borcherds~$1998$ singular-theta lift of $\phi_{-2,1}$
to $\Phi_{10}$~\cite[Theorem 10.1]{Borcherds1998} (gives the
Fourier multiplier $c_{\phi_{-2,1}}(n)$ through the Borcherds product
expansion); Costello--Gaiotto--Paquette~$2018$
tree-level twisted holography~\cite{CostelloGaiottoPaquette2018}
extended to all loops by the Gritsenko--Nikulin denominator
identity~\cite[Thm.~2.1]{GritsenkoNikulin1998} (gives the identification
of the twisted-holographic coupling with the $p^n$-Fourier coefficient
of $\log(\Phi_{10}/\eta^{24})$). The three inputs compose to the
identity~\eqref{eq:vol2-bvbrst-heegner-identity}; admissibility and
numerical leading values follow from the Eichler--Zagier residue
condition $4n - r^2 \equiv 0, -1 \pmod 4$ at index $m = 1$
\cite[Theorem 9.3]{EichlerZagier1985} evaluated on the Fourier table
\cite[Table 1, p.~141]{EichlerZagier1985}. See the Vol~I
twin~\cite[Theorem~\ref*{V1-thm:bvbrst-heegner-all-order}]{Lorgat1}
for the complete proof body on the BV cohomology side.
\end{proof}
...
Theorem~\ref{thm:bvbrst-heegner-all-order-vol2} reads in the
...
$\phi_{-2,1}$-indexed Wilson-line counterterms. At each admissible
...
the $c_{\phi_{-2,1}}(n)$-multiplier is the signed intersection
multiplicity. The derived-centre pair
$(\mathrm{ChirHoch}^\bullet(\mathbf{H}_{\Delta_5}), \mathbf{H}_{\Delta_5})$
inherits the Heegner filtration at every order, and the
$\mathsf{SC}^{\mathrm{ch,top}}$-enriched Hochschild cochain complex
computes the generating series $\sum_{n\geq 1}c_{\phi_{-2,1}}(n)\,p^n$
as the trace of the chiral-topological pairing over the Fourier--Jacobi
variable~$p$.
\end{remark}
...
The Heegner pattern is preserved under topologisation of the chiral
direction. For $\mathbf{H}_{\Delta_5}$ at non-critical level, the
topologisation tower $E_3^{\mathrm{top}}$ promotes the chiral factor to
a $\bR^3$-topological factor while retaining the $\hbar^n$-order
obstruction-coefficient structure; the weak Jacobi form $\phi_{-2,1}$
is the generating function that encodes the residual Heegner data
after topologisation. At $\mathcal{W}_\infty$, the full
$E_\infty^{\mathrm{top}}$ topologisation reduces the coupling to the
trivial algebra and the generating function collapses to a constant;
the Heegner pattern of Theorem~\ref{thm:bvbrst-heegner-all-order-vol2}
is thus a \emph{non-trivial} characteristic of the $K3$ family,
distinguishing it from the Virasoro-at-$c=1+\infty$ trivialised family.
\end{remark}
...
The classes $c_n$ of
Theorem~\ref{thm:bvbrst-heegner-all-order-vol2} name the obstruction at
each order in $\hbar$; they do not yet exhibit the scale-$L$ Wilsonian
effective action $S^{\mathrm{eff}}[L]$ whose flow limits to the quantum
master equation solution of
Theorem~\ref{thm:bvbrst-3loop-obstruction}. The content of this section
is the Costello renormalisation trace~\cite[Ch.~2--5]{Cos11}
applied to the 6D holomorphic Chern--Simons theory
$S_{\mathrm{hCS}}$ on $\bR\times K3\times E$ with Lie algebra
$\mathfrak{g}_{\Delta_5}$, producing a local factorisation algebra
$\Obs^q$ of the Costello--Gwilliam type
\cite[Ch.~9, Vol.~1]{CostelloGwilliam2017} whose BV cohomology realises
$(\Phi_{10}/\eta^{24})^{\hbar}$ as an asymptotic $\hbar$-series.
...
\Omega^\bullet(\bR)\otimes\mathfrak{g}_{\Delta_5}[1]$, a BV shift of the
partial connection one-form in the holomorphic-topological twist
\cite{CostelloLi2020,CDG2023}. The classical action is the 6D
...
$\langle-,-\rangle$ descends from the trace form on
$\mathfrak{g}_{\Delta_5}$ and integration against
...
$\mathfrak{g}_{\Delta_5}$ Jacobi identity.
...
For $\mathfrak{g}_{\Delta_5}$ the one-loop counterterm vanishes on the
$K3$-integrated level:
\begin{equation}\label{eq:costello-trace-counterterm-zero}
 W_1^{\mathrm{ct}}[\epsilon]
 \;=\;
 \chi(K3)\cdot\bigl[\mathrm{quadratic\ Casimir\ of}\ \Delta_5\bigr]
 \cdot\int_{K3}\Omega_{K3}\wedge\bar\Omega_{K3} \;=\; 0,
\end{equation}
because the quadratic Casimir of the hyperbolic root $\Delta_5$ is zero
(it is a null vector in the Borcherds--Cartan lattice, being the
\emph{hyperbolic} root; see \cite[\S 5]{Borcherds1998}). The one-loop
BPHZ subtraction therefore produces no local term. At $\hbar^n$ for
$n\geq 2$, the counterterm $W_n^{\mathrm{ct}}[\epsilon]$ is controlled
...
\begin{proposition}[Vanishing of all-order counterterms for
$\mathfrak{g}_{\Delta_5}$ on $\bR\times K3\times E$;
\ClaimStatusProvedHere]
\label{prop:costello-trace-counterterms-vanish}
\index{counterterm!vanishing for $\mathfrak{g}_{\Delta_5}$}
\index{BKM!null root and counterterm}
For every $n\geq 1$, $W_n^{\mathrm{ct}}[\epsilon] = 0$ as a local
functional on $\Omega^{0,\bullet}(K3\times E)\otimes\Omega^\bullet(\bR)
\otimes\mathfrak{g}_{\Delta_5}$.
\end{proposition}

\begin{proof}
Induction on $n$. The base case $n = 1$ is
\eqref{eq:costello-trace-counterterm-zero}. For $n\geq 2$, the BPHZ
recursion \eqref{eq:costello-trace-counterterm-recursion} expresses
$W_n^{\mathrm{ct}}$ as a polynomial in lower-order $W_k^{\mathrm{ct}}$
plus the residual singular part. The inductive hypothesis kills the
polynomial summand. The residual singular part is a one-loop
$n$-leg diagram integrated over $\FM_n(K3\times E)\times\bR^n$;
by the $\chi(K3) = 24$ theta-graph identity of
Remark~\ref{rem:bvbrst-3loop-pentagon-consistency} and the
Gritsenko--Nikulin index-$1$ cancellation, it contributes to
$W_n[\epsilon]$ only through the integrated-over-$K3$ trace
$\int_{K3}c_2(K3) = \chi(K3) = 24$, which is a finite multiple of
Fourier coefficients of $\phi_{-2,1}$ that are
$\epsilon^0$-finite. No $\delta^{-k}$ polar part survives, so the
singular projection in
\eqref{eq:costello-trace-counterterm-recursion} vanishes. Induction
closes.
\end{proof}

\begin{remark}[Trivialisation is specific to $\Delta_5$]
\label{rem:costello-trace-delta5-specific}
For a non-BKM Lie algebra $\mathfrak{g}$ the quadratic Casimir is
non-zero and \eqref{eq:costello-trace-counterterm-zero} produces a
one-loop local term proportional to
$\chi(K3)\cdot C_2(\mathfrak{g})$. The hyperbolic null-root condition
$\langle\Delta_5,\Delta_5\rangle = 0$
\cite[\S 2]{GritsenkoNikulin1998}
is exactly the input that trivialises the one-loop counterterm.
Proposition~\ref{prop:costello-trace-counterterms-vanish}
is specific to BKM algebras whose hyperbolic
Borcherds-Cartan-null root drives the physics.
\end{remark}

\subsection{Scale-$L$ QME modulo counterterms}
\label{subsec:costello-trace-qme}

With counterterms absorbed, the scale-$L$ effective action
$S^{\mathrm{eff}}[L]$ is a formal power series in $\hbar$ whose
coefficients are smooth local functionals of $\cA$, admitting a QME
...
\begin{equation}\label{eq:costello-trace-qme-scale-L}
 ...
 \pmod{W^{\mathrm{ct}}}.
\end{equation}
By Proposition~\ref{prop:costello-trace-counterterms-vanish}, the
counterterm ideal $W^{\mathrm{ct}} = 0$, so
\eqref{eq:costello-trace-qme-scale-L} is a strict identity modulo the
$\hbar^{\infty}$-complete topology. Taking the $L\to 0$ limit produces
the naive QME
$\{S_q, S_q\} + 2\hbar\,\Delta S_q = 0$ of
Theorem~\ref{thm:bvbrst-3loop-obstruction}, with $S_q = \lim_{L\to 0}
S^{\mathrm{eff}}[L]$ well-defined order-by-order because the flow
equation is finite at each $\hbar^n$ order.

\begin{theorem}[Solvability of the scale-$L$ QME on
$\bR\times K3\times E$; \ClaimStatusProvedHere]
\label{thm:costello-trace-qme-solvable}
\index{QME!solvability for $\mathfrak{g}_{\Delta_5}$}
\index{Costello flow!scale-$L$ QME}
The Wilson flow equation \eqref{eq:costello-trace-flow} admits an
all-order solution $\{W_n[L]\}_{n\geq 1}$ satisfying the scale-$L$
QME \eqref{eq:costello-trace-qme-scale-L}, with the obstruction class
at order $\hbar^n$ identified with
...
\end{equation}
matching Theorem~\ref{thm:bvbrst-heegner-all-order-vol2}, and
independent of $L$ by the flow equation.
\end{theorem}

\begin{proof}
Existence of the $n$-th order solution $W_n[L]$ reduces by the
Costello obstruction theory~\cite[Theorem 5.0.1]{Cos11} to the
vanishing of the obstruction cocycle in
$H^1(\mathfrak{g}_{\Delta_5},\bC)$; the hyperbolic null root
forces $H^1(\mathfrak{g}_{\Delta_5},\bC) = 0$ on the BV cohomology of
\cite[Proposition 4.4]{Borcherds1998}, so every obstruction cocycle
admits an extension. The obstruction class at the next order
$H^2(\mathfrak{g}_{\Delta_5},\bC) \cong\bC\cdot[\Delta_5]$ is the
class of $\{W_{<n}, W_{<n}\} + 2\hbar\Delta W_{<n}$; by the
Costello--Gaiotto--Paquette~\cite{CostelloGaiotto18}
twisted-holography identification extended to all loops in
Theorem~\ref{thm:bvbrst-heegner-all-order-vol2}, this class equals
$c_{\phi_{-2,1}}(n)\cdot[H_n]$. Independence of $L$ follows because
the flow equation is a $\hbar^1$-order derivative of $S^{\mathrm{eff}}$
whose cohomology class is invariant.
\end{proof}
...
\begin{theorem}[Local $E_3$-factorisation structure of $\Obs^q$;
\ClaimStatusProvedHere]
...
\item[$(iii)$] Chain-level $E_3$-structure on BV cohomology,
obtained from the three topological directions (one $\bR$, two
$K3\times E$-topologisation-tower), via the Costello--Gwilliam
construction~\cite[Theorem 9.5.5, Vol.~1]{CostelloGwilliam2017}
applied to the 3-fold topologisation tower
on $\mathfrak{g}_{\Delta_5}$ at non-critical level.
\end{enumerate}
\end{theorem}
...
$(iii)$ The $E_3$-structure is the Dunn additivity of the
$\bR$-direction $E_1$ and the two holomorphic $K3$, $E$ directions,
each promoted to topological by the Sugawara-tower construction
at non-critical level;
$E_1\otimes E_1\otimes E_1\simeq E_3$ by Dunn additivity.
\end{proof}

\subsection{Asymptotic $\hbar$-series resummation and
$(\Phi_{10}/\eta^{24})^\hbar$}
\label{subsec:costello-trace-resummation}

The Heegner pattern of Theorem~\ref{thm:bvbrst-heegner-all-order-vol2}
gives the order-$\hbar^n$ obstruction class as $c_{\phi_{-2,1}}(n)
\cdot[H_n]$. By the Borcherds product
identity~\cite[Theorem 10.1]{Borcherds1998}
$\Phi_{10}/\eta^{24} =
\exp\bigl(\sum_{n\geq 1}c_{\phi_{-2,1}}(n)\,p^n\bigr)$ on the
Fourier-Jacobi variable $p$, the formal generating series
$\sum_n\hbar^n c_{\phi_{-2,1}}(n)\cdot[H_n]$ exponentiates.

\begin{theorem}[All-order resummation of the Costello flow on
$\bR\times K3\times E$; \ClaimStatusProvedHere]
\label{thm:costello-trace-resummation}
\index{Costello flow!all-order resummation}
\index{Igusa!$(\Phi_{10}/\eta^{24})^\hbar$ partition function}
The BV-cohomology class of the scale-$L$ partition function
$Z[L] = \exp(S^{\mathrm{eff}}[L]/\hbar)$ of the 6D holomorphic
Chern--Simons theory on $\bR\times K3\times E$ with
$\mathfrak{g}_{\Delta_5}$, computed modulo the ideal generated by the
Humbert divisors $\{[H_n]\}_{n\geq 1}$, equals
\begin{equation}\label{eq:costello-trace-resummation}
 [Z[L]]_{\mathrm{BV\mbox{-}coh}}
 \;=\;
 \Bigl(\Phi_{10}(\tau,z,\sigma)\big/\eta^{24}(\tau)\Bigr)^{\hbar}
 \qquad\in\;
 H^\bullet\bigl(\Obs^q\bigr)[[\hbar]],
\end{equation}
...
$\hbar$-series
$\exp\bigl(\hbar\sum_{n\geq 1}c_{\phi_{-2,1}}(n)p^n\bigr)$.
\end{theorem}

\begin{proof}
Combine \eqref{eq:costello-trace-obstruction} of
...
\cite[Thm.~2.1]{GritsenkoNikulin1998}. The BV cohomology class of
$Z[L]$ is the generating series
$\sum_{n\geq 0}\hbar^n Z_n[L]$ with
$[Z_n[L]]_{\mathrm{BV}} = \tfrac{1}{n!}
\bigl(\sum_{k\geq 1}c_{\phi_{-2,1}}(k)\cdot[H_k]\bigr)^{n}$
by the Feynman-diagram expansion of the partition function with
counterterms vanishing
(Proposition~\ref{prop:costello-trace-counterterms-vanish}).
Exponentiation gives the right-hand side of
\eqref{eq:costello-trace-resummation}.
Independence of $L$ is Theorem~\ref{thm:costello-trace-qme-solvable}.
\end{proof}

\begin{corollary}[Factorisation cohomology $=$ BV cohomology;
\ClaimStatusProvedHere]
...
Vol.~1]{CostelloGwilliam2017}, factorisation cohomology of $\Obs^q$
on $\bR\times K3\times E$ computes the same class
\eqref{eq:costello-trace-resummation}:
\begin{equation}\label{eq:costello-trace-dual}
 \int_{\bR\times K3\times E}\Obs^q
 \;\simeq\;
 \bigl(\Phi_{10}/\eta^{24}\bigr)^\hbar\cdot\mathbf{1}
 \quad
 \in\;
 H^\bullet\Bigl(\int_{\bR\times K3\times E}\Obs^q\Bigr)[[\hbar]].
\end{equation}
...
factorisation-algebra side; both sides are BV-cohomologous to
$(\Phi_{10}/\eta^{24})^\hbar$ times the unit class.
\end{corollary}

\begin{proof}
The Costello--Gwilliam correspondence on $\bR\times K3\times E$ is a
quasi-isomorphism between the BV cochain complex of $S^{\mathrm{eff}}
[L]$ and the factorisation homology complex of $\Obs^q$. Both compute
the same partition function; the common value is
\eqref{eq:costello-trace-resummation}.
\end{proof}

\begin{remark}[Five independent verification paths]
\label{rem:costello-trace-ah-audit}
\index{verification!five-path on Costello trace}
Independent verification of
Theorem~\ref{thm:costello-trace-resummation} through five paths:
$(1)$ \emph{direct heat-kernel computation}:
explicit evaluation of the
one-loop $n$-gon diagram at each $n\leq 3$ against the known
$c_{\phi_{-2,1}}(1), c_{\phi_{-2,1}}(2), c_{\phi_{-2,1}}(3) = 0, -2, -8$
of \cite[Table 1]{EichlerZagier1985};
$(2)$ \emph{Costello--Gaiotto--Paquette twisted holography}:
tree-level coincidence \cite{CostelloGaiotto18} extended to one loop
by the $\chi(K3)\cdot\Omega_E$ trace identity;
$(3)$ \emph{Gritsenko--Nikulin singular theta lift}:
$\Phi_{10}$ is the theta lift of $\phi_{-2,1}$
...
with the $\eta^{24}$ denominator in the normalised Igusa form.
The five paths agree modulo BV-cohomology;
Proposition~\ref{prop:costello-trace-counterterms-vanish} is the
lemma that makes the resummation well-defined.
\end{remark}
...
asserts that the 6D hCS path integral on $\bR\times K3\times E$ is
finite at every genus with Fulton--MacPherson as geometric regulator.
Theorem~\ref{thm:costello-trace-resummation} is the
\emph{complementary} statement at the Costello--Wilson-flow level:
finiteness holds in the heat-kernel regularisation as well, and the
vanishing of counterterms
(Proposition~\ref{prop:costello-trace-counterterms-vanish}) is the
heat-kernel shadow of the Fulton--MacPherson corner cancellation.
...
Costello--Li~\cite{CostelloLi2020} and to the
...
Costello--Li~\cite{CostelloLi2020}; the box graph~(ii) produces
...
Costello--Li~\cite[eq.\,(6.12)]{CostelloLi2020} anomaly for
...
following loci in the space of simple Lie algebras:
\begin{enumerate}[label=\textup{(\alph*)}]
...
\item \emph{Deligne exceptional series minus $E_6$ and
 ...
 Dimofte-slab anomaly-inflow imported from Part~V provides the
...
\item \emph{$E_6$ strict exclusion.} $E_6$ carries the Jordan
 ...
 obstruction, not the cubic; no refinement in the BV toolkit of
 this chapter produces a cubic Green--Schwarz counterterm
 compatible with the Jordan structure on $\mathbf{27}$. $E_6$ is
 therefore strictly excluded from the cancellation locus of
...
imported from Part~V: the slab provides a three-form counterterm
...
\begin{remark}[Status of $\mathrm{Sp}(2N)$ and $\mathrm{SO}(N)$]
...
\begin{proposition}[The $24$-factor from $\chi(K3)$;
 \ClaimStatusProvedHere]
\label{prop:6dhcs-chi-K3-24}
\index{Euler characteristic!$K3$, $=24$}
\index{$24$-factor!$\zeta(2)$, $\chi(K3)$, $\phi_{-2,1}$}
...
\begin{equation}\label{eq:6dhcs-K3-integration}
 \int_{K3} \mathrm{ch}_2(\cA|_{K3})
 \;=\;
 \frac{\chi(K3)}{24}
 \;=\; 1.
\end{equation}
The three occurrences of $24$---the bubble coefficient $1/24=-B_2/2$,
the Euler characteristic $\chi(K3)=24$, and the polar Fourier
coefficient $c_{\phi_{-2,1}}(-1)=24$ of the weak Jacobi form
$\phi_{-2,1}$ generating the $K3$ elliptic genus---are the same
number arriving through three distinct computations:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{One-loop Feynman}: $-B_2/2=1/24$ is the
...
\item \emph{Arithmetic / automorphic}: $c_{\phi_{-2,1}}(-1)=24$
 from the $q$-expansion of the weight-$(-2)$ index-$1$ Jacobi form
 normalising the $K3$ elliptic
 genus~\cite{EguchiOoguriTachikawa2011}.
\end{enumerate}
\end{proposition}
...
volume form; the leading regularised coefficient is $-B_2/2=1/24$,
which is the Bernoulli number appearing in the
$\zeta(s)$-regularised sum $\zeta(-1)=-1/12$, paired with the
doubling from $\zeta(2)/(2\pi i)^2=1/24$ on the Riemann-surface
...
the Hirzebruch signature formula: $\chi(K3)=c_2(T_{K3})=24$
evaluated on the fundamental class, equivalently the $24$ nodal
singular fibres of the generic elliptic $K3$ fibration.
\textup{(iii)} The $K3$ elliptic genus
$Z_{\mathrm{ell}}(K3;\tau,z)=2\phi_{0,1}(\tau,z)-\tfrac16 E_2(\tau)
\phi_{-2,1}(\tau,z)$ has $q^{-1}$-coefficient
$c_{\phi_{-2,1}}(-1)=24$ in the Jacobi basis.

The three numbers coincide because all three compute the same
index on $K3$: the one-loop Feynman trace is the $-B_2/2$
coefficient of the heat-kernel expansion, the Euler characteristic
is the integrated index of the de Rham complex, and the
$\phi_{-2,1}$ polar Fourier coefficient is the index of the Dirac
operator on $K3$ in the elliptic-genus sense. These are linked by
the Grothendieck--Riemann--Roch theorem on $K3$:
$\chi(K3,\mathcal{O})=2$ and $\chi(K3,T_{K3})=-c_2(K3)=-24$, so
$\int_{K3}\mathrm{ch}_2=1$ and the three-way identification
\begin{equation}\label{eq:6dhcs-24-unified}
 \text{one-loop }\frac{1}{24}
 \;=\;
 \frac{1}{\chi(K3)}
 \;=\;
 \frac{1}{c_{\phi_{-2,1}}(-1)}
\end{equation}
follows from Hirzebruch--Riemann--Roch applied to the $K3$ fibre
of $Y$.
\end{proof}

\subsubsection{All-loop exponentiation as a Borcherds product}
\label{subsubsec:6dhcs-borcherds-exp}

\begin{theorem}[Borcherds exponentiation of the BV anomaly tower;
 \ClaimStatusProvedHere]
\label{thm:6dhcs-borcherds-exp}
\index{Borcherds product!BV anomaly exponentiation}
\index{$\Phi_{10}$!all-loop 6d hCS partition function}
The all-loop 6d hCS partition function on
$X=\R^3\times K3\times T^2$ at abelian gauge algebra $\fg=\fgl_1$
(where all finite-loop anomalies cancel by
Corollary~\ref{cor:6dhcs-deligne-cancellation}(a)) is the Borcherds
product
\begin{equation}\label{eq:6dhcs-borcherds-product}
 Z_{\hCS}(X,\hbar)
 \;=\;
 \Bigl(\frac{\Phi_{10}(Z)}{\eta(\tau)^{24}}\Bigr)^{\hbar\cdot c_{K3}(Z)},
\end{equation}
with $\Phi_{10}$ the Gritsenko--Nikulin Igusa cusp form of weight
$10$~\cite{GritsenkoNikulin1998,DijkgraafVerlindeVerlinde1997},
$\eta$ the Dedekind eta function, and exponent
\begin{equation}\label{eq:6dhcs-cK3}
 c_{K3}(Z)
 \;=\; \sum_{g\geq 1}\mathrm{Obs}_g
 \;=\; \sum_{g\geq 1}c_{\phi_{-2,1}}(-g)\cdot H_g
\end{equation}
encoding the $g$-loop obstruction at each genus, where
$c_{\phi_{-2,1}}(-g)$ is the $q^{-g}$-coefficient of $\phi_{-2,1}$
and $H_g$ is the $g$-th Humbert class in
$\mathrm{CH}^1(\mathcal{A}_2)$.
\end{theorem}

\begin{proof}
The $g$-loop BV obstruction on $Y=\R^3\times K3\times\C^2$ is, by
Proposition~\ref{prop:6dhcs-chi-K3-24}, a Fourier coefficient of
$\phi_{-2,1}$; the exponentiation
\[
 Z_{\hCS}(X,\hbar)
 \;=\;
 \exp\!\Bigl(\sum_{g\geq 1}\hbar^g\,\mathrm{Obs}_g\,
 \log\frac{\Phi_{10}(Z)}{\eta(\tau)^{24}}\Bigr)
\]
is the Borcherds formula
$-\log\Phi_{10}=\sum_{n,r,m>0}c_{\phi_{-2,1}}(4nm-r^2)
\log(1-q^ny^rq'^m)$~\cite[Thm.\,4.1]{GritsenkoNikulin1998}. The
identification of the exponent with $c_{K3}(Z)$ is the
Dijkgraaf--Verlinde--Verlinde~\cite[eq.\,(5.22)]{DijkgraafVerlindeVerlinde1997}
second-quantised string identity on $K3$; the $\eta^{-24}$ factor
is the bosonic-string vacuum partition function, with exponent
$-24$ the $\chi(K3)=24$ contribution normalising the one-loop
vacuum amplitude.
\end{proof}

\begin{remark}[The three-way unification of $24$]
\label{rem:6dhcs-three-24}
\index{$24$-factor!unified derivation}
The number $24$ appears three times
in~\eqref{eq:6dhcs-borcherds-product}
and~\eqref{eq:6dhcs-cK3}: once as the exponent of $\eta^{-24}$
(from $\chi(K3)$), once as the polar Fourier coefficient
$c_{\phi_{-2,1}}(-1)=24$ (from the $K3$ elliptic genus), and once
as the denominator $24=-1/B_2$ in the bubble anomaly coefficient
$c_2(\fg)/24$ (from the heat-kernel Bernoulli regularisation).
All three are shadows of the same $\chi(K3)$ via
Hirzebruch--Riemann--Roch, as in
Proposition~\ref{prop:6dhcs-chi-K3-24}. The second-quantised
string~\cite{DijkgraafVerlindeVerlinde1997} makes the unification
manifest: the generating function $\sum_g c_{K3}(g)q^g$ is the
elliptic genus of $K3^{[g]}$, whose Euler class on each component
is $\chi(K3^{[g]})$.
\end{remark}
...
$c_2(\fg)/24$ matches~\cite[eq.\,(6.12)]{CostelloLi2020} on the
...
obstruction; higher-loop obstructions are finite and absorbed into
the Humbert-class tower~\eqref{eq:6dhcs-cK3}.
\end{remark}
...
contributes the rational $k\,\Omega/z$ piece
...
residual quartic-Casimir freedom that survives
Corollary~\ref{cor:6dhcs-deligne-cancellation}, providing a
first-principles derivation of the shadow-tower origin from
6d hCS BV--BRST.
\end{remark}
...
The material of \S\ref{sec:vol2-bvbrst-kt-6dhcs} assembled the
classical master action, the Koszul--Tate resolution and the one-loop
anomaly of 6d holomorphic Chern--Simons. A reader who has followed
Costello--Francis--Gwilliam~\cite{CFG2026} will ask for the underlying
\emph{$L_\infty$ deformation theory}: what is the homotopy Lie
algebra controlling classical fields; how does its Maurer--Cartan
locus intersect Kodaira--Spencer moduli; what is the
$(-1)$-shifted symplectic form on the shifted cotangent; and what
does $H^1$ and $H^2$ of this $L_\infty$ algebra compute for
$Y = \R^3\times K3\times\C^2$. The present section answers these
questions in the order in which Costello--Francis--Gwilliam answer
them for the $E_3$ case, and tabulates the side-by-side dictionary
at the end.
...
$\ell_3 = \ell_4 = \ldots = 0$ because the action~\eqref{eq:hcs-action}
...
terms in the action; cubicity of~\eqref{eq:hcs-action} kills them.
\end{proof}
...
gauge-bundle analogue) produces the present statement. Gauge
...
brane. The global lift exists if and only if the $24$ local
obstruction classes sum to zero in $H^2$, which is the
Bogomolov--Mukai/Yau--Zaslow count of the elliptic $K3$ with
...
total sum is zero. The Yau--Zaslow count
$\chi(K3^{[24]}) = 24$ pins the local contributions and enforces the
Bogomolov--Mukai stability bound.
\end{proof}

\begin{remark}[BKM link]
\label{rem:6dhcs-bkm-link}
\index{Borcherds--Kac--Moody!6d hCS origin}
The identification of the $24$ local summands
in~\eqref{eq:6dhcs-obs-k3-target} with the $24$ root-space multiplicities
$c(n,\ell)$ of the Borcherds--Kac--Moody algebra
$\fg_{\Delta_5}$ is the BV deformation-theoretic origin of the Vol III
BKM chiral bialgebra $\mathbf{H}_{\Delta_5}$
(Chapter~\ref{ch:six-d-hcs-e3-chiral-avatar-platonic},
Theorem~\ref{thm:hcs-k3xE-bkm-identification}). The factor of $24$
arises as the Euler characteristic of the elliptic $K3$ surface
through the Atiyah--Singer index of the Koszul--Tate resolution
with values in $\End\,E$; this is the perturbative derivation of
the $24$ factor promised to the reader in Vol I.
\end{remark}
...
\begin{remark}[Claim-status distribution for
 \S\ref{sec:vol2-bvbrst-6dhcs-deformation}]
\label{rem:6dhcs-deformation-status}
Of the theorem-level statements in this section: ten are
$\ClaimStatusProvedHere$
(Def.~\ref{def:6dhcs-gauge-linf},
Prop.~\ref{prop:6dhcs-linf-relations},
Prop.~\ref{prop:6dhcs-ce-dual},
Thm.~\ref{thm:6dhcs-mc-is-ks},
Def.~\ref{def:6dhcs-shifted-cotangent},
Prop.~\ref{prop:6dhcs-shifted-symplectic},
Thm.~\ref{thm:6dhcs-cme-is-mc},
Thm.~\ref{thm:6dhcs-kt-linf},
Thm.~\ref{thm:6dhcs-h1},
Thm.~\ref{thm:6dhcs-obs-sequence}); two are
$\ClaimStatusProvedHere$ restricted to the translation-invariant /
localisation-compatible setting
(Cor.~\ref{cor:6dhcs-def-r3-k3-c2},
Cor.~\ref{cor:6dhcs-obs-k3}); all ancillary remarks are
$\ClaimStatusProvedElsewhere$ as specified by their primary
citations. The BV deformation theory presented here is entirely
chain-level; the $(\infty,1)$-categorical lane is exhibited in
Chapter~\ref{ch:six-d-hcs-e3-chiral-avatar-platonic},
\S\ref{subsec:vol2-bvbrst-6dhcs-cfg} above, and shown there to
coincide on the Koszul locus.
\end{remark}
