...
\begin{proposition}[Cross-conjecture relation]
\label{prop:celestial-cross-conjecture}
\ClaimStatusProvedHere
The three provisional conjectures are not independent.  The ordered-to-symmetrized correction satisfies:
\[
Q^{\mathrm{sym}}_{110} - Q^{\mathrm{ord}}_{110} \;=\; -\frac{B_{\mathrm{mix}}}{10} \;=\; \frac{11}{90},
\]
where the numerator~$11$ appears in both $B_{\mathrm{mix}}^{\mathrm{alg}} = -11/9$ and the correction $11/90$, reflecting the shared cubic structure constants $c^{AB}_{p,q}$.
\end{proposition}
