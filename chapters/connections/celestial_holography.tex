...
conjectures. They are not used elsewhere in this file; each requires an
independent symbolic derivation from Zeng's explicit $A_\infty$ formulas.
...
Schur cocycle of the $M_{24}$ cover.
\end{enumerate}
\end{theorem}
...
specific order-$6$ cocycle fixed by the umbral Schur-multiplier
calculation.
\end{proof}
...
quantum group; \ClaimStatusProvedHere]
...
part~(iii) and the umbral Schur-multiplier calculation:
...
$\mathsf{B}$-family locus $K^{\kappa_{\mathrm{ch}}} = 8$
