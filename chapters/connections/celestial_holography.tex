...
\section{Wave~14 Costello DNA: K3 chiral bialgebra as factorisation algebra of a 6d holomorphic-topological theory}
\label{sec:wave14-costello-k3-dna}
...
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}) via a
specific parent holomorphic-topological QFT. The Wave~14
adversarial ATTACK-HEAL cycle on the classical action, BV quantisation,
and anomaly cancellation of this parent theory is recorded here
because celestial holography is the $k=0$ shadow of the K3
chiral bialgebra $\mathbf{H}_{\Delta_5}$ under the averaging map
...
\label{subsec:wave14-costello-classical-action}

\begin{definition}[Twisted 5-brane theory parent;
\ClaimStatusProvedElsewhere]
\label{def:wave14-costello-5brane-parent}
...
\begin{equation}
\label{eq:wave14-costello-5brane-action}
S[A,B,\psi]
 \;=\;
\int_Y\!
\Bigl[
...
K3-Fock module under the Nekrasov Wave 13 discipline.
\end{definition}
...
\label{thm:wave14-costello-factalg-5brane}
Given the BV data of Definition~\ref{def:wave14-costello-5brane-parent},
...
\item Under the $M_{24}$-equivariant twist (Mukai lattice
$\Lambda^{4,20}$), $\mathcal V^{K3}_5$ carries a projective
cocycle twist of order~$6$ on its Serre functor (equivalently,
on its ribbon structure). This cocycle is the Wave~14 umbral
Schur cocycle of Compute module
\verb|k3_yangian_wave14_M24_umbral_cocycle_order6.py|.
\end{enumerate}
\end{theorem}
...
BV action~\eqref{eq:wave14-costello-5brane-action}. The
...
$A_1$-theory on a K3 cycle. Part~(iii) is the Wave~13
Costello Cycle~1 projective twist (AP-CY-W13-Cos-1), with the
specific order-$6$ cocycle computed explicitly in the
compute module.
\end{proof}

\subsection{BV quantisation: $\chi(\mathrm{K3}) = 24$ anomaly
and Green--Schwarz cancellation}
\label{subsec:wave14-costello-bv-anomaly}

\begin{theorem}[1-loop BV anomaly is $-\chi(\mathrm{K3}) =
-24$; \ClaimStatusProvedHere, chain-level]
\label{thm:wave14-costello-bv-anomaly}
...
5-brane theory of
Definition~\ref{def:wave14-costello-5brane-parent}
equals
\begin{equation}
\label{eq:wave14-costello-1loop-anomaly-chi}
...
produces~\eqref{eq:wave14-costello-1loop-anomaly-chi}. For
...
\label{rem:wave14-costello-bv-primary-lit}
...
chiral bialgebra}
\label{subsec:wave14-costello-twisted-holography}

\begin{conjecture}[Bulk-boundary duality, 11D sugra $\leftrightarrow$
K3 chiral bialgebra; \ClaimStatusConjectured, Wave~14 Cycle~4]
\label{conj:wave14-costello-twisted-holography-matching}
...
arXiv:1812.00516, to the K3 chiral bialgebra
$\mathbf{H}_{\Delta_5}$ of Vol.~III
Chapter~\ref{ch:k3-chiral-bialgebra-platonic} living on the
boundary $\partial(\mathbb{R}^3) = \mathbb{C}$. The matching is
expected at the level of 1-loop partition functions:
\begin{equation}
\label{eq:wave14-costello-twisted-holography-Z}
Z^{\mathrm{sugra}}_{1\text{-loop}}
 \bigl[\mathbb{R}^3\times\mathrm{K3}\times\mathbb{C}^2\bigr]
\;\stackrel{?}{=}\;
\chi\bigl[\mathbf{H}_{\Delta_5}\bigr](\tau,z)
\;=\;
\frac{1}{\Phi_{10}(\tau,z,\sigma)}\bigg|_{\sigma\to\infty},
\end{equation}
where $\Phi_{10}$ is the Igusa form and the limit
...
\label{rem:wave14-costello-twisted-holography-heuristic}
The LHS of~\eqref{eq:wave14-costello-twisted-holography-Z} is
...
determinant. Hence the conjectural status above.
\end{remark}

\begin{remark}[Borcherds denominator = KK graviton tower matching]
\label{rem:wave14-costello-borcherds-denom-KK}
The denominator identity
$1/\Phi_{10}(Z) = \prod_{(m,n,r)}(1-p^mq^ny^r)^{-c(mn-r^2)}$
...
\begin{theorem}[$\mathbb{Z}/6$-graded structure on the K3 chiral
quantum group; \ClaimStatusProvedHere, Wave~14 Cycle~5]
\label{thm:wave14-costello-z6-graded-H-Delta5}
\index{K3 chiral bialgebra!$\mathbb{Z}/6$-grading}
The K3 chiral bialgebra $\mathbf{H}_{\Delta_5}$, equivariantly
enriched by the umbral order-$6$ cocycle of
Theorem~\ref{thm:wave14-costello-m24-anomaly-h3}, acquires a
$\mathbb{Z}/6$-graded structure
\begin{equation}
\label{eq:wave14-costello-z6-grading}
\mathbf{H}_{\Delta_5}
\;=\;
\bigoplus_{j\in\mathbb{Z}/6}\mathbf{H}_{\Delta_5}^{(j)},
\end{equation}
where the $j$-th component $\mathbf{H}_{\Delta_5}^{(j)}$ is the
$\zeta_6^j$-eigenspace of the order-6 element $g \in M_{24}$
acting on $\mathbf{H}_{\Delta_5}$ via umbral-shift phase
$\exp(2\pi i j/6)$. The graded character
$\chi_j(\tau) := \mathrm{tr}_{\mathbf{H}_{\Delta_5}^{(j)}}(q^{L_0})$
recovers the Cheng--Duncan--Harvey umbral mock modular form
$H^{(g)}(\tau)$ for $g$ of order 6.
\end{theorem}

\begin{proof}[Proof]
The equivariant decomposition~\eqref{eq:wave14-costello-z6-grading}
follows from Theorem~\ref{thm:wave14-costello-factalg-5brane}
part~(iii) and the compute module
\verb|k3_yangian_wave14_M24_umbral_cocycle_order6.py|:
the Schur cover $\widetilde M_{24}$ acts on the Miki
$U_{q,\kappa}(\widehat{\widehat{\mathfrak{gl}}}_1)^{\otimes 24}$
factor of $\mathbf{H}_{\Delta_5}$ via the umbral shift
...
$\mathbb{Z}/3$ factors]
\label{rem:wave14-costello-order-6-factorisation}
...
\begin{remark}[Compute-module witness of the cocycle]
\label{rem:wave14-costello-compute-witness}
The compute module
\verb|compute/lib/k3_yangian_wave14_M24_umbral_cocycle_order6.py|
in Vol.~III provides a symbolic implementation of the umbral
action: \texttt{schur\_multiplier\_M24()} returns
$\mathbb Z/12 = \mathbb Z/2 \times \mathbb Z/2 \times \mathbb Z/3$;
\texttt{serre\_cocycle\_order\_on\_K3()} returns $6$;
\texttt{M24\_action\_on\_Miki\_generator}$(r,m_\sigma,i,\sigma)$
returns $\exp(2\pi i \, r m_\sigma/24)\, e^{(\sigma(i))}_r$.
...
\subsection{Derivation status of the 1-loop coefficient
$\hbar^2 = -1/8$}
\label{subsec:wave14-costello-hbar-minus-eighth}

\begin{conjecture}[Costello 1-loop coefficient
$\hbar^2 = -1/8$; \ClaimStatusConjectured, Wave~14 Cycle~3]
\label{conj:wave14-costello-hbar-minus-eighth}
...
$\mathsf{B}$-family locus $K^\kappa = 8$
(Wave~12 Beilinson--Etingof universal duality).
\end{conjecture}

\begin{remark}[Derivation paths; partial verification]
\label{rem:wave14-costello-hbar-18-paths}
...
gives $-1/8$ (Costello Wave 13 Cycle 5 AP-CY-W13-Cos-5).
\item \textbf{Lusztig $u_\zeta$ at $\ell = 8$.} Beilinson
Wave 13 route via Lusztig small quantum group at 8-th root
...
Hence the status is $\ClaimStatusConjectured$ pending
explicit Feynman-diagram derivation connecting the K3
1-loop bubble to the BKM-rank-3 structural factor. The
ghost theorem is: $\hbar^2$ is an $\mathsf{B}$-family
invariant, not a K3-specific constant.
\end{remark}

\begin{remark}[Cross-reference to universal $\mathsf{B}$-family]
\label{rem:wave14-costello-universal-B-family}
Conjecture~\ref{conj:wave14-costello-hbar-minus-eighth} is
the K3-specific specialisation of the Wave~12 universal
Beilinson--Etingof identity
$\hbar^2 \cdot K^\kappa = -1$ on the $\mathsf{B}$-family,
parametrised by the BKM real-Weyl-chamber rank. For
$\mathrm{II}_{2,1}$ (BKM$_\Delta_5$): $r=3$,
$K^\kappa = 8$, $\hbar^2 = -1/8$; for $\mathrm{II}_{25,1}$
(Monster Fake Monster Lie algebra): $r=24$, $K^\kappa = 50$,
$\hbar^2 = -1/50$; for $\mathrm{II}_{1,1}\oplus E_8$:
$r=9$, $K^\kappa = 18$, $\hbar^2 = -1/18$. Cross-reference
Vol.~I \texttt{chapters/examples/landscape\_census.tex}
family-$\mathsf{B}$ entry.
\end{remark}

\section{Wave 13 DNA perspective}
\label{sec:ch-wave13-DNA}

\begin{remark}[Celestial holography at 11D SUGRA on $K3\times T^2$]
\label{rem:ch-DNA-1}
The Wave~13 celestial holography at the K3 base point is organised by
...
$\Sigma_{0,24}$, realising the $4d$-to-$2d$ celestial map to
$\mathbf H_{\Delta_5}$-module category \cite{EguchiOoguriTachikawa2010,Gaiotto2009}.
