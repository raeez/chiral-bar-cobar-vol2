...
\cite[Lem~6.1]{Costello2011}). The contour integral over
...
(Costello \cite[\S10]{Costello2011}) computes the chiral two-loop
...
of Costello \cite[\S11]{Costello2011}. The
...
Applying Proposition~\ref{prop:disk3-action-classical-observables}
...
Theorem~\ref{thm:costello-factalg-5brane}(ii), produces
\begin{equation}
...
\mathcal V^{K3}_5,
\end{equation}
a $\Disk_3$-algebra on $E\times\R$. The factorisation homology
\begin{equation}
...
\cite{AyalaFrancisTanaka2017}, Thm~4.1.3): the K3-factor yields the
umbral-twisted character $\chi[\mathbf H_{\Delta_5}](\tau,z)$ of the
K3 chiral bialgebra
...
Theorem~\ref{thm:costello-m24-anomaly-h3}, acquires a
...
\begin{proof}[Proof]
The equivariant decomposition~\eqref{eq:costello-z6-grading}
follows from Theorem~\ref{thm:costello-factalg-5brane}
...
$\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$ on the $\mathsf{B}$-family,
with $K^{\kappa_{\mathrm{ch}}} = 2c_+(L)$ the Mukai-doubling
...
$K^{\kappa_{\mathrm{ch}}} = 2c_+(\mathrm{II}_{4,20}) = 8$,
$\hbar^2 = -1/8$. For the Monster ($L = \mathrm{II}_{1,1}$,
Koike--Norton--Zagier denominator $j(\sigma) - j(\tau)$):
$c_+ = 1$, $K^{\kappa_{\mathrm{ch}}} = 2$, $\hbar^2 = -1/2$. For
the Fake Monster ($L = \mathrm{II}_{25,1} = \Lambda_{24}\oplus\mathrm{II}_{1,1}$,
$\Phi_{12}$ denominator): $c_+ = 25$, $K^{\kappa_{\mathrm{ch}}} = 50$,
$\hbar^2 = -1/50$. Cross-reference Vol.~I
