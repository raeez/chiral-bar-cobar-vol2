...
$\SCchtop$ itself, and part~(i) is the content of
Chapters~\ref{ch:factorization-swiss-cheese}
and~\ref{ch:modular-sc-operad}. The conjecture asserts that
...
By Dunn additivity, $\Ethree \simeq \Etwo \circ \Eone$.
An \emph{$\Ethree$-chiral algebra} is an $\Eone$-algebra in
$\Etwo$-chiral algebras: concretely, a factorisation algebra on
$X \times \R$ that is holomorphic (chiral) on the Riemann
surface~$X$ and topological on~$\R$. This is the
holomorphic-topological (HT) bulk structure governed by
$\SCchtop$. It \emph{depends} on the complex structure of~$X$.
The derived chiral center $\Zder^{\mathrm{ch}}(A)$ of a boundary
chiral algebra~$A$ carries this structure: chiral factorisation
in the $X$-directions from the $\Etwo$-chiral factor, topological
factorisation in the $\R$-direction from the $\Eone$ factor. This
is the structure the present volume constructs for the $3$d HT bulk.
\end{definition}
...
\begin{conjecture}[The $\Ethree$-topological climax; \ClaimStatusConjectured]
\label{conj:E3-topological-climax}
\index{E3-topological algebra@$\Ethree$-topological algebra!climax conjecture}
\index{derived center!E3 topologization@$\Ethree$ topologization}
\index{conformal vector!climax conjecture}
\index{Feigin--Frenkel center!critical level obstruction}
Let $A$ be a chiral algebra with conformal vector $T(z)$ at
non-critical level $k \ne -h^{\vee}$. Then the derived chiral
center $\Zder^{\mathrm{ch}}(A)$ admits an $\Ethree$-topological
structure: the conformal vector topologizes the $\Etwo$-chiral
factor, yielding a fully topological $3$d theory independent
of the complex structure of~$X$.

At critical level $k = -h^{\vee}$, the conformal vector
degenerates and topologization fails: $\Zder^{\mathrm{ch}}(A)$
remains $\Ethree$-chiral but not $\Ethree$-topological. In
the extreme case where the conformal vector is absent entirely,
the bulk is stuck at $\Etwo$-chiral, recovering the
Feigin--Frenkel center $\mathfrak{z}(\hat{\fg})$ as the
$\Etwo$-chiral derived center at critical level.
\end{conjecture}
...
the volume's climax.
Movements~I--V construct the $\Ethree$-chiral structure on
$\Zder^{\mathrm{ch}}(\mathrm{Vir}_c)$: bar-cobar duality gives
the $\Eone$ factor (topological, on~$\R$), while the Virasoro
OPE gives the $\Etwo$-chiral factor (holomorphic, on~$X$).
The conformal vector $T(z)$ at $c \ne 0$ (non-critical for
$\mathfrak{sl}_2$) should topologize the $\Etwo$-chiral factor,
promoting the full structure to $\Ethree$-topological.
Conjecture~\ref{conj:E3-topological-climax} asserts that this
promotion succeeds whenever the conformal vector is non-degenerate.
Without it, the bulk depends on the complex structure of~$X$ and
one has a holomorphic-topological theory rather than a TQFT.
\end{remark}
...
receives contributions from all arities, and the
