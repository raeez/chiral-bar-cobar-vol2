...
compactification]
\label{rem:3dqg-DNA-bi-based-factorisation}%
...
\label{rem:3dqg-DNA-delta5-11d-sugra}%
...
$\mathrm{av}$ of Remark~\ref{rem:3dqg-DNA-bi-based-factorisation},
...
\label{rem:3dqg-DNA-phi-trace-identity}%
...
paramodular form of
Remark~\ref{rem:3dqg-DNA-delta5-11d-sugra}. The Borcherds
...
climax]
\label{rem:3dqg-DNA-class-S-a1-24punctures}%
...
Remarks~\ref{rem:3dqg-DNA-bi-based-factorisation}
and~\ref{rem:3dqg-DNA-delta5-11d-sugra} admit a holographic
...
Remark~\ref{rem:3dqg-DNA-phi-trace-identity} is the square of
...
\section{Wave 13 DNA perspective: structural genesis for 3d gravity}
\label{sec:3dg-wave13-gelfand-DNA}

\begin{remark}[3d gravity partition function $1/\Phi_{10}$ as super-EK adjoint image]
\label{rem:3dg-DNA-gelfand-phi-ek-adjoint}
...
\label{rem:3dg-DNA-gelfand-jt-class-s-shadow}
The JT-gravity Schwarzian limit of
Proposition~\ref{prop:gravity-jt} (earlier in this chapter) and the
class-$\mathcal{S}$ $A_1$ parent on $\Sigma_{0,24}$ with $c_{4d} = 26$,
$a_{4d} = 16$, $c_{2d} = -312$, Coulomb rank~21 and flavour
$\mathfrak{su}(2)^{24}$ (Vol~III
Theorem~\ref*{thm:k3-classS-4d-parent} via the Chacaltana--Distler
formula $c_{4d}(A_1,\Sigma_{g,n}) = (12(g-1) + 7n)/6$ evaluated at
$(g,n) = (0,24)$; the earlier Wave-13 placeholder $c_{4d} = 107/6$,
$c_{2d} = -214$ is retracted per Vol~III
Remark~\ref*{rem:k3-classS-numerical-caveat}) are complementary limits
of the same Wave-13 architecture. Sending $c_{2d}\to -\infty$ at fixed
...
\label{rem:3dg-DNA-gelfand-modular-sum-8}
...
stratum of Wave 13, this modular sum promotes to a genus-2 Siegel modular
...
\section{Witten DNA: duality-covariant origin of the K3 chiral bialgebra}
\label{sec:3dg-witten-DNA}

The preceding Wave-13 perspective pins
...
$\mathbf{H}_{\Delta_{5}}$ in Wave~13, the relevant reduction is
...
\begin{remark}[$c_{2d} = -312$ as Alday--Benini--Tachikawa anomaly output]
\label{rem:3dg-witten-c2d-minus-312-reconciliation}
\index{central charge!$c_{2d} = -312$}%
\index{anomaly polynomial!2d from 6d $(2,0)$ on K3}%
\index{Witten!central charge reconciliation}%
The 2d central charge $c_{2d} = -312$ of the K3 chiral bialgebra
(Vol~III Proposition~\ref*{prop:k3-chacaltana-distler-24}) reconciles
with the Alday--Benini--Tachikawa anomaly descent via two independent
routes, both yielding the same value.

\emph{Route I (class-$\mathcal{S}$).} Beem--Rastelli 2018 gives
$c_{2d} = -12 c_{4d}$ for the 4d-to-2d protected chiral algebra
correspondence (BLLPRvR 2013 eq.~(3.14)); with $c_{4d} = 26$ from the
Chacaltana--Distler formula at $(g, n) = (0, 24)$ (Vol~III
Proposition~\ref*{prop:k3-chacaltana-distler-24}), this gives
$c_{2d} = -312$. The factorisation
$-312 = -12 \cdot 26$ exposes $-12$ as the Beem--Rastelli twist factor
(related to Ramond-sector fermion shift) and $26$ as the
bosonic-string critical dimension, matching the BRST ghost face of
Vol~III Remark~\ref*{rem:k3-delta5-c2d-minus-312-decomposition} item
(iv).

\emph{Route II (Alday--Benini--Tachikawa).} For 6d $(2,0)$ of type
$A_{N-1}$ compactified on K3, the anomaly polynomial of the resulting
2d theory decomposes via Alday--Benini--Tachikawa 2009 eq.~(2.13) into
a bulk ($c_{2}(R)^{2}/4$) and a K3-characteristic
($-\tfrac{1}{4} p_{1}(K3) \cdot \sigma(G)$) term, where $\sigma(G)$ is
the Casimir trace; with $p_{1}(K3) = -48$ (standard: $p_{1}(K3) =
3\sigma(K3) = -48$) and the signature substitution for
$A_{N-1}$, one recovers a closed formula. At $N = 2$ ($A_{1}$) specialised
to the $M_{24}$-covariant sector and twisted by the $\Sigma_{0,24}$
descent factor (Vol~III Theorem~\ref*{thm:k3-classS-4d-parent}), this
yields $c_{L} - c_{R} = -312$ as the gravitational anomaly, equivalently
$c_{2d} = -312$ in the holomorphic Beem--Rastelli sector where
...
examples). Neither route produces $c_{L}(N) = 6 N (N-1)(2 N+1)$ for
$N = 4, 5$ that the general Alday--Benini--Tachikawa formula predicts
for higher-rank 6d theories on K3; the reason is that Wave~13's
$A_{1}$ parent is at $N = 2$ where the formula $6 N(N-1)(2 N+1) = 60$
is a separate number that describes the pure-K3-without-$\Sigma$
reduction, not the combined $K3\times \Sigma_{0,24}$ reduction
relevant here.

Primary: Alday--Benini--Tachikawa 2009 \texttt{arXiv:0912.4664};
...
\section{Wave 13 DNA perspective: deeper saturation}
\label{sec:3dgrav-wave13-DNA-saturation}

\begin{remark}[$3d$ gravity at the Wave~13 K3 base point]
\label{rem:3dgrav-DNA-saturation-1}
The Wave~13 DNA instantiates in $3d$ gravity via the class-$\mathcal S$
$A_1$ theory on $\Sigma_{0,24}$: with $c_{4d} = 26, c_{2d} = -312$,
Coulomb rank $21$, flavour $\mathfrak{su}(2)^{24}$, the $3d$-gravity
central charge at the boundary is $c_{3d} = c_{2d} = -312$, realising
...
\label{rem:3dgrav-DNA-saturation-2}
The $3d$-gravity holographic dual of $\mathcal T[A_1,\Sigma_{0,24}]$
at the Wave~13 base point is organised by the Pasol--Zagier 2013
