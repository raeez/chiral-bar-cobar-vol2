...
For $\cW$-algebras obtained by principal DS reduction
\textup{(}including Virasoro\textup{)}, it is also a theorem
\textup{(}Theorem~\ref{thm:E3-topological-DS}\textup{)}:
the Costello--Gaiotto theorem provides the $3$d HT theory,
and the BRST identity
$T_{\mathrm{DS}} = [Q_{\mathrm{CS}}, G']$ is verified by
constructing the DS-transported antighost~$G'$ from the
