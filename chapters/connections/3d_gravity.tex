...
is a freely generated Poisson vertex algebra. Assume that the
Khan--Zeng Poisson-vertex sigma model attached to
\(\operatorname{gr}_{\mathrm{Li}}(\cA)\) admits a chosen renormalized
half-space BV quantization whose boundary factorization algebra is
\(\cA\), satisfying the hypotheses of
...
of \cite[Theorem~4.1]{KZ25}. The assumed half-space BV quantization
then produces a quantum $3$d bulk with boundary~$\cA$. Applying
Construction~\ref{constr:topologization} to this quantized bulk gives
the $\Ethree$-topological structure.
\end{proof}
...
The K3 synthesis instantiates in $3d$ gravity via the class-$\mathcal S$
