...
degree-$4$ vanishing at the symmetric point.
...
The leading derivative in $m_4$ is $\partial T$: the depth-$2$ and
...
-\partial^4 T
\;-\;
(4\lambda_{12} + 5\lambda_{23}
 + 2\lambda_{34} + 3\lambda_{45})\,\partial^3 T
\\[3pt]
&\quad -\;
(2\lambda_{12}^2 + 19\lambda_{12}\lambda_{23}
 + 6\lambda_{12}\lambda_{34} + 9\lambda_{12}\lambda_{45}
 + 8\lambda_{23}^2 + 6\lambda_{23}\lambda_{34}
\\
&\qquad\qquad
 + 12\lambda_{23}\lambda_{45}
 + 3\lambda_{34}^2
...
$Q_5(\lambda,\ldots,\lambda) = -60\lambda^6$.
...
\item \textup{($n = 4$ anomaly.)}
 At degree~$4$, the two lowest depths $d = 0$ (the field $\partial^3 T$)
 and $d = 1$ (the field $\partial^2 T$) both vanish by cancellation
 among the Stasheff compositions, not by structural obstruction. This
 is the anomaly observed in~\eqref{eq:gravity-m4}: the leading
 derivative term is $2(\lambda_{12}-\lambda_{34})
 (\lambda_{12}-\lambda_{23}+\lambda_{34})\,\partial T$ at depth~$2$,
 with no $\partial^3 T$ or $\partial^2 T$ term, so
...
(iii): At degree~$4$, the depth-$0$ term ($\partial^3 T$ with constant
coefficient) and the depth-$1$ term ($\partial^2 T$ with linear
coefficient) both arise from the five Stasheff compositions, but their
coefficients cancel in the total $m_4$, as verified by direct
computation in Proposition~\ref{prop:gravity-m4}: the surviving leading
derivative is $\partial T$ at depth~$2$. At degrees~$2$,
