...
% AP1: kappa(Vir) from CLAUDE.md C2; c=0 -> 0, c=13 -> 13/2 verified
...
% AP1: kappa(KM) from CLAUDE.md C3; k=0 -> dim(g)/2, k=-h^v -> 0 verified
...
\item The bulk theory $\Zder^{\mathrm{ch}}(\mathrm{Vir}_c)$
  ...
  $\R_{\ge 0} \times \C$.  These cochains are the bulk
  gravity observables.  Bar-cobar inversion provides the
  dictionary.
\item The Drinfeld double $U_A = A \bowtie A^!$, with
  $A^! = \mathrm{Vir}_{26-c}$, reconstructs the full algebra of
  bulk observables coupled to boundary states.  Bulk line
  operators are $\mathrm{Vir}_{26-c}$-modules.
