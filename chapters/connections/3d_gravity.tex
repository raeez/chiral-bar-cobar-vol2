...
is open only for conformal vertex algebras whose Li-filtration
associated graded is \emph{not} freely generated (e.g., the
Moonshine module~$V^\natural$).
\end{conjecture}

\begin{remark}[The orbifold route for $V^\natural$]
\label{rem:monster-orbifold-route}
\index{Monster VOA!orbifold route to E3-topological}
The Moonshine module $V^\natural = V_\Lambda^+$ is the
$\Z/2$-orbifold of the Leech lattice VOA~$V_\Lambda$.
Since $V_\Lambda$ is $\Ethree$-topological
(Theorem~\ref{thm:E3-topological-free-PVA}, abelian
holomorphic CS provides the 3d bulk), the invariant
subalgebra $V_\Lambda^+$ inherits an $\Ethree$-topological
structure by taking $\Z/2$-invariants of the factorisation
algebra (finite group invariants preserve $E_n$-algebra
structure). The conformal vector $T$ and the antighost~$G$
are both $\Z/2$-invariant (quadratic in the currents), so
$T = [Q,G]$ descends to the orbifold.
The full $V^\natural$ (including the twisted sector) requires
the $\Z/2$-gauged holomorphic CS theory, an orbifold BV
quantisation not yet constructed in the Costello--Li framework,
but expected to be tractable (the anomaly vanishes by modular
invariance of~$V^\natural$).
Thus Conjecture~\ref{conj:E3-topological-general} for
$V^\natural$ reduces to a bounded technical construction:
orbifold BV quantisation of abelian holomorphic CS.
\end{remark}
...
and freely-generated-PVA lanes
\textup{(}Theorems~\ref{thm:E3-topological-km},
\ref{thm:E3-topological-DS},
\ref{thm:E3-topological-DS-general},
\ref{thm:E3-topological-free-PVA}\textup{)}; a general
chain-level lift remains conjectural
