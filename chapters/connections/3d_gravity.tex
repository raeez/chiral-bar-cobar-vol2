...
tower).  The remaining open problem is the
\emph{operadic verification} at genus $\ge 2$: that
the clutching maps satisfy the full modular operad
...
%    thm:E3-topological-km, conj:E3-topological-DS,
...
Movements~I--V conjecturally construct the $\Ethree$-chiral
structure on $\Zder^{\mathrm{ch}}(\mathrm{Vir}_c)$: bar-cobar
duality gives the $\Eone$ factor (topological, on~$\R$), while
the Virasoro OPE gives the $\Etwo$-chiral factor (holomorphic,
on~$X$). The $\Ethree$-chiral structure requires a $3$d HT
gravity theory whose boundary is~$\mathrm{Vir}_c$.
The Costello--Gaiotto theorem provides such a theory
(holomorphic CS with DS boundary conditions); the
remaining gap is the BRST identity
$T_{\mathrm{DS}} = [Q_{\mathrm{tot}}, G']$ in the
DS-modified BV complex
\textup{(}Conjecture~\ref{conj:E3-topological-DS}\textup{)}.
