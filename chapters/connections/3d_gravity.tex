...
\begin{proposition}[Pole-order classification]%
...
\begin{corollary}[Gauge-gravity-matter complexity trichotomy]%
...
\begin{proposition}[Formality--depth--discriminant trichotomy]%
...
\begin{proposition}[Quaternary Virasoro $\Ainf$ operation]
...
\begin{proposition}[Quinary Virasoro $\Ainf$ operation]
...
\begin{theorem}[Period-$2$ parity and Catalan factorisation]
...
\begin{proposition}[Graviton tracelessness]
...
(The normalized low-degree values $S_2 = \kappa = c/2$ and $S_3=2$
determine the shadow metric but are not themselves generated by it.
The $-c$ ternary scalar belongs to the unnormalised gravity transfer
lane.)
On the scalar lane, the resolvent reduces to an integral.

\begin{theorem}[Graviton resolvent: closed form]
...
$S_2 = \kappa = c/2$ and $S_3=2$; these determine the shadow
metric~\eqref{eq:shadow-metric-Q} but are not generated by it.  The
unnormalised ternary gravity scalar is proportional to $-c$ and is
kept out of the $S_3$ notation.
...
\begin{theorem}[Gravitational Koszul triangle]
...
\item $\Omega(D_{\Ran}(\barB)) \simeq \mathrm{Vir}_{26-c}$ on
 ...
 Theorem~A, and that bar complex is then inverted by the same
 bar-cobar counit);
...
\begin{proposition}[Virasoro Koszul dual]
...
\begin{proposition}[Gravitational YBE]
...
\begin{proposition}[SDR for the DS complex]\label{prop:ds-sdr}
...
\begin{theorem}[DS-HPL transfer]\label{thm:ds-hpl-transfer}
...
\begin{proposition}[Coproduct primitivity at degree~$2$]\label{prop:coproduct-degree2-vanishing}
...
\begin{theorem}[Gravitational MC element]
...
\begin{proposition}[Gravitational primitive package]
\label{prop:grav-open-six-requirements}
Definition~\textup{\ref{def:grav-open-algebraic}} satisfies
requirements \textup{(i)--(iii)} and \textup{(v)--(vi)} of the
gravitational primitive package. It satisfies requirement~\textup{(iv)}
exactly under Conjecture~\textup{\ref{conj:gravity-line-identification}}:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{theorem}[$\Ethree$-topological for affine Kac--Moody]
...
\begin{theorem}[$\Ethree$-topological via DS reduction]
...
\begin{theorem}[$\Ethree$-topological via non-principal DS reduction]\label{thm:E3-topological-DS-general}
...
\begin{theorem}[$\Ethree$-topological for quantized freely generated PVAs]\label{thm:E3-topological-free-PVA}
...
$K(\fg,k)$, and logarithmic triplet algebras $\cW(p)$ lie in
case~\ref{item:scope-orbifold}. A conformal chiral algebra outside
these two cases must supply a non-critical conformal vector, a
boundary HT realisation, and a quantized topological lift before
Conjecture~\ref{conj:E3-topological-general} can be applied.
\end{remark}
...
\begin{theorem}[Stasheff cancellation]
...
lanes (these determine the shadow metric but are not its Taylor
coefficients). The unnormalised gravity ternary scalar proportional to
$-c$ is not denoted $S_3$.
...
\begin{proposition}[Denominator formula]
...
\begin{theorem}[Catalan--Dynkin parity]
...
\begin{theorem}[Crossing = $d_{\barB}^2 = 0$]
...
\begin{proposition}[Gravitational Page curve]
...
\begin{proposition}[de~Sitter shadow obstruction tower]
...
\begin{proposition}[JT shadow correspondence]
...
The reduced Bruinier obstruction in the K3 scalar lane is
\[
 c_2^{\triangle}=0,\qquad
 c_3^{\mathrm{Br}}=-8[H_3].
\]
...
non-zero reduced class \(c_3^{\mathrm{Br}}=-8[H_3]\).
\end{proof}
...
$c$ under physical Cardy hypotheses]
...
under analytic continuation hypotheses]
