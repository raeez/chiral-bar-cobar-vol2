...
partition-function bridge as conjectural.
...
closed-form expression rather than a weight-indexed matrix. Cardy and
BTZ formulae are physical interpretations of that scalar data only
after the modular and saddle hypotheses are added.
\end{remark}
...
\item The reciprocal branch points \(A_\pm = 1/t_\pm\) give the
 candidate Stokes directions for the field-sector Borel transform
...
transform; \ClaimStatusConditional]
...
The effective scalar partition series for the SL$(2,\R) \times$
...
for the exceptional-edge contraction at every genus.  The remaining
operadic assertions separate as follows:
\begin{enumerate}[label=\textup{(\alph*)}]
...
\(E_1\)-chiral quantum groups by Proposition~\ref{prop:qt-equivariance};
in the general \(\Ainf\)-chiral construction it is an explicit
\(R\)-matrix monodromy-compatibility hypothesis.
 ...
 stable graph. This is the content of quasi-triangularity in the
 quantum-group branch; for the general \(\Ainf\)-chiral branch it
...
$\Ethree$-topological algebra structure provided the lifted
...
\emph{Step 3 (finite-orbifold anomaly datum).} The obstruction to
...
input together with the trivialized class from Step~$3$. By the
...
$\Ethree$-topological structure is proved in the affine and DS lanes
\textup{(}Theorems~\ref{thm:E3-topological-km},
\ref{thm:E3-topological-DS},
...
$\mathrm{Vir}_c$-descendants. At descendant level~$n$, the fixed-PBW
contraction involves the level-$n$ Shapovalov Gram determinant; this is
a product/nonvanishing input, not a least-eigenvalue estimate. The
total is:
\[
...
factor of $\eta(\tau)^{-1}$. The oscillator exponent
...
Assume the annular trace is the physical thermal partition function of
a unitary CFT, that the Cardy saddle controls the high-temperature
density of states, and that the connected two-level correlation has
GUE sine-kernel statistics in the ergodic window. Then the spectral
form factor decomposes into three regimes whose scalar prefactors are
controlled by the genus-$1$ tower:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\end{enumerate}
The scalar slope depends on the genus-$1$ curvature datum
...
type on the ordered bar. The gauge sector is the unique
...
Assume a completed BTZ partition function whose holomorphic seed has
scalar projection
$F_g^{\mathrm{scal}}=\kappa\lambda_g^{\mathrm{FP}}$, and assume
modular invariance, high-temperature vacuum dominance, and a
non-degenerate saddle-point expansion relating the inverse Laplace
transform to microcanonical degeneracies. Under these hypotheses, the
...
\item For Virasoro at $c$:
 ...
 = 2(20 - c/2)/48 = (40 - c)/48$.
\item The scalar shadow partition function
 $Z^{\mathrm{sh}}
...
\((\mathrm{Vir}_c,\mathrm{Vir}_{26-c})\) suggests a paired entropy:
\[
...
normalizes the two scalar towers attached to the Koszul-dual
Virasoro pair. A Page curve requires more data than these scalar
coefficients: it requires a raw gravitational transseries with
...
Assume Conjecture~\ref{conj:gravity-page-raw-transseries}. The
conjectural Page profile associated to the two transseries sectors is
the piecewise-linear entropy
...
from $F_g=\kappa\lambda_g^{\mathrm{FP}}$ alone.
\end{remark}
...
algebra $\cA = \mathrm{Vir}_c$, and $B(\cA^!)$ built from its
Koszul dual $\cA^! = \mathrm{Vir}_{26-c}$. Complementarity
(Theorem~C) gives the scalar decomposition
\[
...
After the Page time it assigns dominance to $B(\cA^!)$ and entropy
...
where the Koszul dual sector $B(\cA^!)$ provides the larger
...
$F_g=\kappa(\mathrm{Vir}_c)\lambda_g^{\mathrm{FP}}$ is convergent;
it supplies the curvature normalization but not the Gevrey-$1$
growth or the Borel singularity. Under the raw-transseries
hypotheses, the Page time is the corresponding non-perturbative
scale:
...
exchanges the dominant sector from $B(\cA)$ to $B(\cA^!)$.
\end{conjecture}
...
$\sum_g \kappa\lambda_g^{\mathrm{FP}}\hbar^{2g}$ converges with
...
the Hawking sector $B(\cA)$ and the island sector $B(\cA^!)$.
...
$t_P=3S_{\mathrm{BH}}/13$ and the self-dual value
...
the chain-level bar trace; \ClaimStatusConditional]
...
$\{|\zeta| < 1/C\}$. If, in addition, $\widehat{\mathcal F}_{A}$
admits sectorial analytic continuation with exponential bounds away
from a locally finite singular set, and if the relevant alien
derivatives close on the factorisation-BV algebra, then the directional
Laplace integral
...
\begin{proposition}[Virasoro vacuum-module coefficient growth and
conditional Cardy density; \ClaimStatusConditional]
\label{prop:3dg-hardy-ramanujan-cardy}
\index{proposition!Hardy--Ramanujan density Virasoro Vol II}%
Let $\rho_{\mathrm{vac}}(n)$ be the dimension of the weight-$n$ component
$\Vir_c[n] = \{v : L_0 v = n v\}$ of the universal Virasoro chiral
algebra at generic central charge $c \in \R_{> 1}$. Then
\begin{equation}
...
instead studies a unitary modular-invariant CFT with vacuum dominance
and effective central charge \(c_{\mathrm{eff}}\), Cardy's theorem
\cite{Cardy86} gives the physical density
\[
...
$c$ under physical Cardy hypotheses; \ClaimStatusConditional]
...
Fix $c \in \R_{> 1}$. Let $\mathcal F^{\mathrm{phys}}_c(\tau)
...
invariance, vacuum dominance, and the physical Cardy density stated in
...
in the THQG BV-extension programme) is the natural place to prove an
...
Assume the completed factorisation-BV effective action has a
nondegenerate saddle $\Phi_\ast$ whose sectorial expansion controls
the Borel transform of $\mathcal F_A^{\mathrm{raw}}$. If the
renormalised on-shell action of this saddle is
$S^{\mathrm{eff}}[\Phi_\ast]=2\pi\cdot c(c-25)/24$ in the chosen
normalisation, then the corresponding candidate Borel-plane action is
\(\zeta_\ast=1/(2\pi)\). The effective action whose saddle must be
analysed is
\[
 S^{\mathrm{eff}}[\Phi]
 \;=\;
...
\]
The candidate residue is \(\rho_c=c(c-25)/24\), provided the
one-loop determinant at \(\Phi_\ast\) produces the Virasoro beta
coefficient. The proof obligation is the analytic saddle theorem
identifying this candidate with an actual Borel singularity of the
ordered bar trace.
\end{proposition}

\begin{proof}
Under the hypotheses, the location and residue are the standard
instanton-action dictionary for a Gevrey-$1$ transseries:
singularities of the Borel transform occur at saddle actions and
carry the corresponding one-loop determinant/residue. The local
factorisation-BV QME supplies the formal effective action and its
candidate saddle equation; it does not by itself prove sectorial
analytic continuation or identify the ordered bar trace with the
saddle expansion. Those analytic inputs are exactly the hypotheses of
the proposition.
\end{proof}
...
Assume the Borel transform of the completed ordered bar trace has a
singularity at $\zeta_{\ast} = 1/(2\pi)$ and that its alien derivative
is defined by sectorial analytic continuation. The alien derivative
...
the raw chain-level bar trace; \ClaimStatusConjectured]\label{thm:3dg-master-gravity}%
...
direct planar rooted-tree count. The Zamolodchikov-norm and
...
step (1) is unconditional from the heptagon bound; step (2) is
conditional on sectorial Borel continuation and alien-derivative
closure at chain level
(\autoref{thm:3dg-class-M-borel-unconditional}); step (3) is
conditional on the physical Cardy/Zwegers identification rather than
on the bare Verma-module coefficient asymptotic; step (4) is then
...
Step (4), conditional \(\mathrm{SL}_{2}(\Z)\)-orbit Borel--Poincar\'e
...
\(\Phi_{10}^{\mathrm{un}}=\mathrm{Borch}(\phi_{0,1}^{K3})\) realises the
...
\emph{Why the sum over geometries is not fundamental.} The
...
combinatorics); on the $\Phi_{10}^{\mathrm{un}}$ Borcherds product
...
\begin{remark}[Master gravity bridge and the programme loop]%
\label{rem:3dg-master-closes-loop}%
\index{master gravity bridge!programme loop closure}%
...
$\hbar^2\cdot K^{\kappa_{\mathrm{ch}}} = -1$ at
$\hbar^2 = -1/8, K^{\kappa_{\mathrm{ch}}} = 8$
