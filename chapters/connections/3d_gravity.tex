...
The two low-degree coefficients $S_2 = c/12$ and $S_3 = -c$
...
(The low-degree values $S_2 = c/12$ and $S_3 = -c$ are the
...
\item \emph{Affine Kac--Moody $V_k(\fg)$ at non-critical level.}
 The $3$d HT theory is holomorphic Chern--Simons~\cite{CostelloLi2020}.
 The Sugawara element
 $T_{\mathrm{Sug}} = \frac{1}{2(k+h^\vee)}\sum_a {:}J^a J_a{:}$
 is BRST-exact: $T_{\mathrm{Sug}} = [Q, G]$, where
 $G = \frac{1}{2(k+h^\vee)}\sum_a {:}J^a\,\bar c_a{:}$ is the
 antighost contraction
 \textup{(}Volume~I,
 Construction~\textup{\ref*{constr:sugawara-antighost}}\textup{)}.
 The denominator $2(k + h^\vee)$ diverges at the critical level
 $k = -h^\vee$, so the construction applies precisely when
 $k \ne -h^\vee$.
 The $\Ethree$-topological factorisation algebra so obtained is
 perturbative Chern--Simons;
 Costello--Francis--Gwilliam~\cite{CFG26} construct the same
...
\item \emph{Virasoro $\mathrm{Vir}_c$ at $c \ne 0$.}
 ...
 The $3$d HT theory is $3$d gravity.
 The BRST complex of $3$d HT gravity and the identity
 $T = [Q, G]$ therein are \emph{conjectural}
 (Conjecture~\textup{\ref{conj:E3-topological-climax}} below);
 once established, the present construction delivers
 $\Ethree$-topological.
\item \emph{$\cW$-algebras $\cW_N$ at generic level.}
 The conformal vector is the Virasoro subalgebra.
 The $3$d HT theory is higher-spin gravity.
 Again, the BRST input is conjectural; the topologization
...
non-critical level. Then the $3$d holomorphic-topological bulk
theory on $X \times \mathbb{R}$ whose boundary is~$A$ admits a
BV-BRST quantisation with differential~$Q$ and a field~$G(z)$
satisfying $T(z) = [Q, G(z)]$. By
Construction~\textup{\ref{constr:topologization}}, this gives
$\Zder^{\mathrm{ch}}(A)$ an $\Ethree$-topological structure
independent of the complex structure of~$X$.

The conjecture is verified for affine Kac--Moody at non-critical
level
\textup{(}Remark~\textup{\ref{rem:topologization-scope-3d}(i)}\textup{)}.
For Virasoro and $\cW$-algebras, the remaining obstacle is the
construction of the $3$d HT BRST complex ($3$d gravity and
higher-spin gravity respectively) and the verification of
$T = [Q, G]$ therein; the topologization mechanism itself
is unconditional
\textup{(}Construction~\textup{\ref{constr:topologization}}\textup{)}.
...
For affine Kac--Moody at non-critical level, the BRST input
$T = [Q, G]$ is verified
(Theorem~\textup{\ref*{thm:topologization}} of Volume~I).
Conjecture~\ref{conj:E3-topological-climax} asserts that the
same BRST input exists for Virasoro ($3$d gravity) and
$\cW$-algebras (higher-spin gravity).
