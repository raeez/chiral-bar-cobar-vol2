...
charge $c = 3\ell/(2G_N)$, and Witten showed in 2007 that this
boundary algebra determines the BTZ black hole spectrum, the Cardy
degeneracy formula, and the modular structure of the partition function.

The identification was correct. The mechanism was missing.
Witten's analysis identifies the boundary chiral algebra but does not
...
\item The Koszul triangle closes: boundary
 ...
 $S_{\mathrm{BTZ}} =
 2\pi\sqrt{ch/6} + 2\pi\sqrt{c\bar h/6}$ follow from the
...
OPE pole ensures a nonzero residue at every degree
...
the non-vanishing of the wheel integral at every degree
(Proposition~\ref{prop:vir-truncation}).
\end{proof}
...
at large conformal weight (Conjecture~\ref{conj:gravity-cardy})
...
\begin{remark}[$\Delta_5$ as the 1-loop-forced anomaly class of twisted 11D supergravity]
\label{rem:3dqg-delta5-11d-sugra}%
\index{11D supergravity!twisted!K3$\times T^2$}%
\index{M5-brane!24 at Kodaira I$_1$}%
\index{Igusa cusp form!$\Delta_5$!1-loop-forced}%
\index{paramodular form!weight 5}%
The secondary anomaly class $u = \kappa_{\mathrm{eff}}\cdot\omega_g$
of the scalar-specialised model above receives a physical origin at
the K3$\times T^2$ Omega-deformed compactification of 11D
supergravity. In that frame the weight-$5$ Igusa/Gritsenko--Borcherds
...
\]
(Hodge bundle on the genus-$2$ Siegel moduli) is the unique
$1$-loop-forced output of twisted 11D supergravity on
K3$\times T^2$ with $24$ M5-branes wrapping the I$_1$ Kodaira
fibres of the elliptic K3. The weight decomposes as
\[
\mathrm{wt}(\Delta_5)
\;=\;
\chi(\cO_{\mathrm{K3}})
\;+\;
\sum_{\mathrm{I}_1}
\Res_{\mathrm{Kodaira}\,\mathrm{I}_1}
\;=\;
2 \;+\; 3
\;=\; 5,
\]
with the $2$ from the holomorphic Euler characteristic of
$\mathrm{K3}$ (Noether) and the $3$ from the residue sum at the
$24$ I$_1$ fibres (each contributing $1/8$ of the Gritsenko lift
of the genus-$2$ eta product). Four duality frames
\begin{enumerate}[label=\textup{(\alph*)},leftmargin=*]
\item the heterotic frame of Harvey--Moore
  \cite{HarveyMoore1996}\,;
\item the Type IIB D1--D5 on K3$\times S^1$ frame of
  \cite{DijkgraafVerlindeVerlinde1997}\,;
\item the Type IIA Mathieu-moonshine frame of
  \cite{DijkgraafMooreVerlindeVerlinde1997}\,;
\item the M-theory on K3$\times T^3$ frame of
  \cite{Witten1995}
\end{enumerate}
produce the same paramodular form $\Delta_5$. The paramodular
anomaly-cancellation condition is the vanishing of a Deligne
cohomology class in
$H^2\bigl(\overline{\cA}_2,\;\cO^*\bigr) = 0$, forcing $\Delta_5$
to be the unique section up to scalar. Under the averaging map
$\mathrm{av}$ of Remark~\ref{rem:3dqg-bi-based-factorisation},
the gravitational anomaly class
$u = (c-26)/2\cdot\omega_g$ specialises at genus~$2$ to the
divisor class $[5H_1 + 10H_4]$ of $\Delta_5$ inside
$\mathrm{Pic}(\overline{\cA}_2)$: the K3$\times T^2$ compactification
realises the Virasoro class-$\mathbf{M}$ dichotomy geometrically,
with $c_{\mathrm{crit}} = 26$ corresponding to the vanishing of
$[\Delta_5]$ in the quotient bundle.
Primary-lit: \cite{HarveyMoore1996,DijkgraafVerlindeVerlinde1997,
DijkgraafMooreVerlindeVerlinde1997,Witten1995,
CostelloGaiotto2018,Costello2021}; cross-reference to Vol~III
...
\medskip\noindent
\textbf{Step~2: The improvement term for general nilpotents.}
The DS stress tensor for a general nilpotent~$f$ with
neutral element $h_0$ decomposes as
...
\end{equation}
with $x_0 := \tfrac{1}{2}h_0 \in \fh$ the conformal grading
element\footnote{%
...
in the Cartan basis. The key structural fact is that $x_0$
is \emph{always a Cartan element}: for any nilpotent $f$ in any
simple~$\fg$, the neutral element $h_0$ of the
$\mathfrak{sl}_2$-triple $(e, h_0, f)$ lies in the Cartan
subalgebra $\fh \subset \fg$
\textup{(}Jacobson--Morozov theorem; the $\mathfrak{sl}_2$-triple
can always be chosen with $h_0 \in \fh$\textup{)}.
...
\begin{theorem}[$\Ethree$-topological for freely generated PVAs; \ClaimStatusProvedHere]\label{thm:E3-topological-free-PVA}
...
By Theorem~\ref{thm:E3-topological-free-PVA}, this conjecture
holds for every conformal vertex algebra whose Li-filtration
associated graded is freely generated, and by
Theorem~\ref{thm:monster-orbifold-e3} it holds for the
Moonshine module~$V^\natural$ via the Leech lattice orbifold
route. The conjecture therefore remains open \emph{only} for
non-freely-generated conformal vertex algebras that admit
neither an orbifold nor coset presentation from a freely
generated parent with vanishing Dijkgraaf--Witten anomaly;
no example from the standard landscape is currently known
to fall in this frontier case.
\end{conjecture}
...
to the $\sigma$-fixed sublattice. Because $\Lambda$ is
\emph{even unimodular} (signature $(24,0)$, determinant~$1$,
all inner products even), the Arf invariant of the
$\sigma$-fixed quadratic form vanishes
\cite[\S~1.4]{Conway1968}: the $+1$-eigenspace of~$\sigma$
contains only the origin, so the restricted form is the
...
\begin{remark}[Scope and frontier after Theorem~\ref{thm:monster-orbifold-e3}]
\label{rem:monster-orbifold-route}
\index{Monster VOA!orbifold route to E3-topological}
Theorem~\ref{thm:monster-orbifold-e3} closes
Conjecture~\ref{conj:E3-topological-general} for the
Moonshine module. The argument does \emph{not} yet
universalise: for a general holomorphic $c = 24$ VOA, the
van Ekeren--M\"oller--Scheithauer theorem~\cite{EMS20}
expresses it as an orbifold of a Niemeier lattice VOA by a
finite automorphism~$g$, and the corresponding
Dijkgraaf--Witten class
$\alpha_g \in H^3(B\langle g\rangle, U(1))$
must be computed case-by-case. For each of the $71$
Schellekens VOAs where $\alpha_g = 0$, the same four-step
proof promotes $\Ethree$-topologicality to a theorem; the
Monster corresponds to one of these cases (the unique one
with no weight-one primary) and is the deepest test of the
method. The genuine remaining frontier of
...
$\Ethree$-topological structure is proved in the affine, DS,
freely-generated-PVA, and orbifold lanes
\textup{(}Theorems~\ref{thm:E3-topological-km},
\ref{thm:E3-topological-DS},
\ref{thm:E3-topological-DS-general},
\ref{thm:E3-topological-free-PVA},
\ref{thm:monster-orbifold-e3}\textup{)}; a general
chain-level lift outside these lanes remains conjectural
...
one-loop gravitational path integral, and its higher-degree
components encode the genus-$1$ graviton vertices that dress
the BTZ spectrum.

\begin{remark}[Universal $\Phi$-trace identity: $1/\Phi_{10}$ from
the Chiral Hochschild Trinity]
...
\index{$1/\Phi_{10}$!3d gravity partition function}%
The genus-$1$ BTZ partition function admits a genus-$2$ extension
that realises, at the 3d-gravity climax of this volume, the
\emph{universal $\Phi$-trace identity} relating the Chiral
Hochschild Trinity $K(A) = \tr_{Z(A)}(K_A)$ to the weight-$5$
paramodular form of
Remark~\ref{rem:3dqg-delta5-11d-sugra}. The Borcherds
singular-theta pushforward along the Kuga--Satake map evaluates
the Trinity invariant at the K3 chiral bialgebra
$\mathrm{Vir}_c\otimes
V_{\Lambda_{\mathrm{K3}}}$ of Vol~III
Chapter~\ref*{ch:k3-chiral-bialgebra-platonic} to yield
\begin{equation}\label{eq:phi-trace-identity-3dqg}
\kappa_{\mathrm{BKM}}\bigl(\mathfrak{g}_{\Delta_5}\bigr)
\;=\;
\frac{c_{\Lambda}(0)}{2}
\;=\; 5
\;=\; \mathrm{wt}\bigl(\Delta_5\bigr),
\end{equation}
with $c_\Lambda(0)$ the zeroth Fourier coefficient of the elliptic
genus of K3 at the reduced lattice $\Lambda_{\mathrm{K3}}$. The
3d-gravity climax partition function on
$\mathrm{AdS}_3\times\mathrm{K3}$ is correspondingly
\[
Z_{\mathrm{3dQG}}(Z)
\;=\;
\frac{1}{\Phi_{10}(Z)}
...
Z \in \mathfrak{H}_2,
\]
on genus-$2$ Siegel moduli, where $\Phi_{10}$ is the Igusa cusp
form of weight $10$ (\cite{DijkgraafVerlindeVerlinde1997}) and
the square relation $\Phi_{10} = \Delta_5^{\,2}$ is the
Gritsenko--Nikulin identity
\cite{GritsenkoNikulin1998}.
The Chiral Hochschild Trinity on the bulk-boundary pair
$(C^\bullet_{\mathrm{ch}}(\mathrm{Vir}_c,\mathrm{Vir}_c),
\mathrm{Vir}_c)$ of
Part~\ref{part:gravity} therefore specialises, at the K3
compactification, to produce the BPS counting function of
Dijkgraaf--Verlinde--Verlinde's three-dimensional quantum gravity
partition function, bridging
Vol~I's five-theorem core $\leftrightarrow$ Vol~II's $\SCchtop$
framework $\leftrightarrow$ Vol~III's CY-to-chiral functor $\Phi$
via a single trace identity.
\end{remark}
...
\item The cubic shadow $S_3$ is gauge-trivial for Virasoro
 ...
 \end{equation}
\end{enumerate}
At large $c$: $S^{(2)} \sim 2/c^2 \to 0$, confirming that
...
MC equation), so $S_r$ gives a Ward identity at each degree.
...
$\Sigma_{0,24}$ this vertex operator algebra is the
$24$-copy tensor product of $\widehat{\mathfrak{sl}_2}$ at
critical level, with Gaiotto gluings at each of the $24$
punctures. Its Schur index is the genus-$2$ Siegel character of
$\mathbf{H}_{\Delta_5}$ (Vol~III
Chapter~\ref*{ch:k3-chiral-bialgebra-platonic},
Theorem~\ref*{thm:k3-siegel-character}):
\[
\cI_{\mathrm{Schur}}^{4\mathrm{d}}\!\bigl[A_1\text{ on }\Sigma_{0,24}\bigr](\tau)
\;=\;
\tr_{\mathbf{H}_{\Delta_5}}
\!\bigl(q^{L_0 - c/24}\bigr)
\;=\;
\bigl(Z\text{-coefficient of }\Delta_5\bigr)(\tau).
\]
The Schur index is the $(0,24)$-punctured-sphere shadow of the
full $\Delta_5$ character, and the 3d-gravity partition function
$1/\Phi_{10} = 1/\Delta_5^{\,2}$ of
Remark~\ref{rem:3dqg-phi-trace-identity} is the square of
this Schur index in the Kuga--Satake frame. The 4d $\mathcal{N}{=}2$
parent $\leftrightarrow$ 3d-gravity holographic duality realised
here is an avatar of the AdS$_3$/CFT$_2$ correspondence restricted
to the $\kappa$-universality locus: the class-$\mathcal{S}$
Schur sector plays the role of boundary chiral algebra, the bulk
3d-gravity saddle is the K3$\times T^2$ compactification of
twisted 11D supergravity, and the $24$-puncture sphere is the
$M_{24}$ Mathieu-moonshine modular shadow of the $24$ Kodaira
fibres. Primary-lit: \cite{BeemRastelli2018,
...
\begin{remark}[3d gravity partition function $1/\Phi_{10}$ as super-EK adjoint image]
\label{rem:3dg-gelfand-phi-ek-adjoint}
The 3d quantum gravity partition function $Z_{3dQG}(Z) = 1/\Phi_{10}(Z)$ on
the Kuga--Satake side of the averaging morphism
(Vol~III Theorem~\ref*{thm:k3-bi-based-factorization}) admits a Gelfand
structural reading as the projective representation character of the
super-EK adjoint image $\mathbf{H}_{\Delta_5}$ of the BKM Manin pair
$(\mathfrak{g}_{\Delta_5}, \mathfrak{n}_+^{\mathrm{imag}}\oplus
\mathfrak{h}^{\mathrm{imag,rk\,23}})$
(Vol~III Theorem~\ref*{thm:k3-structural-genesis}). Because
$\Phi_{10} = \Delta_5^2$ and the super-EK classification cohomology group is
one-dimensional
($H^2(\mathfrak{g}_{\Delta_5})^{\mathbb{Z}/2,K(1)}\cong \mathbb{C}\cdot\Delta_5$),
every factor of the modular bootstrap of 3d gravity is controlled by the
same cocycle class. The bulk 3d-gravity saddle is the K3$\times T^2$
compactification of twisted 11D supergravity, with 24 M5-branes on the
$I_1$ Kodaira fibres producing $\Delta_5$ as its 1-loop anomaly-cancellation
output (Vol~III Theorem~\ref*{thm:k3-1loop-output}). The protected-sector
boundary chiral algebra is the $M_{24}$-averaged Schur sector of
$\mathcal{T}[A_1,\Sigma_{0,24}]$ whose first ten Fourier coefficients are
$\{1,72,2678,68474,1351775,21945390,304799105,3720945220,40716498035,
405322063500\}$ (Vol~III Proposition~\ref*{prop:k3-schur-q-expansion};
computationally verified at
\texttt{compute/lib/k3\_yangian\_wave14\_schur\_index\_classS\_A1\_24.py}).
The 3d-gravity statement is the square of the boundary Schur index,
$1/\Phi_{10} = 1/\Delta_5^2$, in the Kuga--Satake frame.
\end{remark}
...
Theorem~\ref*{thm:k3-classS-4d-parent} via the Shapere--Tachikawa
...
the intermediate the K3 synthesis Chacaltana--Distler ansatz
$c_{4d} = (12(g-1) + 7n)/6|_{(0,24)} = 26$, $c_{2d} = -312$ is
retracted per Vol~I Chapter~\ref{ch:thqg-open-closed-realization}
line~1932 on the $\mathrm{SU}(2)$ $N_f = 4$ cross-check, and per
Vol~III Remark~\ref*{rem:k3-classS-numerical-caveat}) are
complementary limits
of the same the K3 synthesis architecture. Sending $c_{2d}\to -\infty$ at fixed
...
Vol~III Conjecture~\ref*{thm:k3-stress-tensor-sheaf}. The
...
Theorem~\ref*{thm:schur-to-delta5-composite}.
\end{remark}

\begin{remark}[Why $K^{\kappa_{\mathrm{ch}}}=8$ controls the 3d-gravity modular sum]
\label{rem:3dg-gelfand-modular-sum-8}
In the Maloney--Witten sum-over-saddles for 3d gravity, modular images of
the thermal AdS$_3$ saddle are labelled by $\mathrm{SL}_2(\mathbb{Z})$ cosets
and regularised by the Rademacher-series (MTRR) technology. On the K3-input
stratum of the K3 synthesis, this modular sum promotes to a genus-2 Siegel modular
sum over $\mathrm{Sp}_4(\mathbb{Z})$ cosets, and the relevant Borcherds
product is $1/\Phi_{10}$. The order $K^{\kappa_{\mathrm{ch}}} = 8$ Humbert
monodromy enters as the counting exponent of saddles that cross the
Humbert wall $H_1 \subset \overline{\mathcal{A}_2}$: at any modular image
whose limit lies on $H_1$, the $\mathcal{D}$-module contribution cycles
through 8 nearby-cycles sectors before closing, so the Rademacher sum
splits into 8 arithmetic progressions indexed by this monodromy. The
three-face structural origin of $8$ (Mukai $c_+ = 4$, Humbert monodromy,
Lusztig $\ell = 8$; Vol~I Remark~\ref*{rem:as-DNA-gelfand-8-three-faces})
forces the 3d-gravity modular sum to decompose into $8$ arithmetic streams
at every Humbert wall crossing. Primary lit: Maloney--Witten 2010 (AdS$_3$
gravity partition function); Bruinier 2002 (Heegner divisors); Gritsenko--Nikulin
1997--1998 (Igusa cusp form denominator identity); Dijkgraaf--Verlinde--Verlinde
1996--1997 (1/4-BPS dyon partition function on K3$\times T^2$).
\end{remark}

\section{The Borcherds-product ensemble bridge}
\label{sec:3dg-borcherds-ensemble-bridge}
\index{Borcherds product!ensemble bridge}
\index{Phi_{10}@$\Phi_{10}$!Maloney--Witten assembly}
\index{chiral Poincar\'e series}
\index{Maloney--Witten!Borcherds-product realisation}

The climax partition-function theorem
(Vol~II \texttt{programme\_climax\_platonic.tex}
Theorem~\ref*{thm:gravity-climax-partition-function}) produces every
perturbative saddle contribution as a chain-level bar trace on
$B^{\mathrm{ord}}(\Vir_{c})$, and
Remark~\ref*{rem:gravity-climax-maloney-witten-inheritance} records the
orthogonality: the Maloney--Witten \cite{MaloneyWitten2007}
$\mathrm{SL}_{2}(\Z)$-orbit sum is not a trace on the boundary algebra
alone and exhibits negative integer Fourier coefficients at generic
central charge. The bridge below closes this orthogonality on the
Koszul locus. The chain-level bar trace is the germ; the
Borcherds singular-theta lift of Vol~III promotes the germ to a genus-$2$
Siegel modular sum; the Maloney--Witten assembly is the restriction of
this sum to the diagonal $\tau' = \tau$. The negative-degeneracy
pathology is not a defect of the trace but a signature of the
Borcherds product $\Phi_{10}$ at discriminants $D$ where the vector-valued
elliptic genus has $c(D) < 0$.

\begin{theorem}[Borcherds-product ensemble bridge for 3d gravity]
\label{thm:3dg-borcherds-ensemble-bridge}
\ClaimStatusProvedHere
\index{theorem!Borcherds-product ensemble bridge}
On the Kuga--Satake $(2,2)$-isogeny locus of
$\overline{\cA_{2}}$ (Vol~III
Theorem~\ref*{thm:k3-bi-based-factorization}), at $c = 24$ with
boundary chiral algebra $A = \Vir_{24}$ and Koszul-duality locus
$B^{\mathrm{ord}}(A) \cong B^{\mathrm{ord}}(\Vir_{26-c})$ replaced by
the $\mathbf{H}_{\Delta_{5}}$ frame, the Maloney--Witten 3d-gravity
partition function admits the realisation
\begin{equation}
\label{eq:3dg-mw-borcherds-assembly}
Z_{\mathrm{grav}}^{\mathrm{MW}}(\tau)
\;=\;
\Res_{\tau'\to\tau}\!\Bigl(
-\log \Phi_{10}(\tau, z, \tau')
\Bigr)\Big|_{z = 0}
\;=\;
\tr_{\mathbf{H}_{\Delta_{5}}}^{\,\mathrm{Sp}_{4}(\Z)/P\text{-summed}}
\!\bigl(q^{L_{0} - c/24}\bigr),
\end{equation}
where $P\subset\mathrm{Sp}_{4}(\Z)$ is the Siegel parabolic and the
right-hand trace is the genus-$2$ Siegel-modular-summed Schur index
of the class-$\mathcal{S}$ $A_{1}$ theory on $\Sigma_{0,24}$
(Remark~\ref{rem:3dqg-class-S-a1-24punctures}). The equality is at the
level of $q$-expansions in the Fourier domain.

\textup{(i)} \emph{Orthogonality is a framing artefact on the Koszul
locus.} The chain-level bar trace
$\tr_{B^{\mathrm{ord}}(\Vir_{c})}(q^{L_{0} - c/24})$ is the germ of
$-\log\Phi_{10}$ at $\tau' = \tau$; the modular orbit assembly is the
genus-$2$ Siegel lift, which on the $(2,2)$-isogeny diagonal restricts
to the $\mathrm{SL}_{2}(\Z)$ coset sum of the germ.

\textup{(ii)} \emph{Borcherds-product realisation.} The singular theta
lift
\[
\Psi(Z)
\;=\;
...
\Phi_{10}(Z)
\]
of the weight-$0$ index-$1$ Jacobi form $\phi_{0,1}^{K3}$ of the
K3 elliptic genus \textup{(}Borcherds~$1992$; Gritsenko--Nikulin
$1996/1997/1998$; cross-reference Vol~III
Theorem~\ref*{thm:harvey-moore-threshold}\textup{)} has the
product expansion
\begin{equation}
\label{eq:3dg-phi10-borcherds-product}
\Phi_{10}(Z) \;=\;
\prod_{\substack{(m, n, \ell) > 0}}
\bigl(1 - p^{m} q^{n} y^{\ell}\bigr)^{c(4 m n - \ell^{2})},
\end{equation}
where $c(D)$ are the Fourier coefficients of $\phi_{0,1}^{K3}$ and
$(p, q, y) = (e^{2\pi i \tau'}, e^{2\pi i \tau}, e^{2\pi i z})$. The
Maloney--Witten saddle degeneracies $d_{\mathrm{MW}}(n)$ are the
restriction to $p = q$, $y = 1$ of the coefficient of $q^{n}$ in
$-\log\Phi_{10}$, multiplied by the orbit length.

\textup{(iii)} \emph{Negative-degeneracy pathology = Borcherds sign
signature.} The Maloney--Witten \cite{MaloneyWitten2007} observation
that $Z_{\mathrm{grav}}^{\mathrm{MW}}(\tau)$ has negative integer
coefficients at generic central charge translates, under the identification
of \eqref{eq:3dg-mw-borcherds-assembly}, into the Borcherds-product
sign pattern: on the Humbert divisor $H_{D}\subset\overline{\cA_{2}}$
where $c(D) < 0$, the product factor
$(1 - p^{m} q^{n} y^{\ell})^{c(D)}$ inverts, producing negative
coefficients at the corresponding saddle. The K3 elliptic genus
$\phi_{0,1}^{K3}$ of \cite{EguchiOoguriTachikawa2011} has explicit
negative Fourier coefficients $c(D)$ at $D = 3, 7, 8, 11, \ldots$
\emph{by direct inspection of the weak Jacobi form}, which reproduces
the Maloney--Witten negative-degeneracy locus without further input.

\textup{(iv)} \emph{Mock-modular completion at $c \notin 24\Z$.} When
the central charge is shifted to $c = 24 k$ with $k \notin \Z$ (the
Mathieu $M_{24}$-twined sector; Vol~II
\texttt{chapters/connections/celestial\_moonshine\_bridge.tex}
Theorem~\ref*{thm:shadow-tower-moonshine}), the bar trace pulls back
to the twined mock modular form
$h_{g}(\tau)$ of Cheng--Duncan--Harvey $2014$ umbral moonshine; the
$\mathrm{SL}_{2}(\Z)$-orbit sum becomes the Rademacher sum of $h_{g}$,
whose coefficients match the twined saddle degeneracies
(Vol~II \eqref{eq:mock-modular-decomposition}, Zwegers~$2008$
completion). At $c = 12$ (Conway sector, $V^{f\natural}$), the bridge
uses the Duncan--Mack-Crane Conway-moonshine McKay--Thompson series on
$\mathrm{II}_{1,1}\oplus E_{8}$ in place of $\Phi_{10}$, with
$N = 2$ small-superconformal mock-modular completion
(Remark~\ref*{rem:gravity-climax-conway-superselection}).
\end{theorem}

\begin{proof}
We prove \textup{(i)}--\textup{(iv)} in sequence.

\textup{(i)} The chain-level bar trace is
\[
Z_{0}(\tau)
\;=\;
\tr_{B^{\mathrm{ord}}(\Vir_{c})}\!\bigl(q^{L_{0} - c/24}\bigr)
\;=\;
\chi_{\Vir_{c}}^{\mathrm{vac}}(\tau)
\cdot
\bigl(1 + O(q)\bigr),
\]
equal to the Virasoro vacuum character
\eqref{eq:gravity-vacuum-dominance} at leading order (the bar complex
reproduces the vacuum Verma in its zeroth cohomology; higher bar
cohomology contributes the perturbative shadow tower and matches the
Bernoulli-decay asymptotic of
Corollary~\ref*{cor:gravity-climax-shadow-convergence}). The
germ condition is: the localisation of $-\log\Phi_{10}(\tau, z, \tau')$
at $(z, \tau') = (0, \tau)$ reduces by the Borcherds-product expansion
\eqref{eq:3dg-phi10-borcherds-product} to
\[
-\log\Phi_{10}(\tau, 0, \tau)\big|_{\mathrm{germ}}
\;=\;
-\sum_{\substack{(m, n) > 0}} c(4 m n)\,\log\!\bigl(1 - q^{m + n}\bigr)
\;=\;
\sum_{\substack{(m, n) > 0 \\ r \ge 1}}
\frac{c(4 m n)}{r}\,q^{r(m+n)},
\]
and the restriction to $\ell = 0$ isolates the diagonal
$4 m n = D$ with $D \ge 0$. The vacuum-character coefficient
comparison (match Virasoro $q$-series at $c = 24$) fixes the leading
normalisation and pins the germ identification.

\textup{(ii)} The Borcherds 1998 singular theta lift
\[
\Psi_{\phi}(Z)
\;=\;
...
\frac{d^{2}\tau}{\tau_{2}}
\sum_{v\in L}
\phi(\tau, v)\,e^{2\pi i \langle v, Z\rangle}
\]
on a Lorentzian lattice $L$ of signature $(2, n-2)$ produces
meromorphic Siegel forms whose product expansion is
\eqref{eq:3dg-phi10-borcherds-product} with coefficients $c(D)$
determined by the weak-Jacobi-form input. Taking
$\phi = \phi_{0,1}^{K3}$ on $L = \mathrm{II}_{2,2}\oplus\mathrm{II}_{2,0}$
(signature $(2,4)$), Gritsenko--Nikulin 1997
(\cite{GritsenkoNikulin1995}, \cite{GritsenkoNikulin1998}) identify the
output as $\Phi_{10}$; Harvey--Moore 1996 identify the 1-loop threshold
integral as $-\log|\Phi_{10}|^{2}$; Dijkgraaf--Verlinde--Verlinde 1997
identify the second-quantised BPS partition function as $1/\Phi_{10}$.
These three independent realisations converge on
\eqref{eq:3dg-phi10-borcherds-product}; the coefficient comparison with
$-\log\Phi_{10}$ expanded at $p = q$ gives the Maloney--Witten saddle
degeneracies as the orbit sum of the germ.

\textup{(iii)} The K3 elliptic genus
$\phi_{0,1}^{K3}(\tau, z)
= 2\phi_{0,1}(\tau, z) + 2 E_{4}(\tau)\phi_{-2,1}(\tau, z)/12$
(normalisation of Eguchi--Ooguri--Tachikawa 2011) has weak-Jacobi-form
expansion
\[
\phi_{0,1}^{K3}(\tau, z)
\;=\;
y + 10 + y^{-1}
+ (10 y^{2} - 64 y + 108 - 64 y^{-1} + 10 y^{-2})\,q
+ O(q^{2}),
\]
with negative Fourier coefficient $-64$ at $(n, \ell) = (1, \pm 1)$,
discriminant $D = 4 - 1 = 3$. By
\eqref{eq:3dg-phi10-borcherds-product}, the factor
$(1 - p q y)^{-64}$ contributes a negative exponent, inverting to
produce positive but oscillatory-at-leading-order coefficients in
$1/\Phi_{10}$. The restriction to the diagonal $p = q$ (Maloney--Witten
frame) carries these negative exponents into negative integer
degeneracies at the corresponding saddle, by the same sign combinatorics
as the Gritsenko--Nikulin denominator of the Borcherds--Kac--Moody
superalgebra $\mathfrak{g}_{\Delta_{5}}$. The Maloney--Witten 2007
pathology is therefore a faithful image of the Borcherds-product sign
signature of $\phi_{0,1}^{K3}$, not an independent defect.

\textup{(iv)} At the Mathieu-twined central charge $c_{g} = 24 k_{g}$
with $k_{g} \in \Z[1/N_{g}]$ for $g \in M_{24}$ of order $N_{g}$, the
Cheng--Duncan--Harvey 2014 twined elliptic genus
$\phi_{0,1}^{K3,g}(\tau, z)$ decomposes as
\[
\phi_{0,1}^{K3,g}(\tau, z)
\;=\;
\chi_{1/4\text{-BPS}}^{g}(\tau)\cdot\theta_{1}(\tau, z)^{2}/\eta(\tau)^{3}
\,+\,
h_{g}(\tau)\cdot\mu(\tau, z),
\]
with $h_{g}(\tau)$ a vector-valued mock modular form of weight $1/2$
and shadow $\chi(g)/\eta(\tau)^{3}$
(Zwegers 2008; Cheng--Duncan--Harvey 2014
\cite{ChengDuncanHarvey2014}), and $\mu(\tau, z)$ the Appell--Lerch
sum. The bar trace at twined central charge pulls back to the
Rademacher expansion of $h_{g}$ (Vol~II
\texttt{chapters/connections/celestial\_moonshine\_bridge.tex}
Theorem~\ref*{thm:shadow-tower-moonshine}): the sum over
$\mathrm{SL}_{2}(\Z)/\Gamma_{\infty}$ cosets of
$q^{L_{0} - c_{g}/24}$ converges to the Rademacher-series
reconstruction of $h_{g}$. The mock-modular completion is forced by
the non-holomorphic Zwegers completion term; the Maloney--Witten
assembly at shifted central charge is the Rademacher sum of the
twined mock modular form, with negative degeneracies now tracking
both the Borcherds sign signature and the Zwegers shadow.
\end{proof}

\begin{remark}[Test against $c = 24$ Monster sector and $c = 12$ Conway sector]
\label{rem:3dg-borcherds-test-monster-conway}
\index{Monster sector!c=24}
\index{Conway sector!c=12}
At $c = 24$, the boundary chiral algebra is the Monster vertex
operator algebra $V^{\natural}$ of Frenkel--Lepowsky--Meurman 1988,
and the bar trace
$\tr_{B^{\mathrm{ord}}(V^{\natural})}(q^{L_{0}-1})$ reproduces the
$J$-invariant $J(\tau) = j(\tau) - 744$; the
$\mathrm{SL}_{2}(\Z)$-orbit assembly is the Borcherds 1992 \emph{Monster
Lie algebra} denominator identity
$J(\sigma) - J(\tau) = p^{-1}\prod_{m > 0, n\in\Z}(1 - p^{m}q^{n})^{c(mn)}$,
which is the $\mathrm{II}_{1,1}$ specialisation of
\eqref{eq:3dg-phi10-borcherds-product}. The Maloney--Witten sum at $c = 24$
matches the $J$-series, as Witten 2007 \cite{Witten2007} proposed; the
bridge of Theorem~\ref{thm:3dg-borcherds-ensemble-bridge} identifies
Witten's extremal CFT ansatz with the chain-level bar trace of the
Monster sector.

At $c = 12$, the boundary chiral algebra is the Conway vertex
superalgebra $V^{f\natural}$ of Duncan~$2007$ with
$\mathrm{Aut}(V^{f\natural}) = \mathrm{Co}_{0}$ and input lattice
$\mathrm{II}_{1,1}\oplus E_{8}$. The bar trace pulls back to the
McKay--Thompson series of Conway moonshine (Duncan--Mack-Crane 2015
\cite{DuncanMackCrane2015}); the $\mathrm{SL}_{2}(\Z)$-orbit assembly
reproduces the Conway-moonshine Rademacher sum. The
Remark~\ref*{rem:gravity-climax-conway-superselection} statement that
the Conway sector is a parallel gravity climax, not a Virasoro-rung
corollary, is sharpened here: the Conway climax is the
$\mathrm{II}_{1,1}\oplus E_{8}$ specialisation of the bridge, and its
$E_{\infty}$-operadic horizon is independent of the
$\mathrm{II}_{25,1}$ Monster horizon.
\end{remark}

\begin{remark}[Humbert-wall decomposition of the assembly]
\label{rem:3dg-borcherds-humbert-8-streams}
\index{Humbert divisor!8 arithmetic streams}
The $K^{\kappa_{\mathrm{ch}}} = 8$ Humbert monodromy of
Remark~\ref{rem:3dg-gelfand-modular-sum-8} is now visible in
\eqref{eq:3dg-mw-borcherds-assembly} as the eightfold decomposition of
the Borcherds product at the Humbert wall $H_{1}\subset\overline{\cA_{2}}$:
the residue
$\Res_{\tau'\to\tau}(-\log\Phi_{10})$ picks up eight nearby-cycle
contributions from the $\mathcal{D}$-module on
$H_{1}$, reproducing the eight arithmetic progressions of the
Rademacher decomposition. The Maloney--Witten sum is not a single
Poincar\'e series but a direct sum of eight arithmetic streams, each
labelled by a Lusztig--small-quantum-group nilpotent orbit at
$\hbar^{2} = -1/8$ (Vol~III; Lusztig $\ell = 8$). The chiral
Poincar\'e series $P^{\mathrm{ch}}(\tau)$ of the motivating question
is not a single sum but this eightfold direct sum, matching the
Humbert three-faces structural origin
(Vol~I Remark~\ref*{rem:as-DNA-gelfand-8-three-faces}).
\end{remark}

\begin{remark}[What the bridge does and does not resolve]
\label{rem:3dg-borcherds-bridge-caveats}
Theorem~\ref{thm:3dg-borcherds-ensemble-bridge} closes the orthogonality
of Remark~\ref*{rem:gravity-climax-maloney-witten-inheritance} \emph{on
the Koszul locus} $c = 24$ (Monster) and $c = 12$ (Conway), where the
boundary chiral algebra admits an automorphic product realisation via
Borcherds' singular theta lift. Off this locus, at generic
$c \in \R_{> 1}$, the Maloney--Witten pathology persists: no singular
theta lift produces a modular form on $\mathrm{Sp}_{4}(\Z)$ with the
Virasoro vacuum character as Fourier coefficient sequence, and the
Rademacher sum acquires the Maloney--Witten negative-coefficient
defect without Borcherds-product interpretation. The honest scope is:
\emph{at Koszul central charge, the bridge is a theorem; off Koszul
locus, the bridge is a conjecture about the existence of an
automorphic completion, pending further input}. The Vol~III
$\Phi_{10}$ infrastructure supplies the automorphic completion on the
K3 synthesis locus; no comparable infrastructure exists for generic
$c$.
\end{remark}
...
\textup{(i)}--\textup{(ii)}; at $c = 24$ it reduces to
Theorem~\ref{thm:3dg-borcherds-ensemble-bridge} because the Zwegers
shadow degenerates to zero and the Borel sum truncates to the
Borcherds product $\Phi_{10}$.
\end{conjecture}
...
orbit. At $c = 24$ the Stokes jumps land in $\Z$ with signs
controlled by the Fourier expansion of $\phi_{0,1}^{K3}$
(Remark~\ref{rem:3dg-borcherds-bridge-caveats} and
Theorem~\ref{thm:3dg-borcherds-ensemble-bridge}); off Koszul, the
Stokes jumps are real-valued and become integral only after the
Zwegers completion of \textup{(ii)}, which enforces
$\mathrm{SL}_{2}(\Z)$-covariance and integrality simultaneously.
...
\item \emph{Koszul locus $c = 24$ $(\mathrm{resp}.\ c = 12)$.}
Identified, per Theorem~\ref{thm:3dg-borcherds-ensemble-bridge},
without Zwegers completion (shadow vanishes).
\end{enumerate}
\end{remark}
...
Conditional on both inputs, the equality
\eqref{eq:3dg-off-koszul-bridge-main} is a theorem; the Koszul locus
$c = 24$ is the degeneration where the shadow vanishes, the Borel
sum truncates, and the conjecture specialises to
Theorem~\ref{thm:3dg-borcherds-ensemble-bridge}. Primary lit:
...
\begin{proposition}[Integrality of Stokes jumps at $c = 24$]
\label{prop:3dg-stokes-integrality-c24}
\index{proposition!Stokes jumps integrality $c = 24$}%
At $c = 24$, the Stokes jumps of the chain-level bar trace of
$\Vir_{24}$ across the positive real ray in the Borel plane are
integer-valued, and coincide with the Fourier coefficients of the
Borcherds product $\Phi_{10}$ restricted to the diagonal $p = q$.
\end{proposition}
...
is $2\pi i \cdot \rho_{24} = 2\pi i \cdot 24\cdot(-1)/24 = -2\pi i$,
and the $\mathrm{SL}_{2}(\Z)$-orbit weighting assembles the Stokes
jumps into the coefficient sequence of the diagonal restriction
$\Phi_{10}|_{p = q}$ by
Theorem~\ref{thm:3dg-borcherds-ensemble-bridge}. Integrality
follows from the Borcherds lattice $\mathrm{II}_{2,2}$ structure:
$\Phi_{10}$ has Fourier coefficients in $\Z$ by construction
(Borcherds 1992 singular theta lift), and the Zwegers shadow
$g_{24}(\bar\tau)$ vanishes because the Borel sum truncates to the
finite Fourier expansion of $\Phi_{10}|_{p = q}$.
\end{proof}
...
of \autoref{thm:3dg-master-gravity}\,\textup{(B)} has non-holomorphic
piece
...
$g_{24}(\bar\tau) \equiv 0$ and
\autoref{prop:3dg-stokes-integrality-c24} is recovered as the
shadow-vanishing specialisation. At $c = 25$, $u_c = 0$ degenerates
...
Vol~II setting with the Borel--Zwegers completion language of
\autoref{thm:3dg-master-gravity}. Three independent constraints fix
...
(\autoref{thm:3dg-borcherds-ensemble-bridge}) and the
shadow-vanishing conclusion of
\autoref{prop:3dg-stokes-integrality-c24}.
...
both conditional inputs of
\autoref{thm:3dg-off-koszul-bridge-upgraded}\,\textup{(B)} are now
theorems: the Borel summability via the heptagon + factorisation BV,
the Zwegers completion via the explicit $(u_c, v_c)$ identification.
The class-$\mathsf M$ master gravity identity is therefore
unconditional on the $3d$-gravity side; the only remaining conditional
step is the Verdier-dual identification
...
\item \textup{(Koszul degeneration at $c = 24$.)} At $c \in 24\Z$ the
Zwegers shadow vanishes, the Borel sum truncates, Stokes jumps are
integer-valued \textup{(Proposition~\ref{prop:3dg-stokes-integrality-c24})},
and the bridge specialises to
Theorem~\ref{thm:3dg-borcherds-ensemble-bridge}.
\end{enumerate}
\end{theorem}
...
\textup{(C)} At $c \in 24\Z$ the $\beta$-function residue
...
Borcherds-lattice pole, the shadow vanishes
(Proposition~\ref{prop:3dg-stokes-integrality-c24}), and
\eqref{eq:3dg-off-koszul-bridge-main} specialises to the
$\Phi_{10}$-restricted equality of
Theorem~\ref{thm:3dg-borcherds-ensemble-bridge}.
\end{proof}
...
non-critical level, of central charge $c\in\R_{>1}$, the raw
chain-level bar trace $\mathcal F_{A}^{\mathrm{raw}}$ is
Borel-summable in every direction $\theta\ne\arg(\zeta_{\ast})$,
unconditionally; the Borel sum $\mathcal B\mathcal F_{A}$ is the
holomorphic seed of the Borel--Zwegers completion of
\autoref{thm:3dg-master-gravity}\,\textup{(B)}.
\end{theorem}

\begin{proof}
The heptagon Gevrey-$1$ bound (\autoref{prop:3dg-heptagon-growth-bound},
equivalently \autoref{prop:3dg-fact-bv-l0-gevrey}) is unconditional.
The single-instanton Borel singularity at $\zeta_{\ast} = 1/(2\pi)$
with residue $\rho_{c} = c(c-25)/24$ is unconditional by
\autoref{prop:3dg-fact-bv-bcov-instanton}: it is the critical
value of the factorisation-BV effective action, internal to the
Costello--Gwilliam prefactorisation algebra. The alien-derivative
closure of $\Delta_{\zeta_{\ast}}$ on the factorisation
$L_{\infty}$-algebra is unconditional by
\autoref{thm:3dg-alien-derivative-fact-bv-closure}: it is the
derived-centre bracket applied to the factorisation bar
presentation, with the chain homotopy witness $h^{\mathrm{hept}}$
from the heptagon and the Verdier-dual transfer from Theorem~B.
Borel summability in every direction $\theta\ne\arg(\zeta_{\ast})$
then follows by Nevanlinna--Sokal's criterion for Gevrey-$1$ series
with discrete alien-derivative closure
\cite[\S 3]{AnicetoSchiappa2015}: the analytic continuation to
$\{\mathrm{Re}\,\zeta > 0\}\setminus\{\zeta_{\ast}\}$ is supplied
by the factorisation-algebra axiom, the Laplace integral converges
absolutely off the Stokes ray, and the resummation ambiguity across
$\arg(\zeta_{\ast})$ is exactly the image of
$\Delta_{\zeta_{\ast}}$, chain-level witnessed.

The matrix-model large-$N$ input of \cite[Thm.~3.1]{AnicetoSchiappa2015}
is eliminated. The Nevanlinna--Sokal resummability criterion needs
only Gevrey-$1$ growth + analytic continuation through the
single-instanton singularity + alien-derivative closure, all three
of which are now chain-level theorems over
$\bR_{t}\times\bC_{z}$.
\end{proof}

\begin{remark}[Class-split theorem unconditional upgrade]
\label{rem:3dg-class-split-unconditional-upgrade}%
\index{class $\mathsf M$!unconditional upgrade}%
\autoref{thm:3dg-class-M-borel-unconditional} (stated in the
pro-object / weight-completed ambient on $B^{\mathrm{ord}}(A)$ with
the heptagon chain-homotopy witness $h^{\mathrm{hept}}$) upgrades
\autoref{thm:3dg-off-koszul-bridge-upgraded}\,\textup{(B)} and
\autoref{thm:3dg-master-gravity}\,\textup{(B)}: the
Aniceto--Schiappa--Vaz conditional of
\autoref{rem:3dg-off-koszul-remaining-conditionals}\,\textup{(1)}
is now a theorem via factorisation-BV. Thread \textup{(2)} ---
identification of the Zwegers $\mu$-function shadow --- remains
conditional on Bringmann--Ono 2007 Eichler-integral construction:
it is an automorphic statement about the $(4\pi\,\mathrm{Im}\,\tau)^{-1/2}$
shadow of the Borel-resummed seed, not about its Borel summability.
\end{remark}
...
(\autoref{thm:3dg-borel-summability-chain-level}) closes the
Laplace direction; the Borcherds-product bridge
(\autoref{thm:3dg-borcherds-ensemble-bridge}) identifies the
$c = 24$ specialisation with $\Phi_{10}$; the class-split upgrade
...
\begin{theorem}[Master gravity theorem: Borel--Zwegers completion of
...
$c \in \R_{> 1}$. The $\mathrm{SL}_{2}(\Z)$-covariant
$3d$-gravity partition function and the Borel--Zwegers completion of
the raw chain-level bar trace are identified:
...
\item \emph{Koszul locus $c \in 24\Z$.} Unconditional; the shadow
degenerates to zero, the Borel sum truncates to a rational function,
the Stokes jumps are integers
\textup{(\autoref{prop:3dg-stokes-integrality-c24})}, and the
master identity specialises at $c = 24$ to the Borcherds product
identification
\begin{equation}\label{eq:3dg-master-c24-phi10}
  ...
  \Bigr)\Big|_{z = 0}
\end{equation}
of \autoref{thm:3dg-borcherds-ensemble-bridge}, and at $c = 12$ to
the Duncan--Mack-Crane \cite{DuncanMackCrane2015} Conway-moonshine
McKay--Thompson series on $\mathrm{II}_{1,1}\oplus E_{8}$.
\end{enumerate}
\end{theorem}
...
\textup{(C)} At $c \in 24\Z$ the residue $\rho_{24k}$ degenerates to
the Monster-covariant locus: the Borel-plane singularity coincides
with a pole of the Borcherds lattice $\mathrm{II}_{2,2}$, the shadow
vanishes (\autoref{prop:3dg-stokes-integrality-c24}), the Laplace
integral truncates to the diagonal restriction
$\Phi_{10}|_{p=q}$ (\autoref{thm:3dg-borcherds-ensemble-bridge}),
and the Rademacher assembly becomes the genus-$2$ Siegel residue of
\eqref{eq:3dg-master-c24-phi10}. The analogous statement at $c = 12$
is the $\mathrm{II}_{1,1}\oplus E_{8}$ specialisation with
Conway-moonshine McKay--Thompson series
\cite{DuncanMackCrane2015} in place of $\Phi_{10}$.
\end{proof}
...
Step (3), the Koszul-locus specialisation to $\Phi_{10}$, is the
Vol~III Borcherds--Igusa theorem
(\autoref{V3-thm:harvey-moore-threshold} of
\autoref{V3-ch:k3-chiral-bialgebra-platonic}): the
singular-theta-lift identification
$\Phi_{10} = \mathrm{Borch}(\phi_{0,1}^{K3})$ pins the
$c = 24$ Zwegers shadow to zero and realises the
$\mathrm{Sp}_{4}(\Z)$ cusp form as the automorphic completion of
the Monster-sector bar trace. The K3 chiral bialgebra
$\mathbf{H}_{\Delta_{5}}$ of Vol~III is the bi-based
$\mathrm{Sp}_{4}(\Z)$-covariant trace carrier, and
\eqref{eq:3dg-master-c24-phi10} is the diagonal residue of the
Siegel lift.
...
The master identity \eqref{eq:3dg-master-gravity-identity}
resolves the status of 3d quantum gravity as a mathematical object
across the programme's three volumes in the following sense.

\emph{What 3d gravity is.} The 3d-gravity partition function
$Z^{\mathrm{MW}}_{\mathrm{grav}, A}(\tau)$ is the
...
\emph{What is conditional.} Class $\mathsf G$ and the
$c \in 24\Z$ Koszul locus are unconditional theorems. Class
$\mathsf M$ at generic $c$ is conditional on two analytic inputs:
the alien-derivative closure at chain level, and the Zwegers
$\mu$-function shadow identification. Both are tractable and
primary-literature-adjacent; neither is expected to fail. The
honest scope is:
unconditional at $\mathsf G$, at $c = 24$ (Monster), at $c = 12$
(Conway); conditional on two tractable analytic inputs elsewhere.
...
The master identity closes the loop from the Vol~II
...
K3 elliptic genus $\phi_{0,1}^{K3}$. The master identity
\eqref{eq:3dg-master-gravity-identity} fixes these two objects to
each other at $c = 24$ (step (4) + step (3) Koszul degeneration):
the heptagon count $b_{n}(\Vir_{24})$, assembled via the
Rademacher sum over $\mathrm{SL}_{2}(\Z)/\Gamma_{\infty}$, equals
the Fourier coefficient of $-\log \Phi_{10}(\tau, 0, \tau)$ at
level $n$. The combinatorial and automorphic descriptions of
3d gravity at the Monster central charge are the same object.
...
Theorem~\ref*{thm:k3-classS-4d-parent}) and then evaluating its
...
Proposition~\ref*{prop:k3-chacaltana-distler-24}) reconciles with the
...
factorisation $-214 = -2\cdot 107$ (with $107$ prime) replaces the
retracted factorisation $-312 = -12\cdot 26$: $107 =
...
Theorem~\ref*{thm:k3-classS-4d-parent}), this yields
...
The intermediate the K3 synthesis value $c_{2d} = -312$ (via the
Chacaltana--Distler ansatz $c_{4d} = (12(g-1) + 7n)/6$ evaluated at
$(g, n) = (0, 24)$ to give $26$) is retracted: the ansatz fails the
$\mathrm{SU}(2)$ $N_f = 4$ cross-check at $(0, 4)$, returning
$8/3$ whereas Shapere--Tachikawa $2008$ Table~4 and Tachikawa $2013$
eq.~(13.6) give $7/6$; see Vol~I
Chapter~\ref{ch:thqg-open-closed-realization} line~1932 for the
numerical audit. Primary: Alday--Benini--Tachikawa $2009$
...
superalgebra $\mathfrak{g}_{\Delta_{5}}$ that features in the 3d
gravity partition function $Z_{3dQG} = 1/\Phi_{10} = 1/\Delta_{5}^{\,2}$
admits a direct string-theoretic realisation through the
...
level (Vol~III Theorem~\ref*{thm:k3-abelian-at-lie}) is the
...
Theorem~\ref*{thm:harvey-moore-threshold},
Theorem~\ref*{thm:bkm-simple-roots-walls}, and
...
\section{deeper saturation}
\label{sec:3dgrav-saturation}

\begin{remark}[$3d$ gravity at the K3 synthesis base]
\label{rem:3dgrav-saturation-1}
The the K3 synthesis DNA instantiates in $3d$ gravity via the class-$\mathcal S$
$A_1$ theory on $\Sigma_{0,24}$: with canonical Shapere--Tachikawa
first-principles values $c_{4d} = 107/6$, $c_{2d} = -214 = -2\cdot 107$,
Coulomb rank $21$, flavour $\mathfrak{su}(2)^{24}$ (Shapere--Tachikawa
$2008$ \S 3; intermediate the K3 synthesis $(c_{4d}, c_{2d}) = (26, -312)$
retracted per Vol~I
Chapter~\ref{ch:thqg-open-closed-realization} line~1932), the
$3d$-gravity central charge at the boundary is $c_{3d} = c_{2d} = -214$,
realising
the BKM-monster slice via the Cheng--Duncan--Harvey 2014 umbral
moonshine mapping of the $M_{24}$-twined K3 elliptic genus to a $3d$
gravity partition function. The bi-based Ran pair
$(\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24}),\bar{\mathcal A}_2)$ provides
the $3d$-gravity configuration space (the $24$ Kodaira $I_1$ fibres as
boundary defects) and the Siegel-modular holographic dual (via Kuga--Satake)
\cite{ChengDuncanHarvey2014,Gaiotto2009,KugaSatake1967}. Cross-ref
Vol.~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Siegel-modular holography and universal identity]
\label{rem:3dgrav-saturation-2}
The $3d$-gravity holographic dual of $\mathcal T[A_1,\Sigma_{0,24}]$
at the base point is organised by the Pasol--Zagier 2013
universal Igusa form $\Phi_{10}$ on $\mathrm{Sp}_4(\mathbb Z)\backslash\mathbb H_2$:
the denominator identity $\prod(1-q^{r\alpha})^{c(r\alpha)} = \Phi_{10}$
encodes the $3d$-gravity one-loop determinant with K3-BKM algebra
$\mathfrak g_{\Delta_5}$ as gauge algebra. The universal identity
$\hbar^2\cdot K^{\kappa_{\mathrm{ch}}} = -1$ at $\hbar^2 = -1/8, K = 8$
is the $3d$-gravity semiclassical limit $\hbar\to 0$ (Newton coupling)
intersected with the BKM stack $K^{\kappa_{\mathrm{ch}}} = 8$
