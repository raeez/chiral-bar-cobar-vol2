...
The bar complex records the same local collision data that a
configuration-space Feynman expansion uses.  The edge form is fixed in
...
complex therefore carries logarithmic edge kernels and OPE vertex
residues. The Fulton--MacPherson compactification replaces
short-distance collisions by normal-crossing boundary strata.
Theorem~\ref{thm:loop-genus-formula} gives the exact correction to the
common loop/genus slogan: graph loop number is $h^1(\Gamma)$, whereas
ribbon-surface genus also depends on the number of boundary faces.
\end{remark}
...
graph.  Surface genus is obtained only after adding ribbon or modular
...
\(\kappa(\mathrm{Vir}_c)=c/2\), while the divided-power
...
To \emph{define} $m_n$ recursively, for example by homotopy transfer, one
...
\(\eta_{ij}=d\log(z_i-z_j)\) instead.  The common structure is the
...
\in \frac{c}{12}\,\bC
\]
with the scalar determined by the two oriented transfer trees and the
