...
Half-space BV quantization of the affine Khan--Zeng Poisson sigma model on $\C \times \R_{\ge 0}$ produces the affine Kac--Moody boundary VOA; the genus-0 projection of $\Theta^{\mathrm{oc}}$ is one-loop exact. The affine Poisson--vertex sigma model on $\C \times \R_{\ge 0}$ is mixed BF/Chern--Simons theory, solved to all loop orders by the Gwilliam--Williams half-space BV theorem. The polynomial logarithmic Khan--Zeng class is not left at this affine boundary: the doubled FM/raviolo calculus constructs the image-choice reflected graph weights, Theorem~\ref{thm:edge-45} identifies the quantum master equation with the $\SCchtop$ convolution Maurer--Cartan equation, and the relative-Feynman/harmonic contraction solves the all-loop recursion on finite jet--weight quotients. Theorem~\ref{thm:general-half-space-bv} is the resulting constructive theorem.
\end{remark}
...
The new content is that the Neumann side is now fully quantized, so the expected linear Koszul pair
...
\begin{theorem}[Polynomial half-space BV theorem for Poisson vertex sigma models; preview]\label{thm:general-half-space-bv-preview}
...
that the holomorphic translation operator is inner. The corresponding
...
\ClaimStatusProvedHere{} (realized by the image-choice
FM/raviolo compactification in Theorem~\ref{thm:doubling-rwi}).
...
The doubling theorem (Theorem~\ref{thm:doubling-rwi}) resolves this:
the half-space calculus is the image-choice, field-parity-corrected
...
nonlinear Poisson--vertex half-space. The development of the equivariant perspective
...
single vertex may approach its own image at $t_i=0$. These faces are
the half-space images of ordinary doubled collision divisors after
the image-choice blowup separates the fixed-locus directions. Their
residues are the \emph{image-charge corrections} that enforce the
Neumann boundary condition, and they produce the equivariant
correction terms in the quantized action.
\end{enumerate}
...
\]
where $K$ is the bulk propagator of the holomorphic-topological theory
(meromorphic in $z-z'$, exponentially decaying in $|t-t'|$) and
$\varepsilon_e$ is the field-parity reflection sign at the image
endpoint. The displayed plus sign is part of the convention: Dirichlet
or odd reflected fields enter through $\varepsilon_e=-1$, not by
removing the image summand. Extending this kernel to boundary faces is
the image-choice compactification problem solved in
Theorem~\ref{thm:doubling-rwi}; the image term is the source of the
plus-diagonal and self-image residues.
\end{construction}
...
half-space BV theory. Then the boundary factorization identities are
equivalent to the reflected weight identity:
\begin{enumerate}
\item The renormalized boundary operations of the Poisson--vertex
...
\end{enumerate}
The implication $(2)\Rightarrow(1)$ is algebraic after the
...
The construction of the Costello effective action, counterterm scheme,
and all-loop QME is supplied below by
Construction~\ref{cons:kz-bv-fields-action},
Construction~\ref{cons:kz-renormalized-weights}, and
Proposition~\ref{prop:kz-analytic-sdr-package}, hence by
Proposition~\ref{prop:kz-all-loop-counterterm-recursion}.
\end{theorem}
...
condition at $t=0$ forces the propagator to satisfy topological-time
reflection symmetry. The reflection is
$(z,t)\mapsto(z,-t)$: it fixes the boundary $\C\times\{0\}$ and does
not conjugate the holomorphic coordinate. Thus the reflected propagator calculus of
...
\begin{enumerate}
 ...
 \item The general nonlinear case is resolved by the image-choice
 doubling construction (Theorem~\ref{thm:general-half-space-bv}), which
 pushes the half-space Kontsevich integral to the standard whole-space
 Stokes identity through field-parity reflection.
\end{enumerate}
...
the first jet-order correction to the Poisson kernel. The construction of
...
\ClaimStatusProvedHere{} (by Theorem~\ref{thm:doubling-rwi}; independent of the all-loop counterterm recursion).
...
\begin{proof}
Immediate from Theorem~\ref{thm:doubling-rwi}, which proves the
reflected weight identity at all jet orders via the doubling
construction. No renormalized BV quantization is used in this
corollary.
\end{proof}
...
reflected weight identity at every jet order by pushing the
image-choice half-space compactification through the standard
whole-space Kontsevich--FM Stokes identity.
\end{remark}
...
\begin{proof}
Immediate from Theorem~\ref{thm:doubling-rwi}.
\end{proof}
...
match the algebraically determined expressions. The resolution of this
...
construction therefore blows up the plus-diagonals and self-image
faces before pushforward to the fundamental domain.
\end{construction}

\begin{proposition}[Reflected propagator as signed image pullback; \ClaimStatusProvedHere]
...
\begin{proof}
On the dense locus this is the method-of-images formula
$K^{-}+\epsilon_{ij}K^{+}$. The ordinary term is evaluated on
$(z_i-z_j,t_i-t_j)$ and the image term on $(z_i-z_j,t_i+t_j)$; the
plus sign is fixed before the parity sign is inserted. The blowup of
the plus-diagonal and self-image faces is functorial for the divisor
arrangement cut out by $z_i=z_j$, $t_i-t_j=0$, and $t_i+t_j=0$, so the
two logarithmic pullbacks extend to the compactification with the same
residue signs as their doubled FM divisors.
\end{proof}
...
(Definition~\ref{def:log-SC-algebra}). The image-choice half-space
compactification is obtained by pulling the doubled FM boundary
arrangement to the fundamental domain, separating plus-diagonal and
self-image faces, and assigning the field-parity image signs of
Proposition~\ref{prop:reflected-pullback}. Then the reflected weight
identity~\eqref{eq:reflected-weight-identity} holds for every
admissible graph $\Gamma$ on the half-space $X=\C\times\R_{\ge 0}$.
\end{theorem}

\begin{proof}
The proof has four steps: expand by image choices, compactify, apply
Stokes, and push forward to the fundamental domain.

\medskip\noindent\textbf{Step~1: Image-choice expansion.}
For each edge $e=(i,j)$ of $\Gamma$, Proposition~\ref{prop:reflected-pullback}
gives a signed sum
\[
...
$z_i=z_j,\ t_i+t_j=0$ and the self-image faces $t_i=0$ before residue
pushforward.
...
$D_{\sigma(S)}$. The orientation of the image-choice face is the
quotient orientation obtained from the outward-normal-first convention
on the paired doubled divisors; the parity factor already appears in
the edge form, so no additional sign is introduced by pushforward. The
fundamental-domain identity is:
...
\end{itemize}
By the image-choice expansion and
Proposition~\ref{prop:reflected-pullback}, the pushed-forward form is
...
$(\Pi_V)^*=-\Pi_V$, where
$(f\partial_z^r)^*=(-\partial_z)^r\circ f$ after integration by parts
in the local-functional quotient. On
...
\Pi^{ij}_{V}(\partial_z;\Phi)\,\eta_j .
\]
Thus $S_{0,V}=I_{\mathrm{kin},V}+I_{\mathrm{int},V}$, and the free
...
condition $i^*\eta=0$. The sesquilinearity of the $\lambda$-bracket is
the separate statement that this local-functional calculus is
compatible with the $\partial_z$-jet variables.
...
chosen HT gauge}.
\]
For an edge $e$ the reflected half-space kernel has singular part
\[
K^{\mathrm{refl}}_e(z,t;z',t')
=
K(z-z',t-t')+\varepsilon_e K(z-z',t+t'),
...
$i^*\eta=0$ carries odd image parity. Self-image edges are
renormalized by the logarithmic compactification and subtraction
scheme, not evaluated by the naive kernel at zero holomorphic
separation.
For an admissible graph $\Gamma$ with vertices labelled by the
finite polynomial tensors in $\Pi_V$, define its scale-$L$
renormalized weight by the FM/raviolo extension
\[
...
Theorem~\ref{thm:doubling-rwi} identifies every boundary residue of
this image-choice half-space integral with the pushed-forward Stokes
residue of the doubled whole-space logarithmic integral.
\end{construction}

\begin{proposition}[KZ finite-quotient analytic SDR package]
\label{prop:kz-analytic-sdr-package}
\ClaimStatusProvedHere.
For the finite-type KZ input $V$, the completed local BV convolution
complex admits a complete filtered system of jet--weight quotients
\[
\widehat C_V^\bullet=\varprojlim_{N,w}C_{V,N,w}^\bullet
\]
with surjective transition maps. On each quotient the renormalized
half-space BV algebra carries operators $Q_{N,w}$, $\Delta_{N,w}$,
the BV bracket, and
$d_{V,N,w}=Q_{N,w}+[I_{\mathrm{int},V},{-}]_{\mathrm{BV}}$, and these
operators commute with truncation. The reflected graph weights of
Construction~\ref{cons:kz-renormalized-weights} extend
logarithmically to the half-space/raviolo compactification and satisfy
the image-choice Stokes identity of Theorem~\ref{thm:doubling-rwi}.
There are quotient-compatible SDR data
\[
\mathrm{id}-H_{V,N,w}
...
\]
preserving locality and the renormalization scheme. For every
inductively defined QME obstruction
\[
\mathrm{Ob}_{\ell,N,w}
=
...
\]
the harmonic projection vanishes:
$H_{V,N,w}(\mathrm{Ob}_{\ell,N,w})=0$.
\end{proposition}

\begin{proof}
Finite jet order and polynomiality give the jet--weight filtration;
each quotient has only finitely many local monomials in a fixed total
weight, and truncation maps are surjective. The operators
$Q$, $\Delta$, the bracket, and $[I_{\mathrm{int},V},{-}]$ are local
bidifferential operators of finite jet order, hence descend to the
quotients and commute with truncation.

Construction~\ref{cons:kz-renormalized-weights} gives the logarithmic
kernel on the dense half-space locus. The blowup functor in
Theorem~\ref{thm:doubling-rwi} extends the ordinary and image
pullbacks across plus-diagonal and self-image faces, and its
orientation pushforward is Stokes-compatible. This supplies the
reflected residue identity on every quotient.

The SDR is the tensor product of two proved contractions. The
tree/open-colour part is the Boardman--Vogt/bar-cobar special
deformation retract furnished by homotopy-Koszulity of $\SCchtop$
(Theorem~\ref{thm:homotopy-Koszul}) and transported through the
QME--convolution identification of Theorem~\ref{thm:edge-45}. In the
loop/handle sectors, the relative-Feynman harmonic projector and
homotopy of Definition~\ref{def:harmonic-projector} and
Theorem~\ref{thm:harmonic-gauge-existence-uniqueness} give the same
side conditions on each finite quotient. The homological perturbation
lemma transfers these special deformation retracts to
$d_{V,N,w}=Q_{N,w}+[I_{\mathrm{int},V},{-}]_{\mathrm{BV}}$: the
affine-linear part is absorbed into the base finite-quotient
differential, and every nonlinear KZ vertex raises polynomial
jet--weight, so the perturbation series terminates on each quotient.

The obstruction class splits by graph type. The tree part is zero by
Lemma~\ref{lem:kz-cme-pva-jacobi}; the half-space boundary part is zero
by Theorem~\ref{thm:doubling-rwi}; the loop/handle part maps under the
relative Feynman skeleton to the curved-Dunn obstruction complex, where
Theorem~\ref{thm:curved-dunn-bridge} and
Theorem~\ref{thm:harmonic-gauge-existence-uniqueness} kill the
harmonic projection. Hence $H_{V,N,w}(\mathrm{Ob}_{\ell,N,w})=0$.
\end{proof}
...
Proposition~\ref{prop:kz-analytic-sdr-package}, define
...
satisfies
\[
QI_V+\frac12\{I_V,I_V\}_{\mathrm{BV}}
+\hbar\Delta I_V=0,
...
$d_V I_{\ell,V}+\mathrm{Ob}_\ell=0$. Suppressing the finite quotient
indices $(N,w)$, the
identities
\[
...
order $\ell-1$ give $d_V\mathrm{Ob}_{\ell}=0$. Under
Theorem~\ref{thm:edge-45}, this closed element is the degree-two
obstruction class in the $\SCchtop$ convolution Maurer--Cartan complex.
By Proposition~\ref{prop:kz-analytic-sdr-package},
$H_V(\mathrm{Ob}_\ell)=0$ on each finite quotient.
Therefore
\[
\mathrm{Ob}_\ell
=d_Vh_V\mathrm{Ob}_\ell
+h_Vd_V\mathrm{Ob}_\ell
=d_Vh_V\mathrm{Ob}_\ell .
\]
The choice \eqref{eq:kz-qme-recursion} solves the order-$\ell$
equation. The quotient-compatibility and surjectivity in
Proposition~\ref{prop:kz-analytic-sdr-package} give the complete
...
Let $V$ be a finite-type freely generated Poisson vertex algebra
whose $\lambda$-bracket is polynomial of finite jet order, and assume
that the holomorphic translation operator is inner. For the
Khan--Zeng Poisson--vertex sigma model on
...
equation from the PVA axioms. Construction~\ref{cons:kz-renormalized-weights}
constructs the reflected logarithmic graph weights on the
half-space. Proposition~\ref{prop:kz-analytic-sdr-package} supplies
the finite-quotient SDR package and obstruction vanishing.
Proposition~\ref{prop:kz-all-loop-counterterm-recursion}
...
by Theorem~\ref{thm:doubling-rwi}. By
...
The doubling argument proves the reflected weight identity as a
fundamental-domain Stokes theorem. On the dense locus the reflected
propagator is the signed method-of-images pullback. At boundary faces
the involution has fixed points; the image-choice compactification,
field-parity image signs, and self-image renormalization are the
half-space data that make the pullback a logarithmic graph-weight
calculus rather than a slogan about fixed loci.

Thus the half-space geometry introduces no unclosed hypothesis. Its
analytic content is the image-choice FM/raviolo blowup and the
orientation-compatible residue pushforward constructed in
...
\]
is implemented here by the image-choice expansion and the
finite-quotient SDR package, not by taking a literal fixed locus of an
ordinary doubled FM divisor.
\end{remark}

\section{Verification of the bridge-theorem inputs}
\label{sec:verification-physics-bridge}

The doubling theorem holds for any logarithmic $\SCchtop$-algebra.
For polynomial finite-jet Khan--Zeng models, the renormalized BV
package, anomaly cancellation, and QME solution are the constructions
of Theorem~\ref{thm:general-half-space-bv}. Verification now means
checking that a named family lies in this polynomial logarithmic class.
...
\item a factorized logarithmic HT parametrix;
...
\smallskip
All four inputs are verified. They are exactly the logarithmic
...
applies to this verified input.
\end{proof}
...
the following hold for the affine family in the verified
...
The half-space programme is complete for the finite-type polynomial
Khan--Zeng class whose BV observables form a logarithmic $\SCchtop$-algebra
...
the nonlinear frontier by replacing literal fixed-locus language with
image-choice blowups, field-parity signs, self-image renormalization,
and finite-quotient homological perturbation. The jet-order scheme
...
consequences: each jet-order identity is the image-choice
fundamental-domain pushforward of the whole-space Stokes identity at
the corresponding filtration level.
...
Proposition~\ref{prop:kz-analytic-sdr-package}.
...
\section{Synthesis}
\label{sec:ahs-supplement}

\begin{remark}[Affine half-space BV and K3 bi-based factorisation]
\label{rem:ahs-K3-bi-based}
The Vol~II affine half-space BV framework specialises at $K3 \times E$ to the bi-based Ran-space datum $(\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24}), \overline{\mathcal{A}_2})$ with averaging morphism $\mathrm{av} = \mathrm{Torelli}_{K3} \circ \mathrm{KugaSatake} \circ j_{\mathrm{Kodaira}}$. Each of the $24$ Kodaira $I_1$ fibres provides a local affine half-space BV boundary; the nearby-cycles $\mathcal{D}$-modules at the nodes glue to the Francis--Gaitsgory factorisation $\infty$-category on $\overline{\mathcal{A}_2}$.
\end{remark}
