...
Half-space BV quantization of the affine Khan--Zeng Poisson sigma model on $\C \times \R_{\ge 0}$ produces the affine Kac--Moody boundary VOA; the genus-0 projection of $\Theta^{\mathrm{oc}}$ is one-loop exact. The doubling theorem proves the reflected weight identity. It does not construct the renormalized BV package. The general half-space quantization statement (Theorem~\ref{thm:general-half-space-bv}) is conditional on the Costello BV data, anomaly vanishing, and a QME solution. The affine Poisson--vertex sigma model on $\C \times \R_{\ge 0}$ is mixed BF/Chern--Simons theory, solved to all loop orders by the Gwilliam--Williams half-space BV theorem.
\end{remark}
...
\begin{theorem}[Conditional half-space BV theorem for Poisson vertex sigma models; preview]\label{thm:general-half-space-bv-preview}
\ClaimStatusConditional{} (Theorem~\ref{thm:general-half-space-bv}).
...
Assume in addition a Costello-renormalized half-space BV package:
a product HT gauge fixing, a factorized logarithmic reflected
parametrix, local counterterms solving the QME to all orders, and
vanishing anomaly class. Then the Neumann half-space theory on
$\mathbb C\times\mathbb R_{\ge 0}$ has a boundary factorization
algebra which is a vertex algebra deforming the original Poisson
vertex algebra.
\end{theorem}
...
image term $K(z,t;\,z',-t')$ enforces the Neumann condition
...
It does not by itself construct the Costello effective action, the
counterterm scheme, or the all-loop QME.
\end{theorem}
...
\ClaimStatusProvedHere{} (by Theorem~\ref{thm:doubling-rwi}; independent of the conditional BV quantization theorem).
...
point is paired with its image $\sigma(p_i)=(z_i,-t_i)$.
\end{construction}
...
=K(z_i,t_i;\,z_j,t_j)+K(z_i,t_i;\,z_j,-t_j)$.
...
$\sigma(p_j)$ with the image point $(z_j,-t_j)$ in the doubled
space.
\end{proof}
...
Assume that the Poisson--vertex sigma model on
$\C\times\R_{\ge 0}$ with Neumann boundary condition admits a
renormalized Costello BV package whose reflected propagator realizes
the logarithmic weights and whose anomaly class vanishes to all orders.
...
for any logarithmic $\SCchtop$-algebra. Under the stated BV package,
the graph weights define the renormalized boundary operations. By
Theorem~\ref{thm:half-space-reduction}, the reflected weight identity
is exactly the Stokes term in the boundary $A_\infty$/factorization
relations, hence these operations assemble into a vertex algebra.
\end{proof}
...
The doubling theorem holds for any logarithmic $\SCchtop$-algebra. The
general BV quantization theorem is conditional on the renormalized BV
package, anomaly vanishing, and a QME solution. Verification means that
the standard examples satisfy the four inputs of the physics bridge
theorem (Theorem~\ref{thm:physics-bridge}), closing the logical chain
from the abstract framework to the named families.
...
\smallskip
All four inputs are verified. By Theorem~\ref{thm:physics-bridge},
