...
Half-space BV quantization of the Khan--Zeng Poisson sigma model on $\C \times \R_{\ge 0}$ produces the affine Kac--Moody boundary VOA; the genus-0 projection of $\Theta^{\mathrm{oc}}$ is one-loop exact. The general case (Theorem~\ref{thm:general-half-space-bv}) reduces to whole-space Kontsevich machinery via the doubling construction (Theorem~\ref{thm:doubling-rwi}). The affine Poisson--vertex sigma model on $\C \times \R_{\ge 0}$ is mixed BF/Chern--Simons theory, solved to all loop orders by the Gwilliam--Williams half-space BV theorem.
\end{remark}
...
hold for any logarithmic $\SCchtop$-algebra. Verification: that the
standard examples satisfy the four inputs of the physics bridge theorem
