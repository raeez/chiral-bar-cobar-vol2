...
Half-space BV quantization of the affine Khan--Zeng Poisson sigma model on $\C \times \R_{\ge 0}$ produces the affine Kac--Moody boundary VOA; the genus-0 projection of $\Theta^{\mathrm{oc}}$ is one-loop exact. The affine Poisson--vertex sigma model on $\C \times \R_{\ge 0}$ is mixed BF/Chern--Simons theory, solved to all loop orders by the Gwilliam--Williams half-space BV theorem. For the nonlinear polynomial Khan--Zeng class, the proved core is the shifted-cotangent BV action and the equivalence between the classical master equation and the PVA Hamiltonian condition. The all-loop extension is the finite-quotient recursion of Proposition~\ref{prop:kz-all-loop-counterterm-recursion}, conditional on the analytic SDR package named in Assumption~\ref{assum:kz-analytic-sdr-package}.
\end{remark}
...
\ClaimStatusConditional{} on the image-choice compactification in
...
The conditional construction of the Costello effective action,
counterterm scheme, and all-loop QME is isolated below by
...
case requires all loop orders, and the missing content is exactly the
...
The conditional general theorem
...
\ClaimStatusConditional{} on the image-choice half-space Stokes package
of Theorem~\ref{thm:doubling-rwi}.
...
construction). The doubling principle isolates the missing analytic
...
\begin{theorem}[Jacobi-reflection decomposition; \ClaimStatusConditional]
...
\begin{corollary}[The cubic identity as analytic obstruction; \ClaimStatusConditional]
...
\begin{proposition}[Reflected propagator as signed image pullback; \ClaimStatusConditional]
...
$d_V I_{\ell,V}+\mathrm{Ob}_\ell=0$. On each finite quotient, the
identities
\[
...
order $\ell-1$ give $d_V\mathrm{Ob}_{\ell}=0$. By
Assumption~\ref{assum:kz-analytic-sdr-package},
...
\]
is the heuristic geometry behind the half-space construction. A theorem
requires the image-choice expansion, field-parity signs, self-image
renormalization, and orientation pushforward recorded in
Assumption~\ref{assum:kz-analytic-sdr-package}.
\end{remark}
...
The doubling theorem is conditional on the image-choice half-space
...
Assumption~\ref{assum:kz-analytic-sdr-package}. Verification for a
named family therefore separates the proved classical Hamiltonian
condition from the analytic SDR and obstruction-vanishing input.
...
BV programme is proved by the affine one-loop exact theorem. For
...
the nonlinear frontier with the image-choice Stokes pushforward. The jet-order scheme
