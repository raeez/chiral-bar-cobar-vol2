...
\begin{remark}[Sharpened nonlinear frontier]
The jet-order scheme reduces the open nonlinear half-space problem to
a sharp sequence:
\begin{enumerate}
\item Jet order $\le 1$ (affine): PROVED
  (Theorem~\ref{thm:affine-half-space-bv}).
\item Jet order $\le 2$ (quadratic): PROVED
  (Theorem~\ref{thm:quadratic-rwi}).
\item Jet order $3$ (cubic): OPEN
  (Conjecture~\ref{conj:cubic-rwi}).  The obstruction is localized in
  the self-image propagator integral of
  Construction~\ref{cons:cubic-localization}.
\item Jet order $\ge 4$: conditional on the cubic case, higher orders
  follow by the same inductive structure, with increasingly
  constrained graph combinatorics.
\end{enumerate}
The general half-space BV conjecture
(Conjecture~\ref{conj:general-half-space-bv}) is equivalent to the
vanishing of $\delta_N(V)$ for all $N\ge 2$.  The jet-order scheme
converts this single global conjecture into a countable tower of
increasingly constrained local identities, each involving finitely
many graph types.
\end{remark}
