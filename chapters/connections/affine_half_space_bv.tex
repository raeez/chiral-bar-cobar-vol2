...
nonlinear Poisson--vertex half-space.  We now present a first-principles attack
on Conjecture~\ref{conj:general-half-space-bv} via the equivariant enhancement
...
$\ge 2$ to vanish by degree counting.  The nonlinear frontier begins at
...
\begin{conjecture}[Quadratic reflected weight identity]
...
\begin{evidence}
The conjecture is supported by three independent lines of argument.

\medskip\noindent\textbf{1.\ Valence-shift evidence.}
The quadratic vertex has valence $4$ compared to the affine vertex's
valence $3$.  In the Kontsevich weight integral on the half-space, the
form degree equals the number of internal edges $|E(\Gamma)|$, and the
configuration-space dimension scales with the number of vertices.  For
purely affine graphs (the proved case), these two quantities match
exactly at one loop.  The extra incoming edge at the quadratic vertex
shifts this balance by $+1$, creating a form-degree excess that
generically forces the weight integral to vanish.

\medskip\noindent\textbf{2.\ Dolgushev equivariant formality.}
By Remark~\ref{rem:dolgushev-equivariant}, the half-space BV theory
is the geometric realization of the $\mathbb C^*$-equivariant
deformation quantization.  The Dolgushev formality theorem guarantees
that the equivariant star product exists for \emph{any} smooth Poisson
structure.  If the identification of the half-space theory with the
Dolgushev quantization extends beyond the affine case (as
Theorem~\ref{thm:half-space-reduction} predicts), the reflected weight
identity holds for all polynomial Poisson structures, and in particular
for quadratic ones.

\medskip\noindent\textbf{3.\ Example verification.}
Every quadratic PVA arising as the semiclassical limit of a known
vertex algebra (e.g., the Zamolodchikov $W_3$ algebra at large central
charge, or symplectic fermion algebras) has, by construction, a
deformation quantization to a vertex algebra.  These provide infinitely
many verified instances of the conjecture.
\end{evidence}

\begin{remark}[The quadratic case as first non-affine test]
The jet-order scheme makes the quadratic case the \emph{minimal} test
of the nonlinear half-space programme.  If
Conjecture~\ref{conj:quadratic-rwi} can be proved by establishing
the valence-shift mechanism rigorously on the compactified half-space
configuration space of
Construction~\ref{cons:half-space-colored-operad}, this would open the
door to an inductive argument for all jet orders.  The cubic case
($\delta_3$, discussed below) is the first case where the
valence-shift mechanism alone does not suffice and genuine cancellation
between graph types is required.
\end{remark}
...
\begin{conjecture}[Cubic reflected weight identity]
...
mechanism is a cancellation between the lollipop graph
$\Gamma_3^{(1)}$ and the \emph{reflected triangle graph} (a tree-level
graph with one cubic vertex, one affine vertex, and one reflection
stratum).
\end{conjecture}
...
\begin{remark}[The Dolgushev perspective]
\label{rem:dolgushev-cubic}
The Dolgushev equivariant formality theorem
(Remark~\ref{rem:dolgushev-equivariant}) guarantees that a
$\C^*$-equivariant deformation quantization of $(M,\pi)$ exists for
any smooth Poisson structure.  If the reflected Kontsevich integral
is the geometric realization of the Dolgushev formality map---as the
affine case confirms---then the
identity~\eqref{eq:cubic-identity} holds automatically at all jet
orders.  The remaining mathematical content is therefore:
\emph{prove that the reflected Kontsevich integral on the half-space is
a geometric model for the Dolgushev equivariant formality map.}

This is a statement about the specific propagator, not about the PVA
or the Lie algebra.  It would follow from showing that the reflected
propagator on $\C\times\R_{\ge 0}$ satisfies the same formality
equations as the Dolgushev propagator on $\R^n$ with
$\C^*$-equivariance.  The affine case is the first verification; the
cubic identity~\eqref{eq:cubic-identity} is the second.
\end{remark}

\begin{remark}[Synthesis]
\label{rem:half-space-synthesis}
The jet-order scheme, combined with the Jacobi-reflection
decomposition, reduces the general half-space BV conjecture
(Conjecture~\ref{conj:general-half-space-bv}) to a sharp sequence:
\begin{enumerate}
\item Jet order $\le 1$ (affine): PROVED.  Both sides
  of~\eqref{eq:refl-equals-alg} vanish
  (Theorem~\ref{thm:affine-half-space-bv}).
\item Jet order $2$ (quadratic): OPEN
  (Conjecture~\ref{conj:quadratic-rwi}).
  The algebraic side $\delta_2^{\mathrm{alg}}$ is determined by the
  Jacobi identity; the analytic side $\delta_2^{\mathrm{refl}}$
  involves the self-image propagator at quadratic order.
\item Jet order $3$ (cubic): OPEN
  (Conjecture~\ref{conj:cubic-rwi}).
  The identity~\eqref{eq:cubic-identity} is a single concrete
  equation.
\item Jet order $\ge 4$: each level adds one more equation, with the
  algebraic side determined by Jacobi and the analytic side
  by the self-image propagator.
\end{enumerate}
The PVA Jacobi identity determines WHAT the reflection integrals must
equal.  The half-space geometry determines what they ARE.  The
reflected weight identity is the statement that these two
determinations agree.  The Dolgushev theorem predicts that they do;
the jet-order scheme provides the framework for verification.
\end{remark}
