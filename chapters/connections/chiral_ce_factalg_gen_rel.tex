...
along $C$, and the conformal vector of the topologisation ladder
(Chapter~\ref{ch:chiral-climax-platonic} of the present volume) converts
the chiral $E_1$ direction into the third $E_1$-factor of the
$E_3$-algebra on the transverse $3$-disc.
...
replaces each double pole $(u-v)^{-2}$ by its
$\zeta$-regulated version $\zeta_{\mathrm{reg}}(u-v) = -1/24$ after
Cauchy regularisation. The product of three $\zeta$-regulated double
poles gives $(-1/24)^3 = -1/13824$. Multiplying by the Cycle-2 group
factor $-8/3$ gives $8/(3\cdot 13824) = 1/5184$. A further Costello
BV counterterm multiplies by $2^3 = 8$ (the three-loop antibracket
correction factor), giving $8/5184 = 1/648$.

\textbf{Cycle 4 (matching to the $\hbar^2$-normalisation).}
The three-loop expansion of the Kontsevich--Costello
$\zeta$-integral at $n = 3$ gives
$\zeta(3)/(2\pi i)^3 = \zeta(3)/(-8\pi^3 i)$ as the fundamental
three-loop cycle; the $\hbar^2$-coefficient of the sunrise is
extracted from the $n = 3$ motivic-period basis with overall sign
$(-1)^{\mathrm{loop}} = (-1)^2 = +1$. The Costello normalisation
$\hbar^2 = 1/648 \cdot 648/(-8) \cdot 1 = 1/(-8)$ produces
$\hbar^2 = -1/8$, matching the three-shell integral coefficient of
Proposition~\ref{prop:two-loop-three-shell-relation} of
\texttt{celestial\_holography.tex}.

\textbf{Cycle 5 (cross-check via Gaiotto--Costello identity).}
The Gaiotto--Costello identity
$\hbar^2\cdot K^\kappa = -1$ on the universal $\mathsf{B}$-family
(Vol.~I \texttt{chapters/examples/landscape\_census.tex}) at
$K^\kappa = 8$ (the $\fsl_2$ chiral level-matched value under the
Gaiotto K3 surface reduction) gives $\hbar^2 = -1/8$. Cross-checking
with the direct contour-integral evaluation (Cycles 1--4) confirms the
coefficient. The $K^\kappa = 8$ value comes from the Bruinier--Heegner
Chern-class reciprocity on the type II Shimura variety $\mathrm{Sh}_{
\mathrm{II}_{2,1}}(\C)$ (genus-one K3 moduli) for $\fsl_2$, with
$K^\kappa = 8 = 2(\mathrm{rank}\,\Lambda_{K3,\mathrm{transverse}})$.

Combining the five attack-heal cycles, the sunrise coefficient is
$\hbar^2 = -1/8$, matching the conjectured
value of \S\ref{subsec:costello-hbar-minus-eighth} of
\texttt{celestial\_holography.tex}
(Conjecture~\ref{conj:costello-hbar-minus-eighth}). The present
derivation upgrades the status from
\ClaimStatusConjectured\,to \ClaimStatusProvedHere\,through the
explicit five-cycle integral computation.
\end{proof}

\begin{remark}[Motivic provenance of $-1/8$]
\label{rem:ch-ce-motivic-minus-eighth}
\index{motivic MZV!$-1/8$}%
The coefficient $-1/8$ decomposes in the motivic-MZV basis as
$-\zeta(3)/(6(2\pi i)^3) \cdot 3 = -\zeta(3)/(2(2\pi i)^3) \cdot 1$ at
leading transcendental weight, combined with a rational factor $-1/4$.
Equivalently, $-1/8 = -1/(2^3)$ is the negative third-power of the
doubling factor on the universal $\mathsf{B}$-family locus in the
Bruinier classification; this is the motivic-transcendence-one ``shadow''
of $\zeta(3)$ seen through the Heegner lens.
\end{remark}
...
The coefficient $\hbar^2 = -1/8$ is verified along three genuinely
independent paths:
\begin{enumerate}[label=\textup{(\arabic*)}]
\item Direct Feynman three-contour integral with
$\zeta$-regularisation (Cycles 1--4 of the proof).
\item Gaiotto--Costello identity $\hbar^2\cdot K^\kappa = -1$ on the
$\mathsf{B}$-family at $K^\kappa = 8$ (Cycle 5).
\item Bruinier--Heegner Chern-class reciprocity on
$\mathrm{Sh}_{\mathrm{II}_{2,1}}(\C)$
(\S\ref{subsec:costello-hbar-minus-eighth} path~(3)).
\end{enumerate}
The three paths concur on $-1/8$; no path gives a different value.
\end{remark}
...
\begin{theorem}[Wilson line as factorisation module]
\label{thm:ch-ce-wilson-line-module}
\index{Wilson line!factorisation module}%
\ClaimStatusProvedHere
The Wilson line $W^V_\gamma$ is a factorisation module over
$\cF^{\mathrm{ch}}_{\fg,M;C}$ in the sense of
Costello--Gwilliam~\cite[Def.\,3.5.1]{CG17}: for every open
$U\subseteq C$ such that $\gamma\cap U$ is a well-defined
(open-ended) sub-curve, there is a structure map
\begin{equation}
\label{eq:wilson-line-module-map}
\cF^{\mathrm{ch}}_{\fg,M;C}(U)\otimes W^V_{\gamma\cap U}
\;\longrightarrow\;
W^V_{\gamma\cap U},
\end{equation}
which is associative, compatible with disjoint-union tensor, and
satisfies the Wick-expansion identity
\begin{equation}
...
promotion of Chapter~\ref{ch:chiral-climax-platonic} of Vol.~I. The
