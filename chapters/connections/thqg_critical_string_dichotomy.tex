...
\subsection{The $c=26$ no-ghost theorem via Clifford degeneration}
\label{sec:critical-string-clifford-degeneration}

The preceding sections exhibited the $u\to 0$ degeneration of the
Clifford completion as an algebraic dichotomy at $c=26$. The
observation upgrades to a computation: the Clifford degeneration
at $c=26$ \emph{is} the chain-level avatar of the Goddard--Thorn
no-ghost theorem~\cite{GoddardThornNoGhost}.

\begin{theorem}[$c=26$ no-ghost as Clifford degeneration]
...
H^\bullet(Q_{\mathrm{BRST}})$ the physical state space. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item At $c^{\mathrm{matter}}=26$ (the critical string value), the
Clifford completion (Part~\ref{part:anomaly-completed})
of the matter--ghost BRST pair $\cA_{\mathrm{str}}$ degenerates:
the quartic Clifford relations $\{e_i,e_j\} = 2\,g_{ij}$ acquire a
null eigenvector at $u = \kappa\cdot\omega_g$, in the specific
sense that the \emph{effective BRST anomaly}
$\kappa_{\mathrm{eff}}^{\mathrm{BRST}} =
\kappa(\text{matter, }c=26)+\kappa(\text{ghost, }c=-26)$ vanishes
(this is the matter--ghost cancellation of the bosonic string, not
a Virasoro self-duality statement; Virasoro's Koszul self-duality
lives at $c=13$ as stated in the ``Virasoro self-duality at
$c = 13$'' theorem of this chapter, a different phenomenon).
\item The null eigenvector is the class of the ghost zero-mode
$c_0$, and its pairing with $b_0$ produces the BRST Hodge-theoretic
decomposition of $\cA_{\mathrm{str}}$ into transverse, longitudinal,
and ghost directions.
\item Consequently, the physical state space
$\cH_{\mathrm{phys}}$ is concentrated on transverse oscillators,
with longitudinal and ghost oscillators $Q_{\mathrm{BRST}}$-exact.
This is the Goddard--Thorn no-ghost theorem at chain level, computed
by Clifford degeneration.
\end{enumerate}
\end{theorem}

\begin{proof}[Proof sketch]
(i) Effective anomaly vanishing at $c=26$: matter contributes
...
so the Clifford bilinear form on the handle algebra becomes
degenerate. The null direction is the locus of the secondary
anomaly $u=0$.
(ii) The BRST Hodge theory: at $u=0$, the Clifford degeneration
produces a symplectic pair $(b_0,c_0)$ with $\{b_0,c_0\}=1$ but
$b_0^2 = c_0^2 = 0$, which is exactly the ghost zero-mode pair
of the bosonic string. Filtering by $c_0$-weight gives the Hodge
decomposition (transverse $\oplus$ longitudinal $\oplus$ ghost).
(iii) Physical states: the transverse oscillators are
$Q_{\mathrm{BRST}}$-closed and non-exact; longitudinal modes are
exact by the Kugo--Ojima mechanism realized algebraically through
the Clifford Hodge decomposition. The result is the Goddard--Thorn
no-ghost theorem, now with a bar-cobar / Clifford-degeneration
proof.
\end{proof}
...
proof reorganizes the same computation around the algebraic
mechanism: at $c=26$, the secondary anomaly $u$ vanishes,
the Clifford quartic relations degenerate, the BRST Hodge
decomposition emerges, and transverse oscillators dominate. The
advantage of the Clifford-degeneration view is that it exhibits
$c=26$ as a \emph{computation} in the bar-cobar formalism (the
shadow class $\mathbf C$ becoming $\mathbf G$ at the self-dual
locus), not as an external fact to be checked.
\end{remark}
...
\section{Synthesis}
\label{sec:csd-wave13-DNA}

\begin{remark}[Critical string dichotomy and the $c = -312$]
\label{rem:csd-DNA-1}
The critical-string dichotomy, in its $c_{\mathrm{crit}} = 26$ bosonic
and $c_{\mathrm{crit}} = 15$ superstring forms, admits a the K3 synthesis refinement
through the class-$\mathcal S$ $A_1$-on-$\Sigma_{0,24}$ VOA of central
charge
\[
c_{\mathrm{the K3 synthesis}} \;=\; c_{2d} \;=\; -2\cdot 26 - 260 \;=\; -312,
\]
where the first $-2\cdot 26 = -52$ is twice the class-$\mathcal S$
$4d$ $\mathcal N=2$ central charge $c_{4d} = 26$ via the
Beem--Lemos--Liendo--Peelaers--Rastelli--van~Rees 2015 SCFT-to-VOA
relation $c_{2d} = -2 c_{4d} - 22\cdot\mathrm{rk}(\mathrm{Coulomb}) = -52 - 22\cdot 12 = ...$;
the correct decomposition (cf.~\S\ref{sec:softgrav-wave13-DNA}) is
$c_{2d} = -312$ directly via the $24$-punctured sphere with
$\mathfrak{su}(2)^{24}$-flavour. This central charge is the K3 synthesis
echo of the bosonic-string critical dimension: $c_{\mathrm{the K3 synthesis}} = -312
= -12\cdot 26$ is the $12$-fold K3-cover of the critical bosonic
central charge, matching the $12$-fold cover $\widetilde M_{24}\to M_{24}$
of the Schur multiplier. Primary-lit: Beem et al.\ 2015, Gaiotto 2009,
Nanopoulos--Xie 2010.
\end{remark}

\begin{remark}[Retraction note: $c_{2d} \neq -312$; $-12\cdot 26$ factorisation invalid]
\label{rem:csd-wave17-retract-312}
\index{retraction!$c_{2d} = -312 \to c_{2d} = -214$!critical string dichotomy}
The derivation $c_{2d} = -312 = -12 \cdot 26$ above depends on the
Chacaltana--Distler formula $c_{4d} = (12(g-1) + 7n)/6$, which is
falsified by the $\mathrm{SU}(2)$ $N_f = 4$ cross-check
($(g, n) = (0, 4)$ gives $c_{4d} = 8/3$, incompatible with the
Seiberg--Witten known value $c_{4d} = 7/6$ for SCFT $\mathrm{SU}(2)$
with four massless flavours). The correct Chacaltana--Distler
pants decomposition (\S 5.14, trinion $T_2$ with $n_v = 0$,
$n_h = 4$; $22$ trinions, $21$ tubes; $(n_v, n_h) = (63, 88)$)
yields $c_{4d} = (5n - 13)/6 = (2n_v + n_h)/12$, so at
$(g, n) = (0, 24)$: $(c_{4d}, c_{2d}) = (107/6, -214)$ via
Beem--Rastelli $c_{2d} = -12 c_{4d}$. The $12$-fold-cover
\emph{metaphor} $-12\cdot 26$ is thereby a intermediate numerological
artefact; the correct factorisation is $-214 = -2 \cdot 107$ with
$107$ prime, carrying no $M_{24}$-Schur-multiplier content. The
broader $c_{\mathrm{crit}} = 26$ bosonic-string claim, which is
independent of the $24$-puncture specialisation, is unaffected.
Cross-volume anchor:
...
Shapere--Tachikawa $2008$ \S 3; Seiberg--Witten $1994$; Beem et
al.\ $2015$.
\end{remark}
