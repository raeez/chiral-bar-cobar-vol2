...
\label{def:transgression-algebra}
...
\begin{equation}\label{eq:g9-curved-relation}
d^2(b) = [\lambda, b]
\qquad
\text{for all } b \in B,
\end{equation}
...
$(B, d, \lambda)$ is
\begin{equation}\label{eq:g9-transgression-def}
B_\Theta
\;:=\;
...
$B_\Theta$ by
\begin{equation}\label{eq:g9-transgression-diff}
d_{B_\Theta}(\eta) := 0,
\qquad
...
\label{rem:g9-transgression-signs}
...
&= \bigl(d^2(b) + [\eta, d(b)]\bigr)
   ...
   + (-1)^{|\eta|}[\eta, d_{B_\Theta}(b)]\bigr). \label{eq:g9-dsq-expand}
\end{align}
...
&= d^2(b) + [\eta, d(b)]
   - [\eta, d(b)] - [\eta, [\eta, b]] \notag \\
&= d^2(b) - [\eta, [\eta, b]]. \label{eq:g9-dsq-simplify}
\end{align}
...
&= \lambda b - b \lambda \notag \\
&= [\lambda, b]. \label{eq:g9-bracket-curvature}
\end{align}
...
\begin{equation}\label{eq:g9-BT-decomposition}
B_\Theta = B \oplus B\eta.
\end{equation}
The multiplication rule is
\begin{equation}\label{eq:g9-BT-multiplication}
...
\label{subsubsec:transgression-bar}
...
\begin{equation}\label{eq:g9-specialization}
B := \barB^{(g)}(\cA),
\quad
d := \dfib,
\quad
...
\label{prop:secondary-anomaly-properties}
...
\label{subsec:secondary-connecting}
...
\label{prop:g9-ses-transgression}
There is a short exact sequence of cochain complexes
\begin{equation}\label{eq:g9-ses}
0 \longrightarrow
...
\begin{equation}\label{eq:g9-connecting}
...
\subsection{The Clifford completion}
\label{subsec:clifford-completion}
...
\begin{equation}\label{eq:g9-intersection}
...
\begin{equation}\label{eq:g9-handle-ops}
...
\begin{equation}\label{eq:g9-clifford-compute}
...
\label{rem:g9-clifford-dimension}
...
$\beta := \beta_1$ with
\begin{equation}\label{eq:g9-genus1-clifford}
...
\begin{equation}\label{eq:g9-genus1-spinor}
\alpha \cdot v_+ = v_-,
\quad
\alpha \cdot v_- = 0,
\quad
...
\begin{equation}\label{eq:g9-genus1-morita}
...
&\qquad
\beta_k \cdot v_{\ldots-_k\ldots}
  &= u\, v_{\ldots+_k\ldots}, \label{eq:g9-genus2-spinor}
\end{alignat}
where the subscript indicates the $k$-th sign position.  Hence
\begin{equation}\label{eq:g9-genus2-morita}
...
\label{subsubsec:topological-regime}

\begin{theorem}[Topological regime; \ClaimStatusProvedHere]
\label{thm:topological-regime}
...
\begin{equation}\label{eq:g9-topo-morita}
G_g(\cA)[u^{-1}]
...
\begin{equation}\label{eq:g9-spinor-bimod}
S_g(\cA)
\;:=\;
...
\begin{equation}\label{eq:g9-topo-derived}
...
\begin{equation}\label{eq:g9-raising-lowering}
a_k^+ := u^{-1}\alpha_k,
\qquad
a_k^- := \beta_k,
\qquad k = 1, \ldots, g.
...
\label{subsubsec:gravitational-regime}

\begin{theorem}[Gravitational regime; \ClaimStatusProvedHere]
\label{thm:gravitational-regime}
...
\begin{equation}\label{eq:g9-grav-tensor}
G_g(\cA)\big|_{u=0}
\;\cong\;
\barB^{(g)}(\cA)
\;\otimes\; \bigwedge\nolimits^*(\Bbbk^{2g}).
...
\begin{equation}\label{eq:g9-kappa-vir}
...
\begin{equation}\label{eq:g9-kappa-eff}
...
\label{subsubsec:dichotomy-theorem}

\begin{theorem}[Critical string dichotomy for the Virasoro;
\ClaimStatusProvedHere]
\label{thm:critical-string-dichotomy}
...
\begin{equation}\label{eq:g9-partition-topo}
F_g(\mathrm{Vir}_c)
...
\begin{equation}\label{eq:g9-combined-clifford}
...
\label{subsubsec:vir-clifford-data}

\begin{computation}[Genus-$g$ Virasoro Clifford data]
\label{comp:g9-vir-clifford-data}
...
\begin{equation}\label{eq:g9-ghost-sector}
G_g(\mathrm{Vir}_0)
...
\begin{equation}\label{eq:g9-c13-complementarity}
...
\label{rem:g9-two-self-dualities}
...
\label{rem:g9-depth-zero-revisited}
...
\label{rem:g9-arakelov-clifford}
...
\begin{equation}\label{eq:g9-clifford-E1}
...
\label{prop:clifford-spectral-degeneration}
\begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{equation}\label{eq:g9-combined-final}
G_g(\mathrm{Vir}_{26}) \otimes G_g(\mathrm{Vir}_0)
\cong
...
\begin{equation}\label{eq:g9-physical-content}
...
\subsection{The localization sequence}
\label{subsec:localization-sequence}
...
\begin{equation}\label{eq:g9-localization-les}
\cdots \to
...
\begin{equation}\label{eq:g9-connecting-vir}
\partial\colon
...
\begin{equation}\label{eq:g9-eta-coproduct}
...
\ClaimStatusProvedHere]
\label{cor:g9-u-coproduct}
The secondary anomaly element has coproduct
\begin{equation}\label{eq:g9-u-coproduct}
...
\ClaimStatusProvedHere]
\label{cor:g9-comparison-universal}
...
\label{subsec:clifford-deformation}
...
\ClaimStatusProvedHere]
\label{prop:g9-clifford-deformation}
...
\begin{equation}\label{eq:g9-HH-clifford}
...
\begin{equation}\label{eq:g9-trace-formula}
...
\label{subsec:further-directions}

\begin{remark}[Open questions]
\label{rem:g9-open-questions}
