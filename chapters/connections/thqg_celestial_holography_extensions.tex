...
Assume $\cA$ is chirally Koszul and the shadow obstruction tower stabilizes at degree
$r_{\max}$. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the genus spectral sequence is supported on the $q=0$ line and has
 $d_s = 0$ for all $s \geq 2$;
\item no shadow datum of degree $> r_{\max}$ enters the $q=0$ reconstruction;
...
stabilizes at degree $r_{\max}$, then $\Theta_\cA^{\leq r_{\max}} = \Theta_\cA$ and
all shadow terms of degree $> r_{\max}$ vanish. Hence only the finite jet
...
OPE data of a chiral algebra as a sequence of finite-order invariants. The organization of
...
some finite $R$, then all degree-$r$ shadows vanish for $r > R$, so
$\Theta^{\leq R} = \Theta$.
\end{proof}
...
$\mathrm{Sh}_{0,n}$ reduces to the degree-$2$ shadow, which is $\kappa$ times the
sum of Arnold classes. Flatness $(\nabla_{0,n})^2 = 0$ follows from the MC equation
for $\Theta$, which at degree $2$ is the identity $d(\kappa \omega^{(2)}) = 0$ (the
...
\begin{corollary}[Three data determine the affine scalar modular partition function; \ClaimStatusProvedHere]
% label removed: cor:affine-three-data
The full scalar modular partition function of $\widehat{\mathfrak{g}}_k$ is determined by three
...
at degree~$5$. Therefore $o_5 = 0$ and the tower terminates.
\end{proof}
...
Given a chirally Koszul algebra $\cA$ with known shadow depth class (G, L, C, or M), the
scalar modular partition function is reconstructed by the following algorithm.
...
For classes G, L, C: the scalar partition function is determined by the finite jet
...
For class M: the scalar partition function requires the full inverse limit
...
correction: the genus expansion receives finite corrections from the degree-$3$ shadow
...
the full scalar modular partition function.
\end{proposition}
...
Decomposing it by degree and show that each degree produces a soft theorem.
...
\end{equation}
where $S_r$ is the degree-$r$ soft factor:
\begin{equation}
...
\begin{itemize}
\item \textbf{Degree $2$}: $dS_2 = 0$. The leading soft factor is closed.
\item \textbf{Degree $3$}: $dS_3 + [S_2, S_2] = 0$. The subleading soft factor is
...
\item \textbf{Degree $r$}: $dS_r + \sum_{j+k=r} [S_j, S_k] + \cdots = 0$. The degree-$r$
 soft factor is determined by the lower-degree data and the degree-$r$ obstruction class
 $o_r$.
\end{itemize}
\end{remark}
...
curvature $\kappa = c/2$ to produce a nontrivial degree-$5$ cocycle.
...
adjusting gauge choices at lower degrees).
\end{proof}
...
degree $r$ governs the behavior of an $(n+1)$-point benchmark correlator package
...
\begin{proposition}[Exactness heuristic for the BF symplectic structure; \ClaimStatusConjectured]
...
\begin{proposition}[Heuristic contractibility of the exact Steinberg; \ClaimStatusConjectured]
...
\begin{corollary}[Heuristic BF modular lifting package; \ClaimStatusConjectured]
...
not by itself prove a full class~G collapse of the higher-degree shadow obstruction tower.
\end{theorem}
...
about the full higher-degree shadow obstruction tower follows from these inputs alone.
\end{proof}
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\beta\gamma$ (class~C): four data determine the full scalar modular partition function.
...
Let $k_{\mathrm{BF}}$ denote the BF coupling. We denote by
\begin{equation}
...
For non-abelian BF with gauge group $G$ at coupling $k_{\mathrm{BF}}$, one expects
a modular $R$-matrix with nontrivial genus corrections:
\[
...
The Vol~II celestial-holography framework extends at $K3 \times E$ via the Universal $\Phi$-Trace Identity: $K(\cA) = \mathrm{tr}_{\mathfrak{Z}(\cA)}(\mathfrak{K}_\cA)$ on the chiral side intertwines via Borcherds singular-theta pushforward with $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = c_{\phi_{0,1}^{K3}}(0)/2 = 10/2 = \mathrm{wt}(\Delta_5) = 5$. The $4d$ class-$\mathcal{S}$ $A_1$ on $\Sigma_{0,24}$ parent theory (Gaiotto $2008$; Chacaltana--Distler $c_{4d} = 26$, $c_{2d} = -312$) supplies the celestial-boundary chiral algebra via Beem--Rastelli $2015$.
\end{remark}

\begin{remark}[Retraction note: celestial-boundary anchor $(107/6, -214)$]
\label{rem:thqgche-retract-312}
\index{retraction!$c_{2d} = -312 \to c_{2d} = -214$!celestial holography extensions}
In Remark~\ref{rem:thqgche-phi-trace}, the Chacaltana--Distler
parenthetical $(c_{4d}, c_{2d}) = (26, -312)$ is retracted: the
formula $c_{4d} = (12(g-1) + 7n)/6$ fails the $\mathrm{SU}(2)$
$N_f = 4$ cross-check. Correct first-principles pants-decomposition
values at $(g, n) = (0, 24)$: $c_{4d} = (5n - 13)/6 = 107/6$ and
$c_{2d} = -12 c_{4d} = -214$. The Universal $\Phi$-Trace
identification $K(\cA) = \mathrm{wt}(\Delta_5) = 5$ is orthogonal
to this numerical specialisation (it reads the Borcherds weight,
not the class-$\mathcal{S}$ central charge) and stands. The
celestial-boundary VOA anchoring now reads
$\chi[\mathcal{T}[A_1, \Sigma_{0,24}]]$ with $c_{2d} = -214$; its
$w_{1+\infty}$-truncation content and $M_{24}$-equivariance are
preserved. Cross-volume anchor:
