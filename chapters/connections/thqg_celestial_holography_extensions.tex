...
\begin{corollary}[Conditional BF modular lifting package; \ClaimStatusProvedHere]
...
$\kappa(\widehat{\fg}_k)=\dim(\fg)(k+h^\vee)/(2h^\vee)$ and the exact
...
The affine class~L formula $F_1=\kappa/24$ agrees with the genus-$1$
...
\text{Free } \beta\gamma
& V_{\beta\gamma}
& r_{\max}=4,\ \kappa=6\lambda^2-6\lambda+1
& 0 \\
\text{Kodaira--Spencer}
& U_{\mathrm{KS}}
...
\item $\mathcal{W}_3$: the mixed/infinite higher-spin comparison becomes an exact
 shadow-depth theorem with $\kappa=c_{\mathrm{eff}}/2$ exactly on the
