...
function (Vol~I, eq.~\eqref*{V1-eq:modular-char-gf}) uses the Bernoulli
...
\begin{proof}
Using the universal generating function (Vol~I, eq.~\eqref*{V1-eq:modular-char-gf}),
\[
...
(Vol~I, eq.~\eqref*{V1-eq:modular-char-gf}), expanded as
\[
\frac{x/2}{\sin(x/2)} - 1
...
\section{Synthesis}
\label{sec:thqgche-wave13-DNA}

\begin{remark}[Celestial holography at K3: Universal Phi-Trace]
\label{rem:thqgche-DNA-phi-trace}
The Vol~II celestial-holography framework extends at $K3 \times E$ via the Universal $\Phi$-Trace Identity: $K(\cA) = \mathrm{tr}_{\mathfrak{Z}(\cA)}(\mathfrak{K}_\cA)$ on the chiral side intertwines via Borcherds singular-theta pushforward with $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = c_\Lambda(0)/2 = \mathrm{wt}(\Delta_5) = 5$. The $4d$ class-$\mathcal{S}$ $A_1$ on $\Sigma_{0,24}$ parent theory (Gaiotto $2008$; Chacaltana--Distler $c_{4d} = 26$, $c_{2d} = -312$) supplies the celestial-boundary chiral algebra via Beem--Rastelli $2015$.
\end{remark}

\begin{remark}[Retraction note: celestial-boundary anchor $(107/6, -214)$]
\label{rem:thqgche-wave17-retract-312}
\index{retraction!$c_{2d} = -312 \to c_{2d} = -214$!celestial holography extensions}
In Remark~\ref{rem:thqgche-DNA-phi-trace}, the Chacaltana--Distler
