...
The BV path integral is represented by the bar--cobar pairing once the
configuration-space measure, gauge-fixing Lagrangian, and BV
counterterm scheme are fixed:
...
The comparison requires a measure-theoretic model for configuration
space integrals with BV gauge-fixing data. In that model the BV
partition function $Z_{\mathrm{BV}}$ is the bar--cobar pairing
$\langle \bar{B}^{\mathrm{ch}}(\cA),\Omega^{\mathrm{ch}}(\mathcal C)\rangle$,
and the gauge-fixing Lagrangian fixes the regularisation of the
...
\begin{theorem}[Spectral-curve replacement of $\R^3$-translations]
...
Borcherds denominator $\Delta_5$ has been recognized: the reduced
