...
$\kappa(\hat{\mathfrak{g}}_k) = k\, \dim\mathfrak{g}
/ (2(k + h^\vee))$, which for
$\hat{\mathfrak{sl}}_2$ gives
$\kappa = 3k / (2(k+2))$.  This appears as the
coefficient of $\lambda$ in $m_2(h,h)$ divided by
$\dim\mathfrak{sl}_2 = 3$, consistently with the
general formula $\kappa = c/2$ where
$c = k\,\dim\mathfrak{g}/(k + h^\vee)$.
\end{remark}
