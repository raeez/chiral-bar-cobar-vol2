...
\begin{proof}
The modular characteristic $\kappa(A_{M2,\infty})$ is not determined
...
Borcherds identity $\Phi_{10}=\Delta_5^2$, with
$\kappa_{\mathrm{BKM}}(\mathfrak g_{\Delta_5})=c_1(0)/2=5$ in the
...
\begin{proof}
The identity $\Phi_{10}=\Delta_5^2$ is the
...
$1/\Phi_{10}$~\cite{DijkgraafMooreVerlindeVerlinde1997}. The last
display is exactly the extra assertion supplied by
...
The Borcherds infinite product for $\Phi_{10}$ is the exact scalar
