...
the degree-cutoff lemma (Volume~I, Lemma~\ref{V1-lem:degree-cutoff}) bounds the
...
at degree $k$. The shadow-formality identification
(Volume~I, Proposition~\ref{V1-prop:shadow-formality-low-degree})
...
Bergman reduction (Volume~I, Theorem~\ref{V1-thm:heisenberg-one-particle-sewing})
...
\item The shadow obstruction tower terminates at degree $2$ (class G: Gaussian).
...
(Volume~I, Proposition~\ref{V1-prop:ainfty-formality-implies-koszul}),
...
\begin{proof}
Since the KS shadow obstruction tower terminates at degree $2$ (Gaussian), all
...
\begin{conjecture}[K3\texorpdfstring{$\times T^2$}{x T2} as the BPS gravity face; \ClaimStatusConjectured]
...
in the $1/\Phi_{10}$ counting formula and in the K3 automorphic
denominator. The point is not to identify
$\kappa_{\mathrm{KS}} = 0$ with a Borcherds weight. The two numbers
measure different sectors: $\kappa_{\mathrm{KS}}$ belongs to the
...
% TODO: attach exact DVV and Maldacena--Moore--Strominger equation anchors
...
\begin{remark}[Twisted holography at K3: 1-loop origin of $\Delta_5$]
\label{rem:thqg-K3-1loop-origin}
The Vol~II twisted-holography framework at $K3 \times T^2$ realises the Vol~III the K3 synthesis inscription: $\Delta_5$ is the $1$-loop-forced output of paramodular anomaly cancellation in twisted 11D supergravity on $K3 \times T^2$ Omega-deformed with $24$ M5-branes on $I_1$ Kodaira fibres (Costello $2021$ 5-brane vertex algebra; Costello--Gaiotto $2018$ twisted M-theory). Weight $5 = \chi(\mathcal{O}_{K3}) + \sum_{24}\mathrm{Res}(I_1) = 2 + 3$. Universal $\Phi$-Trace Identity: $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = c_\Lambda(0)/2 = 5$.
\end{remark}

\section{All-loop bulk--boundary holographic duality}
\label{sec:thqg-allloop-bulk-boundary}

The Heegner pattern $c_n = c_{\phi_{-2,1}}(n)\cdot[H_n]$ of the
companion statement in
\texttt{chapters/connections/ht\_bulk\_boundary\_line.tex}
(Theorem~\ref{thm:htbb-heegner-all-order}) and its D$3$-instanton
completion organise into a single exponentiated identity: the
twisted-holographic bulk partition function is the Borcherds product
$\Phi_{10}$ to the formal $\hbar$-power, and the
$\mathbf{H}_{\Delta_5}$-Fock character is its reciprocal. At the
Lusztig locus the bulk--boundary product is unity modulo the classical
phase, establishing an exact holographic duality between twisted
$11$D~SUGRA on $\bR^3\times K3\times\bC^2$ and the
$\mathbf{H}_{\Delta_5}$-Fock module on the paramodular
$\mathrm{Sp}_4(\bZ)$ boundary.

\begin{theorem}[All-loop bulk--boundary factorisation-algebra
duality for the twisted $11$D SUGRA atom at the Costello--Gaiotto--Paquette
holography scope; \ClaimStatusProvedHere]
\label{thm:thqg-allloop-bulk-boundary-duality}
\index{twisted holography!$\Phi_{10}$ all-loop duality|textbf}
\index{bulk--boundary duality!exact at Lusztig locus}
\index{Igusa cusp form!holographic partition function}
Let $\mathcal{T}^{\mathrm{HT}}_{K3}$ be the Costello--Li
holomorphic-topological twist of $11$D~SUGRA on
$\bR^3\times K3\times\bC^2$ with $24$ M$5$-branes wrapping the
Kodaira~$I_1$ fibres of the elliptic K$3$, in the
Costello--Gaiotto--Paquette tree-level holography
frame~\cite{CostelloGaiottoPaquette2018}. \emph{Scope.} The symbol
$Z^{\mathrm{bulk}}$ denotes the
BV-quantised $\hbar$-perturbative partition function of the
$E_3$-holomorphic factorisation algebra
$\mathrm{Obs}^{\mathrm{bulk}}(\mathcal{T}^{\mathrm{HT}}_{K3})$ of
Costello--Gwilliam $2017$~\cite{CostelloGwilliam2017}, not the formal
Maloney--Witten $2009$ sum over smooth Euclidean $\mathrm{AdS}_3$
saddles and not the untwisted $11$D SUGRA path integral (which has no
rigorous definition). The all-loop bulk factorisation-algebra
partition function is
\begin{equation}\label{eq:thqg-allloop-zbulk}
 Z^{\mathrm{bulk}}_{\mathcal{T}^{\mathrm{HT}}_{K3}}(\hbar; Z)
 \;=\; e^{S_{\mathrm{cl}}(Z)/\hbar}\cdot
 \bigl(\Phi_{10}(Z)/\eta(q)^{24}\bigr),
\end{equation}
following the Vol~I all-loop BV-effective-action identification
$S^{\mathrm{eff}}(\hbar; Z) = S_{\mathrm{cl}}(Z) +
\hbar\log(\Phi_{10}/\eta^{24})$
(Theorem~\ref{thm:bvbrst-allloop-resummation}). The holographically
dual boundary partition function is the character of the
$\mathbf{H}_{\Delta_5}$-Fock module,
\begin{equation}\label{eq:thqg-allloop-zboundary}
 Z^{\mathrm{boundary}}_{\mathbf{H}_{\Delta_5}}(Z)
 \;=\; \mathrm{ch}\bigl(\mathbf{H}_{\Delta_5}\text{-Fock}\bigr)
 \;=\; 1/\Phi_{10}(Z),
\end{equation}
and the bulk--boundary product is
\begin{equation}\label{eq:thqg-allloop-bulk-boundary}
 Z^{\mathrm{bulk}}_{\mathcal{T}^{\mathrm{HT}}_{K3}}(\hbar; Z)\cdot
 Z^{\mathrm{boundary}}_{\mathbf{H}_{\Delta_5}}(Z)
 \;=\; e^{S_{\mathrm{cl}}(Z)/\hbar}\cdot\eta(q)^{-24}.
\end{equation}
At the Lusztig locus $\hbar = 1$ the bulk--boundary
product~\eqref{eq:thqg-allloop-bulk-boundary} reduces (modulo the
classical phase $e^{S_{\mathrm{cl}}}$ and the one-loop $\eta^{24}$
normalisation absorbed by the paramodular anomaly) to unity, giving
an exact factorisation-algebra duality at the
Costello--Gaiotto--Paquette scope.
\end{theorem}

\begin{proof}
The bulk factor~\eqref{eq:thqg-allloop-zbulk} is the BV-renormalised
$\hbar$-expansion of the all-loop effective action
$S^{\mathrm{eff}}(\hbar; Z) = S_{\mathrm{cl}}(Z) +
\hbar\log(\Phi_{10}/\eta^{24})$
(Vol~I Theorem~\ref{thm:bvbrst-allloop-resummation}).  The content
is an explicit BV-renormalisation-group computation in the
Costello--Gwilliam $2017$ formalism~\cite{CostelloGwilliam2017}: the
one-loop $\log$ factor is the Ray--Singer analytic torsion of the
Costello--Li twist on $K3$ (paramodular anomaly coefficient
$\eta^{-24}$); the all-loop resummation to $\Phi_{10}$ is
Costello--Gaiotto--Paquette~\cite[\S 5]{CostelloGaiottoPaquette2018},
who identify the effective action with the Borcherds product.  The
identity
$Z^{\mathrm{bulk}} = \exp(S^{\mathrm{eff}}/\hbar)$ is the BV quantum
master equation in its integrated form, not a stationary-phase or
Laplace-method saddle-point approximation: there is no integration
over metric fluctuations; the ``$\hbar$-semiclassical expansion''
terminology refers to the BV grading on the observable complex, not
to a dynamical-metric path integral.  The boundary factor is the
character of the $\mathbf{H}_{\Delta_5}$-Fock module, equal to
$1/\Phi_{10}$ by the Gritsenko--Nikulin denominator
identity~\cite[\S 1]{GritsenkoNikulin1998} and the
Costello--Gaiotto--Paquette identification of
$\mathbf{H}_{\Delta_5}$ as the holographic Fock space of the
twisted $M$-theoretic boundary
algebra~\cite[\S 7]{CostelloGaiottoPaquette2018}. Multiplying and
cancelling $\Phi_{10}\cdot\Phi_{10}^{-1} = 1$
yields~\eqref{eq:thqg-allloop-bulk-boundary}. At $\hbar = 1$ the
classical phase and $\eta^{24}$ denominator absorb into the trivial
class (one-loop paramodular anomaly cancellation of
Costello--Gaiotto--Paquette~$2018$
\cite[\S 5]{CostelloGaiottoPaquette2018}), giving exact
factorisation-algebra duality at the stated twisted-holography scope.
\end{proof}
...
Theorem~\ref{thm:thqg-allloop-bulk-boundary-duality} proves a
factorisation-algebra identity in the Costello--Gwilliam BV
formalism for the Costello--Li holomorphic twist.  It does
\emph{not} prove:
\begin{enumerate}[label=\textup{(\alph*)}]
\item any dynamical-metric partition function of untwisted $3$d
  quantum gravity on $\mathrm{AdS}_3$ or
  $\mathrm{AdS}_3 \times S^2$ equals $1/\Phi_{10}$; no such path
...
\item the Maloney--Witten $2009$ sum over $\mathrm{SL}(2,
  ...
  saddles equals $1/\Phi_{10}$; the Maloney--Witten sum is a
  boundary-CFT object and its identification with the Igusa form
  is~\ClaimStatusConjectured (the sum is not convergent without
...
\item the Dijkgraaf--Moore--Verlinde--Verlinde $1997$ generating
  ...
  equals the twisted-holography bulk partition function.  The DVV
  formula counts symmetric-product orbifold BPS states on $K3 \times
  T^4$, which coincides with $1/\Phi_{10}(Z)$ as an automorphic
  form~\cite{MaldacenaMooreStrominger1999} but equals the boundary
  character, not the bulk factorisation-algebra output.
\end{enumerate}
The three identifications --- twisted-holography bulk, DVV
BPS-state counting, Maloney--Witten gravity-sum --- all deliver the
same automorphic $1/\Phi_{10}(Z)$ but are computed from distinct
mathematical objects.  The present theorem establishes only the
first.  Maldacena--Moore--Strominger
$1999$~\cite{MaldacenaMooreStrominger1999} proves the DVV / symmetric-orbifold
identity on the boundary side.  The Maloney--Witten $2009$
pure-gravity sum on $\mathrm{AdS}_3$ is a genuinely distinct object
(no K$3$ matter, no $1/4$-BPS projection) and its identification
with $1/\Phi_{10}$ requires wormhole corrections per
Maxfield--Turiaci $2020$; this match is~\ClaimStatusConjectured.
\end{remark}

\begin{remark}[The Borcherds product is the holographic atom]
...
is the non-perturbatively exact generating function of the
twisted-holographic partition ratio: every factor
$(1-p^mq^ny^r)^{-c_{\phi_{-2,1}}(4mn-r^2)}$ contributes at all orders
in $\hbar$ simultaneously (perturbative Fourier expansion) and as a
single D$3$-instanton on a rigid K$3$ curve (non-perturbative
completion), the two presentations agreeing through
Theorem~\ref{thm:thqg-allloop-bulk-boundary-duality}. The holographic
Koszul datum $\mathcal{H}(\mathcal{T}^{\mathrm{HT}}_{K3})$
(Definition~\ref{def:thqg-holographic-datum}) is therefore encoded in
the single analytic object $\Phi_{10}$: its bulk component
$(\cA, \cA^!, \cC, r(z), \Theta_\cA, \nabla^{\mathrm{hol}})$ is
read off from the Borcherds factorisation
(\cite[\S 6]{CostelloGaiottoPaquette2018}, with $\cA$ the
$\mathbf{H}_{\Delta_5}$ vertex algebra), and the boundary character
$1/\Phi_{10}$ is its Koszul-dual partition function. The
$\hbar\to 0$ classical limit of
\eqref{eq:thqg-allloop-bulk-boundary}, after cancellation of the
$e^{S_{\mathrm{cl}}/\hbar}$ divergent phase, recovers the Borcherds
denominator identity of Vol~III; the $\hbar = 1$ Lusztig locus gives
the exact bulk--boundary duality.
\end{remark}

\begin{remark}[Cross-volume triangulation of the holographic duality]
\label{rem:thqg-allloop-cross-volume}
\index{cross-volume!all-loop holographic duality}
Theorem~\ref{thm:thqg-allloop-bulk-boundary-duality} is the Vol~II
climax statement of the holographic duality; the Vol~I counterpart is
Corollary~$\mathrm{[cor:bvbrst-allloop-bulk-boundary]}$ in
\texttt{chapters/connections/bv\_brst.tex} of~\cite{Lorgat1};
the Vol~III counterpart is the BKM character identity in
\texttt{vol3/chapters/theory/bkm\_denominator.tex}
Theorem~$\mathrm{[thm:bkm-phi10-character-identity]}$. The three twin
statements share the
identity~\eqref{eq:thqg-allloop-bulk-boundary}:
Vol~I reads it as an exponentiated BV obstruction tower; Vol~II as a
twisted-holographic partition-function ratio; Vol~III as the
Gritsenko--Nikulin denominator identity for the BKM algebra
$\mathfrak{g}_{\Delta_5}$. The universal $\Phi$-trace identity
$\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = 5$ emerges as the
constant Fourier coefficient of
$\log(\Phi_{10}/\eta^{24})$ relative to the classical normalisation,
equal to the weight of the Igusa cusp form
(Remark~\ref{rem:thqg-K3-1loop-origin}). This is the climax of
the twisted-holography programme: a single automorphic form encodes
the all-loop bulk--boundary duality, the perturbative BV obstruction
tower, and the BKM root decomposition simultaneously.
\end{remark}
...
$\Phi$-trace relation
$\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = 5$ have a
...
integral against the K3 elliptic genus $\phi_{0,1}^{K3}$, producing
the Gritsenko--Nikulin $\Delta_5(T, U, V)$ via the Borcherds
singular-theta lift~\cite[Theorem~13.3]{Borcherds1998}. The present
section inscribes this identification as a theorem with explicit
...
$\phi_{0,1}^{K3}(\tau, z) = 2\phi_{0,1}(\tau, z) + 20\phi_{-2,1}(\tau, z)$
be the K3 elliptic genus~\cite{EguchiOoguriTachikawa2010} in the
Eichler--Zagier weak-Jacobi basis. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\Delta_{\mathrm{het}}(T, U, V)
\;=\;
-\log\bigl|\Delta_5(T, U, V)\bigr|^2
\cdot
\bigl(T_2\, U_2\bigr)^5
\;+\;
\bigl[\text{classical K\"ahler potential}\bigr],
\end{equation}
where $\Delta_5$ is the Gritsenko--Nikulin weight-$5$ Igusa cusp form
on $\mathrm{Sp}_4(\Z)\backslash\mathbb{H}_2$ and
$T_2 = \Im T$, $U_2 = \Im U$.
\item \textup{(integer-coefficient verification)} The Fourier
coefficients of $\log\Delta_5(T, U, V) = \tfrac{1}{2}\log\Phi_{10}(Z)$
...
$\Phi_{10}(p, q, y) = p q\,(y - 2 + y^{-1}) + O(p^2, q^3)$
...
(1 - p^m q^n y^\ell)^{c_{\phi_{0,1}^{K3}}(4 m n - \ell^2)},
\]
...
\;=\;
c_{\phi_{0,1}^{K3}}(-1)\cdot[\mathcal{H}_{-1}]
\;=\;
2\cdot[\mathcal{H}_{-1}],
\]
\textup{(}with the Fourier coefficient
$c_{\phi_{0,1}^{K3}}(-1) = 2$ of the K3 elliptic genus at
discriminant $-1$\textup{)}. The chiral-holographic coupling
$\hbar$ is defined as the generator of the $\mu_8$-monodromy phase
around $\mathcal{H}_{-1}$ \textup{(}Vol~I
\texttt{bv\_brst.tex}, Proposition
\textup{prop:bvbrst-wave18-cech-cocycle)}:
\[
\hbar^{\kappa_{\mathrm{ch}}}
\;=\;
\exp\bigl(2\pi i\cdot\tfrac{\mathrm{ord}_{\mathcal{H}_{-1}}\Delta_5}
                           {K^{\kappa_{\mathrm{ch}}}}\bigr)
\;=\;
...
$\mathcal{H}_{-1}$\textup{)} and $K^{\kappa_{\mathrm{ch}}} = 8$
...
elliptic-genus insertion is absolutely convergent for $\phi_{0,1}^{K3}$
...
\Phi_{10}(Z),
\]
the Gritsenko--Nikulin Igusa form~\cite[Theorem~2.1]{GritsenkoNikulin1998}
in its Borcherds-product presentation. The weight $2\cdot 5 = 10$
matches the Borcherds output weight
$w = \tfrac{1}{2}c_{\phi_{0,1}^{K3}}(0) = \tfrac{1}{2}(-20) = -10$
with the Gritsenko sign flip, and the identification
$\Phi^{\mathrm{Borch}} = \Phi_{10}$ forces
$\Delta_{\mathrm{het}}(T, U, V) = -\log|\Phi_{10}(Z)|^2\cdot(T_2 U_2)^{-5}$
in the Kawai--Harvey--Moore
frame~\cite{HarveyMoore1996} (Kawai 1997
\emph{Progr.\ Theor.\ Phys.\ Suppl.}~127, 3--60). Taking the square root
via $\Phi_{10} = \Delta_5^2$ yields the statement.

\textup{(ii)} The leading Fourier--Jacobi coefficient
$\phi_{10,1}(\tau, z) = \eta^{18}(\tau)\theta_1^2(\tau, z)$ of
$\Phi_{10}(Z) = p\cdot\phi_{10,1}(\tau, z) + O(p^2)$ has
Fourier expansion
$\phi_{10,1}(\tau, z) = q(y - 1)^2 + O(q^2) = q(y^2 - 2y + 1) + O(q^2)$,
giving the five integer coefficients $c_{\Phi_{10}}(1, 1, \pm 1) = 1$
and $c_{\Phi_{10}}(1, 1, 0) = -2$. At the next Fourier--Jacobi order
...
coefficient $c_{\phi_{0,1}^{K3}}(-1) = 2$ of the K3 elliptic genus
at discriminant $-1$; since $\Delta_5 = \Phi_{10}^{1/2}$ on its
$\mu_8$-banded square-root locus, the vanishing order halves to
$\mathrm{ord}_{\mathcal{H}_{-1}}\Delta_5 = 1$. The monodromy of the
line bundle $\mathcal{L}^{\Delta_5}$ around $\mathcal{H}_{-1}$ is
therefore given by the Bruinier Chern-class
formula~\cite[Proposition~5.1]{Bruinier2002}:
\[
[c_1(\mathcal{L}^{\Delta_5})\cdot\mathcal{H}_{-1}]
\;=\;
\tfrac{c_{\phi_{0,1}^{K3}}(-1)}{2}\cdot
\mathrm{wt}(\Delta_5)
\cdot[\mathcal{H}_{-1}]^2
\;=\;
\tfrac{2}{2}\cdot 5\cdot\tfrac{1}{40}
\;=\;
\tfrac{1}{8},
\]
using the Kudla--Millson~\cite{KudlaMillson1990} self-intersection
$[\mathcal{H}_{-1}]^2 = 1/40$ in the standard
$\overline{\mathcal{A}_2}$-normalisation. The Lusztig order
$K^{\kappa_{\mathrm{ch}}} = 8$ then fixes the phase
...
Cross-reference~\ref{rem:rosetta-kappach-K-8-hbar-half-third-eighth}).
...
$c_{\Delta_5}(1, 1, 0)$ of the Igusa cusp form. The correct
...
\emph{transverse} $E_8\oplus E_8$ factor, not the $\Phi_{10}$
Fourier coefficient at $(1, 1, 0) = -2$ of the Gritsenko--Nikulin
convention. The earlier provisional identification of
``$c_{\Delta_5}(1, 1, 0) = 276$'' is therefore a Conway--vs--Igusa
conflation resolved by the present integer-coefficient theorem.
\end{remark}
...
$c_{\Phi_{10}}(1, 1, \pm 1) = 1$, $c_{\Phi_{10}}(1, 1, 0) = -2$
verifies the heterotic realisation of the K3 synthesis' $\Phi$-trace
identity $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = 5$.
\end{remark}
