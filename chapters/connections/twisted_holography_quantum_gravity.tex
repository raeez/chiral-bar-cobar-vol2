...
is duality-constrained: $\kappaChHodge(\cA) + \kappa(\cA^!) = 0$ for Kac--Moody and free-field algebras, while $\kappaChHodge(\cA) + \kappa(\cA^!) = K(\fg)$ (a nonzero constant) for $\mathcal{W}$-algebras (Volume~I, Theorem~D).
...
anti-symmetry $\kappa(\cA^!) = -\kappaChHodge(\cA)$; for $\mathcal{W}$-algebras
the complementarity sum $\kappaChHodge(\cA) + \kappa(\cA^!)$ equals a nonzero
...
\begin{proposition}[KS Gaussian scalar genus-$g$ free energy;
\ClaimStatusConditional]
% label removed: prop:thqg-ks-free-energy
For a compact Calabi--Yau threefold $Y$, assume the Kodaira--Spencer
holographic system is restricted to the Gaussian sewing lane of
\textbf{G10}. Then its scalar genus-$g$ free-energy coefficient is
...
\begin{proof}
Under the Gaussian hypothesis, the KS shadow obstruction tower
terminates at degree $2$, so the scalar genus-$g$ coefficients are
determined by the modular characteristic $\kappa$ alone. On that lane
the genus-$g$ free energy is the $g$-th coefficient of
\(\log Z(\tau)\), where \(Z(\tau)=\Det(1-K_g)\) is the Fredholm
determinant supplied by \textbf{G10}. The formula follows from the
standard Gaussian determinant computation on \(\Sigma_g\) weighted by
\(\kappa\). Non-Gaussian BCOV/contact corrections are not included in
this proposition.
\end{proof}
...
$\Phi^{\mathrm{Borch}} = \Phi_{10}^{\mathrm{un}}$ forces
$\Delta_{\mathrm{het}}(T, U, V)
= -\log|\Phi_{10}^{\mathrm{un}}(Z)|^2\cdot(T_2 U_2)^{-5}$
...
via $\Phi_{10}^{\mathrm{un}} = \Delta_5^2$ yields the statement.
...
at discriminant $-1$; since
$\Delta_5 = (\Phi_{10}^{\mathrm{un}})^{1/2}$ on its
