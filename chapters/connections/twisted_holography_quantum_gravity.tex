...
\begin{theorem}[All-loop bulk--boundary holographic duality for the
twisted $11$D SUGRA atom; \ClaimStatusProvedHere]
...
fibres of the elliptic K$3$, in the
Costello--Gaiotto--Paquette tree-level holography
frame~\cite{CostelloGaiottoPaquette2018}. The all-loop bulk partition
function is
...
\end{equation}
following the Vol~I all-loop effective-action identification
...
an exact holographic duality.
\end{theorem}

\begin{proof}
The bulk factor follows from the Vol~I all-loop effective action of
Theorem~\ref{thm:bvbrst-allloop-resummation}: the path-integral
saddle-point evaluation gives $Z^{\mathrm{bulk}} =
e^{S^{\mathrm{eff}}/\hbar} = e^{S_{\mathrm{cl}}/\hbar}\cdot
(\Phi_{10}/\eta^{24})$ as in~\eqref{eq:thqg-allloop-zbulk}. The
boundary factor is the character of the $\mathbf{H}_{\Delta_5}$-Fock
module, equal to $1/\Phi_{10}$ by the Gritsenko--Nikulin denominator
...
\cite[\S 5]{CostelloGaiottoPaquette2018}), giving exact holographic
duality.
\end{proof}
