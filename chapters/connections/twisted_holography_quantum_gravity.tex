...
The Vol~II twisted-holography framework at $K3 \times T^2$ realises the Vol~III Wave~13 inscription: $\Delta_5$ is the $1$-loop-forced output of paramodular anomaly cancellation in twisted 11D supergravity on $K3 \times T^2$ Omega-deformed with $24$ M5-branes on $I_1$ Kodaira fibres (Costello $2021$ 5-brane vertex algebra; Costello--Gaiotto $2018$ twisted M-theory). Weight $5 = \chi(\mathcal{O}_{K3}) + \sum_{24}\mathrm{Res}(I_1) = 2 + 3$. Universal $\Phi$-Trace Identity: $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = c_\Lambda(0)/2 = 5$.
\end{remark}
