...
\end{equation}
The modular characteristic is $\kappa(\mathcal{H}_k^{\otimes d}) = dk$, and the shadow free energy is $F_g(\mathcal{H}_k^{\otimes d}) = dk \cdot \lambda_g^{\mathrm{FP}}$. This is consistent with the independent sum factorization (Proposition~\ref{V1-prop:independent-sum-factorization}): for $\cA = \cA_1 \oplus \cA_2$ with vanishing mixed OPE, $\kappaChHodge(\cA) = \kappa(\cA_1) + \kappa(\cA_2)$.
\end{remark}
...
\item The tensor product $\cA = \cA_1 \otimes \cdots \otimes \cA_m$ satisfies HS-sewing, with
 ...
 \end{equation}
\item The modular characteristic is additive: $\kappaChHodge(\cA) = \kappa(\cA_1) + \cdots + \kappa(\cA_m)$.
...
The scalar factorization formula $F_g(\cA) = \kappaChHodge(\cA) \cdot
