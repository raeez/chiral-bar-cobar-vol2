...
supplies the completed scalar sewing series
...
$c = 3\ell/(2G)$ gives $\kappa = 3\ell/(4G)$. The argument stops at the
scalar shadow series; this chapter does not construct a comparison map
to the full analytic BTZ partition function.
\end{proof}
...
hypothesis $(c>0)$, the two-sided asymptotic behaviour,
...
\section{Trace from the 6d hCS BV master equation to $(\Phi_{10}^{\mathrm{un}})^{-1}$}
...
$\Phi_{10}^{\mathrm{un}}$ from the BV quantisation of the 6d hCS action on
...
$\bigl(\Phi_{10}^{\mathrm{un}}/(\eta(\tau)^{24}\eta(\tau')^{24})\bigr)^{\hbar\cdot
c_{K3}(Z)}$, whose pole along the Humbert discriminant controls the
large-$\hbar$ BV limit. Identification of the automorphic form
$(\Phi_{10}^{\mathrm{un}})^{-1}$ thereby obtained with the symmetric-product BPS
...
$\Phi_{10}^{\mathrm{un}}(Z)/\bigl(\eta(\tau)^{24}\eta(\tau')^{24}\bigr)$ at the cusp
...
\begin{equation}\label{eq:bv-obstruction-jacobi-matching}
 ...
 c_{1/\Delta_{5}}(q^{n})
 \;=\;
 [q^n]\!\left(-\log\frac{\Phi_{10}^{\mathrm{un}}(Z)}{\eta(\tau)^{24}\,\eta(\tau')^{24}}\right)\Big|_{p=0,\,y=1}.
\end{equation}
...
The Gritsenko--Nikulin unnormalised Igusa cusp form
$\Phi_{10}^{\mathrm{un}}$ is the Borcherds lift
...
\begin{equation}\label{eq:hcs-all-loop-resummation}
 ...
 \exp\!\Bigl(
 \sum_{g\ge 1}\hbar^{g}\,
 c_{1/\Delta_{5}}(q^{g})\,\log\frac{\Phi_{10}^{\mathrm{un}}(Z)}{\eta(\tau)^{24}\,\eta(\tau')^{24}}\Bigr)
 \;=\;
 \Bigl(\frac{\Phi_{10}^{\mathrm{un}}(Z)}{\eta(\tau)^{24}\,\eta(\tau')^{24}}\Bigr)^{\hbar\cdot c_{K3}(Z)},
\end{equation}
...
reproduces the automorphic form $(\Phi_{10}^{\mathrm{un}})^{-1}$ along the
...
pole along the Humbert discriminant
...
\log\bigl(\Phi_{10}^{\mathrm{un}}(Z)/(\eta(\tau)^{24}\eta(\tau')^{24})\bigr)$ as
...
$\log\bigl(\Phi_{10}^{\mathrm{un}}/(\eta(\tau)^{24}\eta(\tau')^{24})\bigr)$ as the
Harvey--Moore singular-theta lift of $\phi_{-2,1}$.
Restriction of $\Phi_{10}^{\mathrm{un}}$ to the diagonal $z=0$ in the Siegel period
factors as
\[
 \Phi_{10}^{\mathrm{un}}(Z)\big|_{z=0}
 \;=\;
...
\log\bigl(\Phi_{10}^{\mathrm{un}}/(\eta(\tau)^{24}\eta(\tau')^{24})\bigr) + O(1)$,
where the $O(1)$ remainder is the $\hbar$-independent BV-one-loop
normalisation. The pole structure along $\{\Phi_{10}^{\mathrm{un}}=0\}\subset
...
$\bigl(\Phi_{10}^{\mathrm{un}}/(\eta(\tau)^{24}\eta(\tau')^{24})\bigr)^{\hbar\cdot c_{K3}(Z)}$
...
\[
\sum_{a=1}^{24}\, \mathcal I^{(1)}_a \;=\; \text{constant}\cdot \Phi_{10}^{\mathrm{un}}
\]
