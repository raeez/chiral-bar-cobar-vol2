...
\label{sec:thqg-pf-wave13-DNA}

\begin{remark}[$\Delta_5$ as the $1$-loop-forced output at $K3 \times T^2$]
\label{rem:thqg-pf-DNA-delta5-output}
...
\label{rem:thqg-pf-DNA-classS-parent}
The $4d$ $\mathcal{N} = 2$ parent of the K3 chiral bialgebra (Vol~III the K3 synthesis) is the class-$\mathcal{S}$ $A_1$ theory on the $24$-punctured sphere $\Sigma_{0,24}$ with maximal regular $\mathfrak{su}(2)$ punctures. Chacaltana--Distler $2010$ anomaly arithmetic gives $c_{4d} = (12(g-1) + 7n)/6 = 26$ at $(g, n) = (0, 24)$, $a_{4d} = 16$, Coulomb rank $21$, flavour $\mathfrak{su}(2)^{24}$; Beem--Rastelli $2015$ dictionary $c_{2d} = -12 c_{4d} = -312$. The $24$-point configuration preserves the Steiner system $S(5, 8, 24)$ (Conway--Sloane $1988$), the unique (up to M\"obius) $24$-point configuration on $\mathbb{P}^1$ with $M_{24}$ symmetry. Schur index $M_{24}$-averaged over flavour fugacities reduces to the K3 elliptic genus $\phi_{0,1}$; Borcherds $1998$ singular-theta lift of $\phi_{0,1}$ produces $\Delta_5$. The perturbative finiteness theorems of this chapter extend to the paramodular genus-$2$ case via Bruinier--Funke functoriality.
\end{remark}

\begin{remark}[Retraction note: $(26, -312) \to (107/6, -214)$]
\label{rem:thqg-pf-wave17-retract-312}
\index{retraction!$c_{2d} = -312 \to c_{2d} = -214$!perturbative finiteness}
The formula $c_{4d} = (12(g-1) + 7n)/6$ used in the preceding
remark is falsified by the $\mathrm{SU}(2)$ $N_f = 4$
cross-check: at $(g, n) = (0, 4)$ it yields $8/3$, whereas the
first-principles Chacaltana--Distler $2010$ \S 5.14 pants
decomposition (trinion $T_2$ with $n_v(T_2) = 0$, $n_h(T_2) = 4$;
$22$ trinions, $21$ tubes) gives $c_{4d} = (5n - 13)/6$, so
$(c_{4d}, c_{2d}) = (107/6, -214)$ at $(g, n) = (0, 24)$ via
Beem--Rastelli $c_{2d} = -12 c_{4d}$. The intermediate the K3 synthesis
numerics $(c_{4d}, c_{2d}) = (26, -312)$ in
Remark~\ref{rem:thqg-pf-DNA-classS-parent} are retracted; the
$\Delta_5$-as-$1$-loop-forced-output conclusion
(Remark~\ref{rem:thqg-pf-DNA-delta5-output}) is independent of
this numerical specialisation and stands. Cross-volume anchor:
\texttt{notes/SYNTHESIS\_WAVES\_14\_15\_16.md} §§ 3.3, 10.2; Vol~III
\texttt{k3\_chiral\_bialgebra\_platonic.tex} the K3 synthesis remarks.
Primary: Chacaltana--Distler $2010$ \S 5.14;
Shapere--Tachikawa $2008$ \S 3; Beem--Rastelli $2015$;
Gaiotto $2009$ \S 3.
\end{remark}

\begin{remark}[Universal $\Phi$-Trace Identity at $K3\times T^2$]
\label{rem:thqg-pf-DNA-phi-trace}
The Vol~II Chiral Hochschild Trinity universal-centre supertrace $K(\mathcal{A}) = -c_{\mathrm{ghost}}(\mathrm{BRST}(\mathcal{A})) = \mathrm{tr}_{\mathfrak{Z}(\mathcal{A})}(\mathfrak{K}_{\mathcal{A}})$ specialises via Borcherds singular-theta pushforward at $K3 \times T^2$ to the BKM weight $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = c_\Lambda(0)/2 = 5$. Perturbative finiteness extends to the paramodular case via Bruinier $2002$ regularised lift, and the HS-sewing finiteness frontier propagates through the bi-based Ran-space datum $(\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24}), \overline{\mathcal{A}_2})$. The bridge diagram $\mathfrak{B}_X = \Phi^{\mathrm{Borch}}_*(\mathfrak{K}_{\mathcal{A}_X})$ commutes up to canonical homotopy.
\end{remark}

\begin{remark}[$K^{\kappa_{\mathrm{ch}}} = 8$ Mukai doubling on the $\mathcal{B}$-family]
\label{rem:thqg-pf-DNA-Kkappa-8}
