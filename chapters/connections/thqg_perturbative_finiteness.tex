...
The scalar projection of the Maurer--Cartan element $\Theta_\cA$ onto genus~$g$ is the shadow free energy $F_g(\cA)$.  This is the arity-$0$, genus-$g$ component of the shadow algebra $\cA^{\mathrm{sh}} = H_\bullet(\Defcyc^{\mathrm{mod}}(\cA))$ (Definition~\ref{V1-def:shadow-algebra}).
...
\item \emph{Arity cutoff.}  At each genus~$g$, only finitely many graph topologies contribute (Lemma~\ref{V1-lem:arity-cutoff}).  The number of contributing graphs at genus $g$ and arity $r$ is bounded by a combinatorial function of $(g,r)$, and each graph amplitude is a finite integral.
\end{enumerate}
\end{proof}
...
\begin{proof}
Part~(i) is the arity-cutoff lemma (Lemma~\ref{V1-lem:arity-cutoff}).  The set $\Gmod(g,r)$ of connected modular graphs with loop genus $g$ and total arity $\leq r$ is finite: the number of vertices $v$, edges $e$, and external legs $n$ satisfy $v - e + n = 1 - g$ (the Euler characteristic), $2e + n \leq rv$ (each vertex has at most $r$ half-edges), and $g = e - v + 1 + \sum_{v'} g_{v'}$ where $g_{v'}$ is the genus decoration at each vertex.  These constraints bound $v, e, n$ in terms of $g$ and $r$.
...
lane, $F_g^{\mathrm{scal}} = \kappa \cdot \lambda_g^{\mathrm{FP}}$
\textup{(UNIFORM-WEIGHT)} for
all $g \geq 1$; for arbitrary modular Koszul algebras, the
unconditional scalar statement is $F_1^{\mathrm{scal}}=\kappa/24$
\textup{(}$g = 1$ is unconditional at all weights\textup{)}.  % AP32  For
...
At critical level $k = -h^\vee$, the Sugawara construction is \emph{undefined} (not ``$c$ diverges''; the Sugawara tensor $T(z) = \frac{1}{2(k+h^\vee)} \sum_a :J^a(z) J^a(z):$ has a pole at $k = -h^\vee$).  The Feigin--Frenkel theorem identifies the center $Z(V_{-h^\vee}(\fg)) \cong \mathrm{Fun}\,\mathrm{Op}_{\fg^\vee}(\mathbb{D})$ with the algebra of functions on the space of $\fg^\vee$-opers on the formal disc.  The \emph{scalar} shadow free energy vanishes at this level because $\kappa = 0$ (forcing $m_0 = 0$, hence the scalar lane is uncurved); the higher-arity components of the MC element are not determined by $\kappa = 0$ alone, and $F_1 = 0$ does not imply $F_g = 0$ at higher arity.  The Feigin--Frenkel center provides a replacement for the scalar genus package: the ``classical'' genus-$g$ invariant is the space of $\fg^\vee$-opers on $\Sigma_g$, which is finite-dimensional and does not require the HS-sewing machinery.
\end{remark}
...
\index{monster CFT}
For the monster CFT with $c = 24$ (the Frenkel--Lepowsky--Meurman moonshine module $V^\natural$), $\kappa(V^\natural) = 12$, since $\dim V_1^\natural = 0$ forces the modular characteristic to be determined by the Virasoro sector alone: $\kappa = c/2 = 12$ (AP48; Vol~I, Remark~\ref{V1-rem:lattice:monster-shadow}):
\begin{align*}
F_1 &= 12/24 = 1/2, \\
...
Under HS-sewing, the graph sum~\eqref{V1-eq:thqg-I-graph-sewing} converges absolutely at each genus $g$.  The number of contributing graphs is finite at each arity level, and the arity sum converges:
...
\item If $r_{\max} = \infty$ (mixed class), then the arity sum converges at each genus by the shadow obstruction tower convergence theorem \textup{(}Theorem~\ref{V1-thm:recursive-existence}\textup{)}, so the full genus-$g$ amplitude is well defined for every fixed $g$.
...
The proof of this bound for low-arity amplitudes and identify
...
(one vertex, $g_v = 2$, no edges).  % AP123: total count is 7
...
but contributes to the total stable-graph count.  % AP123
\end{lemma}
...
$V_{1,n}$.  At arity~$0$, the genus-$1$ vertex factor reduces
...
rational number.  For the scalar (arity-$0$) projection,
...
half-edges via the propagator.  At arity~$0$, the genus-$1$
...
\cdot V_{1,1}(\phi^j).
\end{equation}
At arity~$0$, $V_{1,1}(\phi^j) = \kappa_j \cdot
...
\end{equation}
At arity~$0$, this is $\frac{1}{2} \sum_i P_{ii}\, \kappa_i^2
...
algebras, higher-genus components $\Theta^{(g)}_\cA$ at arity~$0$
are determined by the MC recursion from $\kappa$ alone.
For multi-weight algebras, the MC recursion receives
cross-channel contributions from the arity-$2$ component
...
\begin{proposition}[Arity-$0$ MC recursion: scalar and cross-channel decomposition;
...
For uniform-weight algebras, the arity-$0$ component of the
...
from the arity-$2$ component $\Theta^{(g-1,2)}$,
...
the arity-$0$ MC recursion~\eqref{eq:MC-genus-g-arity-0}
...
algebras~$\cA$ \textup{(UNIFORM-WEIGHT)}.  % AP32: scope tag mandatory
\end{proof}
...
diagonal part of
Proposition~\ref{prop:arity-0-weight-independence}.
...
\emph{higher-arity} shadow obstruction tower: the cubic shadow
...
\textup{(UNIFORM-WEIGHT)} is a purely
algebraic identity, following from the MC recursion and the
conformal Ward identity.  % AP32  The HS condition ensures that the
...
\textup{(}Proposition~\textup{\ref{prop:arity-0-weight-independence})}.
In particular:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{proof}
For uniform-weight algebras,
Proposition~\ref{prop:arity-0-weight-independence} gives
