...
\begin{enumerate}[label=\textup{(\arabic*)}]
\item \emph{Non-Koszul algebras.}  The theorem applies to modular Koszul chiral algebras.  For non-Koszul algebras (if any exist in the standard landscape), the MC recursion may not terminate at each arity, and the finiteness argument would need modification.
