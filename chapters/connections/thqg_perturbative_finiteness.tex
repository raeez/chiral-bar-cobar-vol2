...
Perturbative finiteness of twisted quantum gravity is the analytic ground on which the gravitational shadow programme stands. Three implications form a chain: $D_\cA^2 = 0$ (Theorem~\ref*{V1-thm:mc2-bar-intrinsic}) furnishes $\Theta_\cA \in \mathrm{MC}(\gAmod)$ at all genera without truncation; the shadow representation $\operatorname{Sh}_{g,0}(\Theta_\cA)$ produces a cohomology class on the compact orbifold $\overline{\mathcal{M}}_g$ whose integral is finite; Bernoulli asymptotics of the Faber--Pandharipande coefficients give convergence of the genus series with explicit radius $4\pi^2$. Standard perturbative quantum gravity, by contrast, diverges at genus $g \geq 2$: the integration domain is noncompact, the integrand distributional, the expansion asymptotic. In the twisted setting, where the boundary algebra~$\cA$ controls the bulk through the holographic modular Koszul datum $\mathcal{H}(T) = (\cA, \cA^!, \mathcal{C}, r(z), \Theta_\cA, \nabla^{\mathrm{hol}})$, all three pathologies vanish: the Maurer--Cartan equation is algebraic, Fulton--MacPherson compactification provides a geometric regulator, and the genus expansion converges.
...
with $\int_{\overline{\mathcal{M}}_g} \kappaChHodge(\cA)\cdot\lambda_g$; for
...
\boxed{ F_g(\cA) \;=\; \kappaChHodge(\cA) \cdot \frac{2^{2g-1}-1}{2^{2g-1}} \cdot \frac{|B_{2g}|}{(2g)!}\,. }
\end{equation}
...
\boxed{ \sum_{g=1}^{\infty} F_g(\cA)\,x^{2g} \;=\; \kappaChHodge(\cA) \cdot \left( \frac{x/2}{\sin(x/2)} - 1 \right). }
\end{equation}
Equivalently, with the convention $F_0(\cA) = \kappaChHodge(\cA)$:
\begin{equation}% label removed: eq:thqg-I-gf-ahat-alt
\sum_{g=0}^{\infty} F_g(\cA)\,x^{2g} \;=\; \kappaChHodge(\cA) \cdot \frac{x/2}{\sin(x/2)}.
\end{equation}
\end{theorem}
...
$\sum_g F_g\,x^{2g} = \kappaChHodge(x/(2\sin(x/2)) - 1)$ uses the real
...
the genus-$1$ identity $F_1(\cA) = \kappaChHodge(\cA)/24$;
...
$F_g(\cA) = \kappaChHodge(\cA) \cdot \lambda_g^{\mathrm{FP}}$ for all $g \geq 1$
...
\begin{align}
|F_g(\cA)| &\;=\; |\kappaChHodge(\cA)| \cdot \frac{2^{2g-1}-1}{2^{2g-1}} \cdot \frac{|B_{2g}|}{(2g)!} \notag \\
...
$F_1(\cA)=\kappaChHodge(\cA)/24$; higher-genus formulas require the stated
...
\item \emph{Scalar partition function on the proved scalar lane.} If $\cA$ has generators of uniform conformal weight, then the series $\sum_{g=1}^{\infty} F_g(\cA)\,\hbar^g$ converges absolutely for $|\hbar| < 4\pi^2$, with the closed-form expression $Z_{\mathrm{grav}}^{\mathrm{scal}}(\cA;\,\hbar) = \kappaChHodge(\cA) \cdot \frac{\sqrt{\hbar}/2}{\sin(\sqrt{\hbar}/2)}$. For arbitrary modular Koszul algebras, the universal scalar statement is the finite genus-$1$ term $F_1(\cA)=\kappaChHodge(\cA)/24$.
...
as an output of the genus recursion. Concretely:
...
c \;=\; \frac{3\ell}{2G}\,,
\end{equation}
where $\ell$ is the AdS radius and $G$ is the three-dimensional Newton constant~\cite{BH86}. In the twisted holographic setting, $\cA = \mathrm{Vir}_c$ is the boundary chiral algebra, with modular characteristic $\kappaChHodge(\cA) = c/2 = 3\ell/(4G)$.
\end{setup}
...
\[
Z_{\mathrm{grav}}^{W_N}(\hbar) \;=\; \kappaChHodge(\mathcal{W}_N) \cdot \frac{\sqrt{\hbar}/2}{\sin(\sqrt{\hbar}/2)}\,,
\]
...
$F_1(\cA)=\kappaChHodge(\cA)/24$ for every modular Koszul algebra; on the
scalar lane one further has
$F_g(\cA)=\kappaChHodge(\cA)\lambda_g^{\mathrm{FP}}$ for all $g \geq 1$.
...
\begin{tabular}{|l|c|c|c|c|c|}
\hline
\textbf{Family $\cA$} & $\boldsymbol{\kappaChHodge(\cA)}$ & $\boldsymbol{r_{\max}}$ & \textbf{Class} & $\boldsymbol{F_1(\cA)}$ & $\boldsymbol{F_2(\cA)}$ \\
\hline
...
\item If the tensor product lies on the proved scalar lane
 ...
 Z_{\mathrm{grav}}^{\mathrm{scal}}(\cA;\,\hbar) \;=\; \sum_{j=1}^m \kappaChHodge(\cA_j) \cdot \frac{\sqrt{\hbar}/2}{\sin(\sqrt{\hbar}/2)}.
 \end{equation}
\end{enumerate}
\end{proposition}
...
The scalar factorisation $F_g(\cA) = \kappaChHodge(\cA) \cdot
...
genus~$1$ ($F_1 = \kappa/24$); higher genus requires separate
...
curvature $\kappaChHodge(\cA) \cdot \eta$ (where $\eta$ is the generator
...
$F_g(\cA) = \kappaChHodge(\cA)\cdot\lambda_g^{\mathrm{FP}}$ of
...
\begin{proposition}[Five-path absence of closed-form generating
function for $\delta F_g^{\mathrm{cross}}$;
licensing $\gamma + \delta + \varepsilon$;
\ClaimStatusProvedHere]
\label{prop:cross-channel-no-closed-form}
\index{cross-channel correction!no closed form}
\index{generating function!cross-channel obstruction}
No closed-form $\hat A$-type generating function exists for
$\delta F_g^{\mathrm{cross}}(\cA, c)$; equivalently, any bivariate
ansatz $G(c, \hbar)$ for the cross-channel tower at the standard
multi-weight witness $\cA = \cW_3$ must be \emph{bivariate in
$(c, \hbar)$ and non-separable}. Five independent obstruction paths:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Inhomogeneous $c$-scaling.} At $g = 2$ the
$\cW_3$ closed form
$\delta F_2^{\mathrm{cross}}(\cW_3) = (c + 204)/(16 c)$
is $O(1)$ in $c$ \textup{(}Vasiliev limit $1/16$\textup{)}, while
the scalar tower $F_g^{\mathrm{scal}} = \kappaChHodge\,\lambda_g^{\mathrm{FP}}$
is $O(c)$ at $g \ge 3$.
\item \emph{Super-linear ratio growth.}
$|\delta F_g^{\mathrm{cross}}| / |F_g^{\mathrm{scal}}|
\ge (2g - 2)!\,(2\pi)^{2g} / 2$, giving ratios
$7 \times 10^5$ at $g = 3$ and $2.6 \times 10^{23}$ at $g = 8$.
\item \emph{Irreducible numerator.} $204 = 2^2 \cdot 3 \cdot 17$;
the prime $17$ is sectoral-cusp data absent from the scalar
$\lambda_g^{\mathrm{FP}}$ numerator factorisations
$\{1, 7, 31, 127, 73, 1\,414\,477\}$ through $g \le 6$.
\item \emph{Separable collapse contradiction.} A separable ansatz
$G(c, \hbar) = \phi(c) \psi(\hbar) + \mathrm{const}$ forces
$\phi(c) \sim \mathrm{const}$ and $\phi(c) \sim 1/c$ simultaneously
from the constant and $1/c$ pieces of $(c + 204)/(16 c)$.
\item \emph{Picard--Fuchs rank gap.} The scalar Picard--Fuchs rank
is $1$; the cross-channel Picard--Fuchs rank is $\ge 6$ (Gevrey-$1$
in $\hbar$ rank $\ge 2$; sectoral $c$-poles rank $\ge 3$).
\end{enumerate}
The bivariate non-separable shadow takes the quasi-Jacobi form
\[
 G(c, \hbar)
 \;=\;
 \Phi_{\hat A}(\hbar)\,\Theta(c, \hbar)
 \;+\;
 \sigma_{\mathrm{Vas}}(c, \hbar),
\qquad
 \Phi_{\hat A}(\hbar) = \tfrac{\sqrt\hbar/2}{\sin(\sqrt\hbar/2)} - 1,
\]
with $\Theta$ a weight-$2$ quasi-Jacobi form in
$(z_c, \tau_\hbar) = (\log c/(2\pi i),\, \hbar/(2\pi i))$ and
$\sigma_{\mathrm{Vas}}(c, \hbar)$ the Vasiliev sectoral cusp residue.
\end{proposition}

\begin{proof}
Each obstruction (i)--(v) is individually checkable; their
conjunction forbids a separable closed form. (i) is the
genus-$2$ Vasiliev limit of $\cW_3$: as $c \to \infty$,
$\delta F_2^{\mathrm{cross}}(\cW_3) \to 1/16 = B(3)$, the
$N = 3$ value of the Vasiliev topological invariant
$B(N) = (N - 2)(N + 3) / 96$. (ii) follows from the Gevrey-$1$
genus growth of the cross-channel tower, with constant
$A_{\mathrm{cross}} / A_{\mathrm{scalar}} \in [1.7741, 3.1]$ from the
$g = 2, 3, 4$ data; the lower bound is
$\sqrt{56/(R_4/R_3)} = 1.7741$ with $R_3 = 42/31$,
$R_4 = 9184/381$. (iii) is the unique prime $17$ in the
$\cW_3$ numerator outside the scalar $\lambda_g^{\mathrm{FP}}$
prime spectrum, identifying the sectoral cusp residue
$S_3^W = 40/(5c + 22)$ at the tadpole vertex. (iv) is the
direct contradiction. (v) follows from the rank counting of
the bivariate Picard--Fuchs system; the cross-channel involves
both the Gevrey-$1$ $\hbar$-stratum and the sectoral $c$-strata.
\end{proof}

\begin{proposition}[Falsification of the separable
...
The honest two-point Gevrey-$1$ extraction with $c$-dependent
$(b(c),\, A_{\mathrm{cross}}(c))$ at each $c$ is self-consistent on
$c \in [1, 200]$, with
$b(c) \in [2.46,\, 4.66]$ and
$A_{\mathrm{cross}}(c) / (2 \pi) \in [0.10,\, 1.22]$; both quantities
grow monotonically with $c$. The Frontier band
$A_{\mathrm{cross}} / A_{\mathrm{scal}} \in [1.7,\, 3.1]$ is realised
at $(g = 3,\, c \in \{50, 70, 100\})$ and $(g = 4,\, c \approx 100)$.

Therefore the $\cW_3$ cross-channel resurgent asymptote is not the
canonical separable $A_2$ Frobenius reconstruction; the Gevrey class
remains underdetermined by the $g = 2, 3, 4$ data alone, and the
genus-$5$ datum (Route~(b) graph enumeration on
$\overline{\mathcal M}_{5, 0}$ over $\sim 4000$--$5000$ stable graphs,
or Route~(c) FSZ$10$/PPZ$19$ Pixton recursion at $r = 3$) is required
to fix $(b, A_{\mathrm{cross}})$ uniquely.
\end{proposition}
...
$-95\%$ and $-99.7\%$ relative error. The two-point Gevrey-$1$
extraction solves the pair of ratio equations
\[
 \log\bigl(|\delta F_{g + 1}/\delta F_g|\bigr) \big|_{g = 2}
 \;=\;
 b(c)\, \log(g) + 2 \log A_{\mathrm{cross}}(c) + \mathrm{const},
\qquad g = 2, 3,
\]
and the same identity at $g = 3, 4$, simultaneously in two unknowns
$(b(c),\, A_{\mathrm{cross}}(c))$ at each $c$. Self-consistency
($A_{\mathrm{cross}}^2$ from $g = 2 \to 3$ equals $A_{\mathrm{cross}}^2$
from $g = 3 \to 4$ to nine decimal digits) holds throughout
$c \in [1, 200]$.
\end{proof}
...
component is $\Theta^{(1,0)}_\cA = \kappaChHodge(\cA) \cdot \eta_1$ where
...
\lambda_g^{\mathrm{FP}} \;=\; \kappaChHodge(\mathcal{A}) \cdot
\lambda_g^{\mathrm{FP}}.
\end{equation}
...
Therefore $F_g(\cA) = \kappaChHodge(\cA) \cdot \lambda_g^{\mathrm{FP}}$
...
Verification: the \emph{diagonal} (sector-by-sector) part of the
...
\item For uniform-weight $\mathcal{W}_N$-like families \textup{(}hypothetical
 ...
 $Z_{\mathrm{grav}}^{\mathrm{scal}}(\cA;\,\hbar) = \kappaChHodge(\cA) \cdot
 \frac{\sqrt{\hbar}/2}{\sin(\sqrt{\hbar}/2)}$.
...
$F_g(\cA) = \kappaChHodge(\cA) \cdot \lambda_g^{\mathrm{FP}}$ for all
...
paramodular anomaly-comparison problem.  The data available in this
chapter are the one-loop anomaly class and the candidate weight
normalisation; they do not construct $\mathfrak g_{\Delta_5}$ or
$\mathbf H_{\Delta_5}$.  The proposed balance
\[
 \mathrm{wt}(\Delta_5) \;=\; \chi(\mathcal{O}_{K3}) \;+\; \sum_{i=1}^{24}\mathrm{Res}_{z=z_i}(\text{Kodaira } I_1) \;=\; 2 + 3 \;=\; 5
\]
is therefore a target identity: the second equality requires an
independent calculation of the total normalised $I_1$ residue
contribution.  The paramodular anomaly-cancellation condition gives a
...
symmetry. The required scalar check is that the Schur index, after
$M_{24}$-averaging over flavour fugacities, equals the K3 elliptic genus
$\phi_{0,1}$. Once that equality is proved, Borcherds' singular-theta
...
The Vol~II Chiral Hochschild Trinity universal-centre supertrace
$K(\mathcal{A}) = -c_{\mathrm{ghost}}(\mathrm{BRST}(\mathcal{A}))
= \mathrm{tr}_{\mathfrak{Z}(\mathcal{A})}(\mathfrak{K}_{\mathcal{A}})$
has a conditional $K3 \times T^2$ target under the Vol~III compact
Hall--Borcherds recognition package.  Once the finite Hall source,
pairing, PBW/no-extra-relations, inverse-limit, and Borcherds comparison
gates are supplied, the singular-theta pushforward should identify the
...
data, not a consequence of the scalar form alone.
\end{remark}
...
obstruction with Jacobi--Fourier coefficients of the weak Jacobi form
$\phi_{-2,1}$ (the Gritsenko-normalised Borcherds-exponent encoding of
$1/\Delta_5$, not the K3 elliptic genus
$\phi_{0,1}^{K3} = 2\phi_{0,1} + 20\phi_{-2,1}$ on the nose); and the
bimodular-weight-zero scalar target
$\bigl(\Phi_{10}^{\mathrm{un}}/(\eta(\tau)^{24}\eta(\tau')^{24})\bigr)^{\hbar\cdot
c_{K3}(Z)}$. The comparison becomes an identity for the BV trace only
...
coefficient source that must be compared with the K3 elliptic genus.

\begin{proposition}[Coefficient package for the K3 elliptic-genus /
$1/\Delta_5$ comparison]
...
\]
The required comparison package assigns to the $n$-loop BV obstruction
the target $q^{n}$ Fourier--Jacobi coefficient of the
Gritsenko--Nikulin Borcherds product $1/\Delta_{5}$, equivalently the
$q^{n}$-coefficient of the log-expansion of the
bimodular-weight-zero ratio
...
At $n=1$ this target specialises to
$c_{1}^{\mathrm{BV,target}}(X) = \chi(K3) = 24$
via $\chi_{K3}(\tau,z)|_{y=1} = 24 + O(q)$, matching the Costello
$1/24$ denominator of Theorem~\ref{thm:6d-hcs-one-loop-coefficient}
after the Gritsenko--Nikulin normalisation.  The equality
$c_{n}^{\mathrm{BV}}(X)=c_{n}^{\mathrm{BV,target}}(X)$ is a theorem
...
gated scalar comparison target is the bimodular-weight-zero
Siegel--Borcherds object
...
function on $\mathfrak H_{2}\setminus\{z=0\}$.  The identity
...
controls the asymptotic behaviour of
$Z_{\hCS}^{\mathrm{target}}(X,\hbar)$:
$\log Z_{\hCS}^{\mathrm{target}}(X, \hbar) \sim
\hbar \cdot c_{K3}(Z) \cdot
...
Insert the target coefficient package \eqref{eq:bv-obstruction-jacobi-matching}
into the scalar comparison of \eqref{eq:hcs-feynman-expansion}: the
target coefficient at $\hbar^{g}$ is $c_{1/\Delta_{5}}(q^{g})$ times a
logarithmic kernel in the Siegel variable $Z$. The kernel is the
Bruinier--Borcherds regularised lift of $\phi_{-2,1}$ supplied by the
source-recognition comparison; the BV graph calculation alone supplies
only the Costello-side coefficient source.  At each loop order, the
target bubble-chain contribution is
...
From the target identity \eqref{eq:hcs-all-loop-resummation}, taking
the logarithm and isolating the leading $\hbar$ dependence along any ray
$\hbar \to \infty$ avoiding the Humbert divisor gives
$\log Z_{\hCS}^{\mathrm{target}}(X, \hbar) =
\hbar \cdot c_{K3}(Z) \cdot
...
category of K3 with the Fourier coefficients
$c_{1/\Delta_{5}}(q^{g})$ (the Borcherds exponents of $1/\Delta_5$,
equivalent to the $y\to 1$ specialisation of the K3 elliptic-genus
$q^{g}$-coefficients). The two comparison packages agree only after
...
\label{rem:pf-deep-1}
The perturbative finiteness programme has, at the K3 base, the
one-loop scalar target
\[
