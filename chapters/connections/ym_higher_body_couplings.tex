...
\]
% RECTIFICATION-FLAG: [Gate 2 define-before-use] $\widetilde Z_\infty(M_i)$ (the complete reduced derived centre, $\widetilde Z$ of the complete filtered minimal $A_\infty$-algebra $M_i$) is used here and in cor:persistence-criterion-quadratic-law but never given a formal definition in this chapter; only $Z_S := HH^0(M_S)$ (Convention) and $\widetilde Z_i$ for decoupled simplices (Def. w-decoupled-simplex) are defined. Add a one-line definition of $\widetilde Z_\infty(M_i)$ near Def. mixed-deformation-linfty-relative. Not fixed here to avoid inventing notation.
