...
stabilizes arity-by-arity, requiring no analytic convergence
condition. Composition
$(g\circ f)_n = \sum_{i+j=n}g_i\circ(\mathrm{id}^{\otimes i}
\otimes f_j)$ is likewise a finite sum at each arity.
\end{remark}
...
coproduct $\Delta_z$ on $\cA^!=Y_z^\hbar(\mathfrak{sl}_2)$
...
differs from the Yangian $Y_z^\hbar(\fg)$ of a simple Lie
...
\item The Laplace kernel $r^L_c(z)$ has a fourth-order pole
 ...
 $r(z)\sim k\,\Omega/z$; the Virasoro Laplace kernel has the
...
simple pole $k\,\Omega/z$, and the infinite-dimensional algebra
...
the CYBE for $r(z)=k\,\Omega/z$, proved in
Proposition~\ref{prop:ybe-from-d-squared}).
...
Yangian $Y_z^\hbar(\mathfrak{sl}_2)$ with
...
$\cA^!=Y_z^\hbar(\fg)$ (the dg-shifted Yangian of a simple
Lie algebra~$\fg$). The \emph{MC3 problem} is:

\emph{Lift the character-level Clebsch--Gordan
decomposition of $Y_z^\hbar(\fg)$-modules to a categorical
equivalence in the derived module category
$\Ydg(\fg)\text{-}\mathbf{mod}$.}

Concretely: given two $Y_z^\hbar(\fg)$-modules $M$ and $N$,
...
The \emph{chromatic filtration} on $Y_z^\hbar(\mathfrak{sl}_N)$
...
Y_z^\hbar(\mathfrak{sl}_N)
\;=\;
\bigoplus_{\alpha\in Q}
Y_z^\hbar(\mathfrak{sl}_N)_\alpha,
\end{equation}
where $Q=\Z\alpha_1\oplus\cdots\oplus\Z\alpha_{N-1}$ is the
root lattice and $Y_z^\hbar(\mathfrak{sl}_N)_\alpha$ consists
...
$Y_z^\hbar(\mathfrak{sl}_2)$. Then for all $n\ge 0$:
\begin{equation}
...
$\pi_j\in\End_{Y_z^\hbar}(V_n\otimes L^-)$ ($0\le j\le n$)
...
$\operatorname{REnd}_{Y_z^\hbar}(V_n\otimes L^-)$.
\end{definition}
...
\item \textbf{DK-4} (completed bar-cobar equivalence):
 ...
 \texttt{conj:dk-compacts-completion}
...
DK-5 & MC3 + MC4 + MC5 (all genera; analytic and $D^{\mathrm{co}}$ lanes proved, chain-level conjectural)
\end{tabular}
\end{center}
...
$Y_z^\hbar(\mathfrak{sl}_N)\to\Ydg(\mathcal{W}_N{}_{c^!_N})$
...
$r(z)=k\,\Omega/z$. The DS functor sends this to
