...
(Vol~I, Theorem~\ref*{V1-thm:bar-cobar-adjunction}).
...
\end{equation}
where $\kappa_{M2} := \kappaChHodge(A_{M2,\infty})$ is the modular
...
$\kappaChHodge(\cA) = \sum_a \langle e_a, e^a\rangle_\cA$
(Vol~I, Definition~\ref*{V1-def:modular-curvature-direct})
...
\ClaimStatusConditional modulo $\kappa_{M2}$ value (\ClaimStatusHeuristic)]
...
= 0.
\]
For Kodaira--Spencer/Calabi--Yau mirror pairs, the complementarity relation $\kappaChHodge(\cA)+\kappaChHodge(\cA^!)=0$ is
...
Mukai-doubling $K^{\kappaChHodge} = 8$. Cross-ref Vol.~III,
