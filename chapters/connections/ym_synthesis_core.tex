...
%=======================================================================
% Scope and unification with gravity / celestial / N=4
%=======================================================================
...
Costello--Costello--Paquette~\cite{CostelloLiSugraTwist});
...
$T_4^{\mathrm{grav}}$ & $\mathbf M$ & Brown--Henneaux $c=3\ell/(2G)$
\end{tabular}
\end{center}
...
The statement that the algebraic reduction of the mass-gap problem
to screening conditions is a genuine equivalence, not a formal
rewriting, is recorded below as a theorem under explicit analytic
hypotheses.
...
analytic untwisting map $U\colon H_{\mathrm{YM}} \to
\mathrm{Op}(B^\Lambda(\cA^{\mathrm{cel}}_{\mathrm{YM}}))$ identifying
the physical YM Hamiltonian with a canonical operator generated by
the instanton-completed boundary package.
\end{enumerate}
Then the mass-gap statement for $T_4^{\mathrm{YM}}$, namely
$\inf \mathrm{spec}(H_{\mathrm{YM}}) - E_0 > 0$,
is equivalent to the algebraic statement
$Z_{\mathrm{vis}}(B^\Lambda(\cA^{\mathrm{cel}}_{\mathrm{YM}})) = 0$,
i.e.\ the visible centre of the instanton-completed bar complex is
trivial.
\end{theorem}
...
construction inside the Wakimoto realization; (H3) is the untwisting
map required by
Remark~\ref{rem:ym-mass-gap-remaining-gap} (in
\texttt{ym\_instanton\_screening.tex}). Under (H1)--(H3), the
screening domination estimate of the GNS construction translates
the spectral gap $\mathrm{spec}(H)-E_0 > 0$ into the algebraic
non-degeneracy of $Z_{\mathrm{vis}}$ on
$B^\Lambda(\cA^{\mathrm{cel}}_{\mathrm{YM}})$; $Z_{\mathrm{vis}}=0$
forbids center-symmetric deformations, which by Polyakov confinement
(Remark~\ref{rem:polyakov-confinement-algebraized}) is equivalent to
monopole condensation and hence to the mass gap. The remaining analytic
content is (H3), which is the precise formulation of the residual
physics-to-algebra bridge gap.
\end{proof}

\begin{remark}[Status: conditional on (H3), unconditional reduction]
\label{rem:mass-gap-status}
Hypotheses (H1) and (H2) are proved (Theorem~\ref{thm:affine-half-space-bv},
and \S~\ref{subsec:ym-screenings-wakimoto} in
\texttt{ym\_instanton\_screening.tex}). Hypothesis (H3)---the
untwisting map---is the frontier: it requires a rigorous
identification of the physical Hamiltonian with a canonical
boundary-package operator. The statement of
Theorem~\ref{thm:ym-mass-gap-reduction} is thus \emph{the
mass-gap reduction as a genuine equivalence, conditional on (H3)};
without (H3) the bar-cobar formalism gives a strictly weaker
algebraic reduction. We record (H3) as the precisely-named open
problem of the programme, replacing the vaguer ``algebraic
reduction'' of earlier drafts.
\end{remark}
% ===== End {fn} =====
