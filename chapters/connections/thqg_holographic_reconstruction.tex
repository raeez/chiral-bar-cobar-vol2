...
(Theorem~\ref{V1-thm:nms-all-arity-master-equation}),
...
\item The holographic R-matrix is $r(z) = k\,\Omega/z$
  with $k = 1$ \textup{(}classical Yang--Baxter; AP126\textup{)}.
...
(d) $r(z)$ & $r(z) = k\,\Omega/z$ at unit level $k = 1$,
  $\Omega = \sum \delta^{ab}\,J_a \otimes J_b$
  \textup{(}single pole; AP19/AP126\textup{)};
...
The Gerstenhaber bracket is determined at arity~$2$:
\[
  \{J, J\} \;=\; k,
\]
...
A non-trivial arity-$3$ component arises from the cubic Casimir
interaction, scaled by a rational function of~$k$.
The tower terminates: all brackets of arity $\geq 4$ are determined
by the lower ones.
...
The bracket has non-vanishing components at every arity $m_k$,
...
Class~$\mathsf{G}$ brackets close at arity~$2$;
class~$\mathsf{L}$ at arity~$3$;
