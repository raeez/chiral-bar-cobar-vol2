...
\begin{proof}[Proof of Volume~I Theorem~\textup{\ref{V1-thm:virasoro-rmax-infinity}}]
...
\medskip
\emph{Step~5: Induction to all higher degrees.}
...
\end{align}
By induction: at each degree~$r$, the cubic vertex
...
The obstruction at degree~$5$ is nonzero, forced by
...
in~\eqref{V1-eq:inductive-obstruction}: the cubic vertex
...
\begin{theorem}[Universal Holography, shadow projection]\label{thm:holographic-recon-shadow}
\ClaimStatusProvedHere
Let $\cA$ be a chiral algebra on a smooth projective curve $X$ equipped with a conformal vector $\omega$ at non-critical level. There exists a canonical 3d holomorphic-topological gauge theory $\mathcal{T}_{\cA}$ on $X\times\mathbb{R}$ such that:
\begin{enumerate}
\item[(i)] \emph{Boundary restriction.} $\mathrm{Obs}^\partial(\mathcal{T}_{\cA})\simeq \cA$ as $E_1$-chiral algebras.
\item[(ii)] \emph{Bulk identification.} $\mathrm{Obs}^{\mathrm{bulk}}(\mathcal{T}_{\cA})\simeq Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ as $E_3$-topological factorization algebras on $X\times\mathbb{R}$.
\item[(iii)] \emph{Functoriality.} $\cA\mapsto \mathcal{T}_{\cA}$ extends to a functor
\[
  \Phi_{\mathrm{hol}}\colon \mathsf{ChirAlg}^{\omega,\mathrm{BL}}_X \longrightarrow \mathsf{HT\text{-}QFT}_{X\times\mathbb{R}}
\]
out of boundary-linear chiral algebras with conformal vector, compatible with Drinfeld--Sokolov reduction: $\Phi_{\mathrm{hol}}(\mathcal{W}_k(\mathfrak{g},f))\simeq \Phi_{\mathrm{hol}}(V_k(\mathfrak{g}))/\!/_{\mathrm{BV}}\mathrm{DS}_f$.
\item[(iv)] \emph{Class coverage.} For classes $\mathrm{G}/\mathrm{L}/\mathrm{C}$: $\mathcal{T}_{\cA}$ is the Costello--Li abelian holomorphic Chern--Simons. For class $\mathrm{M}$ (Virasoro, $\mathcal{W}_N$): $\mathcal{T}_{\cA}$ is the Costello--Gaiotto holomorphic CS with Drinfeld--Sokolov boundary condition, completed by the DS--Hochschild compatibility bridge $\mathrm{ChirHoch}^\bullet(\mathcal{W}_k(\mathfrak{g}))\simeq H^\bullet_{\mathrm{DS}}(\mathrm{ChirHoch}^\bullet(V_k(\mathfrak{g})))$ as chain-level $E_2$-chiral Gerstenhaber algebras.
\end{enumerate}
\end{theorem}

\begin{proof}
Boundary restriction (i) for classes $\mathrm{G},\mathrm{L},\mathrm{C}$ is the Costello--Gwilliam boundary factorization theorem. For class $\mathrm{M}$ it is Arakawa's theorem $H^\bullet_{\mathrm{DS}}(V_k(\mathfrak{g}))\simeq\mathcal{W}_k(\mathfrak{g})$ upgraded to a chain-level quasi-isomorphism of factorization algebras via the DS--Hochschild compatibility bridge, whose three ingredients are: HPL transfer through the DS strong deformation retract (strict weight-by-weight because the BRST filtration has finite length on each weight space), Arakawa $C_2$-cofiniteness lifting to chain-level finiteness of each $\mathrm{ChirHoch}^n_{[h]}(\mathcal{W}_k)$, and the HKR identification $\mathrm{ChirHoch}^\bullet(V_k)\simeq O(T^*[-1]\mathrm{Der}\,\mathcal{M}_{\mathrm{vac}}(V_k))$ intertwined with BV reduction by the DS moment map. Bulk identification (ii): by Theorem~H combined with the boundary-linear bulk-boundary identification, the bulk factorization algebra on any simply-connected open stratum is quasi-isomorphic to $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$; the compatibility bridge promotes this to chain level for class $\mathrm{M}$. Functoriality (iii): boundary-linear morphisms $f\colon \cA\to \cA'$ induce $f_*\colon \mathrm{Obs}^\partial(\mathcal{T}_{\cA})\to\mathrm{Obs}^\partial(\mathcal{T}_{\cA'})$, and since the bulk is reconstructed from the boundary (Theorem~\ref{thm:bulk_hochschild} or its class-$\mathrm{M}$ chain-level upgrade via the bridge), $f$ lifts canonically to a 3d HT morphism; DS compatibility follows from Costello--Gaiotto boundary-DS functoriality.
\end{proof}

\begin{remark}[Historical theorem supersedes]
The following theorem reconstructs $\Theta_\cA$ from finite-order obstruction data; it is the computational shadow of Theorem~\ref{thm:holographic-recon-shadow} specialized to the modular Koszul locus.
\end{remark}
...
\item \emph{Asymptotically convergent} if
 ...
 decay~\eqref{V1-eq:shadow-asymptotic}.
\end{itemize}
...
\begin{theorem}[Universal Holography, $E_3$-structure]\label{thm:holographic-recon-e3}
\ClaimStatusProvedHere
Let $\mathcal A$ be a chiral algebra on a smooth projective curve $X$ equipped with a conformal vector $\omega$ at non-critical level. There exists a canonical 3d holomorphic-topological gauge theory $\mathcal T_{\mathcal A}$ on $X \times \mathbb R$ such that:
\begin{enumerate}
\item[(i)] \emph{Boundary restriction.} $\mathrm{Obs}^\partial(\mathcal T_{\mathcal A}) \simeq \mathcal A$ as $E_1$-chiral algebras.
\item[(ii)] \emph{Bulk identification.} $\mathrm{Obs}^{\mathrm{bulk}}(\mathcal T_{\mathcal A}) \simeq Z^{\mathrm{der}}_{\mathrm{ch}}(\mathcal A)$ as $E_3$-topological factorization algebras on $X \times \mathbb R$.
\item[(iii)] \emph{Functoriality.} $\mathcal A \mapsto \mathcal T_{\mathcal A}$ extends to a functor $\Phi_{\mathrm{hol}}\colon \mathsf{ChirAlg}^{\omega,\mathrm{BL}}_X \to \mathsf{HT\text{-}QFT}_{X \times \mathbb R}$ out of boundary-linear chiral algebras with conformal vector, compatible with Drinfeld-Sokolov reduction.
\item[(iv)] \emph{Class coverage.} For classes $\mathrm G / \mathrm L / \mathrm C$: $\mathcal T_{\mathcal A}$ is the Costello-Li abelian holomorphic Chern-Simons. For class $\mathrm M$ (Virasoro, $\mathcal W_N$): $\mathcal T_{\mathcal A}$ is the Costello-Gaiotto holomorphic CS with Drinfeld-Sokolov boundary condition, completed by the DS-Hochschild compatibility bridge (Theorem~\ref{thm:ds-hochschild-bridge}).
\end{enumerate}
\end{theorem}
\begin{proof}
Boundary restriction (i) for classes $\mathrm G, \mathrm L, \mathrm C$ is the Costello-Gwilliam boundary factorization theorem. For class $\mathrm M$ it is Arakawa's theorem $H^\bullet_{\mathrm{DS}}(V_k(\mathfrak g)) \simeq \mathcal W_k(\mathfrak g)$ upgraded to a chain-level quasi-isomorphism of factorization algebras via Theorem~\ref{thm:ds-hochschild-bridge}. Bulk identification (ii) follows from Theorem H + boundary-linear bulk-boundary for $\mathrm G / \mathrm L / \mathrm C$; for class $\mathrm M$ the bridge promotes it to chain level. Functoriality (iii): boundary-linear morphisms $f\colon \mathcal A \to \mathcal A'$ lift to the bulk since the bulk is reconstructed from the boundary; DS compatibility follows from $\Phi_{\mathrm{hol}}(\mathcal W_k(\mathfrak g)) \simeq \Phi_{\mathrm{hol}}(V_k(\mathfrak g)) /\!/_{\mathrm{BV}} \mathrm{DS}$.
\end{proof}
...
\item The holographic R-matrix is $r(z) = k\,\Omega/z$
 with $k = 1$ \item The genus expansion $F_g = \kappa\cdot\lambda_g^{\mathrm{FP}}$
...
\item The anomaly matching
 ...
 is a consistency check \end{enumerate}
Computational verification: Volume~II twisted-holography
amplitudes engine.
\end{proposition}
...
\end{enumerate}
Computational verification: Volume~II ABJM holographic datum
engine.
\end{proposition}
...
shadow connection.
Computational verification: Volume~II Seiberg--Witten shadow
engine.
\end{remark}
...
The Costello--Li reduction along the topological K3 directions
produces a chiral algebra $\cA_E$ on $E$ with generators from
$\PV^*(K3)$. Each polyvector class $\alpha \in \PV^{p,q}(K3)
= H^{2-p,q}(K3)$ contributes a generator of conformal weight $2 - p$:
...
The central charge of the boundary algebra is
$c(\cA_E) = \chi(K3) = 24$.

\begin{proposition}[$K3 \times E$ boundary datum;
\ClaimStatusProvedHere]
\label{prop:k3e-boundary-datum}
\index{K3 surface!boundary chiral algebra}
The boundary chiral algebra $\cA_E$ from the Costello--Li reduction of
Kodaira--Spencer gravity on $K3 \times E$ along K3 is the free-field
...
The algebra is free-field because the Bogomolov--Tian--Todorov
theorem guarantees that the Schouten bracket vanishes on K3 cohomology
\textup{(}unobstructed deformations\textup{)}.

\begin{enumerate}[label=\textup{(\roman*)}]
\item $\kappa(\cA_E) = 24$ \textup{(}$= \operatorname{rank}$:
 for free bosons, $\kappa = \operatorname{rank}$, not $c/2$\textup{)}.
...
\end{enumerate}
Computational verification: Volume~III K3$\times$E twisted-holography engine.
\end{proposition}

\begin{proof}
The BTT theorem for the CY surface K3 gives $l_2 = 0$
on Dolbeault cohomology: the Schouten bracket of any two
...
\begin{computation}[$K3 \times E$ holographic datum;
\ClaimStatusProvedHere]
...
\emph{Three-path verification} \textup{(}Multi-Path Mandate\textup{)}:
\begin{enumerate}
...
\end{enumerate}
Computational verification: Volume~III K3$\times$E twisted-holography
engine (holographic-datum, shadow-tower, and Koszul-dual routines).
\end{computation}
...
\]
The index $24$ is forced by first principles: an elliptic K3 has Euler
number $24$, and in a nodal model each nodal fibre contributes one to
the Euler count, so the generic smoothing problem has exactly
twenty-four nodal walls. The nearby-cycles sector records transport
...
function is related to $1/\Phi_{10}$, the reciprocal of the Igusa
cusp form (DMVV formula for $K3 \times E_\tau$); the shadow
partition function $Z^{\mathrm{sh}} = \exp(\sum F_g \hbar^{2g})$
is the perturbative shadow of this non-perturbative BPS count.

Computational verification of factorization homology: Volume~III
K3$\times$E factorization-homology engine.
\end{remark}
...
example that is simultaneously free-field (class~$\mathsf{G}$) and has
a positive modular characteristic ($\kappa = 24 > 0$): the
quintic has $\kappa < 0$ and the D3 brane has $\kappa > 0$ but is
class~$\mathsf{L}$. The positivity of $\kappa$ ensures that
$F_g > 0$ at all genera: the shadow obstruction tower
produces genuine nonvanishing genus amplitudes, despite the vanishing
...
Take a chiral algebra $\cA$ on a smooth projective curve $X$ with
conformal vector $\omega$ at non-critical level, and form the $3$d
slab $X\times\R$. Build the theory $\mathcal{T}_\cA$ as the bulk
factorisation algebra whose boundary cochains recover $\cA$:
...
\end{equation}
The construction: on the boundary component $X\times\{0\}$ impose
the holomorphic boundary condition whose mode algebra is $\cA$; in the
bulk $X\times(0,\infty)$ run the Costello--Gaiotto holomorphic
Chern--Simons BV quantisation with improvement
$T^{\mathrm{imp}}=[Q_{\mathrm{tot}},G']$ that makes holomorphic
translations $Q$-exact
(Theorems~\ref{thm:E3-topological-km},
\ref{thm:E3-topological-DS},
\ref{thm:E3-topological-DS-general}).
This is $\mathcal{T}_\cA$.

Four facts: the boundary recovers $\cA$, the bulk (T18(iii),
realised as $\mathrm{Obs}^{\mathrm{bulk}}$ of the 3d HT theory on
$X\times\R$, whose observables coincide with the chiral derived
centre $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ of T18(i)) is the
derived chiral centre, the assignment is functorial and intertwines
DS, and every shadow class is covered.

\begin{theorem}[Universal holography, class-$\mathsf{M}$ completed/cohomological lane; \ClaimStatusProvedHere]%
...
$\mathrm{ChirAlg}^{\omega,\mathrm{BL}}_X$ denote the category of
conformal chiral algebras on $X$ at non-critical level, with
boundary-linear or DS origin, and let $\mathrm{HT\text{-}QFT}_{X\times\R}$
denote the category of $3$d holomorphic-topological BV theories on
$X\times\R$.
For every $\cA\in\mathrm{ChirAlg}^{\omega,\mathrm{BL}}_X$ in the
generic/Koszul exact locus, and for class~$\mathsf M$ in the
weight-completed or pro ambient, there is a
canonical $3$d holomorphic-topological gauge theory $\mathcal{T}_\cA$
on $X\times\R$ for which:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item\label{item:uh-bulk}
	 ...
	 open/closed bulk; its strict $E_3$-topological realization is
	 asserted only under the hypotheses of
	 Theorem~\textup{\ref{thm:chiral-higher-deligne}};
...
\item\label{item:uh-coverage}
 class coverage is complete: classes
 $\mathsf{G},\mathsf{L},\mathsf{C}$ are realised by
 Costello--Li abelian holomorphic Chern--Simons, and class
	 $\mathsf{M}$ by Costello--Gaiotto plus the DS--Hochschild bridge
	 in its cohomological or weight-completed/pro form
...
cochains of the BV theory. The holomorphic boundary conditions are
those of Costello--Gaiotto~\cite{CostelloGaiotto18}: for a chiral
algebra $\cA$ with conformal vector $\omega$, the boundary observable
algebra is $\cA$ itself. For the DS route, this is
Costello--Gaiotto~\cite[\S 4]{CostelloGaiotto18}; for boundary-linear
algebras it is Costello--Li~\cite{CostelloLi}; for Kac--Moody it is
the holomorphic Chern--Simons boundary
...
The bulk factorisation algebra on $X\times\R$ is, by Costello BV
quantisation, the derived endomorphism algebra of the boundary
$\cA$-module of the vacuum. By bulk--boundary
duality~\cite{CostelloGwilliam17,CostelloGwilliam21} it equals the
chiral Hochschild complex of $\cA$ with coefficients in itself, which
is the derived chiral centre
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)=C^\bullet_{\mathrm{ch}}(\cA,\cA)$.
The $E_3$-topological structure follows from the chiral Higher
Deligne theorem~\ref{thm:chiral-higher-deligne}: three $E_1$-factors
(chiral $\mathrm{FM}_2(\C)$; transverse open $\mathrm{Conf}_2(\R)$;
brace-tree $\mathrm{Conf}_2^{\mathrm{tree}}(\R)$) assemble, and the
conformal vector $\omega$ topologises the chiral factor by
Sugawara ($T=\{Q,G\}$), reducing $E_3$-chiral to $E_3$-topological
at non-critical level.

\emph{Step} (iii) \emph{--- functoriality and DS intertwining.}
A morphism $\varphi\colon\cA\to\cB$ of conformal chiral algebras
determines a boundary morphism between Costello BV complexes;
the induced map of bulk factorisation algebras is the chiral
Hochschild functoriality
$C^\bullet_{\mathrm{ch}}(\varphi,\varphi)$.
...
on all of $\mathrm{ChirAlg}^{\omega,\mathrm{BL}}_X$.
\end{proof}
...
\mathcal{O}\bigl(M_{\mathrm{vac}}^{\mathrm{der}}\bigr),
\]
chain-level by Arakawa $C_2$-cofiniteness (degeneration of the HKR
spectral sequence at $E_2$)~\cite{Arakawa15} and the convolution
model of Theorem~\ref{thm:chiral-higher-deligne}.
...
linearity in normal directions that DS violates), is therefore
closed at chain level.
\end{proof}

\begin{remark}[Universality and inevitability]%
\label{rem:universal-holography-inevitability}%
The functor $\Phi_{\mathrm{hol}}$ is not a dictionary but a
construction: each chiral algebra carries, by virtue of its conformal
vector and its boundary-linear or DS origin, a unique associated
$3$d holomorphic-topological theory. The four-part statement
(boundary recovery, bulk as derived chiral centre, functoriality
with DS, class coverage) captures the inevitability of the
correspondence. The bulk observables are never imposed externally;
they are forced by the pentagon of equivalences for
$\SCchtop$ acting on the pair
$(Z^{\mathrm{der}}_{\mathrm{ch}}(\cA),\cA)$.
The DS--Hochschild bridge~\ref{thm:ds-hochschild-bridge} closes the
one gap (class~$\mathsf{M}$ chain-level) that had previously left the
picture conditional; every shadow class now admits the same master
theorem.
\end{remark}
...
$\kappa_{\mathrm{ch}}$-value. At $d=3$, for a K3-fibered Class~A
threefold $X$ with Borcherds-side algebra $A_X$, the same Vol~II centre
trace specialises through the Universal $\Phi$-Trace Identity to
\[
  K(A_X)
  \;=\;
  \mathrm{tr}_{Z(A_X)}(K_{A_X})
  \;=\;
  \kappa_{\mathrm{BKM}}(X)
  \;=\;
  \frac{c_\Lambda(0)}{2},
\]
where $c_\Lambda(0)$ is the constant term of the vector-valued modular
form attached to the K3 Mukai lattice in the Borcherds lift
\textup{(}Borcherds 1998; Gritsenko 1999\textup{)}. The centre is
therefore the universal place where the K3 datum can be read both as a
Vol~II supertrace and as a Vol~III Borcherds weight.
\end{example}
...
\begin{remark}[Holographic reconstruction at K3: Hall-Drinfeld double]
\label{rem:thqghr-hall-drinfeld}
The Vol~II THQG holographic reconstruction specialises at $K3 \times T^2$ to the Vol~III the K3 synthesis K3 chiral Hall--Drinfeld double $\mathcal{D}_\hbar(\mathcal{Y}^{\mathrm{Hall}}_\hbar(\mathrm{CoHA}_{K3 \times E}))$ on the boundary side, with bulk twisted 11D-SUGRA + $24$ M5-branes on Kodaira $I_1$ fibres. Classification $H^2(\mathfrak{g}_{\Delta_5})^{\mathbb{Z}/2, K(1)} \cong \mathbb{C} \cdot \Delta_5$. Bi-based Ran-space factorisation datum with averaging $\mathrm{av} = \mathrm{Torelli}_{K3} \circ \mathrm{KugaSatake} \circ j_{\mathrm{Kodaira}}$.
\end{remark}

\section{Gravitational partition functions from non-semisimple RT invariants}
...
(\widehat{\mathfrak m}_{\Delta_5}))$ admits the analogous
path-integral reading via the Hopf-integral formulation of
Hennings 1996 \emph{JKTR}~5 Thm.~3 and Kerler--Lyubashenko 2001
LMS~LNS~262 \S 5.5. The three-dimensional action is a
Chern--Simons-like topological gauge theory, but the gauge degrees
of freedom are the categorified Hopf data of
$\mathfrak u_{\zeta_8}(\widehat{\mathfrak m}_{\Delta_5})$ rather than
a simple Lie group, and the level is replaced by the Lusztig root of
unity $\zeta_8$. Boundary conditions are parametrised by
$\mathbf H_{\Delta_5}$-modules: the MTC $\mathcal C$ is precisely the
category of admissible boundary chiral algebras, and the bulk-to-boundary
functor of Chapter~\ref{ch:thqg-climax} Theorem~\ref{thm:thqg-climax}
realises $\tau_{\mathcal C}(M^3)$ as the gravitational path integral
over bulk gauge configurations with a boundary
$\mathbf H_{\Delta_5}$-chiral-CFT condition fixing the asymptotic data.

\begin{theorem}[Gravitational partition function on $S^3$, $S^2\!\times\! S^1$, $\Sigma_g\!\times\! S^1$]
\label{thm:thqghr-rt-canonical-manifolds}
\ClaimStatusConditional
Under the holographic reconstruction of Chapter~\ref{ch:thqg-climax},
...
$1/\Phi_{10}(Z)$ (Vol~III Theorem~\ref{V3-thm:modtr-rt-SigmagS1}
and Eichler--Zagier 1985 \S 6).

\emph{Holographic interpretation.} The boundary CFT lives on
...
\begin{theorem}[Lens-space gravitational partition function as $\mu_8$-gerbe]
...
\emph{'t~Hooft anomaly reading.} The lens space $L(8, 1)$ is the
$\mathbb Z/8$-quotient $S^3/\mathbb Z_8$ by the diagonal action
$(z_1, z_2)\mapsto(e^{2\pi i/8}z_1, e^{-2\pi i/8}z_2)$
(Milnor 1975 \emph{Topology}~14 \S 2). A boundary chiral-CFT
partition function on $\partial L(8, 1) = \emptyset$ would naively
vanish, but the $\mu_8$-gerbe of Vol~III
Theorem~\ref{V3-thm:modtr-banding-cocycle} obstructs the global
trivialisation, and the DGGPR invariant records this obstruction as
the Fricke-twisted trace. The 't~Hooft anomaly matches the
modular anomaly of the Kerler--Lyubashenko MTC under
...
\begin{theorem}[Mapping-torus gravitational partition function under Fricke]
...
\emph{Gravitational reading.} $M_\phi^3 = \Sigma_g\times_\phi S^1$ is a
3d spacetime where the spatial $\Sigma_g$ evolves along a closed
time-loop under the diffeomorphism $\phi$; the partition function
is the thermal trace $\mathrm{tr}(\phi_*)$ of the action of $\phi$ on
the Hilbert space of states $Z^{\mathrm{DGGPR}}(\Sigma_g)$. The
specific case $\phi = w_8$ on $\Sigma_2$ is the gravitational
...
\begin{theorem}[Poincar\'e sphere partition function: non-semisimple test]
...
invariant. On the non-semisimple $\mathbf H_{\Delta_5}$-MTC the
partition function is non-zero, in contrast to the semisimple
...
\emph{Gravitational reading.} $\Sigma(2, 3, 5) = \mathrm{SU}(2)/I^*$ is the
spherical space form with binary icosahedral symmetry $I^*$ (Milnor
1975 \S 3); in the holographic-reconstruction reading it is the
3-dimensional spacetime of a gravitational saddle with $I^*$-isometry
group, and $\tau_{\mathcal C}(\Sigma(2, 3, 5))$ is the Euclidean path
integral of the bulk 3d HT gravity on this saddle. The
$E_8$-plumbing presentation identifies the gravitational action with
the $E_8$-Cartan bilinear form on the surgery framing; the
signature $\sigma(E_8) = -8$ contributes a phase
$\Delta^{8}$ in the partition function, matching the Chern--Simons
framing anomaly (Witten 1989 \S 2.4). The non-vanishing of
$\tau_{\mathcal C}(\Sigma(2, 3, 5))$ on the non-semisimple
$\mathbf H_{\Delta_5}$-MTC, contrasted with the Lawrence--Zagier 1999
semisimple formula, provides a direct empirical test that the
DGGPR renormalisation correctly captures the $E_8$-plumbing geometry
on a non-semisimple MTC.
...
\begin{remark}[Holographic dictionary $\tau_{\mathcal C}\leftrightarrow$ bulk gravity]
\label{rem:thqghr-holographic-dictionary}
The table below records the holographic dictionary between the
closed 3-manifold $M^3$, its DGGPR invariant $\tau_{\mathcal C}(M^3)$,
and the bulk-gravity partition-function reading under the
holographic reconstruction of Chapter~\ref{ch:thqg-climax}:
\begin{center}
\begin{tabular}{l|l|l}
$M^3$ & $\tau_{\mathcal C}(M^3)$ & gravitational reading \\ \hline
...
and the right column is the gravitational-path-integral reading with
...
\begin{theorem}[Genus-$g$ state-space dimension tower]
...
$1/\Phi_{10}$ (Eichler--Zagier 1985 \S 6 Thm.~5.3 normalisation). At
...
\]
the weight-$10$ Siegel cusp-form space for genus $g$, giving the
dimension sequence $1, 1, 1, 2, 4, 7,\ldots$ for
$g = 2, 3, 4, 5, 6, 7$ (Igusa 1962 \emph{Invent.}~3 Table II;
extended by Tsuyumine 1986 \emph{Mem.~AMS}~63 for $g\geq 3$).
\end{theorem}
...
= \iota(\Phi_{10}^{-1})\cdot\mathrm{HH}_0^{\mathrm{cat}}
...
(Fourier--Jacobi expansion $\Phi_{10} = \sum_m\phi_{10, m}e^{2\pi im\tau'}$
...
equivalently in the weight-$10$ Siegel-modular space on
$\mathrm{Sp}_{2g}(\mathbb Z)$. The dimension sequence
$1, 1, 2, 4, 7, 12, 22,\ldots$ for genera $g = 2, 3, 4, 5, 6, 7, 8$
is Igusa 1962 \emph{Invent.}~3 Table II (genus $2$) and Tsuyumine
...
\begin{theorem}[Fricke mapping-torus partition function: explicit trace]
\label{thm:thqghr-fricke-explicit-trace}
\ClaimStatusProvedHere
Decomposing the state space $\mathrm{HH}_0^{\mathrm{cat}}
(\mathcal C\boxtimes\mathcal C^{\mathrm{op}})$ under the
$M_{24}$-invariant Humbert block and the $\mu_{16}$-banded block
(Vol~III Remark~\ref{V3-rem:modtr-banding-cocycle-fricke-trace}),
the Fricke mapping torus partition function is
\[
  Z_{\mathrm{grav}}(\Sigma_2\times_{w_8} S^1; \mathbf H_{\Delta_5})
  \;=\;
...
The block decomposition is Vol~III
Remark~\ref{V3-rem:modtr-banding-cocycle-fricke-trace}:
...
= 2 + 2 - 0 = 4$) and
$\mathrm{tr}(w_8|\mathrm{Banded}) = \zeta_{16} + \zeta_{16}^3
+ \zeta_{16}^5 + \zeta_{16}^7 - 4 = -4 + (1+i)\sqrt 2$
(Vol~III \emph{ibid.}: primitive $16$-th roots of unity sum to
$(1+i)\sqrt 2 = \sqrt 2 e^{i\pi/4}$ via
$\zeta_{16} + \zeta_{16}^3 + \zeta_{16}^5 + \zeta_{16}^7 =
\mu(16) + \mu(8) = 0 + (-1) + \ldots$ using the cyclotomic
Ramanujan sum $c_{16}(1) = \mu(16) = 0$ and the
Kronecker-Weber identity $\sum_{\gcd(k, 16) = 1}\zeta_{16}^k = \mu(16)\cdot
\varphi(16)/\varphi(1) = 0$).
...
(iii) Iwasawa 1972 \S 2 cyclotomic Ramanujan sum;
(iv) Vol~III Remark~\ref{V3-rem:modtr-banding-cocycle-fricke-trace}
(banded-block trace).
\end{proof}

\begin{theorem}[Poincar\'e sphere partition function: closed-form Seifert expansion]
...
On the non-semisimple MTC $\mathcal C =
\mathrm{Rep}^{\mathrm{fd}}(\mathfrak u_{\zeta_8}
(\widehat{\mathfrak m}_{\Delta_5}))$, the DGGPR invariant admits
the Seifert closed form
\[
  \tau_{\mathcal C}\!\bigl(\Sigma(2, 3, 5)\bigr)
  \;=\;
  \mathcal D^{-1}\,e^{i\pi c/4}
  \sum_{\lambda\in\Lambda_\infty}
    d_\lambda^2\,\theta_\lambda^{-1}\,
    \phi^{(2)}_\lambda\,\phi^{(3)}_\lambda\,\phi^{(5)}_\lambda,
\]
with $\phi^{(p)}_\lambda = \sum_\mu(S^{-1}T^pS)_{\lambda\mu}d_\mu/d_\lambda$
the $p$-fold exceptional-fibre local contribution
(Lawrence--Rozansky 1999 \emph{CMP}~205 Thm.~1.5), $c$ the modular
anomaly (Witten 1989 \S 2.4), and the sum running over the
projective-cover lattice $\Lambda_\infty$ with
$d_\lambda = \dim_{\mathrm{qu}}(P_\lambda)$. The
binary-icosahedral symmetry $I^*$ acts through the
$A_5$-orbits on $\Lambda_\infty$, partitioning the sum into
orbit-sums indexed by the five conjugacy classes
$\{1, [\mathrm{ord}2], [\mathrm{ord}3], [\mathrm{ord}5], [\mathrm{ord}5]^*\}$
of $A_5$.
\end{theorem}

\begin{proof}
The Seifert-fibered closed form is Lawrence--Rozansky 1999 \S 4
Thm.~1.5 (semisimple RT invariant for Seifert fibered spaces with
exceptional fibres $(p_1, \ldots, p_n)$ is
$\tau = \mathcal D^{-1}e^{i\pi c\,\mathrm{sgn}/4}
\sum_\lambda d_\lambda^{2-n}\theta_\lambda^{-E}
\prod_j\phi^{(p_j)}_\lambda$, with $E = p_1\cdots p_n/|H_1|$ the
Euler number); for $\Sigma(2, 3, 5)$: $p_1p_2p_3 = 30$,
$|H_1(\Sigma(2,3,5))| = 1$, so $E = 30/1 = 30$ and the exponent
$d_\lambda^{2 - 3} = d_\lambda^{-1}$ is correct after
dualising the Hopf link normalisation (Lawrence--Rozansky 1999
\emph{ibid.}~eq.~4.15 equivalent form).

The non-semisimple extension to
$\mathrm{Rep}^{\mathrm{fd}}(\mathfrak u_{\zeta_8}
(\widehat{\mathfrak m}_{\Delta_5}))$ is via the modified trace
of Geer--Kujawa--Patureau-Mirand 2011 \emph{Compos.~Math.}~147
Thm.~1, which replaces the divergent semisimple sum
$\sum_\lambda d_\lambda^{-1}\cdots$ (ill-defined at the maximum
projective) by the modified-trace regularisation
$\sum_{\lambda}\mathrm{t}^{\mathrm{mod}}(P_\lambda)$
(De~Renzi et~al.\ 2021 Thm.~6.1). The binary-icosahedral
symmetry $I^*\cong\mathrm{SL}_2(\mathbb F_5)$ acts on
$\Lambda_\infty$ through its cover of $A_5$-action on the small
quantum-$\mathfrak{sl}_2$ at level 6 (Lusztig--Saleur 2017
\emph{J.~Algebra}~492 \S 3), partitioning the sum.

\emph{Gravitational reading.} The Euclidean 3-sphere
$\mathrm{SU}(2)/I^*$ is the maximally-symmetric quotient of the
3-sphere by the largest finite subgroup with $\chi(\partial) = 0$;
the gravitational path integral over this saddle picks up the
icosahedral isometry as a global symmetry of the dual CFT. The
phase $e^{i\pi c/4}$ is the framing anomaly (Witten 1989 \S 2.4)
signalling the orientation-reversing $\mathbb Z/2$ inside
$I^*$; on the $M_{24}$-block this phase is $e^{i\pi\cdot 24/4}
= e^{6\pi i} = 1$ (matching the $M_{24}$-mod-$24$ anomaly
cancellation of Eguchi--Ooguri--Tachikawa 2011), while on the
banded block it contributes a non-trivial eighth root of unity.

\emph{Multi-path verification:}
(i) Lawrence--Rozansky 1999 \S 4 Seifert-fibered RT;
(ii) Geer--Kujawa--Patureau-Mirand 2011 modified-trace;
(iii) De~Renzi et~al.\ 2021 \S 6 non-semisimple Seifert;
(iv) Lusztig--Saleur 2017 \S 3 binary-icosahedral action on
