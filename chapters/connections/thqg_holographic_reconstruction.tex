...
\partial(X\times\R)=X\times\{0\}\,\bigr),
\qquad
...
\begin{theorem}[Universal holography, class-$\mathsf{M}$ chain-level; \ClaimStatusProvedHere]%
...
$X\times\R$.
For every $\cA\in\mathrm{ChirAlg}^{\omega,\mathrm{BL}}_X$ there is a
...
\item\label{item:uh-bulk}
 ...
 Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ as $E_3$-topological
 factorisation algebras on $X\times\R$;
...
the bulk is $Z^{\mathrm{der}}_{\mathrm{ch}}(\cW^k(\fg))$ at chain
level by the DS--Hochschild bridge~\ref{thm:ds-hochschild-bridge}.
The union of the four sub-constructions defines $\Phi_{\mathrm{hol}}$
on all of $\mathrm{ChirAlg}^{\omega,\mathrm{BL}}_X$.
\end{proof}

\begin{corollary}[Class~$\mathsf{M}$ chain-level bulk reconstruction;
\ClaimStatusProvedHere]%
...
the DGGPR invariant $\tau_{\mathcal C}(M^3)$ computes the partition
function of the bulk 3d holomorphic-topological quantum gravity
coupled to the boundary chiral algebra $\mathbf H_{\Delta_5}$. On
canonical 3-manifolds:
\begin{align}
...
Theorem~\ref{V3-thm:modtr-banding-cocycle} is the 't~Hooft anomaly
of the 3d gauge theory on $L(8, 1)$: the boundary
$\mathbf H_{\Delta_5}$-CFT cannot be globally consistent on
$\partial L(8, 1) = \emptyset$ (the lens space is closed) without a
phase of order $8$ tracking the eighth-root ambiguity in the
