...
tree formula at all arities.  In the
...
(class~$\mathsf{G}$), the shadow obstruction tower terminates at arity~$2$,
...
$(p,q)$ & $(0,0)$ & $(1,0)$ & $(0,1)$ & $(1,1)$ & $(2,1)$ & $(1,2)$
  & $(3,0)$ & $(0,3)$ \\
\hline
$h^{p,q}$ & $1$ & $1$ & $1$ & $21$ & $21$ & $21$ & $1$ & $1$
\end{tabular}
\end{center}
...
$\kappa(\cA_E) = 24$.  The discrepancy is an instance of AP48:
...
\end{equation}
% RECTIFICATION-FLAG: The Heisenberg OPE uses the unit level matrix
% (positive definite, one boson per cohomology class at level 1),
% not the indefinite Mukai pairing of signature (4,20).  The Mukai
% pairing enters the lattice VOA structure through the vertex operators
% e^alpha, not through the Heisenberg levels.  This is consistent with
% Vol I: kappa(V_Lambda) = rank(Lambda) = d * kappa(H_1) = d, which
% requires each Heisenberg boson to have level 1.  If the level matrix
% were the Mukai pairing, additivity would give kappa = tr(G) = 4-20 = -16,
% contradicting kappa = 24.  See AP48.
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\kappa(\cA_E) = 24$ \textup{(}$= \operatorname{rank}$; AP48:
...
by additivity (Volume~I, Corollary~\ref{cor:kappa-additivity}).
...
\hline
(a) $\cA$ & Free-field: $24$ bosons at Mukai level $G^{ab}$,
  $c = 24$, $\kappa = 24$ \\
(b) $\cA^!$ & Koszul dual: $24$ bosons at level $-G^{ab}$,
  $\kappa(\cA^!) = -24$; complementarity sum
  $\kappa + \kappa' = 0$ \textup{(}AP24: vanishes for
  free fields\textup{)} \\
(c) $\mathcal{C}$ & $\cA^!\text{-}\mathsf{mod}$;
  the derived center $\Zder^{\mathrm{ch}}(\cA_E)$
  has Hilbert polynomial
  $P_{\cA}(t) = 24 + 576\,t + 24\,t^2$ \\
(d) $r(z)$ & $r(z) = \Omega_{\mathrm{Mukai}}/z$
  \textup{(}single pole; AP19\textup{)};
  CYBE trivially satisfied \textup{(}abelian\textup{)} \\
...
$F_g = 0$ \textup{(}AP31\textup{)}: the elliptic genus is a
\emph{refined} trace with $(-1)^F$ insertion, while
$F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ is the bar complex
...
$r$-matrix & $r(z) = \Omega_{\mathrm{Mukai}}/z$ &
  signature $(4,20)$
\end{tabular}
\end{center}
...
The period domain for K3 complex structures is
$\mathrm{SO}(3,19)/(\mathrm{SO}(3) \times \mathrm{SO}(19))$
with arithmetic group $O(3, 19; \Z)$.
