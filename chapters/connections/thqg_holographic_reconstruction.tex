...
The Vol~II THQG holographic reconstruction specialises at $K3 \times T^2$ to the Vol~III Wave~13 K3 chiral Hall--Drinfeld double $\mathcal{D}_\hbar(\mathcal{Y}^{\mathrm{Hall}}_\hbar(\mathrm{CoHA}_{K3 \times E}))$ on the boundary side, with bulk twisted 11D-SUGRA + $24$ M5-branes on Kodaira $I_1$ fibres. Classification $H^2(\mathfrak{g}_{\Delta_5})^{\mathbb{Z}/2, K(1)} \cong \mathbb{C} \cdot \Delta_5$. Bi-based Ran-space factorisation datum with averaging $\mathrm{av} = \mathrm{Torelli}_{K3} \circ \mathrm{KugaSatake} \circ j_{\mathrm{Kodaira}}$.
\end{remark}
