...
iterated-Sugawara ladder: the first visible rung of the Platonic
...
\begin{description}
 \item[\S6.0 Platonic Introduction.] The present chapter: the
 deficiency of the one-tensor climax, the ladder preview, the
 eight-section architecture, the four Platonic theorems.
...
climax is \S6.1, and the Platonic endpoint sits at \S6.8.
...
BRST-commutativity is Theorem~\ref{thm:casimir-antighost-commutativity}.
When the hypotheses of Theorem~\ref{thm:e-infinity-topologization-ladder}
...
$(\cA,\omega,\mathrm{realisation})$ at non-critical level to a
relative 3d holomorphic-topological gauge theory $\cT_\cA$ on
$X \times \mathbb{R}$. The boundary restriction gives
$\Obs^\partial(\cT_\cA) \simeq \cA$, and the bulk identification gives
$\Obs^{\mathrm{bulk}}(\cT_\cA) \simeq \Zderch(\cA)$ in the algebraic,
cohomological, or weight-completed/pro ambient specified by the
admissible datum. After Sugawara topologization this is
...
\end{theorem}
\begin{proof}
Existence of $\Phi_{\mathrm{hol}}$ on the admissible locus is
...
via Proposition~\ref{prop:uhf-shadow-vs-holographic}; symplectic
...
Vol~I Theorem~\ref{V1-thm:v-natural-e3-topological} \textup{(}file
...
stress tower of depth $N$ at non-critical level, the universal
holography construction produces a relative 3d HT gauge theory
$\cT_\cA$ on $X \times \mathbb{R}$ whose bulk is
$\Zderch(\cA) \in \Etopo{N+1}\text{-topological}$ in the ambient
specified by Theorem~\ref{thm:holographic-recon-shadow}.
\textup{(}b\textup{)} In the universal limit $N \to \infty$ realized by
$\cW_\infty[\mu]$, the bulk is $\Einfty$-topological; every Virasoro
gravity theory in the admissible Virasoro locus appears as the $N=2$
truncation $\cT_{\mathrm{Vir}_c}$
of this universal bulk. The Virasoro sector is the $N=2$ rung.
\end{proposition}
\begin{proof}
...
Nine attack vectors against the Part~VI climax (FMs 125, 126, 127,
...
Part~VI is free of the nine attack vectors.
...
($\Einfty$-topological, Platonic endpoint) and descends to \S6.1
...
conformal tower, $\cW_\infty$); each step adds one transverse real
dimension. The two orders meet at \S6.4--\S6.5, where the Koszul
...
the whole tower. Part~VI carries the tower.
