...
Iteration of Dunn gives the $\Etopo{N+1}$ conclusion by the
...
and the filtered inverse limit of the compatible $\Etopo{N+1}$-algebras
under truncation is
...
$\Obs^{\mathrm{bulk}}(\cT_\cA) \simeq \Zderch(\cA)$ as
holomorphic-topological factorization algebras.  After Sugawara
topologization this is $\Ethree$-topological on the ordinary chain
complex for classes G/L/C and on the weight-completed/pro ambient for
class~M; the strict class-M $\Ethree$ chain lift has the conditional
status stated in the chiral Higher Deligne theorem.  For $\cA$ in the
...
conformal vector at non-critical level, with class-M weight-completed
chain-level completion via the DS--Hochschild bridge
Proposition~\ref{prop:uhf-ds-hochschild-bridge}.  Class G/L/C chain-level:
Costello--Li abelian holomorphic Chern--Simons plus
Theorem~\ref{thm:E3-topological-km}. Class M ambient:
Costello--Gaiotto holomorphic CS with Drinfeld--Sokolov boundary plus
the DS--Hochschild compatibility bridge in the weight-completed
ambient; strict class-M $\Ethree$ chain-level topologization is the
conditional chiral Higher Deligne lift. The $\Etopo{N+1}$ lift for
depth-$N$ stress
