...
\item subregular in $E_{8}$: certain orbits require $d_{f} = 2$
      (Premet 2003 tables).
\end{itemize}
In particular the BP case has $d_{f} = 2$ (not $d_{f} = 1$ as
stated in an earlier draft at
\cite[proof-sketch L840]{e-infinity-topologization-draft}).
\end{example}
...
\label{subsec:fm81-explicit-examples}
...
\label{subsec:fm81-all-good-graded}
...
\label{cor:fm81-healed-non-principal}
\ClaimStatusProvedHere{}
\index{FM81!healed at non-principal|textbf}
The failure mode FM81 \textup{(}fractional-weight ghost bilinears
...
\label{prop:three-lane-fm81}
\ClaimStatusProvedHere{}
\index{FM81!three-lane overdetermination}
...
\label{subsec:fm81-closure}

\begin{remark}[on scope]
\label{rem:fm81-scope-closed}
Under the reconstitution of this section, the earlier conservative
scope restriction ``good-integer-graded nilpotents only''
\textup{(}e.g.,
\cite[FM81 scope restriction L15-19]{test-e3-topological-ds-general-docstring}\textup{)}
is superseded. The strongest honest form of
...
Proposition~\ref{prop:three-lane-fm81} confirms the strongest
form; no downgrade to a technical subclass is warranted.
\end{remark}

\begin{remark}[on the principal-case embedding]
\label{rem:fm81-principal-is-trivial-case}
...
\label{rem:fm81-operadic-circle-stability}
...
\begin{remark}[Wave-14 anchor: non-principal DS as universal-conductor
input]%
\label{rem:wave14-anchor-fm81-non-principal-input}%
\index{Wave 14!non-principal DS Sugawara antighost}%
\index{Wave 14!universal conductor input}%
\index{branched-cover descent!chain-level avatar}%
The branched-cover integralisation + Galois-descent technology
inscribed above closes FM81 by construction; the closure feeds the
...
\item \textbf{(Wave-14 climax position.)} The non-principal DS
...
\end{itemize}
This remark is inscribed solely as the wave-14 anchor; the FM81
heal status above is unchanged.
\end{remark}

% ============================================================================
% End of Platonic reconstitution of FM81 heal.
