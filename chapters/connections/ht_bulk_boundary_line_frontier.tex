...
\begin{theorem}[Bulk/boundary/line triangle; \ClaimStatusProvedHere]
...
\item \textbf{Line algebra from $\cA^!$-modules.}
 The open-colour sector of the $\SCop$-datum recovers the
 line category as the module category:
 \[
 \cC_{\mathrm{line}}
 \;\simeq\;
 \mathrm{Rep}(\gSC_T).
...
$r^L(z)=k/z^2$, $r^{\mathrm{coll}}(z)=k/z$, $m_k=0$ for $k\ge 3$, and
...
At the HT bulk-boundary line frontier, $K3 \times E$ realises the Vol~III the K3 synthesis K3 chiral Hall--Drinfeld double on the boundary side. The Mukai-doubling Koszul conductor $K^{\kappa_{\mathrm{ch}}}(\mathcal{H}_{\mathrm{Muk}}(K3)) = 2\,c_+(\mathrm{Mukai}(K3)) = 8$ has three faces via Bruinier $2002$ Proposition~$5.1$: Mukai doubling, Humbert $H_1$ monodromy order, Lusztig $\ell = 8$ at $\zeta^8 = 1$. Universal identity $\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$.
\end{remark}
