...
$\Conf_k^{<}(\R)$.  The $\SCchop$ structure does not live on
...
is the $\SCchop$ datum (bulk acting on boundary).  For the affine
...
\item \textbf{$E_1$ coalgebra structure on $\barB(\cA)$.}
 ...
 The $\SCchop$ structure does \emph{not} live on $\barB(\cA)$:
 $d_{\barB}$ and $\Delta$ are the $E_1$ \emph{coalgebra} data,
 dual to the $A_\infty$-algebra structure on~$\cA$.

\item \textbf{$\SCchop$ datum from the chiral derived center.}
 The $\SCchop$ structure lives on the pair
...
\item \textbf{Bulk-to-boundary from the mixed face.}
 The mixed sector of the $\SCchop$-datum yields the
...
$\SCchop$ brace data on $(C^\bullet_{\mathrm{ch}}(\cA,\cA),\,\cA)$
...
\end{tabular}
\end{center}
The two colours of the $\SCchop$ datum are
$(C^\bullet_{\mathrm{ch}}(\cA,\cA),\,\cA)$: bulk (closed) and
boundary (open).  The bar complex $\barB(\cA)$ is the $E_1$
coalgebra underlying the boundary; the $\SCchop$ brace operations
