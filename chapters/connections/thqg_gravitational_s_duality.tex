...
$\kappa(\cA)$ is replaced by $\kappaChHodge(\cA^!)$
...
\begin{tabular}{lcccc}
\toprule
\textbf{Algebra} & $\kappa(\cA)$ & $\kappaChHodge(\cA^!)$ &
...
\item \textbf{Feigin--Frenkel duality} (genus~$1$, degree~$2$):
 $\sigma(\kappa(\cA)) = \kappaChHodge(\cA^!)$.
...
Adding and using $\kappa(\cA) + \kappaChHodge(\cA^!) = K(\cA)$ yields the
...
(b)~$K(\cA^!) = \kappaChHodge(\cA^!) + \kappa((\cA^!)^!) = \kappaChHodge(\cA^!) + \kappa(\cA) = K(\cA)$.
