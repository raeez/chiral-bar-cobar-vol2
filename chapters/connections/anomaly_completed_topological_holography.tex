...
\[
\kappa_{\mathrm{eff}}
=\kappa(\mathrm{Vir}_{26})+\kappa(\mathrm{ghost})
=13+(-13)=0,
\]
...
Assume the BP duality conjecture of Volume~I:
\[
...
Since $(\mathcal B^k)^!=\mathcal B^{-k-6}$ and, under the BP
duality conjecture, the central-charge sum is $c+c'=196$, the
...
\begin{remark}[Mukai-doubling Koszul conductor $K^{\kappa_{\mathrm{ch}}}=8$ on the $\mathcal B$-family]
