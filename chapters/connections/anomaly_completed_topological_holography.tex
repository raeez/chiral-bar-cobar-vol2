...
\label{rem:anomtop-DNA-delta5-paramodular-output}
...
\;+\;
(\text{Kodaira-}I_{1}\text{ residue contribution})
\;=\;
2+3
\;=\;
5,
...
\label{rem:anomtop-DNA-phi-trace-k3xt2}
...
Remark~\ref{rem:anomtop-DNA-delta5-paramodular-output}.
...
lift to a BKM generator whose Koszul curvature is read off
directly from the weight of the output Igusa form:
\[
\kappa_{\mathrm{BKM}}(g_{\Delta_{5}})
\;=\;
\tfrac{1}{2}c_{\Lambda}(0)
\;=\;
\mathrm{wt}(\Delta_{5})
\;=\;
5.
...
Remark~\ref{rem:anomtop-DNA-delta5-paramodular-output}, and
...
\label{rem:anomtop-DNA-bibased-ran-24}
...
Remark~\ref{rem:anomtop-DNA-delta5-paramodular-output} the two
bases specialise to
...
of
Remark~\ref{rem:anomtop-DNA-delta5-paramodular-output}. In
...
residue contribution to $\mathrm{wt}(\Delta_{5})=2+3=5$.
\end{remark}
...
\label{rem:anomtop-DNA-class-s-a1-sigma024}
...
Remark~\ref{rem:anomtop-DNA-bibased-ran-24} is the VOA associated
...
datum of Remark~\ref{rem:anomtop-DNA-bibased-ran-24} to a
...
of Remark~\ref{rem:anomtop-DNA-delta5-paramodular-output}, and
...
triangle, and the Kodaira residue $3$ in
$\mathrm{wt}(\Delta_{5})=2+3=5$ is the same $3$ as the
...
\label{rem:anomtop-DNA-mukai-koszul-conductor-8}
...
Remark~\ref{rem:anomtop-DNA-phi-trace-k3xt2}, the three faces
...
\section{Synthesis}
\label{sec:acth-wave13-DNA}

\begin{remark}[Pentagon anomaly at $\hbar^3$ in topological holography]
\label{rem:acth-DNA-1}
The the K3 synthesis anomaly-completed topological holography at the K3 base
...
Vol.~I Chapter~\ref{ch:thqg-open-closed-realization} line~1890.
\end{remark}

\begin{remark}[$\widetilde M_{24}$ anomaly as order-$6$ Schur cocycle]
\label{rem:acth-DNA-2}
...
\label{sec:acth-wave15-DNA}

\begin{remark}[Retraction of the K3 synthesis $c_{2d} = -312$ identification;
restoration of $c_{2d} = -214$]
\label{rem:anomtop-wave16-retract-312}
\index{retraction!$c_{2d} = -312 \to c_{2d} = -214$}
The the K3 synthesis read of
$c_{4d}(A_1, \Sigma_{0,24}, \mathrm{all\ max}) = 26$ and
$c_{2d} = -312$ is withdrawn. The Shapere--Tachikawa 2008 class-$\mathcal S$
normalisation
$c_{4d} = (2n_v + n_h)/12$ applied to the Chacaltana--Distler
class-$\mathcal S$ count $(n_v, n_h) = (63, 88)$ for
$(A_1, \Sigma_{0,24})$ returns $c_{4d} = 214/12 = 107/6$ (not $26$),
and Beem--Rastelli $c_{2d} = -12 c_{4d}$ returns $c_{2d} = -214$
(not $-312$). The precise first-principles derivation is inscribed
in Vol~I Chapter~\ref{ch:thqg-open-closed-realization},
Proposition around line~1890; see also the K3 synthesis retraction
remark at line~1932 therein and the K3-bialgebra synthesis
platonic line~1095 in Vol~III. Below, the $6d$ anomaly polynomial,
the shadow-tower spectrum, the mock-modular weight, and the BPS
counting are retracted to the correct central charge $c_{2d} = -214$.
\end{remark}

the K3 synthesis derives $c_{2d} = -214$ from the $6d$ $(2,0)$ $A_1$ anomaly
...
\ClaimStatusProvedHere]
\label{rem:anomtop-wave15-anomaly-polynomial}
...
\ClaimStatusProvedHere]
\label{rem:anomtop-wave15-312-spectrum}
...
Vol~I Proposition~\ref{prop:stqp-wave15-312-factor} specialises to
\begin{align*}
...
Chapter~\ref{ch:thqg-open-closed-realization}, around line~1909),
...
\label{rem:anomtop-wave15-T2-modular}
...
with $\Delta_5 = \Phi_{10}^{1/2}$ through the Borcherds denominator
identity
$\prod_{r\cdot\alpha>0}(1-q^{r\alpha})^{c(r\alpha)} = \Phi_{10}$
...
$\Delta_5 = \sqrt{\Phi_{10}}$; it is independent of the elliptic
coordinate $c_{2d}$ and therefore unaffected by the $-312 \to -214$
retraction. The bridge from Borcherds--Siegel weight to
...
the $M_{24}$-averaged/$\Phi_{10}$-denominator locus; the
...
Remark~\ref{rem:stqp-wave15-mock-weight}. The non-integrality of
...
\label{rem:anomtop-wave15-ceff-bps}
...
the polar part of $1/\Phi_{10}(Z)$ at the maximal-rank Siegel cusp
...
Remark~\ref{rem:anomtop-wave15-T2-modular}: the BPS counting
