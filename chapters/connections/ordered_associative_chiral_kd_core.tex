...
% Locks AP159 four-Yangian-types disambiguation, AP161 five-notion
% convention, AP170 two-Yangian-definition equivalence, AP-RMATRIX
% level-prefix discipline, and Wave-14 cross-references to Vol I unified
% Theorem A (thm:koszul-reflection), chap:climax-platonic, and
% chap:universal-conductor. Closes the ordered Koszul-duality core for
% line operators / chiral Yangians. All labels new; existing material
% unmodified.
% ======================================================================

\section{Adversarial discipline block (Wave-14 anchor)}
\label{sec:ordered-kd-core-wave14-anchor}

\subsection{Five $E_1$-chiral notions, ordered KD reading}
\label{subsec:ordered-kd-core-ap161-five-notions}

\begin{convention}[AP161: ordered KD core uses Notion~B + Notion~E
hybrid; \ClaimStatusProvedElsewhere]
\label{conv:ordered-kd-core-ap161-five-notions}
The ordered Koszul-duality theory of this chapter — the bar coalgebra
$\Barchord(\cA)$, the diagonal bicomodule $C_\Delta$, and the
duality involution
(Theorem~\ref{thm:duality-involution}) — uses two of the five
$E_1$-chiral notions of Vol~I AP161:
\begin{enumerate}[label=\textup{(\Alph*)}]
\setcounter{enumi}{1}
...
\label{subsec:ordered-kd-core-ap159-yangian-types}

\begin{convention}[AP159: ordered chiral Koszul duality is the
universal source of the chiral Yangian
$Y(\fg)^{\mathrm{ch}}$; \ClaimStatusProvedElsewhere]
\label{conv:ordered-kd-core-ap159-yangian-types}
Vol~I AP159 distinguishes four Yangian types: classical
...
\begin{remark}[AP170: two definitions of chiral Yangian; equivalence
OPEN at chain level]
\label{rem:ordered-kd-core-ap170-two-definitions}
Vol~I AP170 records two definitions of chiral Yangian. The weaker
one (def:e1-chiral-yangian) requires only an $E_1$-factorization
...
(Theorem~\ref{thm:duality-involution}) is associator-dependent
in the sense of Vol~I AP171: the bar-side $\kappa$ and shadow
tower are associator-FREE; the ordered cobar coproduct depends on
the chosen Drinfeld associator $\Phi$.
\end{remark}

\subsection{AP-RMATRIX guard for ordered Yangians}
...
(the Sugawara shift, FM11). This is the lossy
...
\label{rem:ordered-kd-core-wave14-uhm-bridge}
...
\label{rem:ordered-kd-core-wave14-conductor-bridge}
The ordered KD core sees the universal ghost-charge conductor
$K = c + c^!$ of Vol~I \cref{V1-chap:universal-conductor}
through the duality involution: $\kappa(\cA) + \kappa(\cA^!)$
is the family-specific complementarity invariant
($K=0$ for KM/free, $K=13$ for Vir, $K=98/3$ for BP, etc., as
catalogued in Vol~I landscape census), and the cohomological
reflection $\cA \mapsto \cA^!$ implemented by $\Cobar\circ
\Barchord$ realises the conductor stratification on the
ordered side. The Vol~I climax binds $\kappa$ (degree-$2$ shadow)
