...
are established in the repository.
...
The repository establishes one-sided module/comodule duality.
...
linked by the universal twisting morphism
$\tau \in \cA^! \otimes \cA$ and by the Verdier intertwining
...
\[
 ...
 \in
 H^3\!\left(
   \Def_{\mathrm{Hopf}}\bigl(\cA,\cA^!,\tau,
...
used above. The bordered FM compactification gives the collision
strata, boundary composition maps, operadic gluing, and compatibility
with the ordered bar coproduct. The annular closure identifies the
cyclic boundary trace with coHochschild homology by
Theorem~\ref{thm:bordered-annular}. These five properties are exactly
the geometric hypotheses used in Theorems~\ref{thm:opposite},
...
configuration spaces: the bar differential is an integral over
$\mathrm{Conf}_n^{\mathrm{ord}}(\C)$, its monodromy around the braid
generator is the $R$-matrix, the Yang--Baxter equation is $d^2=0$,
...
requires the Jacobi identity; on the ordered bar complex,
...
The ordered bar complex uses only consecutive collisions (the
associahedron boundary), and $d^2=0$ follows from associativity.
The unordered bar complex uses every pair collision (the
permutohedron boundary), and $d^2=0$ requires both the Jacobi
identity and the Arnold relation. For $E_\infty$-chiral algebras
both are available; for $E_1$-chiral algebras only associativity
is available, and only the ordered complex is well-defined.
\end{remark}
...
dimensional level. The structural claim, that the double bar of
the Yangian recovers the current algebra, is verified at the
first nontrivial rank.
...
ordered bar complex computes directly. This is the double bar:
two successive applications of open-colour Koszul duality. By
Proposition~\ref{prop:open-colour-double-bar}, the double bar
$\Barchord(Y_\hbar(\fg))$ recovers the current algebra
$U(\fg[t])$ without central extension.
...
\]
The open-colour Koszul dual of the Yangian itself: by
Proposition~\ref{prop:open-colour-double-bar}, the double bar
$\Barchord(Y_\hbar(\mathfrak{sl}_2))
\simeq U(\mathfrak{sl}_2[t])$.
The Koszul dual of the Yangian is
...
\end{equation}
The dual is distinct from $Y_\hbar(\mathfrak{sl}_2)$. The Yangian
is not self-dual under open-colour Koszul duality. The duality
chain:
\[
...
\;\cdots
\]
The three-step chain does not close. Open-colour Koszul duality is
an involution on the $E_\infty$ side (for Kac--Moody:
$\widehat{\fg}_k \mapsto \widehat{\fg}_{-k-2h^\vee} \mapsto
\widehat{\fg}_k$); on the $E_1$ side it is not an involution. The
reason:
\begin{itemize}[nosep]
...
\item The deformation parameter $\hbar$ is invisible
 ...
 residue at $u = v$.
\end{itemize}
The Yangian is not self-dual; self-duality lives at the level of
...
colour alone does not close.

At $\hbar = 0$ (the classical limit), a weaker self-duality
holds: $Y_0(\mathfrak{sl}_2) = U(\mathfrak{sl}_2[t])$, and
\[
...
U(\mathfrak{sl}_2[t])
\]
by the self-duality of the PBW filtration on the universal
enveloping algebra. The classical current algebra $U(\fg[t])$ is
self-dual under open-colour Koszul duality: it is a quadratic
algebra generated by $\fg[t]$ with the Lie bracket as the only
relation, and quadratic Koszul duality is an involution.
...
Cross-ref Vol.~I, \S\ref{V1-sec:e1mkd-supplement}.
\end{remark}
