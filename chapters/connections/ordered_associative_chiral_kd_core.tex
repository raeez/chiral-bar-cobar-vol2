...
C_\Delta,
\]
namely the coalgebra $C$ equipped with its coproduct on the left and on the right.
...
refinement.  In \S\ref{sec:master} we package the main results as a single master theorem and explain its Chriss--Ginzburg architecture.  In \S\ref{sec:e1-five-theorems-genus0} we state the $E_1$ counterparts of the five main theorems of Vol~I.  The large Appendix~\S\ref{sec:ordered-bar-explicit} supplies the explicit constructions: ordered configuration spaces, the Heisenberg and $\widehat{\mathfrak{sl}}_2$ bar complexes, the $R$-matrix as descent datum, the Yang--Baxter equation from $d^2=0$, the dg-shifted Yangian, the Euler-eta identity, bar cohomology concentration, the double bar for $\mathfrak{sl}_3$, the Yangian viewed as $E_1$-input, and the annular bar complex with genus-one curvature.  Finally, in \S\ref{sec:conjectures} (in the frontier chapter) we formulate the remaining open questions.
...
(-1)^{\sum_{i<j}(|a_i|+1)(|a_j|+1)}.
\]
The shift by~$1$ in each exponent reflects the desuspension: a bar word $[s^{-1}a_1|\cdots|s^{-1}a_n]$ has internal degree $|s^{-1}a_i| = |a_i| + 1$ (in the cohomological convention $|s^{-1}| = +1$).  Reversing the word permutes the $n$ suspended elements past each other, and the bilinear exponent $(|a_i|+1)(|a_j|+1)$ is the standard Koszul sign for transposing two elements of degrees $|a_i|+1$ and $|a_j|+1$.
...
The generating functions describe the ordered bar complex at the level of dimensions.  We now return to the structural content and verify the central claim---that the double bar of the Yangian recovers the current algebra---at the first nontrivial rank.
