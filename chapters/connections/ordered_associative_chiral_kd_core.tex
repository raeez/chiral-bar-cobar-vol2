...
In particular, $\kappa(\cA) = \mathrm{av}(r(z))$ at arity~$2$.
...
The \emph{Yangian} $Y_z^\hbar(\mathfrak{sl}_2)$, by contrast,
...
At arity~$2$: $e_1 = \tfrac{1}{2}(\id - \sigma)$
...
The modular characteristic $\kappa(\cA)$ lives at arity~$2$
...
At arity~$2$, $\kappa$ for the Virasoro algebra is entirely
...
$\cA^!_{\mathrm{line}}=Y_z^\hbar(\mathfrak g)$ at deformation parameter
...
\[
 \Barchord\bigl(Y_z^\hbar(\fg)\bigr)
 \;\xrightarrow{\;\sim\;}
 U(\fg[t]).
\]
...
$Y_z^\hbar(\mathfrak{sl}_3)$, hence to the current modes
...
\item $Y_z^\hbar(\mathfrak{sl}_N)$ has only simple poles at
...
\end{enumerate}
Together these give
$\Barchord(Y_z^\hbar(\mathfrak{sl}_N))
...
The Yangian $Y_z^\hbar(\mathfrak{sl}_2)$ is the open-colour
...
treating $Y_z^\hbar(\mathfrak{sl}_2)$ as the \emph{input}
chiral algebra (not the output of duality).

\medskip
\noindent\emph{(1) The ``OPE'' of the Yangian: spectral
commutation relations.}
The Yangian $Y_z^\hbar(\mathfrak{sl}_2)$ in the Drinfeld
...
$\cA^!_{\mathrm{line}}(Y_z^\hbar(\mathfrak{sl}_2))$.}
This is the central structural question. The Yangian
$Y_z^\hbar(\mathfrak{sl}_2)$ is the open-colour Koszul
...
double bar
$\Barchord(Y_z^\hbar(\mathfrak{sl}_2))
\simeq U(\mathfrak{sl}_2[t])$.
The Koszul dual of the Yangian is therefore:
\begin{equation}\label{eq:yangian-koszul-dual}
\cA^!_{\mathrm{line}}\bigl(Y_z^\hbar(\mathfrak{sl}_2)\bigr)
\;=\;
H^*\!\bigl(\Barchord(Y_z^\hbar(\mathfrak{sl}_2))
\bigr)^\vee
\;=\;
U(\mathfrak{sl}_2[t])^\vee.
\end{equation}
This is \emph{not} $Y_z^\hbar(\mathfrak{sl}_2)$ itself.
...
\text{affine, }E_\infty}
\;\xrightarrow{\;\text{KD}\;}
\underbrace{Y_z^\hbar(\mathfrak{sl}_2)}_{
\text{Yangian, }E_1}
\;\xrightarrow{\;\text{KD}\;}
...
\item The Yangian $Y_z^\hbar(\fg)$ has only simple poles
...
on $\Barchord_2(Y_z^\hbar)$ reproduces exactly
...
$\Barchord_3(Y_z^\hbar)$ has elements
...
\begin{array}{c|l|l}
\text{Degree}
 & \Barchord_n(Y_z^\hbar(\mathfrak{sl}_2))
 & \text{Identification} \\
\hline
...
(V2-AP2); the discriminant is provenance, not value.
