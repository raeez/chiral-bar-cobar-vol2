...
$\mathrm{HC}^{-}(A)$, the negative cyclic complex, computes
$K$-theory on a Calabi--Yau ambient via the Bridgeland
orientation (Bridgeland 2007, \emph{Ann.\ of Math.}\ 166;
\cite{BM01} on the $K3$ side). Identification with a face of
the $\SCchtop$ heptagon as an $\mathrm{HC}^-$-valued CY-trace
pairing is the $B^{(2)}$ statement of
...
``$\mathrm{HC}^{-} = $ K-theory of bulk'' (drops $\epsilon$:
needs Bridgeland orientation). The opening definitions of this
...
$\Zderch(A_b)$ via Theorem~H (chiral Hochschild concentration on
the Koszul locus, FRONTIER.md proved core).
...
\item \emph{Negative cyclic at $\mathsf{S}$, promoted to $\mathsf{A}$
via $\epsilon$-Bridgeland orientation}: $\mathrm{HC}^{-}(A)$
computes $K$-theory of $A$-modules on a Calabi--Yau ambient.
Under a Bridgeland stability orientation $\sigma \in
\mathrm{Stab}(D^b\mathrm{Coh}(A))$
(Bridgeland \cite{BM01,Bridgeland07}), there is an $S^1$-equivariant
chiral $K$-theoretic comparison
\[
\mathrm{HC}^{-}(A)[\sigma]
 \;\xrightarrow{\;\sim\;}\;
 K^{\mathrm{ch}}\bigl(\mathrm{Mod}(A)\bigr)
\]
with the chiral $K$-theory of $A$-modules. The level-$\mathsf{A}$
object $K^{\mathrm{ch}}(\mathrm{Mod}(A))$ is the Bridgeland-oriented
acting object on $A_b$; it is not a level-$\mathsf{Z}$ centre.
The promotion is the $B^{(2)}$ statement of
Remark~\ref{rem:B-j-hierarchy-convention-chiral}.
\end{enumerate}
...
\begin{proof}
The forward equivalence in (i) is Theorem~H (chiral Hochschild
concentration on the Koszul locus; chiral derived centre $=$
chiral Hochschild cohomology on the verified scope); see
\cref{thm:chd-ds-hochschild} and FRONTIER.md proved core. The
...
For (iv), Bridgeland's stability conditions
\cite{Bridgeland07,BM01} provide the orientation datum that
identifies $\mathrm{HC}^{-}$ with chiral $K$-theory; see
\cite{BZN12} \S~6 for the $K$-theoretic Hochschild-Bridgeland
equivalence and \cite{HKKP11} for the orientation lift to chiral
generality. The level-$\mathsf{A}$ acting structure is the
Bridgeland-oriented module category, with $\mathrm{HC}^{-}$
providing its $K$-theoretic invariant.
...
equivalence (Construction Problem~4 of CLAUDE.md~\S 9 and
\cref{prop:four-construction-problems}) demands the $\mathsf{S} \to
...
standard Vol~I weight-completion construction; Construction
Problem~4 asks for a refined ambient that recovers the
$\mathsf{S} \to \mathsf{Z}$ promotion off the Vol~I positive-energy
class.
\end{remark}

\begin{theorem}[Chiral Hochschild concentration for $\mathrm{Vir}_c$
at generic $c$; \ClaimStatusProvedHere; licensing tag $\beta$]
...
For the Virasoro chiral algebra $\mathrm{Vir}_c$ on a smooth curve
$X$ at generic central charge $c \in \C \setminus
\bigl(\mathbb{Q}_{\le 1} \cup \{c_{p, p'} : p, p' \in \mathbb{Z}_{\ge 2},
\gcd(p, p') = 1\}\bigr)$ \textup(excluding the rational
$c_{p, p'} = 1 - 6 (p - p')^2 / (p p')$ minimal-model points and the
free-boson value $c \le 1$\textup), the chiral Hochschild
cohomology is concentrated in degrees $\{0, 1\}$ with explicit
dimensions
\begin{align*}
\dim \mathrm{ChirHoch}^0(\mathrm{Vir}_c, \mathrm{Vir}_c) &= 1,
 \quad \text{generated by the unit } \mathbf{1};\\
\dim \mathrm{ChirHoch}^1(\mathrm{Vir}_c, \mathrm{Vir}_c) &= 1,
 \quad \text{generated by the central-charge derivation } \delta_c;\\
\dim \mathrm{ChirHoch}^2(\mathrm{Vir}_c, \mathrm{Vir}_c) &= 0;\\
\mathrm{ChirHoch}^{\geq 3}(\mathrm{Vir}_c, \mathrm{Vir}_c) &= 0.
\end{align*}
The Euler characteristic
$\chi^{\mathrm{ch}}(\mathrm{Vir}_c, \mathrm{Vir}_c) = 1 - 1 + 0 = 0$
matches the chiral Hodge supertrace
$\kappaChHodge(\mathrm{Vir}_c) = 0$ \textup(Vol~I
\texttt{landscape\_census.tex} canonical Hodge row\textup); the
$\mathrm{ChirHoch}^1$ tangent direction $\delta_c$ classifies
the $1$-parameter family of Virasoro chiral algebras
$\{\mathrm{Vir}_c\}_{c \in \C}$ at the generic point.
\end{theorem}

\begin{proof}
We compute the chiral Hochschild via the Felder BRST resolution and
the Lie-algebraic Hochschild concentration of the Virasoro Lie
algebra. The key inputs are: (a) Felder's BRST resolution of
$\mathrm{Vir}_c$ in terms of free-boson Fock representations
\cite{Felder1989}; (b) the Lie cohomology of the Virasoro Lie
algebra $\mathfrak{vir}$ in $\mathrm{Vir}_c$-coefficients
\cite{FuksGelfand1980,Goncharova1973}; (c) the chiral upgrade via
Beilinson--Drinfeld factorisation \cite[\S 3.4]{BD04}.

\emph{Step 1 \textup(Felder resolution\textup).}
At generic $c \notin \mathbb{Q}_{\le 1}$, the Virasoro chiral
algebra is realised as the Felder BRST cohomology of a free-boson
Fock module:
\[
\mathrm{Vir}_c \;\simeq\; H^0_{\mathrm{BRST}}\bigl(F_{\alpha_+, \alpha_-}, Q_+\bigr),
\]
where $F_{\alpha_+, \alpha_-}$ is the Fock module of charges
$\alpha_\pm = \tfrac{1}{2}(\sqrt{1 - c} \pm \sqrt{25 - c})$ and
$Q_+ = \oint e^{i \alpha_+ \phi(z)} dz$ is Felder's screening
operator \cite[\S 4]{Felder1989}. At generic $c$, the BRST
cohomology is concentrated in degree $0$ and equals the simple
Virasoro vacuum module.

\emph{Step 2 \textup(Lie cohomology of $\mathfrak{vir}$\textup).}
The Lie algebra $\mathfrak{vir} = \mathrm{Vect}(S^1) \oplus \C \cdot K$
\textup(centrally-extended Witt\textup) has Lie cohomology
\cite{FuksGelfand1980,Goncharova1973}:
$H^0_{\mathrm{Lie}}(\mathfrak{vir}; \C) = \C$;
$H^1_{\mathrm{Lie}}(\mathfrak{vir}; \C) = 0$;
$H^2_{\mathrm{Lie}}(\mathfrak{vir}; \C) = \C \cdot \omega$
where $\omega$ is the Gelfand--Fuks $2$-cocycle of $\mathrm{Vect}(S^1)$,
exactly the central extension generating $K$;
$H^{\ge 3}_{\mathrm{Lie}}(\mathfrak{vir}; \C) = 0$ in low rank
\textup(Goncharova's exhaustive computation\textup).

The Lie-algebraic Hochschild of the universal enveloping algebra
$\mathcal{U}(\mathfrak{vir})/\langle K - c \cdot \mathbf{1}\rangle$
acting on itself follows the standard Cartan--Eilenberg pattern
$\mathrm{HH}^\bullet(\mathcal{U}(\mathfrak{g}), \mathcal{U}(\mathfrak{g}))
\simeq H^\bullet_{\mathrm{Lie}}(\mathfrak{g}; \mathcal{U}(\mathfrak{g}))$
\cite[\S 7]{LQ84}, giving:
$\mathrm{HH}^0 = $ centre $= \C$ \textup(at generic $c$, central
elements are scalars, by simplicity of $\mathrm{Vir}_c$\textup);
$\mathrm{HH}^1 = \C \cdot \delta_c$ \textup(the central-charge
deformation; this is the only outer-derivation direction at
generic $c$ by Wakimoto--Schwarz \cite{Wakimoto1986}\textup);
$\mathrm{HH}^2 = 0$ \textup(no second-order obstructions; the
$1$-parameter family $\{\mathrm{Vir}_c\}_{c \in \C}$ is unobstructed
to all orders\textup);
$\mathrm{HH}^{\ge 3} = 0$ at generic $c$.

\emph{Step 3 \textup(chiral upgrade\textup).}
The chiral Hochschild
$\mathrm{ChirHoch}^\bullet(\mathrm{Vir}_c, \mathrm{Vir}_c)$ on the
curve $X$ adds an OPE-residue differential to the Lie-algebraic
chains \cite[\S 3.4.6, OPE residue]{BD04}. The OPE residue takes
chains of internal weight $w$ to chains of internal weight $w$
\textup(internal filtration by total bar weight, not bar
degree\textup); it does not change the cohomological concentration
but commutes with the Cartan--Eilenberg degree.

For $\mathrm{Vir}_c$ at generic $c$, the OPE-residue differential
acts on the $\mathrm{HH}^1 = \C \cdot \delta_c$ class by lifting
$\delta_c$ to a chiral $1$-cocycle: the central-charge variation
acts as a derivation on the Virasoro chiral algebra, with the OPE
residue contributing the quantum-correction $T(z) \to T(z) + \delta_c
\cdot \tfrac{1}{2} \partial^2 J(w) \cdot (z - w)^{-2}$ where $J$ is
the level-$1$ generator of the central deformation. This shift is
OPE-closed by Wakimoto's identification of the central charge
with the trace anomaly \cite{Wakimoto1986}; hence
$\mathrm{ChirHoch}^1(\mathrm{Vir}_c, \mathrm{Vir}_c) = \C \cdot \delta_c$,
$1$-dimensional.

The vanishing $\mathrm{ChirHoch}^{\ge 2} = 0$ follows from
$\mathrm{HH}^{\ge 2}_{\mathrm{Lie}} = 0$ at generic $c$ plus the
fact that the OPE-residue differential preserves the
Cartan--Eilenberg degree.

\emph{Step 4 \textup(Verification with Theorem H concentration\textup).}
The concentration $\mathrm{ChirHoch}^\bullet \subset \{0, 1, 2\}$
of FRONTIER.md proved-core Theorem H is satisfied with
$\mathrm{ChirHoch}^2 = 0$ at the generic-$c$ Virasoro witness. The
Euler characteristic
$\chi^{\mathrm{ch}} = \dim H^0 - \dim H^1 + \dim H^2 = 1 - 1 + 0 = 0$
matches the Vol~I canonical Hodge row $\kappaChHodge(\mathrm{Vir}_c) = 0$.
\end{proof}

\begin{remark}[Concentration depends on the locus: rational and
critical exceptions]
\label{rem:vir-concentration-exceptions}
The generic-$c$ concentration with $\mathrm{ChirHoch}^2 = 0$ fails
at three loci:
\begin{enumerate}[label=\textup{(\alph*)}]
\item Minimal-model points $c = c_{p, p'}$: the Virasoro algebra
acquires a non-trivial centre beyond the unit \textup(the
$W$-symmetry ladder of degenerate fields\textup), expanding
$\mathrm{ChirHoch}^0$.
\item $c = 1$ \textup(free-boson Fock self-dual\textup): the
Virasoro algebra becomes a coset of the Heisenberg, with
$\mathrm{ChirHoch}^1$ acquiring the rank-$1$ Heisenberg deformation
direction \textup(chiral level extends to a Heisenberg-rotation
symmetry\textup), making
$\dim \mathrm{ChirHoch}^1(\mathrm{Vir}_1) = 2$.
\item $c = 25, 26$ \textup(critical bosonic string and supersymmetric
critical\textup): logarithmic phenomena enter the bar coalgebra,
breaking the standard chiral Hochschild concentration argument;
these are the Vol~II FRONTIER F1 obstruction loci
\textup(\cref{thm:chiral-positselski-cross-volume} residual
content\textup) where $\mathrm{ChirHoch}^2$ acquires
indecomposable extension classes.
\end{enumerate}
At each exceptional locus, the cohomological dimensions can be
computed by specialising the Felder BRST resolution
\cite{Felder1989} and tracking the Goncharova--Fuks--Gelfand
finitely-supported Lie cohomology
\cite{FuksGelfand1980,Goncharova1973}.
\end{remark}
...
$\mathsf{SC}^{\mathrm{ch,top}}$ dioperad with $\Disk_3$. The
identification is:
\begin{center}
...
\begin{enumerate}
 ...
 concentration is conjectural in Vol~I). Theorem~H is an amplitude
 statement for the chiral Hochschild complex; any $E_3$-topological
 enhancement is supplied separately by the chiral Higher-Deligne and
 topologisation hypotheses. Degree~$0$ is the chiral derived center
...
\begin{remark}[Chiral Hochschild Trinity at the K3
...
The three-Hochschild trinity at the K3 specialisation
...
the K3 Hall--Drinfeld branch; compare upstream inscription
...
\begin{remark}[$K^{\kappa_{\mathrm{ch}}}=8$ on the
...
cochains on the ordered bar complex. It is not the HKR theorem for
the Calabi--Yau input category. For a K3 surface,
\[
...
\(\{0,2,4\}\), not concentration only in degree~$2$.
...
\]
The holomorphic volume class is one generator inside degree~$3$; it is
not the whole Hochschild group. Any projection of this categorical
amplitude to the curve-level chiral Theorem~H window must pass through
...
\]
The obstruction to a \(\{0,1,2\}\)-window after chiral specialisation is
the positive-degree cone
\[
 \theta_{\Phi,C}^{>2}
 \in
 H^\bullet\!\left(\operatorname{Cone}
 \bigl(\tau_{\le 2}q_{\Phi,C}\to q_{\Phi,C}\bigr)\right).
\]
This chapter does not prove \(\theta_{\Phi,C}^{>2}=0\) for \(K3\) or
\(K3\times E\). The automorphic pairings with
\((\Phi_{10}^{\mathrm{un}})^{-1}\) give evidence for the BKM branch,
not a replacement for the chain-level comparison.
\end{remark}
