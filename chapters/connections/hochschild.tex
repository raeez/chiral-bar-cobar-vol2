...
Theorem~\ref{rem:affine-half-space-bv}.  For Virasoro and $W_N$, this
...
\textup{(}augmentation ideal, \textup{AP132}\textup{)} and its Verdier
...
\;=\; \ChirHoch^\bullet(\cA).
\end{equation}
The bar used throughout is the ordered bar $B^{\mathrm{ord}}$
\textup{(}\textup{AP102}\textup{)}; the equivalence is between the
...
functors of Anti-Pattern~\textup{AP34}.  The left-hand side is the
...
that class $\mathbf{M}$ breaks the ``classical reduction at a point''
...
coproduct on $B^{\mathrm{ord}}(\cA)$ (AP83: deconcatenation, not
coshuffle), and this transform does not commute with stalk restriction
...
not identify the derived centre (an output of the Hochschild functor,
AP34) with any component of the bar--cobar assembly.  Step~(3) must
...
(AP98, AP134: this is a \emph{cohomological amplitude} bound, not a
virtual dimension).
...
Koszul dual is $\cH_k^! = \cH_{-k}$ (AP29: same $\kappa$, different
algebra), and the chiral double is $U_{\cH_k} = \cH_k \otimes \cH_{-k}$
with classical $r$-matrix $r(z) = k\,\Omega/z$ (AP126: the level $k$
survives the $d\log$ absorption; at $k = 0$ the $r$-matrix vanishes and
the double collapses).  The centre is computed by direct inspection:
...
with $k$ for the Heisenberg family, AP39: $\kappa^{\mathrm{Heis}} = k$,
...
degrees $\{0,1,2\}$ with total dimension at most four.  These two statements
are incompatible as written: the power series $\Bbbk[[\kappa]]$ has
total dimension $\aleph_0$, not $\le 4$.
...
by Tamarkin (ordered $B^{\mathrm{ord}}$ versus factorization $B^{\mathrm{fact}}$,
AP3), or (c) one of the two computations is incorrect.  Resolving this
...
identically (AP126 and AP141) and the chiral double collapses: the
