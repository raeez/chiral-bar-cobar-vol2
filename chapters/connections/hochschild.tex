...
see Volume~I, Theorem~\ref*{thm:operadic-center-hochschild}).
...
(Theorem~\ref{thm:annular-HH} and its proof in
Volume~I).
...
\begin{proof}
The punctured disk $D^*$ deformation-retracts onto
$S^1$ via $(r,\theta)\mapsto(1,\theta)$, so
$\FM_k(D^*)\simeq\Conf_k(S^1)$ for each~$k$.
The map $\varphi$ is defined on generators by
integrating the radial component of the logarithmic
forms and restricting the angular component to
$S^1$-configurations.
...
of the colliding points (the local holomorphic coordinate
...
the $R$-matrix monodromy $\mathrm{Mon}(R)$ when transporting
a tensor factor around the angular coordinate.
For an $E_\infty$-chiral algebra, $R(z)$ is derived from
the local OPE and its monodromy depends only on the
homotopy class of the loop.
For a genuinely $E_1$-chiral algebra, $R(z)$ is an
independent datum, but the structural axioms
(Definition~\ref{def:e1-chiral-algebra},
Proposition~\ref{prop:field-theory-r}) require it to be
meromorphic on~$\C^\times$ with poles only at~$z=0$.
On~$D^*$, the circles $\{|z|=r\}$ for $0<r<1$ enclose
only this pole at the origin.  By Cauchy's theorem,
deforming the integration contour from $\{|z|=r_1\}$ to
$\{|z|=r_2\}$ crosses no poles, so the monodromy
$\mathrm{Mon}(R)=\lim_{z\to e^{2\pi i}z}R(z)$ is
independent of the radius~$r$.  The retraction
$D^*\to S^1$ preserves winding numbers, so $\varphi$
commutes with $d_{\mathrm{cyc}}$ for both $E_\infty$ and
$E_1$ algebras.

\emph{Acyclicity of $\ker\varphi$.}
The kernel $\ker\varphi$ is spanned by forms that
have non-trivial radial dependence---in particular,
the logarithmic forms with holomorphic poles at
collision divisors that are not pulled back from
$S^1$.  Consider the radial
deformation retraction $\rho_t\colon D^*\to D^*$,
$z\mapsto(1-t)z+tz/|z|$ for $t\in[0,1]$.
This induces, for each~$k$, a homotopy
$\rho_{t*}\colon\FM_k(D^*)\to\FM_k(D^*)$
terminating at the inclusion
$\Conf_k(S^1)\hookrightarrow\FM_k(D^*)$.
The retraction $\rho_t$ preserves the
collision divisors $D_S$ (since $\rho_t$ does
not change the relative positions of points
within a cluster) and maps logarithmic forms
to logarithmic forms.

We verify that $\rho_t$ is compatible with
the full differential $d^{D^*}$ along the contraction,
which implies that $h=\int_0^1\rho_t^*\,dt$ is a
well-defined contracting homotopy on $\ker\varphi$.
The three components are:
\begin{itemize}
\item $d_{\mathrm{int}}$: acts on $\cA$-factors and
  commutes with the geometric retraction by functoriality.
\item $d_{\mathrm{res}}$: depends on relative local
  coordinates $z_i-z_j$ near collision divisors.  Since
  $\rho_t$ acts as a common dilation-contraction on
  nearby points---for $z_j=z_i+w$ with $w$ small,
  $\rho_t(z_i+w)-\rho_t(z_i)=w\cdot[(1-t)+t\cdot
  \partial_z(z/|z|)|_{z_i}]+O(w^2)$, a smooth
  rescaling of the relative coordinate preserving
  $D_S=\{w=0\}$---the residue extraction
  $\operatorname{Res}_{D_S}$ commutes with $\rho_t^*$.
\item $d_{\mathrm{cyc}}$: the $R$-matrix monodromy
  $\mathrm{Mon}(R)$ is independent of the radial
  position by the Cauchy argument above (meromorphicity
  of $R(z)$ on $\C^\times$ with poles only at $z=0$).
  The retraction $\rho_t$ preserves angular coordinates
  and hence preserves the monodromy insertion, so
  $\rho_t^*$ commutes with $d_{\mathrm{cyc}}$.
\end{itemize}
Since $\rho_t$ commutes with $d^{D^*}$, the operator
$h=\int_0^1\rho_t^*\,dt$ restricts to
a well-defined contracting homotopy
$h\colon\ker\varphi_k\to\ker\varphi_{k+1}$
satisfying $dh+hd=\mathrm{id}$ on $\ker\varphi$,
which gives the acyclicity.

The argument applies uniformly to $E_\infty$ and $E_1$
algebras: the $E_\infty$ case uses local constancy of
the coefficient system along radial fibers (a stronger
property), while the $E_1$ case uses only the
meromorphicity of $R(z)$ on $\C^\times$ (which ensures
radial-independence of the monodromy and compatibility of
the residue extraction with radial deformation).
\end{proof}
...
\textup{(}Volume~I,
Theorem~\textup{\ref{thm:annular-HH}}\textup{)}.
\end{theorem}
...
\begin{proposition}[Compatibility with the modular operad
...
Let $\xi_\Gamma$ denote the clutching maps of the modular
...
\item \emph{Operadic identification.}
  ...
  of~\eqref{eq:sewing-map} realises the clutching map
...
\item \emph{Modular associativity.}
  For nested stable graphs $\Gamma'\subset\Gamma$
  \textup{(}i.e., $\Gamma'$ is obtained from $\Gamma$ by
  contracting a subset of edges\textup{)}, the composition
  $\sigma_{\Gamma} = \sigma_{\Gamma/\Gamma'}\circ\sigma_{\Gamma'}$
  is independent of the order in which edges are sewn.
  At the level of differentials, this gives
  $d_\Gamma^2 = \kappa(\cA)\cdot\omega_\Gamma$ regardless of
  the order of node formation, because the cross-terms in
  $d_\Gamma^2$ from different edges vanish by disjoint support
  \textup{(}as in the proof of
  Proposition~\textup{\ref{prop:nodal-curvature-source}}\textup{)}.
...
Part~(b): if edges $e_1,e_2\in E(\Gamma)$ are sewn in two
different orders, the resulting differentials agree because
