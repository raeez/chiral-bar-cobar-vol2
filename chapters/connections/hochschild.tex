...
\item $d_{\mathrm{res}}$ is the collision residue
 differential, extracting the residue at each
 collision divisor $D_S\subset\FM_k(D^*)$ and
 applying the operation $m_{|S|}$---this is the
 holomorphic component, identical to the collision
 differential on $\FM_k(\C)$;
...
chain-level quasi-isomorphism; \ClaimStatusProvedHere]%
...
$S^1$-configurations.  It is a chain map: the
collision residue differential $d_{\mathrm{res}}$
on $\FM_k(D^*)$ is compatible with the cyclic
differential on $\Conf_k(S^1)$ because the
residue of a collision at a divisor $D_S$ depends
only on the relative positions of the colliding
points, not on the global radial coordinate.  The
wraparound component $d_{\mathrm{cyc}}$ is
preserved by $\varphi$ since it depends only on
...
$S^1$.  The acyclicity of $\ker\varphi$ follows
from the contractibility of the radial fibre
$(0,1)$: the radial homotopy provides a contracting
homotopy $h\colon\ker\varphi_k\to\ker\varphi_{k+1}$
...
\begin{proof}
That $\mu_P$ is a chain map follows from
$(d^P)^2 = 0$ on $\overline{\FM}_n^{\mathrm{ord}}(P)$: the
pair-of-pants is a genus-$0$ surface, so the bar differential is
flat (no curvature correction at genus~$0$).
Associativity of $\mu_P^H$ is the statement that the two
decompositions of the genus-$0$ four-punctured sphere
$\mathbb{P}^1\setminus\{0,1,\infty,\lambda\}$ into pairs of
pants---pinching along the circle separating $\{0,1\}$ from
$\{\infty,\lambda\}$, versus the circle separating $\{0,\lambda\}$
from $\{1,\infty\}$---give chain-homotopic compositions.  This
chain homotopy is provided by the codimension-$1$ strata of the
moduli space $\overline{\cM}_{0,4}$: the cross-ratio $\lambda$
parametrises the two degeneration channels, and the boundary of
$\overline{\cM}_{0,4} \cong \mathbb{P}^1$ gives the associativity
homotopy. The argument at the coalgebraic level is
...
component $\Sigma_v$, and $\delta_e$ is the distributional
$(1,1)$-current supported on the node corresponding to edge~$e$,
arising from the $\bar\partial$-defect of the Arakelov kernel
$K_{\mathrm{Ar}}$ in the thin-annulus limit of the sewing collar:
\begin{equation}\label{eq:node-dbar-defect}
  \bar\partial K_{\mathrm{Ar}}^{(g)}\big|_{e}
  \;=\; -2\pi i\,\delta_e.
\end{equation}
In particular:
\begin{enumerate}[label=\textup{(\alph*)}]
...
when $g = 0$ and $\Gamma$ is a tree, the Arakelov form $\omega_0$
is zero on $\mathbb{P}^1$ (the Arakelov form is
$\omega_g = (i/2\mathrm{Im}(\tau))\,dz\wedge d\bar{z}$ on a
genus-$g$ surface, which is undefined for $g = 0$ since
$\mathbb{P}^1$ has no period matrix; the correct statement is
that the propagator on $\mathbb{P}^1$ is globally holomorphic,
so $\bar\partial K = 0$ and no curvature arises).
...
Statement~(c) is proved by induction on the number of edges of
$\Gamma$.  The base case (a single vertex with $g(v) = g$, no
edges) gives $\dfib^{\,2} = \kappa(\cA)\cdot\omega_g$ from the
vertex bar complex alone.  For each additional edge, the sewing
via $B^{\mathrm{ann}}_e(\cA)$ introduces the node defect
$\kappa(\cA)\cdot\delta_e$
from~\eqref{eq:node-dbar-defect}.  The global relation
\[
  \omega_g
  \;=\;
...
is the distributional identity for the Arakelov form on a nodal
curve, proved in Arakelov geometry
(cf.\ Faltings~\cite{Faltings84}).  Combining this with
the curvature formula
$\dfib^{\,2} = \kappa(\cA)\cdot\omega_g$ on each stratum
...
\begin{remark}[Relation to the modular operad clutching]%
\label{rem:nodal-bar-modular-operad}%
\index{modular operad!nodal bar complex relation}%
\index{clutching map!nodal bar sewing}%
The sewing maps $\sigma_e$ of
Construction~\ref{constr:nodal-bar-model} are the bar-complex
incarnations of the clutching maps $\xi_\Gamma$ of the modular
operad $\mathcal{O}^{\Ainf\text{-ch}}$
(Definition~\ref{def:modular-operad-ainf-chiral}).  The modular
associativity axiom---composition of clutching maps along nested
stable graphs $\Gamma'\subset\Gamma$ is independent of the order
of node formation---is the operadic statement that the nodal bar
differential~\eqref{eq:nodal-bar-differential} satisfies
$d_\Gamma^2 = \kappa(\cA)\cdot\omega_\Gamma$ regardless of which
edges are sewn first.  The $R$-matrix monodromy
$\mathrm{Mon}(R)$ at each edge
(Construction~\ref{constr:genus1-ainf-chiral-operations})
enters through the wrap-around differential $d_{\mathrm{wrap}}$
of the annular sewing complex $B^{\mathrm{ann}}_e(\cA)$: it is
the holonomy of the KZ connection around the sewing circle,
entangling the holomorphic and transverse factors at each node.
\end{remark}
...
\begin{conjecture}[Amplitude constraint on the chiral double]
...
on $Z(U_\cA)$ is conditional on the identification. What
Conjecture~\ref{conj:drinfeld-center-amplitude} records is the
\emph{shape} of the constraint: if the identification holds, the
Drinfeld centre of the chiral double is forced into a four-dimensional
cohomological window. This is a strong structural test on any
future construction of the chiral double.
\end{remark}
...
with Conjecture~\textup{\ref{conj:drinfeld-center-amplitude}}, is a
