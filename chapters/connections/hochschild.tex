...
$d_B^2 = \kappaChHodge(\cA)\,\omega_g$ enters, the operative framing is
...
$\kappaChHodge(\cA) + \kappa(\cA^!) = K(\cA)$, where $K(\cA)$ is
...
over $\overline{\mathcal{M}}_g$. When $\kappaChHodge(\cA) + \kappa(\cA^{!}) \neq 0$ (DS reductions:
...
($\kappaChHodge(\cA) \cdot \omega_g$), both already computed in
Volume~I.
