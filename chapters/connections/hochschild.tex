...
\begin{remark}[Strand: CY-$2$ input shifts the HKR bar-cobar
bookkeeping by $[2]$ and carries a projective Mathieu cocycle;
\ClaimStatusProvedElsewhere]
\label{rem:hoch-DNA-CY2-K3-shift}
...
(Vol~III, Part~\ref{part:frontier}, the K3 synthesis S3), the
...
then sees an additional dimensional bookkeeping from the K3
...
Remark~\ref{rem:bar-cobar-DNA-3-K3-shift2} of the bar-cobar
review for the same content inscribed upstream).
\end{remark}
...
(Proposition~\ref{thm:chiral-hochschild-models-equivalent}). Three homonymous
objects must not be conflated:
\begin{enumerate}
 \item[\textup{(i)}] $\ChirHoch^\bullet(\cA,\cA)$: concentrated in $\{0,1,2\}$
 at non-critical level on the \emph{ordered} bar complex $B^{\mathrm{ord}}(\cA)$
 (Theorem~H proper, Volume~I~Theorem~H, which is now inscribed on the ordered
 bar; the symmetric-bar concentration is a corollary via the averaging map
 $\mathrm{av}\colon \mathfrak{g}^{E_1}\to\mathfrak{g}^{\mathrm{mod}}$ on classes
 $\mathsf{G}/\mathsf{L}$ at generic parameter; for the $E_1$-only (non-Koszul)
 regime and for class-$\mathsf{M}$ non-generic points the symmetric-bar
 concentration is conjectural in Vol~I). Lifted to the chiral Higher-Deligne
 $E_3$-algebra of Theorem~\ref{thm:chiral-higher-deligne}. Degree~$0$ is the
 chiral derived center $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$.
...
$S$: both sides use the same $S$. The two derived
...
(Theorem~\ref{thm:kl-bar-cobar-adjunction}, Step~4 purity) from
...
closes the periodic-CDG conjecture (Remark~\ref{rem:periodic-cdg-mechanism}). Each sub-problem is concrete:
...
duality~\cite{Positselski11,Positselski18}. The protocol~(a)/(b)/(c) decomposition recorded here tracks the
Vol~II frontier~\#3 discipline on non-critical-level admissibility.
\end{remark}
...
over $\overline{\mathcal{M}}_g$. When $\kappa(\cA) + \kappa(\cA^{!}) \neq 0$ (DS reductions:
$\kappa + \kappa^{!} = 13$ for $\mathrm{Vir}_c$,
$\kappa + \kappa^{!} = 250/3$ for $\mathcal{W}_3$, both nonzero
projections of the Trinity conductor $K = c + c^{!}$ of Vol~I
...
Volume~I's categorical logarithm, at all genera (genus-$0$
unconditional; genus-$g\geq 1$ on the chirally Koszul, non-critical,
non-admissible locus per
Theorem~\ref{thm:hochschild-bridge-higher-genus}). The genus-$0$
...
\begin{remark}[Strand: bi-based Ran-space factorisation on the
bulk--boundary datum at the $K3\times E$ specialisation;
\ClaimStatusProvedElsewhere]
\label{rem:hoch-DNA-bibased-Ran}
...
of Remark~\ref{rem:hoch-DNA-CY2-K3-shift}), the datum upgrades to a
...
$24$-puncture configuration of the K3 synthesis S3; nearby-cycles at each node
...
the K3 synthesis S4 \emph{bi-base factorisation identity}
$(\Delta_{\mathrm{ch}}\otimes\id)\Delta_{A_2}
 =(\id\otimes\Delta_{A_2})\Delta_{\mathrm{ch}}$,
the chiral-side bookkeeping of the Vol~III $\Phi$-bridge splitting
$K3\times E \mapsto (K3, E)$
(compare Remark~\ref{rem:bar-cobar-DNA-4-bibased-ran} of the bar-cobar
...
(operadic $\leftrightarrow$ Koszul-dual $\leftrightarrow$
...
\emph{Edge 3--5 (Dunn additivity on $Z$, not on $\cA$).}
Dunn additivity applied to $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$
(bulk, \emph{never} the boundary $\cA$) assembles the two
...
(the geometric realisation of the pentagon edge joining
factorisation, BV, and convolution presentations). The induced coherence is, by direct inspection of
...
\begin{remark}[Strand: Chiral Hochschild Trinity at the K3
specialisation via the Universal $\Phi$-Trace Identity;
\ClaimStatusProvedElsewhere]
\label{rem:hoch-DNA-trinity-phi-trace}
\index{Hochschild!trinity at K3}%
\index{Phi-trace identity@$\Phi$-trace identity!chiral Hochschild trinity}%
\index{Borcherds pushforward!kappa BKM}%
The Universal $\Phi$-Trace Identity of the K3 synthesis S11
(inscribed upstream as
Remark~\ref{rem:bar-cobar-DNA-5-phi-trace-conductor}) organises the
...
\;=\;\frac{c_\Lambda(0)}{2}\;=\;5,
\]
with $c_\Lambda(0)$ the constant Fourier coefficient of the
Borcherds vector-valued modular form $\Lambda$ for
$\mathrm{II}_{1,1}\oplus\mathrm{II}_{1,1}$ and $\Delta_5$ the Igusa
cusp form. The three-Hochschild trinity at the K3 specialisation
therefore carries two distinct numerical outputs across the same
bulk: the free-field branch gives $K=0$ (trivial conductor, matching
...
$E_1$-lift branch gives $\kappa_{\mathrm{BKM}}=5$ (the weight of
...
accordingly the chiral-side bookkeeping of the Universal
$\Phi$-Trace Identity, with the branch split of
the K3 synthesis S11 realised as the degree-$0$ (conductor) versus
...
earlier drafts is wrong (the counit forces this to $1\otimes 1$);
...
class~$M$.
This furnishes DS--Hochschild compatibility at chain level,
class~$M$ universal bulk, chain-level identification, and
bulk--boundary factorisation for $W$-algebras, all unconditionally.
\end{proof}
...
\begin{remark}[Strand: Igusa $\Delta_5$ as the
Hochschild-cohomological gauge class of the $K3$ chiral
Hall--Drinfeld double; \ClaimStatusProvedElsewhere]
\label{rem:hoch-DNA-igusa-gauge-class}
\index{Igusa form!Hochschild gauge class invariant}%
...
the K3 synthesis S2; compare upstream inscription
Remark~\ref{rem:bar-cobar-DNA-1-igusa-gauge}), the classification of
$\Z/2\times K(1)$-invariant classes in
$H^2(\mathfrak{g}_{\Delta_5})$ is one-dimensional:
\[
H^2\!\bigl(\mathfrak{g}_{\Delta_5}\bigr)^{\Z/2,\,K(1)}
\;\simeq\;
\C\cdot\Delta_5,
\]
generated by Igusa's Siegel weight-$5$ cusp form $\Delta_5$ on
$\mathsf{Sp}_4(\Z)$. Under the Hochschild presentation of the chiral
derived centre
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA_{\mathfrak{g}_{\Delta_5}})
=C^\bullet_{\mathrm{ch}}(\cA,\cA)$
(Definition~\ref{def:chiral_hochschild}), this one-dimensional
classification realises $\Delta_5$ as the \emph{gauge-class invariant}
of the $K3$ chiral Hall--Drinfeld double: the Igusa form is the unique
class in the degree-$2$ chiral Hochschild of
$\cA_{\mathfrak{g}_{\Delta_5}}$ that survives both the $\Z/2$ parity
involution and the $K(1)$ isotropic-lattice reduction, and it is
\emph{not} a member of the class-FF stratum above (whose degree-$0$
content is the Feigin--Frenkel oper algebra). Instead, $\Delta_5$
pairs in degree~$2$ against the Koszul-conductor class
$K(\cA_{\mathfrak{g}_{\Delta_5}})$ of
Remark~\ref{rem:hoch-DNA-trinity-phi-trace} via the Universal
$\Phi$-Trace Identity, producing the weight-$5$ output
$\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5})=5$. This is the
Hochschild-cohomological reading of the Vol~III $\Phi$-bridge
payoff: the gauge class of the Hall--Drinfeld double is the Igusa
cusp form, with the pairing structure lifted from
classification-companion to Trinity structural invariant (sitting
alongside the FF fingerprint slots of
...
\textbf{Summary.} The ``$p=1$'' open target flagged above is
resolved for $p=2$ unconditionally, and for $p\ge 3$ reduces to
the single arithmetic question of $\alpha_p=0$. The argument
unifies the strong-vs-free-generation distinction, the
Heisenberg-as-abelian-KM topologization route, the
Monster-orbifold template, and the orbifold case of
Remark~\ref{rem:E3-top-free-PVA-scope}.
\end{remark}
...
\begin{remark}[Strand: $K^{\kappa_{\mathrm{ch}}}=8$ on the
Mukai-$K3$ branch and $\hbar^2 K^{\kappa_{\mathrm{ch}}}=-1$ via
Bruinier--Heegner reciprocity; \ClaimStatusProvedElsewhere]
\label{rem:hoch-DNA-Kkappa-Mukai-Bruinier}
\index{Mukai!K3!K-invariant}%
\index{Bruinier--Heegner reciprocity!K-invariant}%
The genus tower
$\dfib^{\,2}|_{\overline{\cM}_{g,n}}=\kappa(\cA)\cdot\omega_g$ of
...
\bigl(K_\cA\cdot\kappa_{\mathrm{ch}}(\cA)\bigr),
\]
...
\[
K^{\kappa_{\mathrm{ch}}}(\cA_{K3}^{\mathrm{B}})
\;=\;
...
bookkeeping of Remark~\ref{rem:hoch-DNA-CY2-K3-shift}. At the
universal $\hbar^2$-level,
\[
\hbar^2\,K^{\kappa_{\mathrm{ch}}}(\cA_{K3}^{\mathrm{B}})
\;=\;-1,
\]
computed as a Heegner-divisor pushforward via Bruinier's
reciprocity~\cite{Bru02} for the singular-theta lift: the
$\hbar^2$-coefficient of $K^{\kappa_{\mathrm{ch}}}$ is the
...
Remark~\ref{rem:bar-cobar-DNA-5-phi-trace-conductor} of the
...
$\Z/2$ of Remark~\ref{rem:hoch-DNA-igusa-gauge-class}. The
chain-level content is the localisation of the genus-$g$
$\kappa_{\mathrm{ch}}\cdot\omega_g$ current against the
...
\begin{remark}[Strand: Hall--Drinfeld double versus Drinfeld
Yangian, the fourth taxonomic slot of quasi-Hopf chiral quantum
groups; \ClaimStatusProvedElsewhere]
\label{rem:hoch-DNA-hall-drinfeld-fourth-slot}
...
\emph{four} distinguishable quasi-Hopf chiral quantum groups on the
Vol~III the K3 synthesis taxonomy (Vol~III Part~\ref{part:frontier}, the K3 synthesis
S1). The four slots are:
\begin{enumerate}[label=\textup{(\alph*)}]
...
\item \emph{Chiral affine} / \emph{spectral Yangian}: the chiral
 ...
 $r$-matrix $r^{\mathrm{KM}}(z)=k\Omega/z$ on a level prefix, and
 chiral Hochschild concentrated in $\{0,1,2\}$ with Theorem~H
 Hilbert polynomial (classes~$\mathsf{L}$/$\mathsf{M}$ via DS).
\item \emph{Drinfeld chiral double} $U_\cA=\cA\bowtie\cA^!$: the
 Koszul double of Conjecture~\ref{conj:drinfeld-center-equals-bulk},
 whose Drinfeld centre is the chiral derived centre
 $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ of the boundary; it
 lives on a curve and carries a bar-coalgebra quasi-Hopf structure
 on $B^{\mathrm{ord}}(\cA)$ (not on $\cA$ itself).
\item \emph{Hall--Drinfeld double}: the CY-$2$ specialisation of~(c)
 ...
 $[2]$-shift of Remark~\ref{rem:hoch-DNA-CY2-K3-shift}, the bulk
 is the K3 Hall algebra in the sense of
 Kapranov--Vasserot/Cohomological Hall Algebra theory
 (compare Vol~III Part~\ref{part:frontier}, the K3 synthesis S1), and the
 Hochschild gauge-class invariant is the Igusa form $\Delta_5$ of
 Remark~\ref{rem:hoch-DNA-igusa-gauge-class}.
\end{enumerate}
...
input is K3 and the gauge invariant is $\Delta_5$; this is the
...
\section{Synthesis}
\label{sec:hoch-wave13-DNA}

\begin{remark}[Hochschild concentration at the K3 synthesis base: CY-$2$
base and CY-$3$ fibre patterns]
\label{rem:hoch-DNA-1}
Vol~I Theorem~H (chiral Hochschild concentration in cohomological
degrees $\{0,1,2\}$ on the ambient curve) specialises at the K3
synthesis base with two distinct scope patterns, corresponding to the
two CY-dimension slices of the Vol~III $\Phi$-functor.

\noindent\emph{CY-$2$ base (K3 alone).}\quad For
$\cA_{K3} = \Phi_2(D^b\mathrm{Coh}(K3))$ restricted to the K3 synthesis
base, the categorical Hochschild
$HH^\bullet(\cA_{K3}) \cong H^\bullet(K3, \Lambda^\bullet T_{K3})$
concentrates at degree $2$ with signature $(4, 20)$, matching the CY-$2$
Mukai pairing and the Serre-functor shift $S = [2]$. The concentration
is compatible with the averaging morphism
$\mathrm{av} = \mathrm{Torelli}_{K3}\circ\mathrm{KugaSatake}\circ j_{\mathrm{Kod}}$:
$HH^\bullet$ pulls back along $\mathrm{av}^*$ to the Kuga--Satake Hodge
structure on $\bar{\mathcal A}_2$ at genus $2$
\cite{KugaSatake1967,Mukai1987}.

\noindent\emph{CY-$3$ fibre (K3$\times E$).}\quad For
$\mathbf H_{\Delta_5} = \Phi_3(D^b\mathrm{Coh}(\mathrm{K3}\times E))$
on the CY-$3$ fibre, the transverse CY-$3$ direction adds a third
cohomological degree beyond the curve-ambient $\{0,1,2\}$: the precise
statement is
\begin{equation}
\label{eq:hoch-DNA-CY3-scope}
 \ChirHoch^\bullet(\mathbf H_{\Delta_5})\;\subset\;
 \mathrm{CohDeg}\,\{0, 1, 2, 3\},
\end{equation}
with $\ChirHoch^3(\mathbf H_{\Delta_5}) = \mathbb Z[\omega_{\mathrm{K3}\times E}]$
generated by the CY-$3$ volume class. The Schiffmann--Vasserot
degree-$3$ CoHA generator
$K_3^{\mathrm{K3}\times E}\in\CoHA_{\mathrm{K3}\times E}$
transports under $\Phi_3$ to an explicit chain-level representative
$\chi_3 = \Phi_3(K_3^{\mathrm{K3}\times E})\in\ChirHoch^3(\mathbf H_{\Delta_5})$,
whose non-vanishing is witnessed independently by the triangle-cycle
pairing (real simple root of $\mathfrak g_{\Delta_5}$) and the
Maulik--Nekrasov--Okounkov--Pandharipande Donaldson--Thomas pairing
$\mathrm{DT}_{(0,n)}(\mathrm{K3}\times E) =
\mathrm{coeff}_{q^{n-1}}\,\Phi_{10}^{-1}$
(Oberdieck $2018$ Geom.\ \& Topol.\ $22$ \S5).
The CY-$3$ extension class vanishes on the Koszul locus
$\mathcal U_{\mathrm{Kosz}}^{\mathrm{K3}} =
\overline{\mathcal A}_2\setminus(H_1\cup H_4)$
and picks up non-trivial Humbert-divisor contributions along
$H_1\cup H_4$.

\noindent\emph{General CY-$d$ pattern.}\quad
For the $\Phi_d$-image of a CY-$d$ category on a smooth projective
curve, the concentration pattern is
$\ChirHoch^\bullet(\Phi_d(\cF_{\mathrm{CY}_d}))\subset
\mathrm{CohDeg}\,\{0, 1, 2, \ldots, d\}$,
recovering Vol~I Theorem~H at $d = 0$ (classical associative algebra
with cyclic pairing), the K3 base at $d = 2$, and the CY-$3$ fibre at
$d = 3$. The amplitude $d$ is the CY-dimension of the transverse
fibre, not the ambient-curve dimension.
Primary:
Hinich $2010$ Adv.\ Math.\ $223$ \S3 (CY-$d$ concentration);
Francis $2013$ Adv.\ Math.\ $239$ Thm.\ $1.1$ (categorical $\HH^\bullet$
concentration);
Caldararu $2005$ Adv.\ Math.\ $194$ \S4.2 (CY-$d$ volume class in
degree~$d$);
Schiffmann--Vasserot $2013$ Publ.\ Math.\ IHES $118$ Thm.\ $6.1$
(degree-$3$ CoHA generator);
Maulik--Oberdieck--Pandharipande $2017$ arXiv:1605.05473 Thm.\ $1$
(reduced DT on $\mathrm{K3}\times E$).
Cross-ref Vol~III
Rem.~\ref{rem:hochcalc-theoremH-CY3-scope},
Prop.~\ref{prop:SV-e3-sugawara-toroidal},
...
The intermediate the K3 synthesis ansatz
$c_{4d}(A_1, \Sigma_{g,n}) = (12(g-1) + 7n)/6|_{(0,24)} = 26$
(giving $-312 = -12\cdot 26$) is retracted per Vol~I
Chapter~\ref{ch:thqg-open-closed-realization} line~1932 on the
$\mathrm{SU}(2)$ $N_f = 4$ cross-check. The prime factorisation
$-214 = -2\cdot 107$ (with $107$ prime) replaces the the K3 synthesis
numerology; the Mukai signature decomposition
$c_{2d} = -2 c_{4d} - 2 c_-(c_-/4+1)$ at the corrected
$c_{4d} = 107/6$ gives $-2\cdot 107/6 - 260 = -107/3 - 260
= -(107 + 780)/3 = -887/3$, recognising that the Mukai-shape
ratio is no longer aligned with integer Mukai-signature data
at the first-principles central charge: the Shapere--Tachikawa
denominator $6$ rationalises the arithmetic. The Hochschild
concentration at weight $(0, 1, 2)$ survives unchanged, matching
the class-$\mathcal S$ Virasoro shadow
\cite{ShapereTachikawa2008,ChacaltanaDistler2010,Gaiotto2009}.
