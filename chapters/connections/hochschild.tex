...
\begin{proof}
The computation gives the chiral Hochschild via the Felder BRST resolution and
...
\item \textbf{Hochschild differential $\delta_{\text{Hoch}}$}: composition via the $\lambda$-bracket,
\begin{multline}
...
(T18 convention: 3d HT bulk local
operators $=$ chiral derived centre
$Z^{\mathrm{der}}_{\mathrm{ch}}(A_{\partial})=C^\bullet_{\mathrm{ch}}(A_{\partial},A_{\partial})$.)
Let $\mathsf{Obs}$ be a holomorphic--topological prefactorization algebra on $\C\times\R$ arising from a physical realization in the scope of Theorem~\ref{thm:physics-bridge}, and let $A_{\partial}$ be the boundary chiral algebra obtained by restricting $\mathsf{Obs}$ to $\C\times\{0\}$. Then bulk local operators are quasi-isomorphic to chiral Hochschild cochains,
\begin{equation}
...
boundary-linear exact sector, the bulk algebra is the
...
The quasi-isomorphism $\mathcal{O}_{\mathrm{bulk}} \simeq C^\bullet_{\mathrm{ch}}(A_{\partial}, A_{\partial})$ is proved for HT prefactorization algebras in the scope of Theorem~\ref{thm:physics-bridge} via reduction along $\R$ and factorization homology of the chosen prefactorization model $\mathsf{Obs}$. The stronger triangle composition (bulk $\simeq$ derived center of boundary) requires compact generation and the derived center quasi-isomorphism, verified in the boundary-linear exact sector (Theorem~\ref{thm:boundary-linear-bulk-boundary}). The global triangle for all HT theories is conjectural; an abstract bulk/Hochschild comparison for an arbitrary logarithmic $\SCchtop$-algebra without a physical realization is open.
\end{remark}
...
The content of Theorem~\ref{thm:bulk_hochschild} is the chain-level identification of the abstract
...
(Step~4 of the proof) intertwining the bulk
...
chain-level control feeds the obstruction theory of
...
\begin{proof}
Construct an explicit quasi-isomorphism intertwining the differentials. Four steps: reduction along $\R$ via local constancy, identification with chiral Hochschild cochains via the holomorphic weight filtration, differential matching, quasi-isomorphism verification.
...
The holomorphic weight filtration on the bulk matches the Hochschild filtration on $C^\bullet_{\mathrm{ch}}(A_{\partial}, A_{\partial})$: filtration degree $p$ corresponds to cochains supported on $\Ran_{\leq p}(\C)$, i.e.\ cochains of at most $p$ inputs. Each additional insertion point introduces a new collision divisor contributing to the pole order.

\medskip
\textbf{Step 3: Differential identification.}

The bulk differential $d_{\mathrm{bulk}}$ on $\int_{\C} \mathsf{Obs}^{\R}$ has two components matching $d_{\mathrm{ch}} = \delta_Q + \delta_{\mathrm{Hoch}}$.

\begin{enumerate}[label=(\roman*)]
\item \emph{Internal differential $\delta_Q$:} the BV-BRST differential $Q = m_1$ acts on each factor of $A_{\partial}$ separately. On a cochain $f_n$,
\[
(\delta_Q f_n)(a_1, \ldots, a_n) = Q(f_n(a_1, \ldots, a_n)) - \sum_{i=1}^n (-1)^{|f| + |a_1| + \cdots + |a_{i-1}|} f_n(a_1, \ldots, Qa_i, \ldots, a_n).
\]
the graded commutator of $Q$ with $f_n$, the linearized BRST operator on bulk observables.
...
\]
plus the two end terms where $a_1$ or $a_{n+1}$ acts on the output via the $\lambda$-bracket. These are the boundary face maps: the divisor $D_{\{i,i+1\}} \subset \partial \FM_{n+1}(\C)$ at which $z_i$ and $z_{i+1}$ collide implements composition of $f_n$ with $m_2$ at the $i$th slot, with spectral parameter specialization $\Lambda_{\{i,i+1\}} := \lambda_i + \lambda_{i+1}$ (the spectral substitution principle of Definition~\ref{def:ainfty-relations}, the block fusion rule).
\end{enumerate}
...
\end{equation}
so the Ran space decomposition~\eqref{eq:Ran-component} produces a chain map $\int_{\C} \mathsf{Obs}^{\R} \to C^\bullet_{\mathrm{ch}}(A_{\partial}, A_{\partial})$.
...
\end{equation}
is a quasi-isomorphism by filtration. Both sides carry the holomorphic weight/Hochschild filtration; $\Phi$ is a filtered map. On the associated graded, the factorization algebra $\gr^F\!\int_{\C} \mathsf{Obs}^{\R}$ splits: at filtration level $p$ the stratum $\Ran_p(\C) \setminus \Ran_{p-1}(\C)$ contributes $\Hom(A_{\partial}^{\otimes p}, A_{\partial}((\lambda_1))\cdots((\lambda_{p-1})))$ with differential $\delta_Q$ (since $\delta_{\mathrm{Hoch}}$ raises filtration degree). The associated graded of $C^\bullet_{\mathrm{ch}}(A_{\partial}, A_{\partial})$ has the same description. So $\gr(\Phi)$ is an isomorphism, and by filtration completeness and the spectral sequence comparison theorem $\Phi$ is a quasi-isomorphism,
\begin{equation}
C^\bullet_{\mathrm{ch}}(A_{\partial}, A_{\partial}) \;\simeq\; \int_{\C} \mathsf{Obs}^{\R} \;\simeq\; \mathcal{O}_{\mathrm{bulk}}(\C \times \R),
\end{equation}
as dg-objects, $C^\bullet_{\mathrm{ch}}$ the chiral Hochschild cochain complex.
\end{proof}
...
\]
the second identification uses the Lagrangian condition:
$T^*[-1]\cL_b$ is the formal neighbourhood of $\cL_b$ inside
$\cM_{\mathrm{vac}}$. The bulk algebra is the
ambient $(-2)$-shifted symplectic stack seen from the
Lagrangian. The bulk-as-derived-center statement
(Theorem~\ref{thm:CDG_compatibility}\,(iii)) is direct: the center of $\cA_\partial$ is functions
on the self-intersection $\cL_b \times_{\cM_{\mathrm{vac}}}
\cL_b$, and for a Lagrangian this self-intersection is
...
capturing the admissible representation theory of $V$. The zero-mode bridge is precise; the regimes where it is a
quasi-isomorphism versus a strict inclusion follow.
...
\label{thm:bulk-CHC}
(T18 convention: 3d HT bulk observables $\mathsf{Obs}_{\mathrm{bulk}}$ on the left; chiral Hochschild cochains on the right.)
...
\begin{proof}
Three steps: operad-of-shapes recognition providing a common model, filtration analysis via holomorphic weight, well-definedness from the logarithmic $\SCchtop$ structure.
...
\subsection{Bulk from boundary: chiral Hochschild computations}
...
For each standard family, $C^*(\cA,\cA)$ is computed and identified with the bulk observables.
...
\begin{remark}[Package content]
Each ingredient was proved independently. The theorem is a single reference point: for any
logarithmic $\SCchtop$-algebra, the bulk observable complex, boundary
algebra, and spectral braiding fit into one datum
\[
(\mathsf{Obs},\; \cA_\partial,\;
Z_{\mathrm{der}}(\cA_\partial),\;
C_{\mathrm{line}},\; r(z))
\]
while the derived-center and line-module identifications retain the scope of (iii)--(iv). This is the boundary-side holographic datum.
...
\item \textbf{Bulk reconstruction.} Given the boundary vertex
 ...
 the bulk is identified from the boundary chiral Hochschild cochains, with no uniqueness statement.
...
radial coordinate. The induced map
...
at collision divisors — the forms encoding OPE
residues. The cokernel controls the chain-level distinction between $D^*$ and $S^1$.
...
\begin{proof}
Two stages: $\varphi$ is a chain map, $\ker\varphi$ is acyclic. Both rest on flatness of the connection defined by $d_{\mathrm{res}}$ on the
ordered bar complex, and homotopy
...
Verification: $\varphi\circ d^{D^*} = d^{\mathrm{ann}}\circ\varphi$
