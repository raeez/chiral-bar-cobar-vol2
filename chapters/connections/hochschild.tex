...
\begin{remark}[Hochschild concentration at the K3 synthesis base]
\label{rem:hoch-DNA-1}
The Vol~I Theorem~H (Hochschild concentration in a single degree)
refines at the K3 synthesis base to:
$HH^\bullet(\mathbf H_{\Delta_5}) \cong H^{2d}(K3)[2]\otimes \mathrm{id}$
concentrated in degree $2$, matching the CY-$2$ Mukai pairing signature
$(4,20)$ and the Serre-functor shift $S = [2]$. The concentration is
compatible with the averaging morphism $\mathrm{av} = \mathrm{Torelli}_{K3}\circ\mathrm{KugaSatake}\circ j_{\mathrm{Kod}}$:
$HH^\bullet$ pulls back along $\mathrm{av}^*$ to Kuga--Satake Hodge
structure on $\bar{\mathcal A}_2$ at genus $2$
\cite{KugaSatake1967,Mukai1987}. Cross-ref Vol.~III
