...
MC3 is proved for types $A_n$ (Theorem~\ref{thm:mc3-type-a-resolution}), $B_n$ (folding, Proposition~\ref{prop:mc3-type-b-folding}), and $C_n$ (Langlands duality).  The difficulty gradient for remaining types: $D_n$/$E_6$ (grade~$B$, expected from simply-laced structure), $E_7$ (grade~$C^+$), $G_2$ (grade~$C^-$, rank~$2$ amenable to direct computation), $F_4$/$E_8$ (grade~$D$, zero minuscule representations, may require Coulomb branch methods).
\end{construction}
