...
\begin{remark}[Extension to $\mathfrak{sl}_4$: cross-orbit Koszul duality; \ClaimStatusConditional]
...
(Chapter~\ref{chap:log-ht-monodromy}), with a rigidly
...
Section~\ref{app:anomaly-completed-topological-holography}, with
$u = \eta^2$ the secondary
central charge. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
...
(Chapter~\ref{ch:ym-instanton-screening}); identification of $\eta$ as
...
The Vol~II anomaly-completion framework specialises at twisted 11D-SUGRA on $K3 \times T^2$ with $24$ M5-branes on $I_1$ Kodaira fibres to a Deligne cohomology anomaly class in $H^2(\overline{\mathcal{A}_2}, \mathcal{O}^*)$; its unique trivialiser is the Gritsenko--Nikulin Igusa cusp form $\Delta_5$ of weight $5 = \chi(\mathcal{O}_{K3}) + \sum_{24}\mathrm{Res}(I_1) = 2 + 3$. Four duality frames (Harvey--Moore $1996$; DVV $1996$--$1997$; DMVV $1997$; Witten $1995$). Universal $\Phi$-trace identity: $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = c_\Lambda(0)/2 = 5$.
\end{remark}
