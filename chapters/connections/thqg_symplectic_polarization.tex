...
the construction reduces to a bulk--boundary entanglement
computation whose physical BTZ interpretation requires the
AdS$_3$/CFT$_2$ dictionary, modular invariance, vacuum dominance,
and saddle analysis.
...
The complementarity theorem has a holographic interpretation after
the physical AdS$_3$/CFT$_2$ dictionary is supplied. In that
dictionary, the genus-$g$ partition function of a boundary CFT is
compared with a gravitational path integral over bulk geometries with
conformal boundary of genus $g$. At genus~$1$, the high-temperature
saddle contains the BTZ black hole \cite{BTZ92}. Complementarity
provides the algebraic bulk--boundary polarization; it does not by
itself prove saddle dominance, modular invariance, or a physical
entropy formula.
...
\item The complementarity potential at genus $1$ is quadratic:
 $S_\cA^{(1)} = \kappaChHodge(\cA) \cdot x$
...
$S_\cA^{(1)} = \kappaChHodge(\cA) \cdot x$ where $x$ is the
...
\end{itemize}
The Virasoro shadow obstruction tower being infinite is the
algebraic counterpart of the perturbative non-renormalizability of
