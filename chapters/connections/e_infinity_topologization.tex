...
spin $N$, and the full tower is the inverse limit. Axiom
\ref{tower:antighost-commutativity} is supplied at finite depth by
Theorem~\ref{thm:casimir-antighost-commutativity}; on every bounded
weight window the inverse system is eventually constant, so the
finite-depth exact homotopies pass to the completed inverse limit.
...
At generic $c$, criteria \textup{(a),(b),(c)} are equivalent and all
hold; Theorem~\ref{thm:casimir-antighost-commutativity} supplies the
all-spin antighost commutativity needed to identify the limiting
stress tower with the limiting Dunn tower. The
$\Einfty$-topological structure on
