...
\noindent
How does the $E_1$-chiral chart functor of
Part~\ref{part:e1-core} acquire a topological dynamics? A
conformal vector $\omega \in A_b$ at non-critical level
$k \ne -h^\vee$ exhibits the chart algebra $A_b$ as $E_1$-chiral
promoted to $\Ethree^{\mathrm{top}}$ on $Q$-cohomology by the
Sugawara identification $T(z) = [Q_{\mathrm{CS}}, G(z)]$ with
$G(z)$ the antighost partner: the Sugawara identity purchases
one transverse real dimension, the $\Etwo^{\mathrm{top}}$ upgrade
along the holomorphic direction, and Dunn additivity with the
open colour assembles the three-dimensional topological field
theory \textup(Construction~\ref{constr:topologization}\textup).

This chapter asserts that $\Ethree^{\mathrm{top}}$ is the
\emph{first rung} of an infinite ladder. Each higher-spin
conformal tensor $T^{(n)} = [Q_{\mathrm{tot}}, G^{(n)}]$ in the
stress tower
$T_{\bullet}(A_b) = \bigl\{ T^{(2)}, T^{(3)}, T^{(4)}, \ldots \bigr\}$
of a chiral algebra of $\cW$-type purchases \emph{one more}
transverse real dimension once the corresponding independent
translation and homotopy data are supplied; a chart algebra equipped
with a stress tower of depth $N$ reaches $\Etopo{N{+}1}$; the
$\cW_\infty[\lambda]$ algebra of Bakas--Pope--Romans and
Linshaw\,\cite{Linshaw-Winfty,Bakas-Winfty,BMP-Wsymmetry} reaches
the universal limit $\Einfty^{\mathrm{top}}$ under the convergence
and inverse-limit hypotheses
$\hypProchazka, \hypCKL, \hypPRSh, \hypYamada$ recorded in
\cref{thm:e-infinity-specialisation-Winfty}. The
$3$d quantum gravity of Part~\ref{part:gravity} (Virasoro case
$N = 2$) is then the $\Ethree^{\mathrm{top}}$ shadow of a $3$d$+\infty$
topological field theory living on the $\cW_\infty$ boundary. The
operadic circle of \S\ref{subsec:operadic-spiral} is, at Platonic
level, an infinite bidirectional spiral.
...
Fix the bicoloured primitive package $\primPkgBicolour$ on
$\logCurve = (X, D, \tau)$ and let $A = A_b = \mathrm{End}_{\openFactCat}(b)$
be the chart algebra at boundary vacuum $b$, at non-critical level
...
would be a category error. The iterated Sugawara identities
...
and let $A = A_b$ be the chart algebra at boundary vacuum $b$, at
non-critical level, equipped with a higher-spin stress tower of depth $N$
\textup{(}Definition~\textup{\ref{def:higher-spin-stress-tower}}\textup{)}.
...
specialised to non-critical level. Assume the four primary-source
endpoint hypotheses
\[
 \hypProchazka\;\text{\cite{ProchazkaRapcak18}},\qquad
 \hypCKL\;\text{\cite{CKL-parafermion}},\qquad
 \hypPRSh\;\text{\cite{PopeRomansShen-winfty-wedge,Bakas-Winfty}},\qquad
 \hypYamada\;\text{\cite{Yamada}},
\]
discharging respectively the triangular-truncation control of the
universal family, the parafermion / Heisenberg-coset compatibility,
the higher-spin Casimir wedge realisation with its Bakas quantisation,
and the explicit weight-window threshold for the inverse-limit
stabilisation. Assume further the chain-level data:
\begin{enumerate}
 ...
 \item the weight-window strong-limit condition (supplied by
 $\hypYamada$) holds for the chosen generic point;
...
four primary-source endpoint hypotheses
\[
 \hypProchazka\;\text{\cite{ProchazkaRapcak18}},\qquad
 \hypCKL\;\text{\cite{CKL-parafermion}},\qquad
 \hypPRSh\;\text{\cite{PopeRomansShen-winfty-wedge,Bakas-Winfty}},\qquad
 \hypYamada\;\text{\cite{Yamada}},
\]
which respectively license the
triangular-truncation control of the universal family
(criterion~(a)), the parafermion / coset compatibility supplying an
independent route to the Gaberdiel--Gopakumar stabilisation, the
PRS wedge + Bakas quantisation supplying the higher-spin Casimir
realisation, and the explicit Yamada weight-window threshold
$N_0(w_{\max}) = 2 w_{\max} - 1$ controlling criterion~(c). The
following four coupled hypotheses are the chain-level data
(theorem data, not equivalent theorems):
...
topological gravity; licensing $\delta$;
...
conditional on $\hypProchazka, \hypCKL, \hypPRSh, \hypYamada$ — the
four primary-source endpoint hypotheses on triangular-truncation
control, parafermion stabilisation, wedge-algebra bounds, and Yamada
coherence respectively\textup{)}.
...
\emph{physical} gravity has a higher-spin stress tower, that is, whether
