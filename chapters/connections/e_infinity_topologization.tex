...
dimension $2 + \infty + 1 = \infty$; climbs to $\Einfty^{\mathrm{top}}$,
the Koszul self-dual endpoint.
\end{enumerate}
\end{proposition}
...
The dimension bookkeeping on the RHS of~\eqref{eq:tower-dimension-dunn-factorisation}
...
giving $\Einfty^{\mathrm{top}}$ as the inverse limit of the $\Etopo{N+1}$
tower.
...
theory has infinite transverse dimensions, and the iterated Dunn
product converges to the commutative ring $\Einfty^{\mathrm{top}}$
in the stable limit.
\end{remark}
...
constant on $Q_{\mathrm{tot}}$-cohomology to order $N - 1$: all
derivatives $\partial_z^r$ for $1 \le r \le N - 1$ act as
...
$\Etopo{N+1}$: the $N - 1$ antighost directions
$G^{(2)}, \ldots, G^{(N)}$ supply $N - 1$ topologized factors along the
holomorphic direction (each $G^{(n)}$ kills $\partial_z^{n-1}/(n-1)!$),
which combined with the Deligne $\Etwo^{\mathrm{chiral}}$ on
$Z^{\mathrm{der}}_{\mathrm{ch}}$ and the transverse $\Eone^{\mathrm{top}}$
factor on $\R$ assemble by Dunn into
$\Etwo^{\mathrm{top}} \otimes (\Eone^{\mathrm{top}})^{\otimes (N-2)}
\otimes {\Eone^{\mathrm{top}}}_{\mathrm{tr}} = \Etopo{N+1}$.
In the limit $N \to \infty$ the construction delivers an
...
\textup{(}Definition~\textup{\ref{def:higher-spin-stress-tower}}\textup{)}.
Assume that the first $k$ stress levels supply $k$ pairwise commuting
topological translation operators $P_1,\ldots,P_k$ and degree-$-1$
homotopies $G_1,\ldots,G_k$ satisfying
...
with $0 \le k \le N-1$, the derived chiral centre
...
discarding the highest antighost $G^{(k+1)}$. In the limit $N \to \infty$,
...
\begin{enumerate}
 ...
 \item the Yamada weight-window strong-convergence criterion holds
 for the chosen generic point.
\end{enumerate}
...
tower is the inverse limit. Under hypotheses~(2) and~(3), Axiom
...
\begin{theorem}[$\cW_\infty$ $\Einfty$-topological convergence;
\ClaimStatusConditional]
...
following three coupled criteria. Criterion \textup{(a)} is the
Linshaw--Gaberdiel--Gopakumar stability input; criteria
\textup{(b),(c)} are the operadic and chain-level convergence
hypotheses needed to pass from all finite rungs to the inverse limit.
...
When criteria \textup{(a),(b),(c)} hold and the finite-depth
antighost homotopies are compatible in the inverse system, the
$\Einfty$-topological structure on
...
$N \ge 2w_{\max} - 1$, which is the uniform Yamada threshold.
...
the generic OPE stabilisation theorem; criteria \textup{(b),(c)} and
the all-spin antighost compatibility are the remaining inverse-limit
inputs.
...
compatibility and Yamada strong-convergence hypotheses are supplied.
The three convergence criteria \textup{(a),(b),(c)} are the three
interfaces that must agree: Gaberdiel--Gopakumar stabilisation at the
OPE level, co-cofinality at the $\infty$-operadic level, and Yamada
weight-window stabilisation at the chain-complex level.
\end{remark}
...
concrete finite-window test of the stress-tower and Dunn-rung inputs.
...
weight-window): the uniform threshold of
...
the finite stress and Dunn inputs, but it misses the
...
whereas the bar-weight-$4$ Yamada threshold is the spin-$7$ truncation.
...
compatibility and Yamada strong-convergence inputs of
...
targets of the programme. Conditional on the inverse-limit inputs, it
