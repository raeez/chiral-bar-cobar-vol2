...
%
% Canonical upgrade (Vol II UPGRADE-SWEEP, 2026-04-16):
...
%   Section 6. Healing of FM47, FM48, FM81, FM82, FM215.
...
\begin{remark}[Antighost BRST-commutativity and higher-spin $\cW$-commutativity]
...
$[G^{(n)}, G^{(m)}] = Q_{\mathrm{tot}}$-exact on cohomology. In the
Costello--Gaiotto realisation this follows from the classical fact that
the truncated $\cW_\infty[\mu]$ structure constants are degenerate
at the classical level in the sense that the Poisson bracket
$\{T^{(n)}, T^{(m)}\}$ is a normally ordered polynomial of total spin
$n+m-1$ which reduces on cohomology to a total derivative of a
lower-spin Casimir combination (Linshaw\,\cite{Linshaw-Winfty} Theorem
7.1; Bouwknegt--McCarthy--Pilch\,\cite{BMP-Wsymmetry}
§4.3). Applying Theorem~\ref{thm:iterated-sugawara-construction} to this
lower-spin combination exhibits $[G^{(n)}, G^{(m)}]$ as
$Q_{\mathrm{tot}}$-exact.
\end{remark}
...
We now assemble the iterated topologization of
...
$k \le N-1$. Conflating the two (native vs.\ derived) is
AP-CY56 / AP150 and must be resisted; see
Remark~\ref{rem:single-conformal-vector-N2} above for the base case
and Vol~III AP-CY56 for the systematic version.
\end{remark}
...
\ref{tower:antighost-commutativity} is preserved under inverse limits
of topologically complete graded vector spaces (equivalently: the
truncation projections are compatible with the antighost
Poisson-commutativity identities).
...
(Fresse\,\cite{Fresse-book}\,Theorem~14.1.1), the algebraic structure on
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cW_\infty[\mu])$ is fully commutative
up to all higher coherences.
\end{proof}
...
hold; the $\Einfty$-topological structure on
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cW_\infty[\mu])$ is proved
...
tests for $N = 5, 6, \dots$ populate the cache entry
\cite{cache:w-infty-spin-leq-N} and converge to the full
...
\label{prop:healing-fm47-48}
\index{FM47!healed}
\index{FM48!healed}
FM47 \textup{(}``$\Etwo \to \Ethree$-chiral automatic''\textup{)} and
FM48 \textup{(}``$\Ethree$-top from $\Einfty$ alone''\textup{)} are
absorbed into the higher-spin stress-tower clause of
Definition~\ref{def:higher-spin-stress-tower}.

\begin{enumerate}[label=\textup{(\arabic*)}]
\item FM47 was the conflation ``$\Etwo$ on $Z^{\mathrm{der}}_{\mathrm{ch}}(A)$
 automatically upgrades to $\Ethree$-chiral''. Correct statement: the
...
\item FM48 was the conflation ``$\Ethree$-top from $\Einfty$ alone''.
 Correct statement: $\Ethree$-top requires two independent inputs,
 3d HT theory and conformal vector at non-critical level. Both are
 inputs to Definition~\ref{def:higher-spin-stress-tower} at depth~$2$.
\end{enumerate}
Both failure modes are now resolved by the explicit hypothesis structure.
\end{proposition}

\begin{proposition}[Fractional-weight ghosts and branched covers]
\label{prop:healing-fm81}
\index{FM81!healed}
\index{non-principal DS!fractional-weight ghosts!healed}
...
In particular FM82 (``class~M quartic poles incompatible with
Khan--Zeng polynomial $\lambda$-brackets'') was a confusion between the
\emph{original} $\lambda$-bracket (which does contain $\lambda^3$) and
...
\label{prop:healing-fm215}
\index{FM215!resolved}
\index{Stage 9!class M!resolved}
...
is consistent with the stronger FM82 healing statement
(Proposition~\ref{prop:healing-fm82}) once the Li filtration is applied.
The apparent contradiction is resolved: FM82's original scope
...
\begin{proof}
Direct consequence of Proposition~\ref{prop:healing-fm82}.
\end{proof}
...
The $\Einfty$-topologization theorem of this chapter inscribes the
...
Three-dimensional gravity with Virasoro boundary, as inscribed in
...
The physical $3$d quantum gravity that has been inscribed in the
...
\begin{remark}[Consistency with V2-AP catalogue and Vol~II
reconstitution]
\label{rem:consistency-vol2-AP-reconstitution}
...
(anti-patterns V2-AP47, V2-AP48, V2-AP81, V2-AP82, V2-AP215) as consequences of the
stronger $\Einfty$-statement rather than as isolated patches.
