...
\textup{(}Definition~\ref{def:higher-spin-stress-tower}\textup{)}. The
iterated topologisation of
Construction~\ref{constr:iterated-topologization} assembles the following
Dunn factorisation of the derived chiral centre:
...
\end{equation}
The count $2 + (N - 2) + 1 = N + 1$ decomposes as: $2$ dimensions from
...
the higher-spin antighosts $G^{(3)}, \ldots, G^{(N)}$ (each $G^{(n)}$
contributing one topologised direction dual to the $(n-1)$-st holomorphic
jet); and $1$ dimension from the transverse real direction
...
\]
with the finite perturbative correction supplied by the transferred
homotopy satisfies~\eqref{eq:casimir-antighost-commute}. Drinfeld--
Sokolov reduction preserves the argument because both the Casimir
polynomials and the Euler homotopy commute with the screening
differentials after passing to the common kernel.
...
\begin{remark}[Remaining chain-level refinement]
\label{rem:frontier-antighost-BRST-commutativity-higher-spin}
\index{frontier!strict antighost commutativity before cohomology|textbf}
The theorem proves the commutativity required by
Definition~\ref{def:higher-spin-stress-tower}: exactness on
$H^\bullet(A^{\mathrm{BV}}_{3d},Q_{\mathrm{tot}})$ and, in the
$\cW_\infty[\mu]$ case, exactness on every bounded weight-completed
window. A stronger operator identity on the raw uncompleted BV vertex
algebra would require writing the transferred homotopy corrections in
closed normal-ordered form. That stricter chain-level presentation is
separate from the cohomological tower theorem proved here.
\end{remark}
