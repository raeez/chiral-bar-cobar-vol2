...
computation on the full Schellekens row surface using the cyclic
...
outcome is an unconditional stratified anomaly theorem: representatives
of all $71$ Schellekens rows have $\alpha_{\mathrm{orb}} = 0$, with the
vanishing mechanism divided into three disjoint routes (Types~A/B/C).
...
at central charge $c = 24$, with orbifold presentation
$W = V_\Lambda^g$ as in
...
= \mathbb{Z}/n$
of the orbifold presentation equals $0$, for all $71$ row
representatives.
...
(Chapter~\ref*{ch:monster-chain-level-e3-top-platonic}).
 ...
 \end{itemize}
\item \textbf{Chain-level $E_3$-topological descent.} Every such
representative $W$ admits a chain-level $E_3$-topological structure;
hence lies
...
(i) Uniform DW vanishing: every representative on the Schellekens
row surface falls into exactly
...
is defined on representatives of all $71$ Schellekens holomorphic
$c = 24$ rows. For each such $W$, $\Phi_{\mathrm{hol}}(W)$ is a
chain-level HT-QFT on
...
chain-level $E_3$-topological structure on each Schellekens-row
representative. The Universal Holography functor is defined on
...
\begin{itemize}
\item Representatives of all $71$ Schellekens holomorphic $c = 24$
rows have
$\alpha_{\mathrm{orb}} = 0$
...
Types~A and~C.
\item The stratified anomaly theorem accounts for all $71$ rows in
Schellekens's list.
\end{itemize}
