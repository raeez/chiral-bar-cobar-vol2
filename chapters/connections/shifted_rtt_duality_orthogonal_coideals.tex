...
% Locks AP159 four-Yangian-types disambiguation (shifted RTT side: type
% 1 + type 2 + boundary quotient layer (c)), AP161 Notion-B working
% default, FM179 coideal-property safeguard, FM181 ``orthogonal''
% terminology disambiguation (NOT Letzter--Kolb), AP-RMATRIX guard, and
% Wave-14 cross-references. All labels new; existing material unmodified.
% ======================================================================

\section{Adversarial discipline block (Wave-14 anchor)}
\label{sec:shifted-rtt-wave14-anchor}

\subsection{Three layers and the four Yangian types}
\label{subsec:shifted-rtt-ap159-yangian-types}

\begin{convention}[AP159: layers (a)/(b)/(c) match Yangian types
1/2/$+$shift; \ClaimStatusProvedHere]
\label{conv:shifted-rtt-ap159-yangian-types}
The three-layer decomposition of
Definition~\ref{def:three-layers-ordered-theory-appendix} aligns
with the four Yangian types of Vol~I AP159 as follows:
\begin{enumerate}[label=\textup{(\arabic*)}]
\item Layer (a) (\emph{local RTT kernel}): Vol~I AP159 \emph{type 1},
the classical Yangian $Y_\hbar(\mathrm{sl}_N)$ as
$E_1$-topological algebra on $\R$, RTT presentation.
\item Layer (b) (\emph{completed envelope}): Vol~I AP159 \emph{type 2},
...
\subsection{AP161 working notion: shifted RTT in Notion-B}
\label{subsec:shifted-rtt-ap161-notion-b}

\begin{convention}[AP161: shifted RTT data are Notion-B
$E_1$-chiral; \ClaimStatusProvedElsewhere]
\label{conv:shifted-rtt-ap161-notion-b}
...
\subsection{FM179 coideal property: chain vs differential
stability}
\label{subsec:shifted-rtt-fm179-coideal-discipline}

\begin{remark}[FM179 discipline: coideal vs sub-dg-module distinction;
\ClaimStatusProvedElsewhere]
\label{rem:shifted-rtt-fm179-coideal-discipline}
Vol~II FM179 records that ``image of dg algebra map is stable
under dual differential'' is differential stability, NOT
coproduct stability (the actual coideal property
$\Delta(C) \subset C\otimes\text{ambient} +
...
\subsection{FM181 terminology: ``orthogonal coideal'' vs
Letzter--Kolb}
\label{subsec:shifted-rtt-fm181-terminology}

\begin{remark}[FM181 disambiguation: ``orthogonal'' here means
\emph{pairing-annihilator}; \ClaimStatusProvedElsewhere]
\label{rem:shifted-rtt-fm181-terminology}
Vol~II FM181: the term ``orthogonal coideal'' in the
...
\subsection{AP-RMATRIX guard for the local RTT kernel}
\label{subsec:shifted-rtt-rmatrix-level-prefix}

\begin{remark}[Level prefix $\hbar$ on the RTT kernel; AP-RMATRIX/HZ-1;
\ClaimStatusProvedElsewhere]
...
$\hbar = 0$ gives $R(u,v) = 1$ (trivial $R$-matrix, abelian
limit), confirming the AP126 discipline: substituting the level
parameter to zero gives the trivial $r$-matrix on the trace-form
side. The shifted $R$-matrix
