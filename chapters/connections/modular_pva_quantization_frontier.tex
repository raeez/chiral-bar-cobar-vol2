...
The following gives the Chriss--Ginzburg attack on this question, recasting the modular
...
\begin{proposition}[Fixed-pair Steinberg cap action; \ClaimStatusConditional]
...
\begin{conjecture}[Steinberg fundamental class and clutching compatibility]
\label{conj:steinberg-fundamental-class-compatibility}
\ClaimStatusConjectured
For each stable pair \((g,n)\), the derived correspondence
\(\fS_b^{g,n}=\cL_b\times_{\cM_{\mathrm{vac}}^g}\cL_b\)
admits an oriented Borel--Moore virtual fundamental class
\([\fS_b^{g,n}]^{\mathrm{vir}}\). For every modular clutching
graph~\(\Gamma\), the clean-intersection pullback satisfies
\[
 \xi_\Gamma^![\fS_b^{g,n}]^{\mathrm{vir}}
 =
 \frac{1}{|\Aut(\Gamma)|}
...
\begin{proposition}[Modular completion as Steinberg system; \ClaimStatusConditional]
...
Assume
\autoref{conj:steinberg-fundamental-class-compatibility}. Then
...
compatibility in
\autoref{conj:steinberg-fundamental-class-compatibility}: the gluing maps
...
factorization/line-operator package of Part~\ref{part:bbl-core}
(\S\ref*{sec:line-operators}, \S\ref*{sec:spectral-braiding}).
This identifies $G_0(T;B)$ with the convolution data on~$\fS_b^{0}$.

For the modular extension, assume the compatibility in
\autoref{conj:steinberg-fundamental-class-compatibility}. Then the operadic
...
The fundamental classes are compatible by the conjectural
clean-intersection identity
\eqref{eq:steinberg-excess-pullback} with trivial excess and
trivial orientation defect. That class-compatibility assertion is
exactly the missing chain-level Steinberg input.
\end{proof}
...
\autoref{conj:steinberg-fundamental-class-compatibility}.
...
\autoref{conj:steinberg-fundamental-class-compatibility} these defects
...
Under
\autoref{conj:steinberg-fundamental-class-compatibility}, the
...
fundamental-class compatibility,
Conjecture~\ref{conj:tautological-recursion} remains open: the
unaccounted term is the cap-push of the excess factor
...
does not require a primary-source computation of the $D^3$
hemisphere partition function for general bulk theories. The
hemisphere-as-cyclic-pairing identification is predicted by
the three-stage comparison below. Computation~\ref{comp:heisenberg-hemisphere-pairing}
cites \cite{DFPY16} only as a methodological analogue for the $S^3$
sphere partition function; a primary $D^3$ hemisphere source is still
needed for the analytic normalisation. On the locus where the slab
...
denote the augmentation ideal, and recall that
...
collision residue $k/z$. Here the
...
serves only as a methodological analogue. A primary $D^3$ hemisphere
calculation would replace this analogue and fix the normalisation
constant.
...
on Koszul pair; \ClaimStatusConditional]
...
Conjecture~\ref{conj:hemisphere-cyclic-pairing} at the
boundary divisors.
...
across BKM ranks \cite{PasolZagier2013,Borcherds1998}. Cross-ref
