...
$K_{\mathrm{eff}}$ is linear in~$\hbar$, so the rational function
...
(Khan--Zeng analytic SDR \cite{KZ25}; Forsythe--Mukai Stokes data;
...
Khan--Zeng's quantization of the classical $W_3$ PVA yields the quantum
...
direction. The form $\langle\cdot,\cdot\rangle_{\mathrm{slab}}$ is the
...
computation of the $D^3$ hemisphere partition function for abelian CS is
unavailable at present; the expression above is obtained as a direct
perturbative Gaussian integral, and the cross-reference to \cite{DFPY16}
serves as a methodological analogue. A primary $D^3$ hemisphere
