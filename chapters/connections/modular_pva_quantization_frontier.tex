...
Construction Problem~3 (CLAUDE.md~\S9). The Virasoro and $W_3$
...
\]
This is the value selected by
Theorem~\ref{thm:central-charge-shift} after the four endpoint
hypotheses
$\hypKZSDR,\,\hypStokes,\,\hypReflWts,\,\hypTLift$
are supplied (Khan--Zeng analytic SDR \cite{KZ25}; Forsythe--Mukai
Stokes data; reflected weights under analytic continuation; chain-level
lift of $T = [Q_{\mathrm{tot}}, G]$ \cite{Costello2011Renorm,CG17,LurieHA}).
...
Khan--Zeng's quantization problem for the classical $W_3$ PVA leads to
the quantum $W_3$ algebra with a shifted effective central parameter
\cite{KZ25}. The formal algebraic results above prove that this shift is
the only relevant genus-one direction once locality, conformal weight,
and filtration are imposed. The remaining analytic datum is the scalar
\[
...
\section{Required Construction Data}
...
cyclic pairing on the ordered bar complex. Under the universal
...
of the $D^3$ hemisphere partition function is available for every
...
D^3 \cup_{S^2} D^3$. For TQFTs this is the gluing theorem;
for $3$d holomorphic--topological theories it is the requirement that the
...
partition function requires a finite-dimensional localisation
