...
each genus-$g$ contribution projects a single fundamental
class~$[\fS_b^{g}]$.
\end{remark}
...
algebra is the Heisenberg vertex algebra $\Heis_k$$, collision residue $k/z$). Here the
...
with $\kappa + \kappa' = 0$ for KM/free families (Volume~I,
...
statement is closed by
Theorem~\ref{thm:irregular-singular-kzb-regularity} (the
Jimbo--Miwa irregular-singular framework, which gives chain-level
regularity at generic non-integral level $\hbar$ on smooth
genus-$g$ curves); the boundary-stratum extension for $g \ge 1$
is the open frontier
\cref{conj:f5-irregular-singular-genus-g} of
the climax. The cyclic completion of class~$\mathbf{C}$ at
...
\section{Witten DNA: heterotic/IIA duality identification of the K3 chiral bialgebra}
...
$\mathbf{H}_{\Delta_{5}}$ (Vol~III Chapter~\ref*{ch:k3-chiral-bialgebra-platonic},
Definition~\ref*{def:k3-chiral-bialgebra}) maps under this duality to the
...
Theorem~\ref*{thm:k3-abelian-at-lie}) is exactly the lattice VOA
...
Vol~III Theorem~\ref*{thm:k3-classS-4d-parent} carries flavour
...
Theorem~\ref*{thm:k3-abelian-at-lie}) with the heterotic gauge sector
...
bridge: Vol~III Chapter~\ref*{ch:k3-chiral-bialgebra-platonic}
\S\ref*{sec:hall-drinfeld-double-k3}.
\end{remark}

\section{Wave 13 DNA perspective}
\label{sec:mpvaqf-wave13-DNA}

\begin{remark}[Modular PVA quantisation frontier at Wave~13]
\label{rem:mpvaqf-DNA-1}
The Wave~13 modular PVA quantisation frontier extends the Pasol--Zagier
