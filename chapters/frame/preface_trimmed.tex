...
Steinberg variety presents the Hecke algebra.}  The Steinberg
variety $\widetilde{\cN} \times_{\fg} \widetilde{\cN}$ is one
geometric object; its Borel--Moore homology, $K$-theory, and
equivariant derived category extract three different algebraic
invariants.  The bar complex $\barBch(\cA)$ on
$\FM_k(\C) \times \Conf_k^{<}(\R)$ is one coalgebra; three
functors project three objects: cobar
$\Omega(\barBch(\cA)) \simeq \cA$ inverts the construction,
Verdier duality
$\mathbb{D}_{\mathrm{Ran}}(\barBch(\cA)) \simeq \barBch(\cA^!)$
produces the line-side Koszul dual, and chiral Hochschild
cochains $C^\bullet_{\mathrm{ch}}(\cA,\cA)$ compute the bulk
observables.  Every chapter of this volume is a different
projection of this single object.

A chiral algebra is a point in the derived symplectic category: a
Lagrangian $\cL_\cA \hookrightarrow \cM_{\mathrm{vac}}(\cA)$.  The
bar complex is the endomorphism algebra of this point; the
...
of the point over the moduli of curves.  Volume~I computes the
Taylor jets; this volume identifies the local composition law.

The Steinberg analogy is not decoration.  The three projections
play three genuinely different roles in the theory and never
coincide: cobar recovers the algebra up to quasi-isomorphism,
Verdier duality interchanges it with its line-side dual, and
chiral Hochschild produces the bulk as a separate object with its
own module category.  Conflating any two of them collapses the
...
obstruction tower at all arities and the discriminant
...
genus $g \ge 1$; complementarity lifts to the
bulk-boundary-line triangle; the discriminant $\Delta$
classifies the complexity of bulk-boundary pairs.  Classical
Koszul duality is genus~$0$ on a point; Volume~I is genus~$g$ on
a curve; this volume is the dimensional lift to $\C \times \R$.
...
over $\SCchtop$ (Theorem~\ref{thm:rosetta-swiss-cheese}): the
differential is the closed-colour co-structure, the coproduct is
...
(Theorem~\ref{thm:homotopy-Koszul}).  By Koszul self-duality
...
$d^2 = \kappa(\Bbound) \cdot \omega_g$ (Vol~I, Theorem~D).  This
is not a defect but the content of the modular theory: the
nonvanishing of $d^2$ is how the genus tower enters the local
...
The $\SCchtop$-coalgebra structure, the 3d MC element
$\alpha_T$, the ordered/associative $E_1$ side of Koszul
...
Each projection of the arity-$(2,0)$ collision: spectral
...
\item Swiss-cheese identification of the bar complex
...
Swiss-cheese operad is the local composition law, Koszul duality
is the symplectic complement, the genus tower is the deformation
of the point over the moduli of curves, and the holographic
principle is the theorem that a point determines its ambient.
