...
object $\mathbf{H}_{\Delta_5}$. Here the triangle becomes genuinely
...
\]
matches Drinfeld's ordered Hall presentation with the Siegel
paramodular one: Beilinson governs the first base, Kazhdan the
second, and Volume~II identifies the two as one $\SCchtop$ datum. The factorization-homology centre
...
$\Phi_{10}^{\mathrm{un}}/\eta^{24}$, and the remaining obstruction space
\[
H^2(\mathfrak{g}_{\Delta_5})^{\mathbb{Z}/2,\,K(1)}
\;\cong\;
\mathbb{C}\cdot\Delta_5
\]
shows that the Igusa cusp form is the classification invariant
itself. This is where Volume~I's shadow lens, Volume~II's composition
law, and Volume~III's Calabi--Yau origin story meet on a single
object.
...
\text{Argyres--Douglas}, \text{twisted}\}$: three decompositions agree
on one object (the Hall--Drinfeld $\SCchtop$ datum, the Kuga--Satake
Shimura variety
