...
$d_{\mathrm{fib}}^2 = \kappaChHodge(\cA) \cdot \omega_g$ at higher genus,
...
$\FM_k(\C) \times \Conf_k^{<}(\R)$ as an $E_1$ chiral
coassociative coalgebra; the full Swiss-cheese datum lives on the
derived-centre pair.

The two-structure thesis of this volume is the assertion that
these two pieces of data form the local bar datum of a
...
coderivation compatibility between them is the chain witness from
which the genus tower, the spectral $R$-matrix, and the PVA shadow
are read. The bulk-boundary-line triangle also needs the chiral
Hochschild centre and the mixed brace action.
...
differential $d_{\mathrm{fib}}^2 = \kappaChHodge(\cA) \cdot \omega_g$.
...
curved: $d_{\mathrm{fib}}^2 = \kappaChHodge(\cA) \cdot \omega_g$.
...
$\C$ becomes invisible in cohomology, the closed chiral colour
becomes topological, and Dunn additivity
...
on~$\C$. Without conformal vector the pair remains at $\SCchtop$.
...
characteristic $\kappaChHodge(\cA)$ controls the curved $A_\infty$
...
Genus~$1$: $\kappaChHodge(\cH_k) = k$, $F_1 = -k\log\eta(\tau)$. The
...
relation
$\kappaChHodge(\widehat{\fg}_k) + \kappaChHodge(\widehat{\fg}_{-k-2h^\vee})
= 0$.
...
$\mathrm{Ch}(\mathrm{Vect})$ being exactly the finite-propagation
assertion of
...
On cohomology $H^\bullet(A, Q)$, the binary $m_2$ decomposes into a
commutative product (regular
...
Higher operations vanish only on the formal or compatibly
nullhomotopic locus.
...
\paragraph{The K3 crown.}
The cleanest bulk-boundary-line test is the K3 Hall--Drinfeld boundary
target $\mathbf{H}_{\Delta_5}$. Here the triangle becomes genuinely
bi-based: the boundary colour factorises on
$\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24})$ with nearby cycles at the
$24$ Kodaira nodes, the parameter colour factorises on
$\overline{\mathcal{A}_2}$ in holonomic $\mathcal{D}$-modules with
regular singularities along the Humbert divisor $H_1\cup H_4$, and
the averaging map
\[
\mathrm{av}
\;=\;
...
\]
compares Drinfeld's ordered Hall presentation with the Siegel
paramodular one: Beilinson governs the first base, Kazhdan the
second, and Volume~II tests the two as one $\SCchtop$ datum. The factorization-homology centre
$\int_{S^1\times\mathrm{Ran}(X)}\cA$ carries the universal-centre
supertrace, and at this K3 point it reads
\[
...
\mathrm{wt}(\Delta_5),
\]
while the Mukai-self-dual companion has vanishing supertrace under the
Borcherds singular-theta pushforward.

On the holomorphic-topological side this same datum is the
Omega-deformed twisted-$11$D supergravity theory on $K3\times T^2$
with $24$ M5-branes on the Kodaira $I_1$ fibres; the one-loop
partition function is paramodular and the anomaly-cancellation
equation forces the Borcherds product weight
\[
\mathrm{wt}(\Delta_5)
\;=\;
c_{\phi_{0,1}^{K3}}(0)/2
...
5.
\]
The equality $5=2+3$ is the $N=1$ mnemonic for the K3 holomorphic
Euler characteristic and Kodaira-residue contribution. It has no
additive content for $\kappa_{\mathrm{BKM}}$.
Harvey--Moore, Dijkgraaf--Verlinde--Verlinde,
Dijkgraaf--Moore--Verlinde--Verlinde, and Witten give four duality
frames for the same form. Along the Humbert walls the smooth-locus
quasi-Hopf pentagon upgrades to the $\Ainf$ coherences
$\phi^{(n)}$, $n\geq 4$, governed by the higher Jacobi coefficients of
$\Phi_{10}^{\mathrm{un}}/\eta^{24}$, and the obstruction comparison is
...
gates. This is where Volume~I's shadow lens, Volume~II's composition
law, and Volume~III's Calabi--Yau origin story meet on one automorphic
comparator.

\paragraph{Quillen norm as BV one-loop determinant.}
The $\SCchtop$ bar-differential on $\mathbf{H}_{\Delta_5}$-observables
produces a factorisation-algebraic partition function $Z^{(1)}$ whose
Quillen norm $\|\Delta_5\|_Q^2$ is the 1-loop BV determinant:
\[
...
\]
Bismut--Gillet--Soulé 1988 Quillen-norm immersion applied to the
Kuga--Satake embedding
$\overline{\mathcal{A}_2}\hookrightarrow\overline{\mathcal{A}_{21}}$
supplies the identification; Yoshikawa 2004 equates the analytic
torsion with a period of $\Delta_5$; Bruinier--Kühn 2003 equates the
boundary Borcherds-lift contribution along $H_4$ with
$24\,L'(0,\Delta_5,\mathrm{std})$. The coefficient $24$ is
simultaneously the second Chern class of the rank-$21$ K3
transcendental lattice variation and the count of M5-brane sources at
Kodaira $I_1$ fibres: the BV determinant sees both.

\paragraph{Three-loop Arnold cancellation on $\mathrm{Conf}_4(E)$.}
On $\mathrm{Conf}_4(E_\tau)$ the four-point function $\phi^{(4)}$ is a
sum of $10$ Belokurov--Shavgulidze topologies including ladder
splitting $T_5$ and theta-graph $T_6$ in its two wheel orientations.
Every term cancels pairwise under the Arnold--Cohen flip
$H_1 \leftrightarrow H_4$ via the Arnold three-term relation
\[
...
\]
Thus $\phi^{(4)} = 0$ on $\mathrm{Conf}_4(E_\tau)$ for every $\tau$.
Cross-verification: at a generic point of the Humbert divisor $H_4$ the
period degenerates to a pair of elliptic periods and the admissible
$E_8$-weight condition fails by half-integer excess
$\widetilde{D}_4 = 1/2$, forcing $\phi^{(4)}$ to vanish on the
admissible lattice by a second independent route.

\paragraph{Humbert equals Argyres--Douglas.}
At $E_{\tau_1}\times E_{\tau_2}$ on the Humbert divisor the
Seiberg--Witten curve degenerates to a pair of elliptic curves glued
at a nodal point; Gaiotto--Moore--Neitzke 2009 Example~8.3 identifies
this degeneration with an $(A_1, A_{2N-1})$-type Argyres--Douglas
point. The four-fold sibling stratification
$\{\Psi, \Psi^{\deg}, \Psi^{\mathrm{tor}}, \Psi^{\mathrm{metap}}\}$ of
$\mathbf{H}_{\Delta_5}$ is compared with the class-$\mathcal{S}$ join
$\mathrm{CDT}\cup\mathrm{AD} = \{\text{regular}, \text{irregular},
\text{Argyres--Douglas}, \text{twisted}\}$ only after the finite
Hall/CoHA source, pairing, PBW/no-extra-relations, radical, parity,
completion, inverse-limit, and EK/Heegner gates: three decompositions
then become comparison faces of one target (the Hall--Drinfeld
$\SCchtop$ datum, the Kuga--Satake
Shimura variety
$K3\hookrightarrow\operatorname{Sh}(\mathrm{SO}(2,19))$, and the
class-$\mathcal{S}$ Seiberg--Witten curve degeneration).

\paragraph{Class-$\mathcal{S}$ $\mathcal{T}[A_1,\Sigma_{0,24}]$ Schur-index cross-check.}
Gluing $22$ $A_1$-trinions along $21$ tubes on $\Sigma_{0,24}$
assembles $\mathcal{T}[A_1,\Sigma_{0,24}]$ with
$(n_v, n_h) = (63, 88)$, by Chacaltana--Distler 2010 tinkertoy
assembly plus Shapere--Tachikawa 2008 vector/hypermultiplet counting.
The 4D-2D map of Beem--Lemos--Liendo--Peelaers--Rastelli--van Rees
2013 gives
$c_{4d} = (2n_v + n_h)/12 = 107/6$ and
$c_{2d} = -12\,c_{4d} = -214 = -2\cdot 107$. The $\chi_{2d}$ chiral
algebra at $c = -214$ is not $\mathbf{H}_{\Delta_5}$: the numerical
``$24$'' coincidence ($24$ punctures on $\mathbb{P}^1$ versus $24$
Kodaira $I_1$ fibres on elliptic K3) reflects a shared rank-$22$
automorphism pattern, but
$\mathrm{av}(\chi_{2d}[\mathcal{T}[A_1,\Sigma_{0,24}]])$ has
$\kappaChHodge = -107$ and
$\mathrm{av}(\mathbf{H}_{\Delta_5})$ has $\kappa_{\mathrm{BKM}} = 5$.
The decoupling is a theorem: the two rank-$22$ structures play
distinct roles, one as the number of internal tubes in
$\Sigma_{0,24}$, the other as the rank of the transcendental K3
lattice.

\paragraph{Chiral-Hochschild $W_\infty[c=-214]$ cocycles.}
The class-$\mathbf{M}$ topologisation ladder reaches the
$W_\infty^{\mathrm{top}}$ endpoint after the iterated-Sugawara
stress-tower hypotheses are supplied. At the
class-$\mathcal{S}$ value $c = -214$ the chiral-Hochschild generators
$e_3, e_4, e_5, e_6 \in \ChirHoch^2(\mathrm{Vir}_{-214})$ align
explicitly with Pope--Romans--Shen 1990 $W_\infty$-primaries:
$e_5 = W_5$ (Wang 1998 Proposition~4.2), and
\[
e_4
\;=\;
...
\]
The coefficient $107/11$ is fixed by the requirement that $e_4$ be
Virasoro-primary at $c = -214$; primarity is an open condition on a
one-parameter family, and the Pope--Romans--Shen identification picks
out the unique admissible value.

\paragraph{Conway $V^{s\natural}$ as metaplectic $\Psi^s$-image on Leech.}
The Duncan 2007 (Duke~139) Conway module $V^{s\natural}$ of central
charge $c = 12$ is the $\mathbb Z/2$-orbifold of the $24$-fermion
superalgebra $A(\Lambda_{24})$ on the Leech lattice, distinct from
the Monster $V^\natural$ of central charge $c = 24$. The metaplectic
extension of Scheithauer 2008 (Invent.\ Math.\ 172) realises
$V^{s\natural}$ as the $\Psi^s$ super-Borcherds image of
$\Lambda_{24}$, separate from the bosonic $\Psi$ image producing
$V^\natural$. The topologisation ladder therefore has two
Leech-lattice fixed points at different central charges: $c = 12$ via
$V^{s\natural}$, $c = 24$ via $V^\natural$. The factor of two is the
super-bosonisation ratio, not a gauge choice.

\section*{XIII.\quad Curved genus expansion}

At genus $1$ the fiberwise differential is curved:
$d^2_{\mathrm{fib}} = \kappaChHodge(\Bbound) \cdot \omega_1$ (Vol~I, Theorem~D).
For $g \ge 2$ the continuation
$d^2_{\mathrm{fib}} = \kappaChHodge(\Bbound)\cdot\omega_g$ uses the
modular-bootstrap acyclicity and degree-$2$ curved-Dunn comparison
hypotheses of Theorem~\ref{thm:curved-dunn-H2-vanishing-all-genera}. This
...
Proof obligations: completeness of the higher-genus Lagrangian,
the global geometry of $\cM_{\mathrm{vac}}(\cA)$, the
Calabi--Yau origin story of Volume~III, and the centre-equals-bulk
comparison map of the programme.
...
\item Root-sector scalar spectral Drinfeld strictification criterion
