...
%%   inscribe the explicit convergence radius rho_*(c) = c/6, extend
...
\begin{proof}
From Theorem~\ref{thm:shadow-tower-asymptotic-closed-form}
...
\ref{thm:shadow-tower-tier-4-closed-form} (Vol~I
...
(Frontier F6, 2026-04-17). The tempering conclusion
...
(cf.\ \ref{thm:shadow-tower-tier-4-closed-form}). Each polynomial
...
frontier of the present chapter, inscribed as
...
formula for principal $\cW_N$ (HZ-4 of Vol~I CLAUDE.md, with $H_N =
\sum_{j=1}^N 1/j$). At $N = 3$: $H_3 = 1 + 1/2 + 1/3 = 11/6$, so
...
$\beta_N = 6 + 4 + \ldots + (2N)$ at $N = 3$ (stress + W_3 = $6 + 4 = 10$).
...
(Conjecture~\ref{conj:wn-tempered-general}, Open Frontier F5 in
the Beilinson audit).
...
\item \emph{Derived from}: Vol~I
  \texttt{thm:shadow-tower-asymptotic-closed-form} (leading Laurent
...
(CLAUDE.md 2026-04-17 Wave 15 frontier question, paraphrased).
...
\item \textbf{Cross-links}: Vol~I
  \texttt{shadow\_tower\_higher\_coefficients.tex}
  (\texttt{thm:shadow-tower-asymptotic-closed-form},
  \texttt{thm:shadow-tower-tier-4-closed-form}) supplies the
...
\begin{remark}[Wave-14 anchor: tempered stratum closures across the
universal-conductor master]%
\label{rem:wave14-anchor-tempered-stratum-master}%
\index{Wave 14!tempered stratum closures}%
\index{Wave 14!frontier accounting}%
...
\item \textbf{(Schellekens 71 + Monster $V^\natural$.)} Closed by
 ...
 = 0$).
\end{itemize}
The non-degenerate locus on which the wave-14 climax theorem
...
all six wave-14 anchors of the climax theorem are otherwise
unconditional on this locus. This remark is inscribed solely as the
wave-14 cross-reference; the present chapter's status is unchanged.
\end{remark}
