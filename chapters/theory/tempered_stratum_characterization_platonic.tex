...
\emph{(3), (4)} The Fateev-Lukyanov $\cW_3$ cubic OPE has leading
coupling $\alpha_{\mathrm{FL}} = 2/\sqrt{5}$ (normalised so that the
$\cW$-generator $W$ has weight 3, OPE $T \cdot W \sim W \cdot W /
(z-w)^3$ at leading order). The binary-collision Riccati recursion for
$\Phi_r^{\cW_3}(c) := c^{r-2} S_r(\cW_{3, c})$ runs with the
replacement of the Virasoro master-equation factor $f_{\mathrm{Vir}}
(3, r-1) \cdot 3 \cdot S_3 = 6$ (at large $r$) by the $\cW_3$ factor
$f_{\cW_3}(3, r-1) \cdot 3 \cdot S_3 = \alpha_{\mathrm{FL}}^2 \cdot 3
\cdot 2 = 12/5 \cdot 3 = \text{coefficient dependent on the Fateev
normalisation}$. The derivation follows the four-step pattern of
Theorem~\ref{thm:shadow-tower-asymptotic-closed-form} (Vol~I)
generalised to include both the Vir-stress-tensor channel and the
$\cW$-generator channel. Explicit computation of the leading-tier
recursion yields $A_r^{\cW_3} = -10 (r-1) / r \cdot A_{r-1}^{\cW_3}$
(the factor $10$ is $6 + 4 = $ Vir-channel $6$ plus
$\cW$-generator-channel $4$, matching the $\kappa(\cW_3) = 5 \kappa(\mathrm{Vir}) /
3$ ratio at large $c$).
\end{proof}
