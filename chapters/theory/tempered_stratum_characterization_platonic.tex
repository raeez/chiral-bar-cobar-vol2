...
%% conj:wn-tempered-general (compatibility label for the conditional
%% finite-beta W_N criterion)
...
class~$\mathsf{M}$ at each fixed non-zero $\kappaChHodge(\cA)$, non-critical
...
the finite-envelope criterion below, with the harmonic beta law retained
as a conditional radius refinement and the old
\texttt{conj:wn-tempered-general} label retained as a compatibility
anchor.
...
the harmonic spin-lane candidate
\[
 \beta_N
 \;=\;
...
$\kappaChHodge(\cW_N)=c(H_N-1)$. The triangular and quadratic
extrapolations agree at $N=2,3$ only by accident and are separated by
the harmonic candidate at $N=4$.
\end{remark}
...
$15$, and the quadratic extrapolation gives $16$. All three differ from
the harmonic candidate. The harmonic candidate is rational-valued rather than polynomial-valued:
$\beta_5=77/5$ already rules out every integer-valued extrapolation as
a candidate under the kappa-ratio scaling law.
\end{remark}
...
The harmonic closed form for the ordinary convergence radius is
conditional on the beta-law surface.
...
\emph{Possible non-tempered loci.} The remaining
...
loci are the remaining non-principal cases not decided by this
chapter.
\end{corollary}
...
Theorem~\ref{conj:wn-tempered-general} on the finite-envelope locus. The
...
sector analysis respectively; both are explicit proof obligations.
\end{proof}
...
\texttt{compute/tests/test\_tempered\_stratum.py}. Each item below has
disjoint derivation and verification paths.
...
stratum of class~M at non-trivial central charge.
...
\item \textbf{Tempered principal locus}: every generic
 ...
 finite-envelope locus. The Banach $E_3$-model has its analytic input
 on that locus; raw direct-sum descent remains the finite-propagation assertion of
 Theorem~\ref{thm:chain-level-e3-original-complex-conditional}.
...
\item \textbf{Cross-links}: Vol~I
 ...
 and the chain-level $E_3$ theorem under
 $(\mathrm{H}_S)$. Vol~II
...
any principal $\cW_N$ rank where the finite envelope is proved) is
tempered. This supplies the Banach topologisation input. Raw direct-sum
topologisation is governed by finite propagation. The unproved
...
\item \textbf{($\cW_N$, $N \ge 4$.)}
 ...
 (the Platonic endpoint $\cW_\infty[\mu]\to\Einfty^{\mathrm{top}}$)
 ...
 \textsf{e\_infinity\_topologization.tex}; the present chapter does
 not assume that endpoint.
\item \textbf{(Logarithmic $\cW(p)$ triplet.)} The remaining theorem is
 ...
 and its proof obligation is the Adamovi\'c-Milas character expansion
 together with the corresponding log-modular coefficient bound. This does not
...
The single residual stratum is logarithmic $\cW(p)$;
