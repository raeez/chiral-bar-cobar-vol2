...
%% Local macro safety (providecommand only; AAP per V2-AP39)
...
The chapter constructs the five classical faces
(Chapter~\ref{ch:factorization-swiss-cheese} fixes the operadic face;
Chapter~\ref{ch:theory-equivalence} the Koszul dual; the
factorization, BV/BRST, and convolution faces follow)
and then inscribes two new faces: the Drinfeld-centre face
via the factorization-module category $\Rep_{\mathrm{fact}}(\cA)$,
and the derived-algebraic-geometry face via global sections of a
stack classifying $E_1$-chiral algebras with $\SCchtop$-datum.
...
$\End^{\mathrm{ch}}_\cA$ (Chapter~\ref{ch:theory-hochschild},
AP-CY62/AP-CY63 convention). The pair
\begin{equation}
...
(Proposition~\ref{prop:sc-koszul-dual-three-sectors}; FM155/FM156 heal).
...
Deligne; AP-CY64 three-Hochschild convention, chiral incarnation). By
...
chain-level presentation depends on $\Phi$ (FM158 heal; AP171). On the
Koszul locus, two associators yield strictly equivalent half-braidings
after applying the $\Phi_1\Phi_2^{-1}$ twist.
\end{remark}
...
target, yielding a $1$-shifted output (AP-CY34 shifted-cotangent
convention).
\end{remark}
...
$\Phi\in\mathrm{GRT}_1(\mathbb{Q})$ (FM158 heal). Strict chain-level
equivalences hold on the Koszul locus; away from it, the equivalences
are $\infty$-quasi-isomorphisms that become strict after passing to
$\Phi$-transported presentations (AP171). We suppress $\Phi$ from
edge names; each name carries its $\Phi$-dependence implicitly.
\end{convention}
...
Chapter~\ref{ch:theory-equivalence} (FM155 heal). The bar and
...
equivalence is $\Phi$-independent (AP171).
\end{theorem}
...
FM209 heal): the QME reads
$\partial S_{\mathrm{bulk-bdy}} + \tfrac{1}{2}\{S_{\mathrm{bulk-bdy}},
S_{\mathrm{bulk-bdy}}\}=0$ with coloured BV bracket extending
the bulk BV bracket by a boundary-to-bulk bracket, and this is the
MC equation for the pair (not for $\cA$ alone; AP-CY62).
\end{proof}
...
$\Phi$-independent at the level of cohomology (AP171); strictly
chain-level presentations track a single $\Phi$.
\end{theorem}
...
tensor $T(z)$ degenerates (FM78 heal): the normalisation factor
...
structure is FM155 heal. Face~(3) is Costello--Gwilliam Vol.~II;
brace structure is Tamarkin 2000. Face~(4) is Costello--Gwilliam Vol.~I
and Butson--Yoo 2018; logarithmic refinement is Chapter~\ref{ch:bv-construction}.
Face~(5) is Vallette coloured Koszul duality. Face~(6) is new to
this chapter, built on the half-braiding construction of
Definition~\ref{def:half-braiding-explicit} of
Chapter~\ref{ch:theory-hochschild} (AP-CY25 heal). Face~(7) is
