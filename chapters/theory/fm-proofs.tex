...
After quotienting $\C^3$ by translations and dilations, $\FM_3(\C)$ has real dimension $2(3-1) = 4$. Use quotient coordinates $w_1 = z_1 - z_3$, $w_2 = z_2 - z_3$, so the complex orientation form is
\[
...
\label{comp:D123}
The divisor $D_{\{1,2,3\}}$ corresponds to all three points approaching a common limit simultaneously. Then
\[
...
The FM compactification $\FM_4(\C)$ has real dimension $2(4-1) = 6$. Use quotient coordinates $w_i = z_i - z_4$ for $i = 1, 2, 3$. The boundary divisors $D_S$ with $|S| \geq 2$ are:
\begin{itemize}
...
\begin{theorem}[Truncation for cubic Landau--Ginzburg]
...
A non-zero integral requires a well-defined top-form. The naive measure $\prod d^2z_{v_i}$ is a top-form, but the holomorphic structure of the propagators imposes a sharper holomorphic-degree constraint, developed next.
...
Under the HT gauge-fixing of the cubic LG model,
