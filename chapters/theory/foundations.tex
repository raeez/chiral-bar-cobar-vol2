...
\begin{proof}
\textbf{Part~(a).}
By definition, $d_{\barB}$ acts on a bar element
$[a_1|\cdots|a_n] \in T^c(s^{-1}\bar{\cA})$ as
\[
  d_{\barB}[a_1|\cdots|a_n]
  = \sum_{1 \leq i < j \leq n}
    \pm\, [a_1|\cdots|m_2(a_i,a_j)|\cdots|\widehat{a_j}|\cdots|a_n],
\]
where $m_2(a_i,a_j) = \Res_{z_i = z_j} \mu(a_i,a_j)$ is the OPE
residue along the collision divisor $D_{\{i,j\}} \subset
\partial\FM_n(\C)$.  This is precisely the convolution recipe: form
the external product of $a_i$ and $a_j$ on the two-point space
$\FM_2(\C)$, restrict to the fiber product over the boundary stratum
$D_{\{i,j\}} \hookrightarrow \FM_n(\C)$, and push forward along the
collapse map $\FM_n(\C) \to \FM_{n-1}(\C)$ that merges $z_i$ and
$z_j$.  The higher operations $m_k$ for $k \geq 3$ arise from
deeper boundary strata --- iterated collisions --- which correspond
to iterated convolutions along higher fiber products.
...
\begin{proposition}[Koszul duality as Serre duality;
\ClaimStatusProvedHere]
\label{prop:koszul-serre}
\index{Koszul duality!as Serre duality|textbf}
\index{Serre duality!derived symplectic category}
Koszul duality $\cA \leftrightarrow \cA^!$ is Serre duality in the
derived symplectic category.  Precisely:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item The involution $(\text{-})^{!!} \simeq \id$
  ...
  locus\textup{)} is the geometric statement that complementarity is
...
bar and cobar.  (iii)~On the Koszul locus, the bar-cobar
...
T\cL_{\cA^!}$ is symmetric in its two summands, so
exchanging the roles of $\cA$ and $\cA^!$ gives
$T\cM|_{\cL_{\cA^!}} \simeq T\cL_{\cA^!} \oplus T\cL_\cA$, and
the second complement recovers $\cL_\cA$.
\end{proof}
