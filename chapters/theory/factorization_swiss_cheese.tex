...
\begin{align}\label{eq:cross-contribution}
  ...
  \cdot [s^{-1}a].
\end{align}
% Resolved: dbar K_Ar = +omega_Ar (eq:dbar-arakelov) with
% omega_Ar = (pi*i/2) Sum (Im tau)^{-1} omega wedge omega-bar.
% The boxed formula d_fib^2 = kappa*omega_g follows from Vol I Theorem D.

The bar differential $\dfib$ uses the propagator $K^{(1)}_{\mathrm{Ar}}$
of \eqref{eq:arakelov-genus-1}, whose $\bar\partial$ equals $+\omega_{\mathrm{Ar}}$
(Equation~\ref{eq:dbar-arakelov}; see also the
