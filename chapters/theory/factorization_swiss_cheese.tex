...
collection $\{m_k\}_{k \geq 2}$: the arity-$n$ Stasheff relation
$\sum_{i+j=n+1}\sum_s (-1)^{\epsilon} m_i(\ldots, m_j(\ldots),
\ldots) = 0$ is precisely the arity-$n$ Wess--Zumino
...
are all non-formal in the GKW sense, but differ in the arity at
...
into a single object whose arity-$k$ projection recovers
...
is the all-genera, all-arity master equation of which the
GKW consistency conditions are the genus-$0$, arity-$n$
projections.
...
Open & $F_{\mathsf{op}}$ on $\Ran(\R)$ &
 dg-shifted Yangian $Y_z^\hbar(\fg)$ \\
...
\smallskip\noindent
\emph{Part~(ii): Open-colour projection recovers $Y_z^\hbar(\fg)$.}
...
$\pi_{\mathsf{op}}(F_{V_k(\fg)}) \simeq Y_z^\hbar(\fg)$.
