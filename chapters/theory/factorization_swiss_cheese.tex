...
(Principle~\textup{\ref*{princ:e1-primacy}}), the ordered bar
...
Volume~I, Theorem~\textup{\ref*{thm:three-bar-complexes}}
...
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lccl}
\textbf{Class} & $r_{\max}$ & \textbf{Nonzero $m_k^{\mathrm{SC}}$} &
  \textbf{Mechanism} \\
\hline
$\mathbf{G}$ (Gaussian) & $2$ & none ($k \geq 3$) &
  abelian OPE; SC-formal at $d'=1$ \\
$\mathbf{L}$ (Lie) & $3$ & $m_3$ only &
  Lie bracket; Jacobi kills $m_4$ \\
$\mathbf{C}$ (contact) & $4$ & $m_4$ only &
  contact quartic; rank-one rigidity kills $m_5$ \\
$\mathbf{M}$ (mixed) & $\infty$ & all $m_k$, $k \geq 3$ &
  non-nilpotent $A_\infty$ cascade
\end{tabular}
\end{center}
Class~$\mathbf{G}$ is the key refinement: these algebras
(Heisenberg, lattice VOAs) are formal \emph{even at} $d' = 1$,
...
$r(z) = \Omega/z$
\textup{(}Remark~\textup{\ref{rem:mc3-ordered-bar-bridge}}\textup{)}.
...
\emph{only} in the closed sector $\gAmod^{\mathrm{mod}}$
