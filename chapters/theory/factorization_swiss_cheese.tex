...
The three canonical examples already exhibit the full range of complexity.
The Heisenberg algebra has trivial bar differential ($d_{\barB} = 0$, class~$\mathbf{G}$).
Affine Kac--Moody has nontrivial $m_2$ but $m_{k \geq 3} = 0$ (class~$\mathbf{L}$):
double-pole OPE, cubic shadow terminates.
Virasoro has genuinely infinite $A_\infty$ structure driven by the quartic pole (class~$\mathbf{M}$):
the shadow obstruction tower does not terminate.
Each computation is carried out in explicit coordinates with all signs and structure maps written out.
...
absorbing one power via the $d\log$ kernel (AP19: the bar kernel
...
$\SCchtop$ governs.  The next step is the global setting of
