...
\item \emph{Transferred SC operations.}
  Our $m_k^{\mathrm{SC}}$ arise from homotopy transfer of the
  $\SCchtop$-operad structure to the bar cohomology $H^*(\barB(\cA))$
  via the Kadeishvili tree formula.  The transferred operations
  satisfy the Stasheff $A_\infty$ relations by construction.
\end{enumerate}
...
\item the \emph{deconcatenation} coproduct on
  ...
  $\barB^{\mathrm{ord}}(\cA)$.
\end{enumerate}
The factorization Swiss-cheese algebra is the joint data of both
coproducts: the bar differential is the closed colour, the
deconcatenation coproduct is the open colour, and
the mixed-sector composition maps~\eqref{eq:bulk-action-composition}
intertwine them.  The symmetric bar, which retains only coproduct~(i),
...
bar complex is homotopy-Koszul dual to $\cA$ as an
$\SCchtop$-algebra; the symmetric bar complex is Koszul dual to
...
\emph{primitive} bar complex: it is the $E_1$ colour of the
$\SCchtop$-bar coalgebra from which the symmetric bar
