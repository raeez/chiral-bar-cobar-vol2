...
% Pentagon of equivalences for SC^{ch,top}: the BD/CG pair proved in this
% section extends to three further presentations (Koszul dual, BV/BRST,
% convolution). Installed as a Remark pending the dedicated Pentagon
% chapter; upgrade to \begin{theorem}[Pentagon]... when
% chapters/foundations/sc_chtop_pentagon.tex is created.
\begin{remark}[Pentagon of equivalences for $\SCchtop$]
\label{rem:sc-chtop-pentagon}
Theorem~\ref{thm:BD-CG-equivalence} proves the equivalence
$\mathsf P_1 \simeq \mathsf P_3$ of two presentations of the
Swiss-cheese chiral-topological coloured operad: the \emph{operadic}
presentation $\mathsf P_1$ (Voronov generators on
$\FM_k(\C) \times \Conf_m(\R)$, bulk-to-boundary structure) and the
\emph{factorization} presentation $\mathsf P_3$ (BD D-modules on
$\Ran(X)$ with boundary stratum along $\R \subset \C$, acting on the
pair $(\Zderch(A), A)$, never on $A$ alone; cf.\ AP165). Three further
presentations fit into a pentagon of pairwise equivalences:
...
chiral formality (2015); a chain-level proof is pending the dedicated
chapter \texttt{sc\_chtop\_pentagon.tex} in preparation. The
single-colour projection of the pentagon onto the closed colour
recovers the Chiral Hochschild Trinity of the three models
$C^\bullet_{\mathrm{chiral}}$ / $\mathrm{Ext}^\ast_{A^e}(A,A)$ /
$\mathrm{RHH}_{\mathrm{ch}}(A)$ (AP-CY63 bridge). The bulk-on-boundary
...
% universal_trace_identity.tex chapter is installed.
\begin{remark}[Pentagon substrate for the Universal Trace Identity; \ClaimStatusProvedElsewhere]
...
(Vol I \texttt{thm:K-Atiyah} $/$ GHOST IDENTITY).
...
conditional on \texttt{conj:trace-identity-chain-level} (Vol III).
The identity is verified numerically on the Vol III $\Z/N$ orbifold
table for $N = 1, \ldots, 8$; the naive $\kappa_{\mathrm{ch}} + \chi$
coincidence at $N=1$ is the shadow of this projection
structure at the boundary of the parameter range, not an additional
identity.
\end{remark}
...
Ch.~IV.5; cf.\ the presentability lemma of
\texttt{coderived\_models.tex}), hence combinatorial, and
...
\begin{remark}[Ordered vs.\ symmetric bar primacy; resolution of
  FM69]\label{rem:ordered-symmetric-bar-primacy}
Proposition~\ref{prop:R-twisted-sigma-n-descent} resolves the
FM69 critique that the classical Koszul argument for Theorem~A
...
% -------------------------------------------------------------------
\section{Wave-14 anchors: pentagon, heptagon, and the climax}
\label{sec:factorization-sc-wave14-anchors}
% -------------------------------------------------------------------
...
(\S\ref{sec:factorization-properads-theorem-A-infinity-2}, where
present), and the modular Steinberg correspondence
(\S\ref{sec:modular-steinberg-correspondence}, where present)---is
a finite atlas on the $\SCchtop$-pentagon of equivalent
presentations: (1) operadic, (2) Koszul-dual, (3) factorization,
(4) BV/BRST, (5) convolution. Wave-14 reconstitution upgrades the
pentagon to a heptagon by adjoining (6) Drinfeld-centre face
$\mathcal{Z}(\mathrm{Rep}_{\mathrm{fact}}(\cA)) \simeq
\mathrm{Rep}_{\mathrm{fact}}(\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA))^{E_2}$
via categorified bar--cobar with half-braiding (AP-CY25), and
(7) derived-AG face via PTVV on
$\mathrm{Map}(X \times \R_{\geq 0}, B\,\mathsf{SC}\text{-}\mathsf{Alg})$.
The seven edges are the named theorems of
\textsf{sc\_chtop\_heptagon.tex}; the present chapter contributes
the factorization edges (3) and (4) explicitly. Edge (1)
$\leftrightarrow$ (3) is the BD$\leftrightarrow$CG equivalence; edge (3)
$\leftrightarrow$ (5) is the factorization-convolution comparison via
...
canonical $\SCchtop$-datum on which all seven
presentations agree.
...
\index{Wave 14!universal-holography anchor (factorization)}%
...
\textsf{programme\_climax\_platonic.tex}) takes as input the
present chapter's factorization-Swiss-cheese apparatus.
For an algebra $\cA$ on a smooth projective curve $X$ with
conformal vector at non-critical level, the canonical $3$d HT
gauge theory $T_\cA$ on $X \times \R$ has boundary observables
$\Obs^\partial(T_\cA) \simeq \cA$ as
$\Eone$-chiral algebras and bulk observables
$\Obs^{\mathrm{bulk}}(T_\cA) \simeq \cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$
as $E_3$-topological factorization algebras after topologization.
The class coverage is
\begin{enumerate}[label=(\roman*)]
\item Class G/L (Heisenberg, affine KM at non-critical level)
via Costello--Li abelian holomorphic Chern--Simons applied to the
present chapter's factorization model;
\item Class C ($\beta\gamma$, $bc$) via FMS bosonization
reducing to Class G;
\item Class M (Virasoro, $\cW_N$) via Costello--Gaiotto
holomorphic Chern--Simons with DS boundary, completed by the
DS--Hochschild compatibility bridge \textsf{thm:chd-ds-hochschild}
(Volume~II, \textsf{chiral\_higher\_deligne.tex}) at chain level
in the weight-completed category;
\item Class FF (critical-level KM): stuck at $\SCchtop$, no
topologization, Feigin--Frenkel center realises the bulk
observables.
\end{enumerate}
The five rung corollaries
\textsf{cor:rung-heisenberg, cor:rung-affine-km, cor:rung-virasoro,
cor:rung-w-N, cor:rung-w-infty} are the family-by-family
specialisations; the iterated-Sugawara $E_\infty$-topological
ladder \textsf{thm:e-infinity-topologization-ladder}
(\textsf{e\_infinity\_topologization.tex}) realises the limit
rung. The non-logarithmic standard landscape sits inside the
unconditional reach of \textsf{thm:programme-climax}; logarithmic
$\cW(p)$ tempering (\textsf{conj:tempered-stratum-contains-wp})
remains conjectural pending the Adam\-ovi\'{c}--Milas
...
The Volume~I Koszul-reflection theorem \textsf{thm:koszul-reflection}
(in Vol~I's \textsf{chap:climax-platonic}) and the universal
ghost-charge formula \textsf{thm:universal-conductor}
(Vol~I's \textsf{chap:universal-conductor}) anchor into the
present factorization apparatus through the universal
trace-identity bridge
\textsf{conj:universal-trace-identity-volii-bridge}, which (when
realised) identifies the Vol~I universal-conductor functorial
trace with the Volume~II SC-pentagon convolution-complex trace
on the chiral derived center pair.
\end{remark}
