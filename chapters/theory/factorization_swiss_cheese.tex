...
% NOTE: This result is now a theorem; label kept as conj: for backward compatibility. Use \ref{conj:pentagon-factorization-convolution}.
...
\medskip
\noindent\textbf{Step~3: Quasi-isomorphism on the associated graded
(direct Koszul argument).}
Equip $\cP$ with the holomorphic weight filtration of
Proposition~\ref{prop:gr-chiral}. The Koszul inclusion
$\iota \colon \cP^{\text{\textexclamdown}} \hookrightarrow
\mathbf{B}(\cP)$ of Step~1 is a filtered map with respect to
this filtration (both sides inherit the filtration from~$\cP$;
cf.\ Theorem~\ref{thm:filtered-koszul}).
Since $\iota$ is a quasi-isomorphism of dg cooperads
(Theorem~\ref{thm:homotopy-Koszul}), the induced map on the
associated graded,
\begin{equation}\label{eq:gr-iota}
 \mathrm{gr}(\iota) \colon
 \mathrm{gr}\;\cP^{\text{\textexclamdown}}
 \;\hookrightarrow\;
 \mathrm{gr}\;\mathbf{B}(\cP),
\end{equation}
is a quasi-isomorphism of graded dg cooperads.
...
\[
 ...
 \;\longrightarrow\;
 \Hom_\Sigma(\mathrm{gr}\;\cP^{\text{\textexclamdown}},\,
 \End_\cA),
\]
acts on \emph{all} colour profiles $(k,m,c_{\mathrm{out}})$
simultaneously: no decomposition into pure and mixed sectors
is required. For each colour profile, the source
$\mathrm{gr}\;\cP^{\text{\textexclamdown}}(k,m;c_{\mathrm{out}})$
is levelwise finite-dimensional (the Koszul dual cooperad has
finitely many generators at each arity) and we work over a
field of characteristic~$0$.
The functor $\Hom(-,V)$ for a fixed cochain complex~$V$
sends quasi-isomorphisms between levelwise finite-dimensional
complexes to quasi-isomorphisms (the linear dual of a
quasi-isomorphism between finite-dimensional complexes is a
quasi-isomorphism).
Applying this at each colour profile with
$V = \End_\cA(k,m;c_{\mathrm{out}})$, we conclude that
$\mathrm{gr}(\iota^*)$ is a quasi-isomorphism on the total
$\Hom$-complex.

\medskip
\noindent\textbf{Step~4: Passage from associated graded to
filtered complex.}
...
filtered map of dg~Lie algebras, and Step~3 shows that
$\mathrm{gr}(\iota^*)$ is a quasi-isomorphism. The filtration
is bounded below (pole order~$\ge 0$) and bounded above at
each arity (at each arity~$n$, the maximal pole order on
$\FM_n(\C)$ is finite, since the total pole order of
an $n$-ary operation is bounded by the number and orders of
the collision divisors $D_S$, and there are finitely many such
divisors at each arity),
...
Steps~3--4 establish the quasi-isomorphism $\iota^*$ on all
colour profiles at once, without decomposing into pure closed,
pure open, and mixed sectors. Three observations record how
the argument specialises to each sector.
\begin{itemize}
\item On the \emph{pure closed} sector ($m = 0$,
 $c_{\mathrm{out}} = \mathsf{cl}$), the argument reduces to
 the classical brace-convolution correspondence of
 Getzler--Jones~\cite{GJ94}.
\item On the \emph{pure open} sector ($k = 0$,
 $c_{\mathrm{out}} = \mathsf{op}$), it reduces to the $A_\infty$
 Koszul duality for $E_1 = \mathsf{Ass}$
 (Loday--Vallette~\cite{LV12}, Theorem~10.1.13).
\item On the \emph{mixed} sector ($k \ge 1$, $m \ge 0$,
 $c_{\mathrm{out}} = \mathsf{op}$), the target
 $V_{k,m} = \Hom(\cA_{\mathsf{cl}}^{\otimes k} \otimes
...
\end{itemize}
The K\"unneth decomposition of the bar complex into pure and mixed
sectors (Lemma~\ref{lem:operadic-kunneth}) provides geometric
insight into this argument but is not logically required;
see Remark~\ref{rem:kunneth-geometric-insight}.
...
so both are cofibrant replacements of the same two-coloured
operad, connected by a zigzag
$Q_{\mathrm{Kos}} \xleftarrow{\sim} Q' \xrightarrow{\sim} Q_{\mathrm{bar}}$.
For a cofibrant dg operad~$Q$ and a $Q$-algebra structure
$\phi\colon Q\to \End_\cA$, the deformation dg~Lie algebra is the
derivation complex $\Der_\phi(Q,\End_\cA)$; it computes the
operadic cotangent complex and is independent of the cofibrant
replacement (Loday--Vallette~\cite{LV12}, Chapter~12).
For $Q=\Omega C$ quasi-free, $\Der_\phi(Q,\End_\cA)$ identifies
with the $\phi$-twisted convolution algebra
$\Hom_\Sigma(C,\End_\cA)[1]$ (\cite{LV12}, Theorem~11.3.3).
Taking $C=\cP^{\text{\textexclamdown}}$ gives the brace complex;
taking $C=\mathbf{B}(\cP)$ gives~$\gSC_T$.
The zigzag of cofibrant replacements then induces
the quasi-isomorphism~\eqref{eq:brace-convolution-qiso},
providing an independent proof route.
\end{remark}
...
Proposition~\ref{prop:star-topology}: all ten proved pairwise
equivalences cover all ten pairs, and the star structure alone
covers all ten pairs via paths of length at most~$2$.
\end{proof}
...
proved equivalence. All five non-hub direct links are now
proved, providing full redundancy across every edge and diagonal
of the pentagon. The star
