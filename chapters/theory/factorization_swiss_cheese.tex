...
\begin{align}\label{eq:dbar-arakelov}
  ...
  -\omega_{\mathrm{Ar}}(z),
\end{align}
% RECTIFICATION-FLAG: There is an internal sign inconsistency in this
% computation.  The correct value of \bar\partial_z Im(\int_w^z omega_beta)
% is (i/2) omega-bar_beta (since Im(f) = (f-fbar)/(2i) gives
% dbar Im(f) = -omega-bar/(2i) = (i/2) omega-bar).  Substituting (i/2)
% into line 1335-1347 gives second term = (pi*i/2)*Sum... = -(pi/(2i))*Sum...
% But the displayed intermediate at line 1343 shows +pi/(2i)*Sum... which
% corresponds to the OLD (wrong) value dbar Im = 1/(2i).  Also,
% with omega_Ar := -(pi/(2i))*Sum..., the corrected second term equals
% +omega_Ar (not -omega_Ar as claimed at line 1349).  The full resolution:
% either (a) change 1343 to -pi/(2i), change 1349 to = +omega_Ar, and
% accept dbar K_Ar = +omega_Ar = pi*omega_g (off-diagonal Green's function
% equation with positive sign and pi normalization); or (b) redefine
% omega_Ar with opposite sign.  The FINAL result d_fib^2 = kappa*omega_g
% is correct (Vol I Theorem D, eq:dfib-sq-final-heisenberg); the intermediate
% signs here are a notational inconsistency that requires independent
% verification.  Do NOT use eq:dbar-arakelov without resolving this flag.
...
\begin{equation}\label{eq:dbar-eta-1}
  ...
  dz \wedge d\bar z,
\end{equation}
where $\omega_{\mathrm{Ar}}$ is a $(1,1)$-form on~$\Sigma_1$ satisfying
$\int_{\Sigma_1}\omega_{\mathrm{Ar}} = -1$.  The unit-integral Arakelov
form is $\omega_1 := -\omega_{\mathrm{Ar}} = \tfrac{i}{2\operatorname{Im}\tau}\,
dz \wedge d\bar z$.
\end{proposition}
...
\begin{align}\label{eq:cross-contribution}
  ...
  \cdot [s^{-1}a].
\end{align}
% RECTIFICATION-FLAG: The intermediate computation above uses the propagator
% eta^{(1)} (Proposition prop:arakelov-genus-1-derivation), whose correction
% term C(u) = 2*pi*i*Im(u)/Im(tau) carries a factor 2*pi*i.  The bar
% differential is defined using K_Ar (eq:arakelov-genus-1), whose correction
% term is (pi/Im(tau))*Im(u) (real coefficient, no 2*pi*i).  The relationship
% K_Ar = eta^{(1)}/(2*pi*i) holds only for the holomorphic parts; the
% correction terms involve a passage between complex and real normalizations
% that requires independent verification.  The boxed formula below is correct
% by Volume I Theorem D and the independent affine KM computation
% (eq:dfib-sq-affine); the step ``= kappa*omega_1'' at line 2832 is a
% placeholder for that agreement.

The bar differential $\dfib$ uses the propagator $K^{(1)}_{\mathrm{Ar}}$
of \eqref{eq:arakelov-genus-1}, whose $\bar\partial$ equals $-\omega_1$
(Equation~\ref{eq:dbar-arakelov}; subject to the normalisation
reconciliation flagged there; see also the
affine KM computation \eqref{eq:dfib-sq-affine}).
Contracting with OPE data and applying Volume~I Theorem~D:
...
\begin{equation}\label{eq:correction-dbar}
  ...
  -\omega_{\mathrm{Ar}}(z).
\end{equation}
% RECTIFICATION-FLAG: The sign here (-omega_Ar) is consistent with the
% internal computation of eq:dbar-arakelov only pending resolution of the
% normalisation flag there.  The result d_fib^2 = kappa*omega_g is
% correct (Vol I Theorem D); the intermediate sign of dbar R should be
% verified independently once eq:dbar-arakelov is resolved.
