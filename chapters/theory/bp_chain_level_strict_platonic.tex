...
\[
 ...
 \mathrm{Deck}(\pi) = \Z/2.
\]
Pulling the Kazhdan grading to~$\widetilde X$ integralises it; descending via $\Z/2$-invariants returns statements to~$X$. That route closes the cohomological $\Ethree^{\mathrm{top}}$ statement for BP on the original complex on~$X$ via the general good-graded theorem \cite[Theorem~\ref*{thm:E3-topological-DS-general-all-good-graded}]{Vol2-fractional-ghost-platonic}. The cohomological statement uses $T_{\mathrm{DS}}(f) = [Q_{\mathrm{CS}}, G'_{f,X}]$ on $Q_{\mathrm{CS}}$-cohomology on~$X$; the strict chain-level statement asks for the same identity on cochains in $\cO_{\mathrm{bulk}}$.
...
\eqref{eq:bp-central-charge-FL} above) with the Feigin--Frenkel dual
...
sides of \eqref{eq:bp-central-charge-FL} pull back to $\widetilde X$ via
...
\begin{proposition}[BP Koszul conductor and its $\kappa$-complementarity image]
\label{prop:bp-kappa-conductor}
\index{Bershadsky--Polyakov algebra!Koszul conductor identity|textbf}
\ClaimStatusProvedElsewhere
The BP Koszul conductor $K_{\mathrm{BP}}(k) \coloneqq c^{\mathrm{BP}}(k)
+ c^{\mathrm{BP}}(-k-6)$, the self-contraction of central charge across
the Feigin--Frenkel involution $k \mapsto -k-6$, is a level-independent
constant. In the Arakawa convention $c(\mathrm{BP}_k) = 2 - 24(k+1)^2/(k+3)$,
\begin{equation}\label{eq:bp-kappa-conductor}
 K_{\mathrm{BP}}(k) \;\equiv\; 196 \qquad \text{as a polynomial identity in $k$,}
\end{equation}
where $\mathrm{BP}_{-k-6} = \cW^{-k-6}(\fsl_3, f_{\min})$ is the
operadic Koszul-dual BP-algebra at shifted level. The conductor
$K_{\mathrm{BP}} = -c_{\mathrm{ghost}}(\mathrm{BRST})$ is the Trinity
invariant of the boundary Koszul pair, matching Vol~I
Theorem~\ref{V1-thm:bp-koszul-conductor-polynomial}; the Fateev--Lukyanov
convention~\eqref{eq:bp-central-charge-FL} gives $K^{\mathrm{FL}}_{\mathrm{BP}}
\equiv 50$ (Remark~\ref{rem:anchor-bp-self-duality}). The derived-centre
$\kappa$-complementarity invariant is the distinct value
\begin{equation}\label{eq:bp-kappa-complementarity}
 \kappaChHodge(\mathrm{BP}_k) \;+\; \kappaChHodge(\mathrm{BP}_{-k-6})
 \;=\; \varrho_{\mathrm{BP}}\, K_{\mathrm{BP}}
 \;=\; \tfrac{1}{6}\cdot 196 \;=\; \tfrac{98}{3},
\end{equation}
the BP entry of the complementarity spectrum
$\{0, 8, 13, 250/3, 98/3\}$, with generator-profile coefficient
$\varrho_{\mathrm{BP}} = 1/6$.
\end{proposition}

\begin{proof}
In the Arakawa convention $c(\mathrm{BP}_k) = 2 - 24(k+1)^2/(k+3)$,
the involution $k \mapsto -k-6$ sends $(k+1) \mapsto -(k+5)$ and
$(k+3) \mapsto -(k+3)$, so $c(\mathrm{BP}_{-k-6}) = 2 +
24(k+5)^2/(k+3)$; summing,
\[
 K_{\mathrm{BP}}(k) = 4 + \frac{24\bigl[(k+5)^2 - (k+1)^2\bigr]}{k+3}
 = 4 + \frac{24 \cdot 8(k+3)}{k+3} = 4 + 192 = 196,
\]
the level-independent constant of \eqref{eq:bp-kappa-conductor};
the pole at $k=-3$ is removable because the two summands carry
equal-and-opposite residues. The full symbolic verification across
both conventions is recorded in Vol~I
\Cref{V1-thm:bp-koszul-conductor-polynomial}. The
$\kappa$-complementarity image \eqref{eq:bp-kappa-complementarity}
follows from the bridge identity
$\kappaChHodge + \kappaChHodge^! = \varrho_A K$ with the BP
generator-profile coefficient $\varrho_{\mathrm{BP}} = 1/6$ read off
the strong-generator weights $(J_{h=1}, G^{\pm}_{h=3/2}, T_{h=2})$;
the Koszul conductor $K = c + c^!$ and the $\kappa$-complementarity
sum $\kappa + \kappa^!$ are distinct invariants of the same Koszul
pair (the former a central-charge self-contraction, the latter a
chiral-Hodge supertrace), related by $\varrho_{\mathrm{BP}}$ and never
to be conflated.
\end{proof}

\begin{remark}[Role of $K_{\mathrm{BP}} = 196$ in the present chapter]
\label{rem:bp-role-of-K-196}
The Koszul conductor $K_{\mathrm{BP}} = 196$ does not enter the
...
\section{Independent verification by three disjoint routes}
...
$c^{\mathrm{BP}}(-3/2) = -(0)(-7/2)/(3/2) = 0$ at the boundary level,
and $c^{\mathrm{BP}}(0) = -(3)(1)/3 = -1$ at generic level $k = 0$.
...
\begin{remark}[Companion computation]
\label{rem:bp-compute-module}
The symbolic verification lives in
\verb|compute/tests/test_bp_chain_level_strict.py|, with derivation
input declared as the class-L antighost closure of Vol~I and the
branched-cover Galois descent of Vol~II, and verification input as
the Kac--Roan--Wakimoto BRST cohomology, the de Boer--Tjin screened
free-field realisation, and the symbolic $\fsl_3$ Wick at $k = -3/2$.
The three verification routes operate on disjoint derivation paths.
\end{remark}

\section{Position in the topologisation programme}
...
$K_{\mathrm{BP}}(k) = c^{\mathrm{BP}}(k) + c^{\mathrm{BP}}(-k-6) \equiv 196$
(Arakawa convention), with derived-centre $\kappa$-complementarity
image $\kappaChHodge(\mathrm{BP}_k) + \kappaChHodge(\mathrm{BP}_{-k-6})
= \varrho_{\mathrm{BP}} K_{\mathrm{BP}} = 98/3$
...
Three prior structures supply the inputs: the Vol~I file
\texttt{chapters/theory/topologization\_chain\_level\_platonic.tex}
supplies the class-L antighost closure for affine $V_k(\fg)$ at
non-critical level upstairs; the Vol~II fractional-ghost chain-level
companion supplies the branched-cover integralisation and Galois
descent for good $(1/d_f)$-graded nilpotents; the Vol~II file
\texttt{chapters/connections/e\_infinity\_topologization.tex} carries
the general $E_\infty$-topologisation programme, of which the
class-M-adjacent BP strengthening is a piece.
...
Remark~\ref{rem:bp-disjointness}. The companion computation is
\verb|compute/tests/test_bp_chain_level_strict.py| per
