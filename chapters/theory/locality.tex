...
\subsection{The Two-Colored Operad $\mathsf{SC}^{\mathrm{ch,top}}$}
\label{subsec:sc-def}

An HT observable lives on a rectangle $D \times I \subset \C \times
\R$: holomorphic in the $\C$-factor and locally constant in the
$\R$-factor. Its operations must therefore compose configurations
in both factors while respecting the directionality of the coupling
(collision data flows from $\C$ to $\R$, not the reverse). The
two-colored operad encoding exactly this content is built
componentwise from $\FM_k(\C)$ and $E_1(m)$, with the
no-open-to-closed rule imposed by setting the would-be
ch-producing mixed operations to the empty space.
...
\subsection{Recognition: HT Prefactorization $\Leftrightarrow$ $W(\SCchtop)$-Algebra}
...
$D \times I \subset \C \times \R$ and show that their algebra of
decompositions is exactly the operadic algebra structure prescribed
by $W(\SCchtop)$.
...
To extract the full operadic structure one needs a descent principle
that propagates the rectangle data to arbitrary opens. Weiss cosheaf
descent is the correct principle: it is tailored exactly to the
factorization axiom (cover by opens that see every finite
configuration), and for product manifolds it factors along each
factor.
