...
$\SCchtop$ is a \emph{coloured dioperad} in the sense of
Gan~\cite{Gan03}, not a standard (uncoloured or symmetric-coloured)
operad. Directional colour-restriction is intrinsic: an HT
...
topological boundary ($\mathsf{top}$). Equivariance holds within
each colour class but not across the colour boundary: a permutation
interchanging a $\mathsf{ch}$ input with a $\mathsf{top}$ input
would produce operations whose output colour is not preserved. This
is the defining content of a dioperad: directed edges in the
underlying composition graph admit colour-restricted (rather than
$\Sigma$-symmetrised) composition. Dunn additivity, an operadic
additivity statement for uncoloured little-disk operads, therefore
does \emph{not} apply to $\SCchtop$; the tensor-product formulas
$E_p \otimes E_q \simeq E_{p+q}$ are statements about symmetric
operads and are unavailable for coloured dioperads with directional
colour-restriction.

The holomorphic--topological observable content is built componentwise
from $\FM_k(\C)$ and $E_1(m)$, with the no-open-to-closed rule
imposed by setting the would-be ch-producing mixed operations to the
empty space.
...
\begin{proof}
The dioperad axioms follow from the component structure.
\emph{Associativity:} composition in $\SCchtop$ is componentwise FM insertion on the $\FM_k(\C)$ factor and interval composition on the $E_1(m)$ factor; since these factors are independent, associativity of the composite follows from associativity of each factor separately.
...
\emph{Within-colour $\Sigma$-equivariance:} the symmetric group $\Sigma_k$ acts on the $\FM_k(\C)$ factor by permuting input labels (with trivial action on $E_1(m)$, whose inputs carry a fixed linear ordering); $\Sigma_k$-equivariance of FM operadic insertion gives within-closed-colour equivariance for $\SCchtop$. No equivariance across the colour boundary is postulated, consistent with the dioperad axiomatic.
...
structure $\C \times \R$ essentially: the factorization of the
...
(conformal vector, non-critical level, chain-level $Q$-exactness
witness), proved for affine Kac--Moody at non-critical level in
