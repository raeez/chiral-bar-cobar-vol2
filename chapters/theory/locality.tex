...
The open-sector factorization category~$\cC$ of a 3d HT theory on $\C \times \R$ has a boundary algebra $A_b = \End_\cC(b)$ that carries two interacting structures: a holomorphic OPE from collisions in $\FM_k(\C)$, and an $A_\infty$ composition from the ordered topology of $\Conf_k^<(\R)$.  The two-colored operad $\SCchtop$ encodes precisely this coupling: its closed color $\cP^{\mathrm{ch}}$ governs holomorphic collisions, its open color $\Eone$ governs topological ordering, and the Swiss-cheese constraint (no open-to-closed) reflects the bulk-to-boundary directionality.  An $A_\infty$ chiral algebra is an algebra over $\OHT := C_\ast(W(\SCchtop))$.
...
The space $\Conf_n(\C)$ is a smooth manifold of real dimension $2n$, noncompact.  Integration of differential forms over $\Conf_n(\C)$ (the mechanism by which chiral OPE residues are extracted) requires a compactification that records the relative rates at which points approach each other.  The Fulton--MacPherson compactification \cite{FM94} is the canonical choice:
