...
$\OHT := C_\ast(W(\SCchtop))$; the boundary algebra $A_b =
...
Closing the gap between the prefactorization data (defined on
...
cosheaf descent on the basis of rectangles. Applying the
...
\label{rem:locality-wave14-anchor}%
\index{HT locality!universal-holography anchor}%
\index{Wave 14!HT locality anchor}%
The HT prefactorization recognition theorem
(Theorem~\ref{thm:recognition-SC}) is the local-to-global engine
behind the universal-holography master theorem
\textsf{thm:universal-holography-master} (Volume~II,
\textsf{programme\_climax\_platonic.tex}). The product Weiss
...
recognition theorem assembles the two factors via the
Boardman--Vogt resolution of $\SCchtop$, producing a
$C_*(W(\SCchtop))$-algebra structure on the boundary observables.
...
remains in the generic regime: this is the FF stratum of the
infinite-fingerprint classification
(\textsf{infinite\_fingerprint\_classification.tex}, $\kappa = 0$).
The directionality guard $\SCchtop \neq E_3$ (AP163, AP167)
is enforced throughout: the present recognition theorem produces
$\SCchtop$ structure, never $E_3$ structure directly; the
topologization step is a separate theorem with explicit
hypotheses (conformal vector + non-critical level + chain-level
$Q$-exactness witness, proved for affine KM at non-critical level
in \textsf{thm:E3-topological-km}; conjectural for general
chiral algebras with conformal vector;
chain-level for class M in the weight-completed category via
\textsf{thm:programme-climax}).
\end{remark}
