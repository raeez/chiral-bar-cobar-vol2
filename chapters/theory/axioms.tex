...
The K3 chiral bialgebra $\cA = \mathbf{H}_{\Delta_5}$ of Vol~III,
Ch.\,\ref{ch:k3-chiral-bialgebra-platonic}, fits the axiomatic
framework above exactly once the curved $A_\infty$ extension
(Definition~\ref{def:curved-ainfty-chiral}) is invoked: the
chain-level generators realise $V\subset\cA$ of homogeneous
OPE-weight~$1$, the quadratic Miki commutators realise $m_2$
purely in $V\otimes V$, and the Borcherds-imaginary-root relations
$R_{\mathrm{Borcherds}}$
\textup{(}Vol~I, Ch.\,\ref{ch:chiral-climax-platonic},
Remark~\textup{\ref{rem:k3-koszul-structure-precise}}\textup{)}
...
\]
equal to the Gritsenko--Nikulin denominator cocycle read
automorphically. The $n=1$ curved relation of
Definition~\ref{def:curved-ainfty-chiral}, $m_1^2 = [m_0, -]$,
encodes the fact that the bar differential on the K3 input fails
to square to zero by exactly the Humbert-divisor obstruction: on
...
\smallskip
\noindent\emph{Hochschild concentration is $\{0,1,2,3\}$, not
$\{0,1,2\}$.} Theorem~H of the shared five-theorem core states
$\ChirHoch^\bullet(\cA) = 0$ for $\bullet \notin \{0,1,2\}$ on the
standard chiral landscape \textup{(}Vol~I,
Theorem~\textup{\ref{thm:theorem-h-platonic-pointer}}\textup{)}.
This concentration extends on the K3 input to one degree higher:
\[
  \ChirHoch^n\bigl(\mathbf{H}_{\Delta_5}\bigr) \;=\; 0
  \quad \text{for } n \notin \{0,1,2,3\}.
\]
The extra degree is forced by the CY-$3$ geometry of $K3\times E$
under the CY-to-chiral functor $\Phi$: the Hochschild cohomology of
$D^b(\mathrm{Coh}(K3\times E))$ has cohomological support in
$\{0,1,2,3\}$ by Gerstenhaber--Serre duality on the CY-$3$
category, and $\Phi$ transports this support verbatim to the
chiral side. Theorem~H therefore \emph{scopes} on K3 input: the
concentration $\{0,1,2\}$ is the standard-landscape statement, and
the $\{0,1,2,3\}$ extension is a K3-specific boundary feature.
The extension is tight: $\ChirHoch^3\bigl(\mathbf{H}_{\Delta_5}\bigr) \neq 0$
because the Borcherds imaginary-root direction carries a non-trivial
degree-$3$ class, represented chain-level by the Schiffmann--Vasserot
...
bar-cobar inversion of Theorem~B \textup{(}Vol~I,
Theorem~\textup{\ref{thm:theorem-b-platonic-pointer}}\textup{)}
holds for $\mathbf{H}_{\Delta_5}$ in the curved-$A_\infty$
category with the curved bar functor of Positselski\,2011, Memoirs
AMS\,212, Ch.~$7$; the weight-completed coderived-contraderived
equivalence survives the $m_0\ne 0$ deformation because
...
Remark~\textup{\ref{rem:k3-koszul-structure-precise}}, valued in the
Positselski curved CDG bar resolution.
\end{remark}

\section{Synthesis}
\label{sec:axioms-wave13-DNA}

\begin{remark}[Vol~II axioms of $\mathsf{SC}^{\mathrm{ch,top}}$ at the K3 synthesis base]
\label{rem:axioms-DNA-1}
The Vol~II axioms of $\mathsf{SC}^{\mathrm{ch,top}}$ ($3$D HT QFT)
instantiate at the K3 synthesis base with: chiral algebra
$\mathbf H_{\Delta_5}$ at the Schur sector of class-$\mathcal S$ $A_1$
on $\Sigma_{0,24}$, central charges $c_{4d} = 107/6, c_{2d} = -214$
...
with averaging $\mathrm{av} = \mathrm{Torelli}_{K3}\circ\mathrm{KugaSatake}\circ j_{\mathrm{Kod}}$.
The $3$D HT QFT's holomorphic-twist axiom matches Costello 1-loop output
giving $\Delta_5$ as character \cite{CostelloHolomorphic,Gaiotto2009}.
Cross-ref Vol.~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[axiomatic refinement]
\label{rem:axioms-DNA-2}
At the K3 synthesis base, the Vol~II axioms refine: (i) factorisation
axiom restricts to bi-based Ran; (ii) holomorphic-twist axiom produces
Igusa $\Phi_{10}$ denominator; (iii) Drinfeld-double axiom produces
$\mathbf H_{\Delta_5} = \mathcal D_\hbar(\mathcal Y^{\mathrm{Hall}}_\hbar, \widetilde\Phi^{\mathrm{Sieg\text{-}Bor}}_{\mathrm{Sp}_4}, R_{\mathrm{Sieg,dyn}})$;
(iv) $R$-matrix axiom produces $R_{\mathrm{Sieg,dyn}}$ via Pasol--Zagier
2013. The universal identity is the axiom (v) that ties (i)-(iv)
\cite{PasolZagier2013,EtingofKazhdan2007PartV}. Cross-ref
