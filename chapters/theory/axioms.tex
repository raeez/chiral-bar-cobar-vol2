...
The boundary chart $A=A_b$ is only one member of the primitive
package. The reduced chiral bar coalgebra
$\barB^{\mathrm{ch}}(A)=T^c(s^{-1}\bar A)$ is a coalgebra resolving
twisting morphisms out of~$A$. The linear Koszul dual $A^!$ is the
cobar algebra built from the dual cooperadic data; it is not
$\barB^{\mathrm{ch}}(A)$ and is not the same object as the
coaugmentation coideal $A^i$. The derived chiral centre
...
dioperad whose open colour acts on~$A$ and whose closed colour acts
...
$f(\lambda) \mapsto f(-\lambda-\partial)$, which is precisely the
vertex-algebra skew-symmetry substitution. The visible exchange sign
is the ordinary Koszul sign $(-1)^{|a||b|}$; the shifted signs enter
the Jacobi and Leibniz identities through the bar desuspension.
...
cover. The Hall--Drinfeld-double axiom produces
...
the $R$-matrix axiom produces $R_{\mathrm{Sieg,dyn}}$ via the
Pasol--Zagier Kronecker--Eisenstein series. The conductor identity
\[
\hbar_{\mathrm{QG}}^2 K^{\kappa_{\mathrm{ch}}}=-1,\qquad
