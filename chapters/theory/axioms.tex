...
tautological swap $\tau$. This is the precise content of
V2-AP1: $\Einf$ admits pole-bearing realisations; the discriminant
from $\Eone$-chiral is locality, not pole content.
\end{enumerate}
\end{proposition}
...
The programme uses the Kac--Kazhdan / Shapovalov determinant as a
\emph{vanishing-locus control} underlying the class-M shadow tower, the
critical-level (class FF) structure, and the minimal-model null-vector
corrections (FM76 heal). The following three items fix the scope.
...
Proposition~\ref*{V1-prop:minimal-model-non-koszul} and FM76~heal).
Class M thus persists on the simple quotient at minimal-model loci.
This remark makes explicit the vanishing-locus input tacit in the
...
No new theorem; this remark consolidates the Kac--Kazhdan/Shapovalov
input used throughout Vol.~II (42+ citation sites across the holographic
reconstruction, gravitational-Yangian, soft-graviton, and bar-cobar-review
chapters), and fixes the positive-codimension convention invoked (implicitly)
by the $r_{\max}=\infty$ definition of class~M.
\end{remark}
...
purely topological $E_1$ bar.
%
% AP165: the full SC^{ch,top} structure lives on the derived
% chiral center pair (C^bullet_ch(A,A), A), not on C_A alone.
% The lift from (ChirAss)^!-coalgebra to B(SC^{ch,top})-coalgebra
% uses the promoted pair (A,A) and is established in Step~2
...
$r(z)=\Omega/((k+h^\vee)z)$ diverges at $k=-h^\vee$ and
\emph{must} be excluded there; the $k=0$ diagnostic of AP126
(level-stripped $r=0$) is a fact about the rescaled
$r^{\mathrm{Sug}}(z)=k\,\Omega/((k+h^\vee)z)$, not about the
Killing--Casimir $r$-matrix, and both diagnostics coexist with
exclusion of the pole at the critical level.

\textbf{Protocol for authors.} Before writing any statement that
uses $(k+h^\vee)^{-1}$ or the Sugawara $T(z)$:
\begin{enumerate}
\item[(a)] state the hypothesis $k\neq -h^\vee$ explicitly, or
state that the formula is a rational function with a simple pole
at the critical level;
\item[(b)] if a corollary is claimed at \emph{all} levels, either
produce the companion critical-level statement (pointing to the
Feigin--Frenkel invariants) or restrict the corollary's scope;
\item[(c)] never write ``for all $k$'' when the proof uses
$T_{\mathrm{Sug}}$, Sugawara conformal-vector hypotheses, or the
KZ/KZB denominator $(k+h^\vee)$.
\end{enumerate}
...
algebraic structure (the Feigin--Frenkel centre) takes over;
per the fingerprint classification, the programme's Platonic
target is the five-class stratum $\{G,L,C,M,FF\}$ with $FF$
the canonical $\kappa=0$ companion class.
\end{remark}
...
This remark closes V2-AP24: the $S$-transform (closed, complex
structure) and the Wick rotation of $R$ (open, $E_1$ ordering) are
distinct algebraic data that coincide numerically on the Euclidean
torus slice in the proved scope above, not universally.
\end{remark}

\subsubsection{$(H1)$--$(H5)$ as the FMP on the derived chiral algebra stack}
\label{subsec:h-axioms-from-fmp}

The enumerated hypotheses $(H1)$--$(H5)$ circulating in Vol~II
(one-loop finiteness; propagator meromorphicity and $\R$-regularity;
configuration-space convergence and AOS relations; factorization
compatibility; conformal-vector admissibility) are not a four- or
five-element list of ad hoc analytic assumptions.  They are the
axioms of a \emph{formal moduli problem} (FMP, in the sense of
\cite[Thm.~2.2.4]{Pridham} and \cite[Ch.~15, Thm.~1.3.12]{HA})
attached to the derived stack
...
\begin{remark}[Five $E_1$-chiral notions and the present axioms (AP161)]
\label{rem:axioms-five-e1-notions}%
\index{E_1-chiral algebra@$\Eone$-chiral algebra!five notions}%
\index{AP161!five $E_1$-chiral notions}%
The single phrase ``$\Eone$-chiral algebra'' is used in the
literature for at least five distinct objects, none of which is
interchangeable with another at chain level. We anchor the
present axiomatic notion among them once, canonically.
(A) Strict $\mathrm{ChirAss}$-algebra in $\mathrm{D}\text{-}\mathsf{mod}(X)$
(Beilinson--Drinfeld pseudo-tensor): the bare strict associative
chiral algebra. (B) $A_\infty$-algebra in
$\End^{\mathrm{ch}}_\cA$: the present
...
of Remark~\ref{rem:operadic-formulation}. (C) Etingof--Kazhdan
quantum vertex algebra: the Drinfeld-associator-fixed
...
homotopy-coherent $\Eone$-algebra in
$\Fact^{\mathrm{ord}}(X)$, native to the GR17 IV.5
$(\infty,2)$-categorical ambient. (E) Factorization algebra on
$\Ran^{\mathrm{ord}}(X)$: the Francis--Gaitsgory geometric
incarnation, with $n$-ary operations on
$\Conf_n^{\mathrm{ord}}(X)$ that are not required to descend to
$\Sigma_n$-coinvariants. The present axioms produce~(B);
...
locus via the standard rectification of $A_\infty$-algebras to
algebras over a cofibrant operad
(\textsf{thm:rectification}); (D) $\leftrightarrow$ (E) by
Francis--Gaitsgory factorization recognition; (A) is
\emph{strictly weaker} than (B)--(E) (no homotopy data,
no associator). The downstream chapters of this volume use
(B) on the formal disk and (D)/(E) on the global curve;
no statement of this volume requires (A) alone, and every
statement that mentions (C) carries an explicit Drinfeld-associator
choice. The five notions are not synonyms; the axioms here
\emph{specifically} produce (B), and the bridges to (C)/(D)/(E)
are theorems, not definitions.
\end{remark}

\begin{remark}[Wave-14 anchors: bar axioms feed the universal-holography master]
\label{rem:axioms-wave14-anchor}%
\index{Wave 14!axioms anchor}%
...
\textsf{programme\_climax\_platonic.tex}).
Each rung corollary specialises this axiomatic input:
\textsf{cor:rung-heisenberg} takes
$\cA = \cH_k$ with $\kappa(\cH_k) = k$, abelian Sugawara
$T = \tfrac{1}{2k}{:}JJ{:}$, $c = 1$;
\textsf{cor:rung-affine-km} takes $\cA = V_k(\fg)$ with
$\kappa(V_k(\fg)) = \dim(\fg)(k+h^\vee)/(2h^\vee)$, Sugawara
stress at non-critical level;
\textsf{cor:rung-virasoro} takes $\cA = \mathrm{Vir}_c$ with
$\kappa(\mathrm{Vir}_c) = c/2$, BRST-primitive
$G^{(2)}$ from the Costello--Gaiotto holomorphic-CS construction;
\textsf{cor:rung-w-N} takes $\cA = \cW_N(\fg)$ with
$\kappa(\cW_N) = c \cdot (H_N - 1)$, $H_N = \sum_{j=1}^N 1/j$
(boundary check $N=2$: $\kappa(\cW_2) = c/2 = \kappa(\mathrm{Vir})$,
AP136); \textsf{cor:rung-w-infty} is the iterated-Sugawara
$E_\infty$-topological limit
(\textsf{thm:e-infinity-topologization-ladder} in
\textsf{e\_infinity\_topologization.tex}). All scope tags carry
the canonical convention: $\Sigma_n$-coinvariant projection of
the present ordered axioms is the symmetric-bar shadow used in
the modular-characteristic computation; the ordered axioms
themselves carry the full $R$-matrix data via the
deconcatenation coproduct
(Remark~\ref{rem:sc-three-bar-connection} of
\textsf{foundations.tex} when present).

The Volume~I Koszul-reflection identity
$\bar\partial = \KZ^*(\nabla_{\mathrm{Arnold}})$ together with
$\kappa = -c_{\mathrm{ghost}}(\mathrm{BRST})$
(\textsf{thm:koszul-reflection} in Vol~I's
\textsf{chap:climax-platonic}) is, on the present axiomatic side,
the comparison between the bar differential
$d_{\barB}$ on $T^c(s^{-1}\bar\cA)$ assembled from the
$m_k$ above and the holomorphic Knizhnik--Zamolodchikov
connection on $\Conf_n(\C)$ pulled back via the
collision-residue Arnold relations on $\FM_n(\C)$. The
universal conductor formula
$K(\cA) = \kappa(\cA) + \kappa(\cA^!)$ from
Vol~I's \textsf{chap:universal-conductor} and the universal
trace identity bridge
\textsf{conj:universal-trace-identity-volii-bridge} are the
characteristic-class shadows of the present axiomatic data
under the open/closed pentagon-of-equivalences for
$\SCchtop$ assembled at \textsf{sc\_chtop\_heptagon.tex} when
present.
\end{remark}
