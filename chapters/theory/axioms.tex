...
The reduced chiral bar coalgebra
$\barB^{\mathrm{ch}}(A)=T^c(s^{-1}\bar A)$ resolves twisting morphisms
out of~$A$. The linear Koszul dual $A^!$ is the cobar algebra built
...
\begin{equation}\label{eq:bar-coder-iso}
 ...
 \;\simeq\;
 \mathrm{Coder}\bigl(\BarTwc{A}\bigr),
 \qquad
 d_{\mathrm{Hoch}} \;=\; [d_{B},\, -\,],
\end{equation}
where $\BarTwc{A} = T_{c}(s^{-1}\bar A)$ is the conilpotent
$E_{1}$-chiral bar coalgebra of
\cref{thm:typed-boundary-holographic-realisation}\,(i), and
$\mathrm{Coder}$ denotes the coderivation space.  The equivalence is
the universal property of the cofree conilpotent tensor coalgebra:
...
$\mathrm{Coder}(\BarTwc{A})$ on $A$ is the two-colour assignment
\begin{description}
\item[\textup{closed colour}] $\mathrm{Coder}(\BarTwc{A}) \simeq
...
brace insertions $\nu^{(m)}_{k} \colon
\mathrm{Coder}(\BarTwc{A})^{\otimes k} \otimes A^{\otimes m} \to A$
...
Definition~\ref{def:ainfty_chiral} equips $A$ with a translation operator $\partial$ (from the $\C$-action on configuration space) and operations $m_k$ valued in formal Laurent series in the spectral parameters $\lambda_i$. The two data are coupled: translation of an input point matches a shift of the associated spectral parameter, and the unit is transparent to both. Sesquilinearity and unitality record the compatibility as polynomial identities in $\partial$ and~$\lambda_i$.
...
The $A_\infty$ chiral algebra is a \emph{planar} (non-commutative) structure: inputs are ordered by the topological direction~$\R_t$; no symmetry under input permutation is imposed. The passage to commutative structures (PVA on cohomology, symmetric bar complex) goes through the exchange cylinder of~\S\ref{sec:PVA_descent}, using connectivity of the full three-dimensional configuration space $\Conf_2(\C \times \R)$ to relate the two orderings.
...
depending on $\varepsilon$. The computation gives $F(\varepsilon)$ in two
independent ways.
...
\]
Verification: each axiom of Definition~\ref{def:ainfty_chiral}.
...
Verification: that the $C_\ast\!\bigl(W(\mathsf{SC}^{\mathrm{ch,top}})\bigr)$--algebra
