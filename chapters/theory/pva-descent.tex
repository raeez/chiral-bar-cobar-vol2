...
(Khan--Zeng analytic SDR; Forsythe--Mukai Stokes datum;
...
Construction Problem~3 (CLAUDE.md \S9): a unified PVA-quantum HT theory
with the classical $\lambda$-Jacobi limit and an $E_3$-lift on
$Q$-cohomology. Throughout this section, every theorem statement that
...
A direct derivation from the OPE mode expansion $\{a_\lambda b\} = \sum_{n \geq 0} a_{(n)} b / \lambda^{n+1}$ using the vertex algebra identity $(\partial a)_{(n)} = -n \cdot a_{(n-1)}$ (see \cite{Kac98}, Proposition~4.4) yields
\[
...
\begin{lemma}[Jacobi Identity Verification; \ClaimStatusProvedHere{} (classical $\lambda$-Jacobi)]
...
and FM regularity). Let $[a],[b],[c]\in H^\bullet(\A,Q)$ be cohomology classes with representatives $a,b,c$ satisfying $Qa=Qb=Qc=0$. Then the $(-1)$-shifted $\lambda$-bracket satisfies the Jacobi identity:
\begin{equation}
...
\begin{proof}
The proof proceeds in six steps: write the $n=3$ $A_\infty$ identity
with all signs; project to cohomology; decompose into regular/singular
parts; identify the singular--singular compositions with iterated
$\lambda$-brackets; use the three pairwise FM boundary faces to recover
the missing Jacobi term; and verify AOS cancellations at codimension-2
corners.
...
It remains to name the extra datum under which $m_k = 0$ in
cohomology for $k \geq 3$.

\begin{theorem}[Higher operations vanish under compatible nullhomotopies;
\ClaimStatusConditional{} on compatible topological nullhomotopies]
...
for all $k \geq 3$ and all $[a_1], \ldots, [a_k] \in H^\bullet(\A, Q)$
(cohomology classes), the higher operation $m_k$ is $Q$-exact:
\[
...
Forsythe--Mukai compactification; reflected weights / wall-crossing
...
Q(m_2(a,b)) = m_2(Qa,b) + (-1)^{|a|} m_2(a,Qb).
\]
If $Qa = Qb = 0$, then $Q(m_2(a,b)) = 0$, so $m_2(a,b)$ is $Q$-closed. If $a' = a + Q\alpha$, then
\[
...
\begin{proof}
Construct an explicit chain homotopy $s(a,b)$ witnessing that $m_2(a,b) - (-1)^{|a||b|} m_2(b,a)$ is $Q$-exact.
...
\[
\{a_\lambda b\} = \mathrm{ASym}(m_2^{\mathrm{sing}}(a,b)) = -(-1)^{|a||b|} \mathrm{ASym}(m_2^{\mathrm{sing}}(b,a))\big|_{\lambda \to -\lambda - \partial},
\]
...
\begin{proposition}[Higher operations vanish under compatible nullhomotopies;
\ClaimStatusConditional{} on compatible topological nullhomotopies]
...
compactified ordered topological factor. Contractibility of the open
configuration space is only the homological background. The hypothesis
...
\textbf{(i) Commutative product:} The operation $m_2$ descends to
cohomology (Lemma \ref{lem:lambda_descends}), yielding a product that
...
\textbf{(iii) Higher operations vanish under compatible
nullhomotopies:} Established in Proposition \ref{prop:m3_vanish}.
