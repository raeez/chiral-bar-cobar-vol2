...
As in the physics–motivated constructions discussed earlier in the paper, we will rely on the following analytic locality and renormalization hypotheses (stated here for completeness).
...
\begin{remark}
Assumption~\ref{assump:H1-H4} is satisfied in the class of holomorphic–topological theories on $\R\times\C$ treated by the BV renormalization framework discussed earlier; it is precisely the hypothesis needed to make the configuration–space proofs below rigorous.
\end{remark}
...
\begin{lemma}[Well–definedness]\label{lem:product-welldefined}
...
\begin{lemma}[Graded commutativity on $H$]\label{lem:commutativity}
...
\begin{proposition}[Associativity on $H$]\label{prop:associativity}
...
\end{definition}
\begin{lemma}[Well–definedness of the $\lambda$–bracket]\label{lem:bracket-welldefined}
...
\begin{proposition}[Sesquilinearity and skewsymmetry]\label{prop:sesqui-skew}
...
\begin{theorem}[Jacobi identity]\label{thm:Jacobi}
...
\begin{corollary}[PVA structure]\label{cor:PVA-structure}
...
\begin{proposition}[Arnold relations and Jacobi]\label{prop:AOS-implies-Jacobi}
