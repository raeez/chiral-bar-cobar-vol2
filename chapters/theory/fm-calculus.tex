...
\label{sec:wave14-fm-costello-k3}
...
\label{subsec:wave14-fm-24pts-P1}

\begin{definition}[FM space of 24 marked points on $\mathbb{P}^1$]
\label{def:wave14-fm-conf24-P1}
...
\ClaimStatusProvedHere]
\label{thm:wave14-fm-24-node-stratum}
\index{24-node curve}
...
of~\eqref{eq:wave14-fm-strat-24} for the
...
Theorem~\ref{thm:wave14-costello-bv-anomaly}) admits a chain-level
presentation as a boundary sum on
$\overline{\mathrm{Conf}_{24}}^{\mathrm{FM}}(\mathbb{P}^1)$:
\begin{equation}
\label{eq:wave14-fm-stokes-chi24-anomaly}
0
\;=\;
...
(pair-collisions). By
Theorem~\ref{thm:wave14-fm-24-node-stratum}, the 12 pair-collision
...
\eqref{eq:wave14-fm-stokes-chi24-anomaly}.
\end{proof}

\begin{remark}[Double-counting and $\chi(\mathrm{K3})/2$]
\label{rem:wave14-fm-double-counting}
The factor of 24 (not 12) in
Proposition~\ref{prop:wave14-fm-stokes-chiK3} is the number of
...
Theorem~\ref{thm:wave14-fm-24-node-stratum} halves this to 12
...
Vol.~III Theorem~\ref{thm:wave14-costello-m24-anomaly-h3}.
\end{proposition}
...
holography]
\label{rem:wave14-fm-chain-level-twisted-holography}
The FM calculus supplies the chain-level ambient for the
twisted-holography KK matching of Vol.~II
Conjecture~\ref{conj:wave14-costello-twisted-holography-matching}:
