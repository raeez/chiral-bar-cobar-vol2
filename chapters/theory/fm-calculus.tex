...
\begin{proof}
Proof: the formula by a regularization argument and explicit sign computation.
...
Verification: the sign formula for the next case, $\FM_4(\C)$, which has real dimension $2(4-1) = 6$ and quotient coordinates $w_i = z_i - z_4$ for $i = 1,2,3$, with orientation form $\Omega_4 = dx_1 \wedge dy_1 \wedge dx_2 \wedge dy_2 \wedge dx_3 \wedge dy_3$.
...
Proof: the cluster factorization theorem using Fulton--MacPherson boundary calculus.
