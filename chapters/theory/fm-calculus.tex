...
\begin{remark}[Corner cancellation and $(-1)$-shifted Poincar\'{e} duality]
\label{rem:corner-poincare}
Codimension-2 corners cancel because transverse Lagrangian intersections in the $(-1)$-shifted symplectic stack $\FM_k(\C) \times \Conf_k(\R)$ have well-defined intersection numbers: the two orderings of taking residues along $D_S$ and $D_T$ correspond to the two sheets of the self-intersection, and their contributions are related by the $(-1)$-shifted Poincar\'{e} duality pairing. This is why the Arnold relation $d^2\eta = 0$ on $\FM_3(\C)$ gives the Jacobi identity: the three pairwise collision divisors $D_{\{i,j\}}$ meet in codimension-2 corners whose intersection numbers, computed via the symplectic orientation, sum to zero---which is the $(-1)$-shifted Poincar\'{e} duality of the correspondence.
\end{remark}
