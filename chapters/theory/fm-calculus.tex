...
The collision divisors $D_S$ parametrize the boundary strata of the chiral Steinberg correspondence $\FM_k(\C) \times \Conf_k(\R)$: as the Steinberg variety presents the Hecke algebra via convolution over its strata, the bar differential $d_{\barB}$ extracts residues along these divisors---this is Lagrangian convolution. The coassociativity of the bar coproduct $\Delta$ reflects the fact that the diagonal of the self-intersection is a groupoid morphism: the ordered deconcatenation on $\Conf_k(\R)$ respects the boundary stratification of $\FM_k(\C)$, so $\Delta$ is compatible with the decomposition $D_S \cong \FM_r(\C) \times \FM_{|S|}(\C)$.
\end{remark}
...
\begin{remark}[Stokes as Lagrangian intersection arithmetic]
\label{rem:stokes-lagrangian}
Theorem~\ref{thm:Stokes_FM} is the geometric content of the $A_\infty$ Stasheff identity, read as Lagrangian intersection arithmetic on the chiral Steinberg correspondence. The codimension-1 boundary divisors $D_S \subset \partial\FM_k(\C)$ are the codimension-1 faces of the Lagrangian cycle $\FM_k(\C) \times \Conf_k(\R)$; Stokes' theorem equates the sum of residues along these faces to zero, which is the $A_\infty$ relation $\sum m_i \circ m_j = 0$. The orientation signs $\epsilon_{D_S}$ are determined by the symplectic orientation of the Lagrangian, and the factorization $D_S \cong \FM_r(\C) \times \FM_{|S|}(\C)$ is convolution of the corresponding Lagrangian correspondences.
\end{remark}
