...
on $H^\bullet(\cA, Q)$. The bridge is via the universal holography master
theorem (Vol~II climax \texttt{thm:universal-holography-master}): the
$\SCchtop$-algebra on the slab $\R \times \C$ is the bulk; the raviolo
restriction at fixed $t_0$ exhibits the boundary chiral algebra; the PVA
descent extracts the classical $\hbar \to 0$ limit. The Koszul conductor
$K = -c_{\mathrm{ghost}}(\mathrm{BRST})$ (Vol~I \texttt{chap:universal-conductor})
controls the level shift between the chiral and PVA descriptions: at the
self-dual point $K = 0$, the raviolo and PVA encode the same data; off
the self-dual point, the level shift is the Sugawara correction
$\dim(\mathfrak g)/2$ for affine Kac--Moody (AP-RMATRIX in the canonical
reference). The raviolo is the chain-level avatar; the PVA is its classical
projection; both are shadows of the universal bulk $\SCchtop$-algebra.
\end{remark}

\begin{remark}[Recognition: which $\SCchtop$-algebras yield raviolo VAs]
\label{rem:raviolo-recognition-bridge}
The recognition theorem $\texttt{thm:recognition-SC}$ identifies the class
of $\SCchtop$-algebras admitting a raviolo restriction: precisely those
arising from logarithmic $\SCchtop$-algebras (Definition~\ref{def:log-SC-algebra})
satisfying the bridge hypotheses (a)--(c) of Theorem~\ref{thm:physics-bridge}
(BV data, one-loop finiteness, polynomial interactions). The raviolo
restriction $\mathsf V_{\mathrm{rav}}$ of Definition~\ref{def:raviolo-restriction}
is precisely the vertex-algebraic detection invariant: two $\SCchtop$-algebras
$A, A'$ produce isomorphic raviolo VAs iff they agree up to the
spectral data forgotten by collapsing the $E_1$ direction. The PVA descent
of Theorem~\ref{thm:raviolo-PVA} is a further forgetful step: it discards the
$\hbar$-deformation data of the vertex algebra and retains only the
classical $\lambda$-bracket and commutative product. The raviolo VA / PVA
pair forms the chain-level / cohomology-level pair of detection invariants
for the recognition theorem.
\end{remark}
...
\subsection{Wave-14 anchors: raviolo as time-slice shadow of universal holography}
\label{subsec:raviolo-wave14-anchor}

\begin{remark}[Raviolo restriction in the universal-holography master]
\label{rem:raviolo-wave14-anchor}%
\index{raviolo!universal-holography anchor}%
\index{Wave 14!raviolo anchor}%
...
\textsf{thm:universal-holography-master} (Volume~II,
\textsf{programme\_climax\_platonic.tex}). For an algebra $\cA$
with conformal vector at non-critical level, the canonical $3$d
HT theory $T_\cA$ on $X \times \R$ has time-slice observables
...
\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ are computed via
factorization homology along $\R$; the raviolo restriction
extracts the $\C$-direction shadow at fixed time, retaining
the chiral product (closed colour) and discarding the open
colour through the $\Eone$ contractibility identification.

The raviolo apparatus assembled here (the time-slice
restriction (\S\ref{subsec:raviolo-time-slices}); the PVA
descent on cohomology (Theorem~\ref{thm:raviolo-PVA}); the
bridge-from-physics-to-algebra
(Theorem~\ref{thm:physics-bridge}); the chain-level $A_\infty$
chiral structure
...
the family-by-family verification machinery for the rung
corollaries
\textsf{cor:rung-heisenberg, cor:rung-affine-km, cor:rung-virasoro,
cor:rung-w-N, cor:rung-w-infty}
of the master theorem. Each rung specialises the present apparatus
to a particular family ($\cA = \cH_k$, $V_k(\fg)$,
$\mathrm{Vir}_c$, $\cW_N$, $\cW_\infty$); each preserves the
log-$\SCchtop$ hypothesis (G/L/C lineages directly; class M via
the DS--Hochschild compatibility bridge
\textsf{thm:chd-ds-hochschild} of Volume~II
\textsf{chiral\_higher\_deligne.tex}).

The $\SCchtop \neq E_3$ guard (AP163) is enforced by the raviolo
restriction itself: the time-slice raviolo is a vertex algebra
(closed colour), not an $E_3$-topological algebra; the
