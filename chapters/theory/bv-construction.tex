...
\label{ch:bv-construction}% alias for dangling \ref{ch:...}

The bridge theorem (Theorem~\ref{thm:physics-bridge}) identifies the BV-BRST data of a 3d~HT theory on $\C_z \times \R_t$ with a logarithmic $\SCchtop$-algebra: the prefactorization algebra $U \mapsto \mathsf{Obs}(U)$ produces the open factorization category, and the quantum master equation
\begin{equation}
...
\begin{remark}[$\hbar$ convention; AP151 bridge identity]
...
$\hbar=0$ to the classical master equation $\{S,S\}=0$. Other places in the
manuscript use the spectral parameter $\hbar$ in $R(z)=\exp(r(z))=1+\hbar
r(z)+O(\hbar^2)$; the bridge identity is that the quantum-group $\hbar$
\emph{is} the BV $\hbar$ when both are computed from the same boundary
chiral algebra (AP151). No $\pi i$ factor appears in the BV $\hbar$; the
$2\pi i$ convention enters only in residue evaluations of weight forms over
$\FM_k(\C)$ via $\frac{1}{2\pi i}\oint$ (AP120).
\end{remark}

\begin{remark}[The bar complex IS the BV complex; cross-reference to climax theorems]
\label{rem:bar-equals-BV-bridge}
For the boundary chiral algebra $A_\partial$ of a 3d HT theory satisfying
Theorem~\ref{thm:physics-bridge}, the chiral bar complex $\barB^{\mathrm{ch}}(A_\partial)
= T^c(s^{-1}\bar A_\partial)$ \emph{coincides} with the BV-BRST complex
$\Obs_{\mathrm{bulk}} = (\widehat{\Sym}(\mathcal E^\vee), Q_{\mathrm{BRST}})$
of the bulk theory at the level of derived equivalence: the bar differential
$d_{\overline B}$ from OPE residues on $\FM_k(\C)$ matches $Q_{\mathrm{BRST}} =
\{S_{\mathrm{eff}},-\}_{\mathrm{BV}} + \hbar\Delta_{\mathrm{BV}}$ via the
DS--Hochschild bridge \texttt{thm:chd-ds-hochschild} of
\S\ref{sec:chiral_hochschild_expanded}, and this identification is the
chain-level upgrade of the universal holography theorem
\texttt{thm:universal-holography-master} (Vol~II climax in the
3d~quantum gravity Part).
The Koszul conductor $K = -c_{\mathrm{ghost}}(\mathrm{BRST})$
(Vol~I, \texttt{chap:universal-conductor}) equals the level-shift
controlling the QME at one loop: $\kappa(A_\partial) +
\kappa(A_\partial^!) = K$ scalar-channel, with the ghost central charge
absorbing the matter complementarity. The QME of
\eqref{eq:QME-bv-construction} is therefore the chain-level
incarnation of $D_A\Theta_A + \tfrac12[\Theta_A,\Theta_A] = 0$ on the bar
side: open (BV) and closed (modular Maurer--Cartan) faces of the same
Maurer--Cartan datum.
\end{remark}

\begin{remark}[BV $=$ bar scope; class G/L proved, class C/M conditional (AP73)]
\label{rem:bv-equals-bar-AP73-scope}
The chain-level identification $(\Obs_{\mathrm{bulk}}, Q_{\mathrm{BRST}}) \simeq
(\barB^{\mathrm{ch}}(A_\partial), d_{\overline B})$ is \emph{proved
unconditionally} for the abelian--Sugawara class~G (Heisenberg, lattice) and
the affine--Kac--Moody class~L at non-critical level via the explicit
homotopy transfer~\cite{CG17}*{Theorem~5.2.2.1} of \S\ref{subsec:analytic}
Step~4. For the symplectic bosons / $\beta\gamma$ class~C and the
Virasoro / W$_N$ class~M, the identification is \emph{conditional on the
harmonic-decoupling property} (AP73 in the canonical reference): the
BV propagator must split into a holomorphic kernel and a topological
kernel without cross-coupling at the harmonic component, an analytic
hypothesis verified for class~G/L from the explicit Wakimoto/free-field
realization but only conjectural for class~C/M at chain level. The
\emph{cohomological} identification $H^\bullet(\Obs_{\mathrm{bulk}})
\simeq H^\bullet(\barB^{\mathrm{ch}}(A_\partial))$ holds for all four
classes via the DS--Hochschild bridge \texttt{thm:chd-ds-hochschild};
the chain-level upgrade for class~C/M is closed in the
weight-completed category by \texttt{prop:bv-bar-class-m-weight-completed}
(see Vol~I CLAUDE.md MC5 entry, 2026-04-16). The QME~\eqref{eq:QME-bv-construction}
is the open form; $D_A\Theta_A + \tfrac12[\Theta_A,\Theta_A] = 0$ is the closed
form; both are the same Maurer--Cartan element under the bar/BV identification
on the proved scope (G/L) and on the conditional scope (C/M)
