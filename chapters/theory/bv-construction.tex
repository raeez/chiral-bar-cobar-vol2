...
Having established that BV observables form an $\Ainf$ chiral
algebra, we now prove the $\Ainf$ identities via Stokes' theorem
on Fulton--MacPherson compactifications.  The higher
multiplications $m_k$ constructed above are defined by integrating
renormalized propagators over $\FM_k(\C) \times E_1(m)$
(\S\ref{subsec:mk-integrals}), but we have not yet verified that
these integrals satisfy the Stasheff identities.  That
