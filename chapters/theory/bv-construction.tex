...
\emph{compares} with the BV-BRST complex
$\Obs_{\mathrm{bulk}} = (\widehat{\Sym}(\mathcal E^\vee), Q_{\mathrm{BRST}})$
only after the relevant comparison datum has been constructed. At
genus zero the chain comparison is the cited Costello--Gwilliam /
Drinfeld--Sokolov--Hochschild bridge. In higher genus the manuscript
proves a coderived bar comparison; a strict equality between
$d_{\overline B}$ and
$Q_{\mathrm{BRST}}=\{S_{\mathrm{eff}},-\}_{\mathrm{BV}}
+\hbar\Delta_{\mathrm{BV}}$ additionally requires the
diagonal-residue BV operator to exist, be second order, square to zero,
and be functorial. In this hypothesis-bound sense the comparison is the
