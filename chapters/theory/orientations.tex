...
\]
The induced boundary orientation $\iota_{-\partial_\varepsilon}\Omega_3 = -\varepsilon\,dx_1\wedge dy_1\wedge d\theta$ is compared with the standard \emph{product} orientation on $D_{\{2,3\}} \cong \FM_2(\C) \times \FM_2^{\mathrm{red}}(\C)$, namely $dx_1\wedge dy_1$ (outer, recording $w_1$) followed by $d\theta$ (inner $S^1$); the outer-then-inner reordering supplies the compensating sign, so that
\begin{equation}
\label{eq:eps-D23}
\boxed{\epsilon_{D_{\{2,3\}}} = +1.}
\end{equation}
% RECTIFICATION-FLAG: [GATE 1] The four canonical signs (+,-,+,-) are used consistently downstream and yield a consistent n=3 Stasheff relation, but their per-face derivations here apply the outward-normal flip non-uniformly: D_{1,3} and D_{1,2,3} pick up the -1 flip explicitly, while D_{1,2} and D_{2,3} obtain +1 from the even transposition count without it. The reconciling ingredient is the induced-vs-product orientation comparison, written out explicitly only for D_{1,3} (and now noted for D_{2,3}); a uniform application of Convention conv:outward-normal to every face, with the product-orientation comparison tracked face-by-face, would make the bookkeeping consistent without changing the values.
...
coordinate change, the parity of the transposition moving
$d\varepsilon_S$ to the first slot, and the single outward-normal sign.
\end{proposition}
...
minus sign in \eqref{eq:sigma-S-formula}. All remaining signs are
therefore exactly the sign of the displayed Jacobian coefficient.
\end{proof}
