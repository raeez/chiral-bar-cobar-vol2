...
lifts \(\xi\) to a degree-\(2\) class \(\tilde{\xi} \in
\fg^{A}_{\mathrm{mod}}\). \(\Phi\) is a chain map, so
\(d_{\Theta^{(0)}}\tilde{\xi}\) maps to zero on the curved-Dunn
side; the degree-\(2\) cohomology comparison adjusts
\(\tilde{\xi}\) by a coboundary to make it
\(d_{\Theta^{(0)}}\)-closed. From
\(H^{2}\bigl(\fg^{A}_{\mathrm{mod}}, d_{\Theta^{(0)}}\bigr)=0\),
\(\tilde{\xi}=d_{\Theta^{(0)}}\eta\), hence
...
\begin{itemize}
\item Virasoro: \(K_{T}(\mathrm{Vir}_{c}) = \kappaChHodge(\mathrm{Vir}_{c})
 + \kappaChHodge(\mathrm{Vir}_{26-c}) = c/2 + (26-c)/2 = 13\) per genus,
 the derived-centre complementarity sum of archetype \(\mathsf C\);
 the critical central charge \(c + (26-c) = 26\) is the distinct scalar
 setting the Hodge-line shift.
\item Affine Kac--Moody \(\widehat{\mathfrak{g}}_{k}\):
 ...
 the complementarity sum of a Sugawara Koszul pair.
 % RECTIFICATION-FLAG: [gate 1, KM conductor] The previously displayed
 % closed form (dim(g)/2)[(k+h^vee)+(-k-h^vee)]/h^vee * h^vee evaluates to
 % 0 (the bracket vanishes identically), so it could not equal 2dim(g);
 % the correct Sugawara complementarity scalar needs the Vol I Theorem C
 % archetype value for KM, not asserted here.
\item Bershadsky--Polyakov \(W_{k}(\mathfrak{sl}_{3}, f_{\min})\):
 \(K_{T}(\mathrm{BP})\) is the BP self-contraction conductor on
 \(\overline{\mathcal{M}}_{g}\), the Dynkin factor \(\alpha_{3}/2\)
 of Vol I's W-algebra Koszul-pair table.
 % RECTIFICATION-FLAG: [gate 0, BP conductor] The bare value K_T(BP)=196
 % is outside the canonical rho_K set {0,8,13,250/3,98/3} and unsupported
 % by the sketch; pin against the Vol I W-algebra Koszul-pair table before
 % asserting a numeral.
\end{itemize}
\end{proposition}
...
(\(\rho_{K} = 13\) for Virasoro, the derived-centre complementarity
sum \(\kappaChHodge(\mathrm{Vir}_{c}) + \kappaChHodge(\mathrm{Vir}_{26-c})
= c/2 + (26-c)/2\); \(=2 h^{\vee}\) for KM,
...
% RECTIFICATION-FLAG: [gate 1, conductor values] (i) Virasoro rho_K
% corrected 26 -> 13 here: the derived-centre complementarity sum
% kappa+kappa! = c/2+(26-c)/2 = 13 is the canonical Theorem-C archetype-C
% value (CLAUDE.md \S3, rho_K in {0,8,13,250/3,98/3}); 26 is the critical
% central charge, a distinct scalar. With rho_K=13 the self-duality
% kappa!(A)=13-c/2=(26-c)/2 closes, and K_T(Vir)=13 matches the statement.
% (ii) The statement gives K_T(Vir)=13 per genus but also "total over genus
% = 26"; per-genus 13 is the conductor, the 26 is the critical charge sum,
% not a second conductor value -- the "K_T=26" phrasing conflates the two.
% (iii) KM statement K_T=2dim(g) vs proof rho_K=2h^vee disagree (sl_2: 6 vs
% 4); BP K_T=196 is outside the canonical set and unsupported by the sketch.
% These need the Vol I Theorem C archetype assignment for KM/BP/W_N before
% asserting numerals; flagged pending that cross-volume input.
\end{proof}
...
\]
$c_{g,n}$ the Arakelov intersection of the Hodge class $\lambda_g$
...
$\lambda_g = (1/12)\sum_{n}\phi^{(n)}\cdot c_{g,n}$, reducing the
...
\begin{remark}[Tower bound $n \le g+1$]
...
is $n \le 3g$. Markl--Shnider--Stasheff 2002 Theorem 3.42
sharpens to $n \le g + 1$ via the associator Jacobi identity.
\cite{Mumford1983,MarklShniderStasheff2002,BeilinsonBloch2005}.
\end{remark}

\section{Explicit obs$_g$ at $g = 9, 10$ and pro-object limit}
...
\]
% RECTIFICATION-FLAG: [gate 1, MZV dimensions] The motivic-MZV total
% dimensions stated here and in the following remarks (d_10=5, d_11=7,
% d_12=9) disagree with the standard Zagier/Broadhurst--Kreimer sequence
% 1,0,1,1,1,2,2,3,4,5,7,9,12 (weights 0..12), which gives d_8=4, d_9=5,
% d_10=7, d_11=9, d_12=12 under exactly the recurrence d_n=d_{n-2}+d_{n-3}
% cited. The recurrence is correct; its evaluation is not (true
% d_11=d_9+d_8=5+4=9, not 4+3=7). The explicit bases listed (5/7/9
% elements at weights 10/11/12) are correspondingly incomplete. The
% catalogue Padovan row (notes/antipatterns_catalogue.md row 45) is itself
% internally inconsistent with its own seeds, so the canonical value is
% flagged rather than silently rewritten: correcting requires re-deriving
% the Vol I phi^(n) basis content (V1-rem:nc-phi8..12) and re-checking the
% Arakelov coefficients c_{g,n}. Cross-volume coherence must be preserved.
At genus~$9$ the sum reaches $\phi^{(10)}$, decomposing into
$d_{10}$ motivic MZV legs (Deligne~$2013$ weight-$10$ basis containing
...
weight-$11$ MZV basis has $d_{11}$ irreducibles governed by
the Padovan recurrence $d_{11} = d_9 + d_8$, plus a new
Borcherds leg
...
\]
The genus-$10$ sum reaches $\phi^{(11)}$, adding the
Brown $2011$ Ann.~Math.~$175$ weight-$11$ basis, containing
...
given by the Padovan recurrence $d_m=d_{m-2}+d_{m-3}$,
and the Borcherds imaginary-root
...
surjection with kernel the rank-$d_{12}$ free module on the weight-$12$
