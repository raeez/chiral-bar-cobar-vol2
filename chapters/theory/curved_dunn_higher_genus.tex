...
\(g = 2\). The construction extends to every \(g \ge 2\),
conditional on the boundary KZB formal-type input, by the clutching
recursion of
...
\autoref{cor:modular-operad-composition-all-levels}.
The vanishing
\(H^{2}(\mathrm{TCo}^{\bullet}_{g}(A), d_{0}) = 0\) holds
unconditionally by
\autoref{thm:curved-dunn-H2-vanishing-all-genera}; the displayed
primitive is the conditional chain-level clutching witness.
\end{corollary}
...
slopes \(> 1\), controlled by the edgewise pole orders in
...
regularisation, and formal/Stokes classification of meromorphic
connections replaces direct convergence.  The two frameworks agree on
...
\begin{itemize}
  ...
  \item \emph{$R$-twisted clutching and curved-Dunn $H^2$.} The two
    algebraic inputs are closed by
    \autoref{thm:curved-dunn-H2-vanishing-all-genera} and the
    Eilenberg--Zilber twisting cochain \(\tau\) constructed in its
    proof; the analytic boundary input is
    \autoref{conj:kzb-boundary-formal-type-extension}.
\end{itemize}
\end{remark}
...
Siegel--Borcherds associator (Remark~\ref{V1-rem:nc-phi3-4-5} of
...
Remark~\ref{V1-rem:nc-Ainfty-open} and
...
Remark~\ref{V1-rem:nc-obs-infty} and
\texttt{compute/lib/k3\_yangian\_wave17\_pentagon\_coboundary\_hbar8\_9\_10.py}.
...
(Vol.~I Chapter~\ref{V1-app:nilpotent-completion}) with the curved-Dunn
$H^2$-vanishing (\autoref{thm:curved-dunn-H2-vanishing-all-genera})
