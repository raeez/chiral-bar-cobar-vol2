...
%% while the E_3-rigidity reinterpretation remains an open route.
%%
...
bulk structure only after three inputs have been separated: the
$E_2$-chiral brace action, the two-coloured $\SCchtop$ bulk-boundary
lift on $(Z^{\mathrm{der}}_{\mathrm{ch}}(\cA),\cA)$ under the
two-coloured cobar-homotopy hypothesis, and the strict
$E_3^{\mathrm{top}}$ refinement supplied by explicit topologisation
data. The class-$M$ global-triangle gap is exactly the failure of these
three inputs to collapse to a single theorem. The chapter constructs
the $E_2$-chiral bridge, isolates the strict $E_3$ chain-lift
obligation, and treats concentration-in-$\{0,1,2\}$ as the output of a
...
chiral-topological $\SCchtop$ datum through the heptagon under the
two-coloured cobar-homotopy hypothesis. Fourth
(\S\ref{sec:chd-rigidity}), the $E_3$-PBW route: concentration follows
from $E_3$-rigidity-at-a-point once the derived centre admits a
chiral-$E_3$-PBW presentation. Two applications close
the chapter
...
\begin{theorem}[Chiral Higher Deligne: braces, two-coloured lift,
and topologised $E_3$]%
...
(Remark~\ref{rem:chd-universality-hypotheses}). Then the pair
...
shuffle operations. On cohomology the $E_2$ brace structure does not
use this homotopy. The two-coloured universal $\SCchtop$ statement and
the strict $E_3$ refinement use precisely the extra hypotheses stated
in the theorem.
\end{remark}
...
Part (2). Under the two-coloured cobar contracting homotopy, one
contracts the space of homotopy
...
single-colour comparison; the proof obligation here is the
two-coloured homotopy.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The $E_3$-PBW route to concentration}
...
Koszulity of the braid arrangement. The route studied here
...
\begin{conjecture}[Concentration from $E_3$-rigidity and chiral-$E_3$-PBW]%
...
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA|_{D_x})$) implies
\[
 \ChirHoch^\bullet(\cA, \cA) \;\text{concentrated in degrees}\; \{0, 1, 2\}.
\]
The missing input is a chiral-$E_3$-PBW theorem of this derived-centre
form (see
...
$E_3$-rigidity route is not used in that proof.
\end{conjecture}
...
its stated hypotheses) in Vol.~I,
...
\begin{remark}[Proof route under
...
Assume the chiral-$E_3$-PBW hypothesis. The proof has three steps; each
removes the type issue isolated in
Remark~\ref{rem:H-concentration-via-E3-rigidity-type-error-retraction}
before invoking rigidity.
...
Step 3. Assuming the chiral-$E_3$-PBW presentation of the derived centre,
...
Range of the argument. Steps~1 and~3 are sound only under the
...
Orlik--Solomon Koszulity), the missing PBW input does not
...
\begin{remark}[Concentration as a consequence of chiral-$E_3$-PBW]%
...
hypotheses and the two-coloured cobar homotopy of
Remark~\ref{rem:chd-universality-hypotheses}.
\end{theorem}
...
\begin{corollary}[Universal holography for class M in the
...
\]
on cohomology as an $E_2$-chiral/Gerstenhaber algebra. Assuming the
heptagon hypotheses and the two-coloured cobar homotopy of
Remark~\ref{rem:chd-universality-hypotheses}, the identification
refines to a mixed chiral-topological chain-level equivalence in the
sense of Convention~\ref{conv:chd-e3-chiral-meaning} on the
\emph{weight-completed} ambient (equivalently, on the pro-object /
$J$-adic topological ambient of
...
identification at chain level uses
Thm~\ref{thm:chiral-higher-deligne}(2) and the chain-level mixed lift
clause of Thm~\ref{thm:chd-ds-hochschild}; both require the
two-coloured cobar homotopy of
Remark~\ref{rem:chd-universality-hypotheses}. The underlying
$E_2$-chiral/Gerstenhaber identification is the cohomological
comparison above.
\end{proof}
...
direct-sum complex carries the nonzero class
...
$h_{\mathrm{oc}}$ in Remark~\ref{rem:chd-universality-hypotheses}.
...
The proof obligation is a chiral-$E_3$-PBW theorem presenting
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA|_{D_x}) = U_{E_3}(W_x)$ for a
polynomial-growth $W_x$; not established in any volume.
Shelton--Yuzvinsky 1997 (\emph{Koszulness of Orlik--Solomon algebras
of braid arrangements}) supplies the canonical proof of concentration
via $OS(A_{n-1})$-Koszulity in Vol.~I,
Theorem~\ref*{V1-thm:hochschild-concentration-E1}; Fuks 1986
(\emph{Cohomology of Infinite Dimensional Lie Algebras}) supplies the
formal-disc Gel'fand--Fuchs bound.
...
Kac--Wakimoto 1988 (\emph{Modular invariance representations of affine
algebras}) supplies an independent DS cohomology characterization of
$\cW$-characters; Feigin--Frenkel coset duality for
$\cW_k(\fg) \cong \mathrm{coset}$ supplies a coset-commutant derivation
of the same Hochschild comparison. The mixed lift uses the same
two-coloured cobar homotopy $h_{\mathrm{oc}}$.
\end{enumerate}
...
(Vol.~I~C19--C21), so \emph{if} the chiral-$E_3$-PBW
theorem required by
...
outside the range of Theorem~H regardless. Theorem~H itself is
