...
%%
%% Closes the Hochschild cluster (chain-level class M of the global
%% triangle, holographic reconstruction for class M, chain-level
%% HKR compatibility, universal-holography class-M scope) via a single
...
global-triangle gap is the bill that comes due. The deficiency dictates the theorem: supply the missing
...
The explicit degree-$4$ case is inscribed; the degrees $\geq 5$ follow
...
\begin{theorem}[Chiral Higher Deligne; clauses (1) and (3)
\ClaimStatusProvedHere, clause (2) \ClaimStatusConditional]%
...
$(3)\leftrightarrow(4)\leftrightarrow(5)$. \emph{(Proved here.)}
...
intertwining the bulk-to-boundary structure. \emph{(Conditional on
the Vol.~I chiral Positselski co-contra correspondence in its
associator-chain-level form; see
...
complex on the Koszul locus; we cite this at the citation level as
the $\SCchtop$-variant of the chiral Positselski co-contra
correspondence (Vol.~I, chapter on Theorem~B;
cf.~\texttt{thm:chiral-positselski-weight-completed}). The specific
$(\SCchtop)^!$-formulation required here lifts the weight-completed
chiral Positselski statement to the two-coloured cooperad setting;
that lift is not independently inscribed in Vol.~I at chain level
(the phantom label \texttt{thm:chiral-positselski-7-2} recorded in
the Vol.~I preface is a placeholder, not a proved body;
cf.~HZ-11 / AP255 discipline). Accordingly, clause (2) carries
\ClaimStatusConditional{} with explicit attribution: universality
is proved here \emph{up to} the $(\SCchtop)^!$-cobar contracting
homotopy, whose chain-level existence is the cited Vol.~I result.
On cohomology the universality statement is unconditional, since the
associator dependence collapses (clause (3)) and the cohomological
derived centre is the intrinsic $E_3$-chiral-algebra determined by
...
Theorem~\ref*{thm:chiral-positselski-7-2}, applied to
$(\SCchtop)^!$).
...
for a polynomial-growth generating space $W_x$; no such inscribed
...
The prior inscribed proof of concentration via $E_3$-rigidity is
...
$\cA|_{D_x}$ by its derived centre), Step~2 breaks: no inscribed
...
inscription closes the route, the two arguments would share
...
remains the canonical inscribed proof. This retraction records the
error under the local-AP discipline (AP2021 in the CLAUDE.md
catalogue).
\end{remark}
...
presentation of
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA|_{D_x})$ is inscribed.
...
Step 2 (CONJECTURAL INPUT; not presently inscribed). PBW gives
...
presently inscribed in any volume and is registered as the open
...
future inscription, Lemma~\ref{lem:chd-e3-rigidity-point} applies to
...
\begin{proof}
Class M identification at chain level was previously open and had
been the residual obstruction to uniform universal holography.
Using Theorem~\ref{thm:chd-ds-hochschild}, we reduce the class M
derived centre to the DS cohomology of the class L derived centre.
...
polynomial-growth $W_x$; not inscribed in any volume.
