...
McClure--Smith, Berger--Fresse, Hu--Kriz--Voronov), one dimension
...
(Theorem~H, concentration in $\{0,1,2\}$). The bulk structure of
...
chain-lift obligation, and treats concentration-in-$\{0,1,2\}$ as
the output of a separate chiral-$E_3$-PBW theorem for the derived
centre.
...
concentration follows from $E_3$-rigidity-at-a-point once the
derived centre admits a chiral-$E_3$-PBW presentation.
...
The explicit degree-$4$ case is given; degrees $\geq 5$ follow
by the same template without modification. The phrase ``Stasheff
coherences through all degrees'' below is warranted by this
uniform Stokes argument.
\end{remark}
...
Convention~\ref{conv:chd-e3-chiral-meaning} and forbidden slogan~\#3
of the voice table in CLAUDE.md~\S 8).
\end{proof}
...
$T_{\mathrm{DS}} = [Q_{\mathrm{CS}}, G'_{f}]$ of
\cref{thm:intro-e3-ds} is the class-$\mathsf{L}$ analogue when
the DS-reduced $T_{\mathrm{DS}}$ replaces the Sugawara $T$).  For
...
\begin{remark}[Forbidden slogan~\#3 of the voice table is the negation
of the obligation list]%
...
by Dunn additivity'' is forbidden slogan~\#3 of the voice table
(CLAUDE.md~\S 8).  The allowed form is the conditional
...
finite-dimensional $H^0$ case: the $E_3$-Koszul cohomology is
still concentrated in bar-weights $\{0,1,2\}$, with $H^0$ replaced
by the corresponding finite-dimensional commutant. This extended
form is used in Step 3 below; the narrow Francis~2013 input
($E_n$-centre of a unital field is the field) only cross-checks
the unit component, not the rigidity bound itself.
