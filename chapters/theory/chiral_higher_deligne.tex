...
Honest disclosure (AP115 / AP40). We claim PROVED HERE for the
chiral-Deligne action statement, the concentration-from-rigidity
statement, and the DS-Hochschild compatibility; each is self-contained
on the specified locus with an explicit chain map. Associator
dependence is tracked: the chiral Higher Deligne action is associator-dependent
at chain level, associator-independent at cohomology, with the
cohomological statement being the associator-fixed invariant
...
derived-centre construction. Writing "$E_3$ on $\cA$" is a type error
; writing "$E_3$ on $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$" is
the correct native/derived distinction.
\end{remark}
