...
Write $\rho_K(\cA) := \kappaChHodge(\cA) + \kappaChHodge(\cA^!)$ for the Koszul
...
$\kappaChHodge(\cA) + \kappaChHodge(\cA^!) = \rho_K$ exploited below; while
...
with Vol~I's $\kappaChHodge(\mathrm{Vir}_c) = c/2$.
\end{proposition}
...
$\kappaChHodge(\mathcal{W}_N) = c(H_N - 1)$ with $H_N = \sum_{j=1}^N 1/j$
(C4 of the Vol~I census). Falsification of the linear law:
substituting $\kappa_w = \alpha w$ into Virasoro gives
$\kappa_2 = 2\alpha$; solving against $\kappaChHodge(\mathrm{Vir}_c) = c/2$
