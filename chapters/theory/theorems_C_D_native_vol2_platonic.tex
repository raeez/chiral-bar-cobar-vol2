...
\providecommand{\kappaChHodge}{\kappaChHodge^{\mathrm{Hodge}}}
\providecommand{\kappaChHeis}{\kappaChHodge^{\mathrm{Heis}}}
...
$\obs_g = \kappaChHodge(\cA) \cdot \lambda_g$ on the
...
\begin{remark}[Theorem~A vs.\ Theorem~B separation; signpost to the
GLZ caveat; Type~$\varepsilon$]
\label{rem:theoremC-A-vs-B-signpost}
The hypothesis ``chirally Koszul $E_1$-chiral algebra'' load-bearing
in Theorem~\ref{thm:theoremC-total-shifted-symplectic} and
Theorem~\ref{thm:theoremD-tensor-arakelov} below is precisely the
Koszul-effectiveness condition $\effKoszul$
(Type~$\varepsilon$). It separates the two Vol~I anchors that the
$E_1$-chiral lane carries:
\emph{Vol~I Theorem~A} (bar--cobar inversion
$\Omega(\barB(\cA)) \xrightarrow{\sim} \cA$ on the Koszul locus) is
the structural input that gives a well-defined Koszul dual~$\cA^!$
together with the Lagrangian complementarity
$\kappa(\cA) + \kappa(\cA^!) = \rho_K$ exploited below; while
\emph{Vol~I Theorem~B} (chiral Positselski coderived equivalence,
extending~\cite{Positselski11}) is the comparison-functor input
that promotes the quadratic Maurer--Cartan injection
\eqref{eq:GLZ_MC_injection} of
Theorem~\ref{thm:GLZ_MC} to a bijection on the Koszul-effective
locus. The two theorems are not interchangeable: Theorem~A controls
the underlying object~$\cA^!$ (whence the orthogonality and the
shifted-symplectic structure of
Theorem~\ref{thm:theoremC-total-shifted-symplectic}) and
Theorem~B controls the comparison functor that makes the bar
classifier surjective onto $\MC(\cA^! \otimes \cB)$. The
chapter on bar--cobar review
(Remark~\ref{rem:GLZ_MC_eff_caveat} of
Section~\ref{sec:bar_cobar}) records the injection-vs-bijection
caveat and frontier-references Construction Problem~4 (chiral
Positselski) as the open chiral-generality analogue of Theorem~B.
The C/D theorems below take ``chirally Koszul $E_1$-chiral'' as the
...
the sixth slot $\kappaChHodge$ of the Infinite Fingerprint,
...
\begin{remark}[K3$\times E$: Beilinson-cut witness for the
 $\kappaChHodge$-row reading of Theorem~C]
...
The Koszul conductor $\rho_K(\cA) = \kappa(\cA) + \kappa(\cA^!)$
of \cref{thm:theoremC-total-shifted-symplectic} is read on the
\emph{Hodge row}: $\rho_K(\cA) = \kappaChHodge(\cA) +
\kappaChHodge(\cA^!) \in \{0,\, 8,\, 13,\, 250/3,\, 98/3\}$ on
the five archetypes $\mathsf{G/L/C/M/B}$. The reading is forced,
not a convention. The witness is the K3$\times E$ object: its
...
The five rows arise from four distinct constructions on the same
geometry. $\kappaCat = \chi(\cO_{K3}) \chi(\cO_E) = 2 \cdot 0 = 0$
by K\"unneth multiplicativity (Hartshorne, Ch.~III, Prop.~8.5; the
...
\emph{Falsified additive collapse.} The naive additive identity
...
\end{tabular}
\end{center}
Even on the K3-fibre chart where $\chi(\cO_{K3}) = 2$, the
prediction $0 + 2 = 2$ at $N = 1$ disagrees with the actual $5$;
at $N \ge 2$ the elliptic-fibre chart returns $\chi(\cO_E) = 0$
and the prediction collapses to $0$. The mnemonic
``$5 = 3 + 2$'' at $N = 1$ reads
$\kappaBKM = \kappaChHeis + \chi(\cO_{K3})$, mixing the
rank-additive Heisenberg row with the categorical fibre Euler
characteristic; this is not the proposed additive law and does
not extend to any other $N$.

\emph{Beilinson-cut witness.} The K3$\times E$ counterexample is
the structural witness that
\cref{thm:theoremC-total-shifted-symplectic} is stated on
...
$\mathbf{B}$, \emph{not} the Borcherds weight $\kappaBKM = 5$,
\emph{not} the Heisenberg rank $\kappaChHeis = 3$, \emph{not} the
fibre trace $\kappaFiber = 24$. The Borcherds row is independent
of the Theorem~C ceiling and is read off the paramodular
denominator (Vol~III Rem.~\ref*{V3-rem:k3e-cy3-two-anchor}, the
two-anchor identity); the Mukai conductor
...
invariant, again not a row of the κ-tuple.

\emph{Status.} The five-row construction of $\kappaTuple{K3 \times E}$
is \ClaimStatusProvedHere\ as a counterexample to the additive
identity, with the additive collapse
$\kappaBKM = \kappaChHodge + \chi(\cO_{\mathrm{fiber}})$ explicitly
\emph{retracted} at every $N \in \{1, 2, 3, 4, 6\}$. The κ-tuple
is genuinely primitive in the sense of the universal stage chain
$\mathsf{P} \to \mathsf{C} \to \mathsf{S} \to \mathsf{Z} \to
\mathsf{A}$; identifying any two rows without naming the
licensing $\alpha$-arrow (chart / scope projection) is the master
shadow-collapse pattern.
\cref{thm:theoremC-total-shifted-symplectic} is on the
$\kappaChHodge$-row;
\cref{thm:theoremD-tensor-arakelov} below is the weight-graded
refinement of the same row; the Borcherds row of K3$\times E$
enters the universal trace identity through $\Phihol$ in the
climax chapter
(\texttt{chapters/connections/programme\_climax\_platonic.tex},
Conj.~\ref*{conj:universal-trace-identity-volii-bridge};
counterexample inscription
