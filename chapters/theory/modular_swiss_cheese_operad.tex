...
operator $D_1 = \kappa \cdot \Deltacyc$ on bar cohomology. We compute
it explicitly.
...
$\kappa(\Heis_\kappa) = \kappa$, and $\kappa \cdot \omega_g$
...
all-genus modular statement for $\SCmod$ holds under three hypotheses:
\begin{enumerate}[label=\textup{(\roman*)}]
...
We record it, and isolate the $H^2$-vanishing hypothesis on which
...
associativity at $(g,n)$ follows from the assumed Stokes cocycle
compatibility at the boundary corners.
\end{theorem}
...
quasi-triangularity is not inferred from the formal type alone.
...
\item \textbf{(Rung-$\cW_N$ and rung-$\cW_\infty$.)}
 ...
 Conjecture~\ref{thm:beta-N-closed-form-proved-all-N}; the
...
to strict coassociativity in the comparison target. Cross-ref
