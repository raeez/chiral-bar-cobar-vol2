...
%%
%% Input data:
%% * Theorem thm:wn-tempered-all-N: finite-beta tempering criterion
%% for principal W_{N,c}.
...
%% beta_4 = 13 (separates A = 15, B = 16)
%% beta_5 = 77/5 (separates A = 21, B = 24; also predicts that
...
%% The Vol II finite-beta criterion thm:wn-tempered-all-N uses only
%% beta-independent Stirling dominance after the envelope is supplied.
%% The exact convergence radius is rho_*^{W_N}(c) =
%% |c| / [12 * (H_N - 1)] under the harmonic law.
%%
%% Labels defined:
...
%% compute/tests/test_beta_N_closed_form.py (17 tests, 3 independent
%% verification-decorated paths)
%%
%% The finite-beta tempering criterion is independent of the exact closed
%% form of beta_N once the envelope is supplied; the harmonic formula is
%% the current all-rank
...
proved the finite-\(\beta_N\) criterion for $\cW_N$ tempering. The
low-rank data $\beta_2 = 6$ and $\beta_3 = 10$
also admit two polynomial extrapolations: the triangular formula
...
uses only finite $\beta_N$ and Stirling dominance after a Riccati
envelope is supplied; the harmonic closed form supplies a conditional
exact convergence radius.
...
finite-envelope locus. If Conjecture~\ref{thm:beta-N-closed-form-proved-all-N}
holds, the ordinary-generating convergence radius has the closed form
\[
 \rho_*^{\cW_N}(c)
 \;=\;
...
Stirling dominance) with Conjecture~\ref{thm:beta-N-closed-form-proved-all-N}
...
envelope before the exact arithmetic of $\beta_N$ is used. Conditional
on the harmonic closed form, substitution gives $|c|/(12(H_N - 1))$.
\end{proof}
