...
The torsor construction replaces the star by a \emph{torsor}. The
...
comparison identifying $F_8$ and $F_9$ as geometric and
...
The seven-way equivalence of presentations is the orbit relation of the
$\GRT(K)$-action. The bar hub is a distinguished base point, not the
whole gauge groupoid: every non-hub edge is an associator transport in
the same torsor.
\end{remark}
...
transports are associator-valued, not tautological equalities; the
torsor statement separates the higher cochain faces from the binary
residue.
\end{remark}
