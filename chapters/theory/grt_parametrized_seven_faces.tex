...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Anti-pattern register: AP171, AP174, slab-bimodule scope}
\label{sec:grt-ap-register}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The $\GRT$-torsor master Theorem~\ref{thm:seven-faces-as-GRT-torsor}
inherits three standing scope qualifiers from the Vol II programme-level
register. Recording them here pins the precise content of the torsor
statement and connects the chapter to the Wave-14 reconstitution
anchors.

\begin{remark}[AP171: cohomology vs.\ cochain associator action]
\label{rem:grt-AP171-cohomology-vs-cochain}
The $\GRT(\mathbb{Q})$-action on $\Face(\cA)$ of
Proposition~\ref{prop:grt-action-face} is a strict groupoid action
at every level of the chain-level presentation; this is the content
of Tamarkin's theorem combined with Kontsevich--Tamarkin formality.
...
\item[(a)] \emph{Cohomological face} $r(z)$ \emph{is associator-independent.}
  Two associators $\Phi_1, \Phi_2 \in \GRT(\mathbb{Q})$ produce the
...
\end{itemize}
The cohomological reading is the canonical one for Wave-14 cross-volume
...
\begin{remark}[AP174: abstract Koszul-locus scope of the torsor theorem]
\label{rem:grt-AP174-abstract-vs-concrete}
Theorem~\ref{thm:seven-faces-as-GRT-torsor} is proved
\emph{abstractly} on the chirally Koszul locus for any
$\SCchtop$-algebra in the standard 3d HT landscape: the
$\GRT(\mathbb{Q})$-torsor structure is a consequence of Tamarkin's
formality and is independent of the specific algebra. The
\emph{concrete} construction of all face transports
$\Phi_{1i}\in\GRT(\mathbb{Q})$ as explicit Q-rational associators is
verified for $\fsl_2$-Yangian and affine KM at non-critical level
(per Theorem~\ref{thm:seven-faces-as-GRT-torsor} proof structure);
...
absent at the present scope. Cross-link to
Remark~\ref{rem:AP174-concrete-vs-abstract-scope} of
\texttt{unified\_chiral\_quantum\_group.tex} and
Remark~\ref{rem:dnp-master-AP174} of
\texttt{dnp\_identification\_master.tex}.
\end{remark}

\begin{remark}[Slab is a bimodule: $r(z)$ as diagonal-bimodule data]
\label{rem:grt-slab-bimodule-scope}
The face space $\Face(\cA)$ enters 3d HT physics as the space of
chain-level presentations of the $r$-matrix datum on the
\emph{diagonal} of the Dimofte $D|T|N$ slab bimodule
$(\cA_{\mathrm{L}} = \cA_{\mathrm{R}} = \cA)$ (Vol II memory
``slab=BIMODULE NOT SC disk'', 2026-04-08; Vol I RS-9). The
off-diagonal slab carries an $r$-matrix in
...
recovers the present chapter's $\GRT(\mathbb{Q})$-torsor on
$\Face(\cA, \cA) = \Face(\cA)$.
\end{remark}

\begin{remark}[Wave-14 anchor cross-references]
\label{rem:grt-wave14-anchors}
The torsor master Theorem~\ref{thm:seven-faces-as-GRT-torsor}
threads three Wave-14 reconstitution anchors:
\begin{itemize}
\item[(a)] \emph{Universal holography master.} Each face
  $F_i \in \Face(\cA)$ is a coordinate presentation of the
  boundary-to-bulk obstruction $r(z)$ in the universal-holography
  functor $\Phi_{\mathrm{hol}}\colon \cA\mapsto T_{\cA}$
  (\texttt{thm:universal-holography-master}). The five rungs
  \texttt{cor:rung-heisenberg},
  \texttt{cor:rung-affine-km},
  \texttt{cor:rung-virasoro},
  \texttt{cor:rung-w-N},
  \texttt{cor:rung-w-infty}
  each produce a $\GRT(\mathbb{Q})$-torsor of seven-face presentations
  via the present theorem; the $\fgrt$ generators
  $\sigma_3, \sigma_5, \dots$ specialise to the higher-spin stress
  tensors of the $E_{n}$-topologisation ladder
  (\texttt{e\_infinity\_topologization.tex}).
\item[(b)] \emph{Universal conductor.} The Vol I universal conductor
  $K = c + c^{!}$ (\texttt{chap:universal-conductor}) is the
  $\GRT^{\mathrm{fin}}$-invariant scalar pinning the absolute
  Casimir normalisation of $F_1$. The Wave-14 conductor formula
  expresses the ghost charge as the trace of a specific
...
\item[(c)] \emph{Vol III $\Phi$ functor.} The Vol III CY-to-chiral
  functor $\Phi$ pulls back the present $\GRT$-torsor structure to
  $\mathrm{Coh}(X)$: each CY target carries a seven-face torsor
  associated with $\Phi(X)$, with explicit identifications at
  K3 (via $Y(\fgl(4|20))$), $\bC^{3}$ (via Schiffmann--Vasserot
  CoHA), and toric CY${}_{3}$ (via Maulik--Okounkov $r$-matrix). The
  Tamarkin chain of Section~\ref{sec:f9-willwacher} is the operadic
  bridge; the motivic face $F_8$ specialises in Vol III to the
  Brown motivic associator on the moduli of CY-period
  configurations. FM43 scope ($n=2$ for $d\le 2$, $n=1$ for $d\ge 3$)
  applies.
\end{itemize}
\end{remark}
