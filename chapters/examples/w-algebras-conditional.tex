...
Explicit formulas beyond the displayed arities require a separate
finite-pole compute engine.
...
\begin{remark}[Conditional $W$-algebras at the K3 base point]
\label{rem:wac-1}
At the K3 base point the conditional $W$-algebras are the
$W$-algebra variants parameterised by auxiliary data (level, central
charge, twist). With $\mathbf H_{\Delta_5}$ as anchor, they fibre
over the moduli of class-$\mathcal S$ data on $\Sigma_{0,24}$
(puncture positions, flavour holonomies). The
$\widetilde M_{24}$ Schur cocycle of order $6$
...
that $\mathbf{H}_{\Delta_5}$ is forced by \emph{three topological
inputs},
...
Siegel-form weight or affine-Kac--Moody level of the
...
K3 hypotheses hold. The $\Phi_2$-image is
...
$\hbar^2\cdot K^{\kappa_{\mathrm{ch}}} = -1$ gives
$(\hbar^2, K^{\kappa_{\mathrm{ch}}})
