...
\begin{remark}[Conditional $W$-algebras at the K3 synthesis K3]
\label{rem:wac-DNA-1}
The the K3 synthesis conditional $W$-algebras at K3 base point are the $W$-algebra
...
(punctures positions, flavour-holonomies). The the K3 synthesis conditional
...
\label{sec:wac-wave15-DNA}

\begin{remark}[The K3-specific scope of $\mathbf{H}_{\Delta_5}$]
\label{rem:wac-W15-DNA-1-scope}
The K3 chiral bialgebra~$\mathbf{H}_{\Delta_5}$ is an output of the
CY-to-chiral functor~$\Phi$ (Volume~III) evaluated specifically at the
K3 surface. The conditional $W$-algebras above parameterise
deformations of $W_3$ at central charge~$c$ and of the
$W_{1+\infty}$/Miki tower at levels $(q_1,q_2,q_3 = (q_1 q_2)^{-1})$
on the Frenkel--Kac vertex closure side. The the K3 synthesis scope sharpening
is that $\mathbf{H}_{\Delta_5}$ is forced by \emph{three topological
inputs},
...
\item $c_1(X) = 0$ (Calabi--Yau condition; Serre functor~$S_X = [d_X]$
  without twist);
\item $h^{2,0}(X) = 1$ (shift~$[2]$ bar-cobar, forcing the
  super-quasi-Hopf refinement of~$\mathbf{H}_{\Delta_5}$).
\end{enumerate}
...
\item Enriques surface: $\chi_{\mathrm{top}} = 12$, $h^{2,0} = 0$,
  fails (T1) and (T3). The analogue is a~$\mathbb{Z}/2$-gauging
  of~$\mathbf{H}_{\Delta_5}$ at level~$2$; the Igusa-form
...
\item Kummer surface $\mathrm{Kum}(T^4)$: topologically K3 with 16
  special $(-2)$-curves; the~$\Phi$-image is~$\mathbf{H}_{\Delta_5}$
  restricted to the Kummer sublattice~$\Lambda_{\mathrm{Kum}}\subset
  \mathrm{II}_{3,19}$; Igusa datum restricts
  to~$\Delta_5|_{\mathcal{A}_2^{\mathrm{Kum}}}$.
...
\end{itemize}
The conditional $W$-algebras of this chapter are the deformation
catalogues \emph{at the generic stalk} of the family; the
K3-specific crown~$\mathbf{H}_{\Delta_5}$ is the fibre at the
K3~base point, and sisters (Enriques/Kummer/bielliptic) are
different fibres at different base points, not specialisations of a
universal family. Cross-ref Vol.~III preface
Remark~\ref{rem:v3-preface-W15-DNA-3-specific-vs-general} and
Vol.~III introduction
Remark~\ref{rem:v3-intro-W15-DNA-3-scope}.
\end{remark}

\begin{remark}[Half-K3 $= \mathrm{dP}_9$ is not CY-2; 12 fibres, not 24]
\label{rem:wac-W15-DNA-2-halfK3}
...
The only~conditional $W$-algebra at half-K3 is the
...
\cite{MorrisonVafa1996II,Borcherds1998}. The conditional ``half-K3
$W$-algebra'' is therefore 1-loop-trivial, not a Lusztig-closed
specialisation.
\end{remark}

\begin{remark}[Heterotic decoupling limit isolates $\mathbf{H}_{\Delta_5}$ from Vol II]
\label{rem:wac-W15-DNA-3-decoupling}
From the Vol II perspective, the decoupling limit is the
statement that $\mathbf{H}_{\Delta_5}$ admits a presentation as a
chiral algebra living on a curve~$C$ with topological~factorisation
coproduct on an~$\R$-direction, with no residual gravitational
bulk coupling. On the heterotic frame, this is realised by taking
...
NahmWendland2001,TaorminaWendland2011}. The decoupled limit is the
stand-alone chiral bialgebra whose
...
closure. This is what makes $\mathbf{H}_{\Delta_5}$ a~fourth
Drinfeld taxon (Etingof--Kazhdan super Part~V super-quasi-Hopf;
see Vol.~III preface
Remark~\ref{rem:v3-preface-DNA-4-etingof-super-EK}) rather
than an anomaly contribution inside a 10d~heterotic or
11d~M-theory~action. The fourth taxon is characterised by its
admission of a decoupling limit, not by its 1-loop output
character: both sides (1-loop character, decoupled isolation)
read the same object through different lenses. Cross-ref
Vol.~III preface~Remark~\ref{rem:v3-preface-W15-DNA-1-decoupling}
and Vol.~III introduction~Remark~\ref{rem:v3-intro-W15-DNA-2-decoupling}.
\end{remark}

\section{the CY-2 Siegel-weight column}
\label{sec:wac-wave17-DNA}

the K3 synthesis flagged the four sister-CY-2 surfaces (Enriques, $T^4$,
Kummer, bielliptic, plus the non-CY half-K3) and noted, in a
single paragraph per sister, which Wave-$15$ topological
hypothesis fails. the K3 synthesis computes, for each sister, the explicit
Siegel-form weight or affine-Kac--Moody level of the resulting
$\Phi$-image, and verifies the CY-$2$ dichotomy across all
five surfaces.

\begin{theorem}[Enriques Siegel weight $= 5/2$;
$\mathbf{H}_{\Delta_5}^{\mathrm{Enr}}$ as a~$\mathbb{Z}/2$-orbifold]
\label{thm:wac-W17-DNA-enriques-weight}
\ClaimStatusProvedElsewhere
Let $\mathrm{En} = K3/\iota$ be an Enriques surface. The
$\Phi_2$-image
$\mathbf{H}_{\Delta_5}^{\mathrm{Enr}} = \mathbf{H}_{\Delta_5}\,/\!\!/\,\langle\iota\rangle$
has genus-$2$ Siegel character
\[
  \chi^{\mathrm{Enr}}_2(Z)
  \;=\;
  1/\Delta_5^{\mathrm{Enr}}(Z),
\]
with $\Delta_5^{\mathrm{Enr}}$ a Siegel paramodular form of
weight~$5/2$ on the paramodular subgroup $K(2)\subset\mathrm{Sp}_4(\mathbb{Q})$,
obtained as the Borcherds multiplicative lift on the Enriques
lattice $\mathrm{II}_{2,10}(2)$ (Gritsenko--Nikulin 1998
\textup{J.~reine angew.~Math.}~507, Theorem~5.2;
Hulek--Sankaran 2011 \textup{Algebra and Number Theory}~5,
Theorem~3.4). Equivalently, $\Delta_5^{\mathrm{Enr}}$ is the
Gritsenko--Nikulin additive lift of the Enriques elliptic
genus~$\phi^{\mathrm{Enr}}_{0,1}(\tau, z) = (1/2)\phi_{0,1}^{K3}(\tau, z)$
(halved because Enriques elliptic genus is one-half of K3 elliptic
genus; Borisov--Libgober 1998
\textup{Commun.~Math.~Phys.}~191, Theorem~4.2). The weight
degradation $5\mapsto 5/2$ is dictated by
...
the lattice requires a metaplectic input. The BKM denominator
$\Phi_{10}^{\mathrm{Enr}} = (\Delta_5^{\mathrm{Enr}})^2$ is a
level-$2$ paramodular Siegel form, matching the B-side
Kirillov--Ostrik orbifold gauging
(Vol.~III \texttt{derived\_categories\_cy.tex},
Theorem~\ref{thm:dcy-W17-DNA-enriques-analogue}).
\end{proof}

\begin{remark}[Kummer Siegel weight $= 5$ with Kummer sublattice
restriction]
\label{rem:wac-W17-DNA-kummer-weight}
\ClaimStatusProvedElsewhere
The Kummer K3 $\mathrm{Kum}(A)$ is topologically K3, so
$\chi_{\mathrm{top}} = 24$, $c_1 = 0$, $h^{2,0} = 1$: all three
the K3 synthesis hypotheses hold. The $\Phi_2$-image is
$\mathbf{H}_{\Delta_5}\big|_{\Lambda_{\mathrm{Kum}}}$, with
Siegel character
\[
  \chi^{\mathrm{Kum}}_2(Z)
  \;=\;
...
\label{rem:wac-W17-DNA-T4-weight}
\ClaimStatusProvedElsewhere
...
$= 0$]
\label{rem:wac-W17-DNA-halfK3-level}
\ClaimStatusProvedElsewhere
...
\label{rem:wac-W17-DNA-weight-column}
...
of Vol.~III \texttt{derived\_categories\_cy.tex},
Remark~\ref{rem:dcy-W17-DNA-landscape-partition}):
\begin{center}
\begin{tabular}{lccl}
\toprule
...
content of the Wave-$15$ claim that three topological invariants
$(\chi, c_1, h^{2,0}) = (24, 0, 1)$ select the K3 crown: the
Borcherds weight~$5$ arises exactly when
$(\chi/24, c_1, h^{2,0}) = (1, 0, 1)$, and any degradation of
one invariant collapses the weight to a lower value
(half-integer) or to~$0$. No conditional $W$-algebra of this
chapter supports any of the non-trivial automorphic products
beyond K3 and Enriques; the deformation catalogues specialise
to the K3-column entry at
$(\chi, c_1, h^{2,0}) = (24, 0, 1)$ and to nothing else.
\end{remark}

\begin{remark}[Specific-vs-general discipline on the CY-2
landscape]
\label{rem:wac-W17-DNA-specific-vs-general}
The Wave-$15$ specific-vs-general discipline holds columnwise:
the K3 row is the specific crown of the CY-$2$ column; the
Enriques row is the specific $\mathbb{Z}/2$-orbifold at
weight~$5/2$; the $T^4$/bielliptic rows are the general Narain
...
$\Phi_2$. The conditional $W$-algebras of this chapter
...
\section{Enriques BKM and the four-row $\Psi$-landscape}
\label{sec:wac-wave18-DNA}

With the Enriques Siegel weight fixed at $5/2$ as the
$\mathbb{Z}/2$-gauged companion of K3's weight-$5$ crown
(Theorem~\ref{thm:wac-W17-DNA-enriques-weight}), the
$\mathsf{SC}^{\mathrm{ch,top}}$-consequences follow: the
Enriques BKM superalgebra $\mathfrak{g}_{\Delta_5}^{\mathrm{Enr}}$
sits as a distinguished deformation face of the
fourth-taxon column, with explicit Cartan, imaginary-root
halving, and conjectural $2.M_{12}$-moonshine module structure.
Full construction in Vol.~III
\texttt{k3e\_bkm\_chapter.tex},
\S\ref{sec:bkm-enriques-W18}; the present section records the
$W$-algebra-conditional-landscape view.

\begin{theorem}[Enriques elliptic-genus halving drives the BKM
multiplicity halving]
\label{thm:wac-W18-DNA-enriques-halving}
\ClaimStatusProvedElsewhere
...
$\iota$-invariant spectrum. The positive-imaginary-root
multiplicities of the Enriques BKM
$\mathfrak{g}_{\Delta_5}^{\mathrm{Enr}}$ are
\[
  \mathrm{mult}(\alpha)
  \;=\;
  f^{\mathrm{En}}(N,\ell)
...
\]
This halving propagates through the
Weyl--Kac--Borcherds denominator identity to match the
weight-$5/2$ Borcherds lift $\Delta_5^{\mathrm{Enr}}$ of
Theorem~\ref{thm:wac-W17-DNA-enriques-weight} as the orbifold
denominator. Primary: Borisov--Libgober 2000 \emph{Duke Math.~J.}~104,
Thm~4.1; Gritsenko--Nikulin 1998 \emph{J.~reine angew.~Math.}~507,
Thm~5.2; Borcherds 1992 \emph{Invent.~Math.}~109, Thm~10.4;
Eguchi--Ooguri--Tachikawa 2010 \emph{Exp.~Math.}~19, Table~1.
\end{theorem}

\begin{remark}[First ten Fourier coefficients]
\label{rem:wac-W18-DNA-ten-fourier-coeffs}
\ClaimStatusProvedHere
...
\end{tabular}
\end{center}
Negative $c^{\mathrm{En}}$ at $(1,0)$ witnesses odd (fermionic)
simple roots: the Enriques BKM is a genuine \emph{super}algebra,
inheriting the super-grading from~K3. Sign-sum verification:
...
(Kirillov--Ostrik 2002 \emph{Adv.~Math.}~171, Thm~5.1; the
Enriques-involution lifts as an automorphism of both colour
slots), then
\[
...
\label{rem:wac-W18-DNA-enriques-topologisation}
...
Theorem~\ref{thm:k3-abelian-at-lie}). The $\mathbb{Z}/2$-gauged
sibling $\mathbf{H}_{\Delta_5}^{\mathrm{Enr}}$ inherits an
...
\label{rem:wac-W18-DNA-four-row-landscape}
\ClaimStatusProvedHere
\index{$\Psi$-landscape!Vol~II reading}
\index{four-row landscape}
Reading the $\Psi$-landscape of Vol.~III
\texttt{k3e\_bkm\_chapter.tex} along the Vol.~II
Siegel-weight and $\mathsf{SC}^{\mathrm{ch,top}}$-structure axes
gives:
\begin{center}
\begin{tabular}{lccl}
\toprule
...
$c = 24$. The two CY-$2$-input rows (K3, Enriques) are linked by
$\mathbb{Z}/2$-gauging; the two pure-automorphic rows (Monster,
Fake-Monster) are the FLM and Borcherds endpoints of the
four-taxon census. The universal three-faces identity
$\hbar^2\cdot K = -1$ gives
$(\hbar^2, K) \in \{(-1/2, 2),\, (-1/4, 4),\, (-1/8, 8),\, (-1/50, 50)\}$
along the four rows.
\end{remark}

\begin{remark}[Enriques-specific vs CY-$3$ Enriques conjecture]
\label{rem:wac-W18-DNA-enriques-cy2-vs-cy3}
\ClaimStatusProvedHere
...
moduli spaces. No conditional $W$-algebra of this chapter
supports either; the conditional tower has its domain of
validity inside the K3 row, and the Enriques/CY-$3$
orbifold extensions are separate
$W_{1+\infty}$-Miki-tower lifts. Cross-ref
Vol.~III \texttt{derived\_categories\_cy.tex},
Remark~\ref{rem:dcy-W18-DNA-enriques-cy2-vs-cy3}.
\end{remark}
