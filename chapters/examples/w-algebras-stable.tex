...
% RECTIFICATION-FLAG: This is the Laplace kernel r^L(z), NOT the collision
% residue r^{coll}(z). The collision residue (= the actual r-matrix in the
% bar complex, per AP19) is r^{coll}(z) = (c/2)/z^3 + 2T/z (two poles at
% odd orders). The Laplace kernel r^L(z) has poles at z^{-1}, z^{-2}, z^{-4}
% and agrees with the computation at line~\ref{comp:vir-CYBE}.
% The definition title was previously "Virasoro Spectral r-Matrix"; clarified.
...
\end{equation}
The collision residue (bar complex r-matrix, one pole order lower per AP19)
is
\begin{equation}
...
$F_g(c^*) = \tfrac{1}{2}\,F_g(0)$ for all~$g \ge 1$.
...
depth} $d_{\mathcal{W}}(\mathcal{W}_N)$: the smallest arity~$r$
...
datum at arity~$N$.  The perturbative depth says ``the tower is
infinite''; the $\mathcal{W}$-essential depth says ``after
arity~$N$, the recursion is fully seeded.''
\end{remark}
...
After $d\log$ absorption (AP19), the collision residue
...
\item \textbf{Gap migration.}
  At arity~$n$, the depth gap in the binding
...
Part~(v) is Theorem~\ref{thm:wn-class-M} combined with the
...
At arity~$2$, the binding sector $(W_N, W_N)$ has maximum depth
$2N - 1$ with a gap at $d = 2N - 2$.  Each additional input
at arity~$n$ contributes at most depth~$1$ (from the $T$-sector
...
\end{equation}
This is the largest arity-$2$ depth gap in rank~$2$: the
...
\end{center}
\noindent
$E_8$ has the deepest arity-$2$ depth gap in the entire classification
...
\item Gravitational coproduct primitivity at all arities
...
with rank $\ge 1$;
(3)~Theorem~\ref{thm:wn-class-M} (whose proof uses only the
