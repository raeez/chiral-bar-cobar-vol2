...
(Proposition~\ref{prop:su2-hypotheses}) gives the precise hypotheses under which
$SU(2)$ Chern--Simons lies in the scope of
Theorem~\ref{thm:physics-bridge}. The explicit
$\Ainf$ computation through degree~$4$ is unconditional at the
chain level; cohomological tower termination uses the chosen
one-loop Koszul affine comparison model with compatible
nullhomotopies. The Jacobi mechanism for $m_4^H = 0$ and the
conditional tower bound $r_{\max} = 3$ appear in
...
\begin{proposition}[$\Ainf$ tower termination at degree~$3$;
\ClaimStatusProvedHere]
...
\item $m_4 \neq 0$ at chain level: the square Feynman diagram
 ...
 $\FM_4(\C)$. Its transferred cohomology class is killed by the
...
\item On the chosen one-loop Koszul affine comparison model, the
 transferred higher operations vanish on cohomology. The PVA on
 $H^\bullet$ is $\widehat{\mathfrak{sl}}_2$ at level~$k$ with the
 standard $\lambda$-bracket. Shadow depth $r_{\max} = 3$
 (class~$\mathbf{L}$).
\end{enumerate}
\end{proposition}
...
Part~(iv): by induction on degree on the certified comparison model.
...
are killed by the nullhomotopy hypothesis. The compositions
...
$r(z) = k\,\Omega/z$, the standard rational
...
and $h^\vee = 2$). The curvature $\kappa(V_k(\mathfrak{sl}_2)) = \dim\mathfrak{g}\cdot(k+h^\vee)/(2h^\vee) = 3(k+2)/4$, matching Volume~I's computation.
Under $k \mapsto k' = -k - 2h^\vee = -k - 4$:
$\kappa(V_{k'}(\mathfrak{sl}_2)) = 3(k'+2)/4 = 3(-k-4+2)/4 = 3(-k-2)/4 = -3(k+2)/4 = -\kappa(V_k(\mathfrak{sl}_2))$,
confirming complementarity $\kappa(V_k(\mathfrak{sl}_2)) + \kappa(V_{k'}(\mathfrak{sl}_2)) = 0$.
\end{proof}
...
\hline
$SU(2)$ CS & $\frac{f^{ab}{}_c J^c}{z_{12}} + \frac{k\delta^{ab}}{z_{12}^2}$ & $\neq 0$ (explicit) & $m_4^H = 0$ on the certified comparison model; $m_{\geq 5} = 0$ by degree counting & $\widehat{\mathfrak{sl}}_2$ at level $k$ \\
\hline
\end{tabular}
\end{center}
