...
Classes~$\mathbf{G}$, $\mathbf{L}$, $\mathbf{C}$ are chirally
Koszul (bar cohomology concentrated, $m_k = 0$ for $k \ge 3$
on cohomology); class~$\mathbf{M}$ is not
...
\item \emph{Virasoro} (class~$\mathbf{M}$, depth~$\infty$):
  ...
  The self-intersection is not clean, and the Virasoro
  is \emph{not} chirally Koszul: the fourth-order pole
  causes purity failure, bar cohomology is
  non-concentrated, and $m_k \ne 0$ for all $k \ge 3$
  on cohomology.  The Virasoro does possess a
  well-defined Koszul dual
