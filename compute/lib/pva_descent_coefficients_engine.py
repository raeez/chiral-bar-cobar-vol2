...
H*(A,Q) to a (-1)-shifted Poisson vertex algebra, whose five structural
axioms are the descent coefficients
    ...
    D_6 : Higher vanishing      m_k vanishes in cohomology for k >= 3
                                (Conf_k^<(R) is contractible)

The descent functional D_k : (A, {m_k}, Q) -> truth value returns a
formal/numeric residue measuring axiom failure.  For every PVA on the
standard HT landscape (Heisenberg = abelian current algebra, affine
sl_2, Virasoro), each D_k residue is zero.  This engine encodes the
universal Borel-transformed formulas from the chapter:
...
compute_D6(spec, k)           Higher-vanishing predicate (k >= 3)
descent_coefficients(spec, triples)
                              Return DescentResult for all five axioms
...
class PVASpec:
    ...
    modes : dict
        ...
        keys are treated as identically zero (all regular).
    product : callable
...
@dataclass
class DescentResult:
    """Residues of all five descent coefficients on one test triple."""
    ...
    def is_pva(self) -> bool:
        """Return True iff every residue is the zero expression."""
        residues = [self.D2_left, self.D2_right, self.D3, self.D4, self.D5]
        return all(simplify(r) == 0 for r in residues) and self.D6
...
def compute_D3(
    ...
    For degree-zero generators (|a| = |b| = 0) the shifted sign is +1
    and the axiom reduces to {a_lam b} + {b_{-lam - d} a} = 0.
    ...
    shifted_sign = (-1) ** ((deg_a + 1) * (deg_b + 1))
    ...
    residue = expand(lhs + shifted_sign * substituted)
...
def compute_D6(spec: PVASpec, k: int) -> bool:
    """Return True iff m_k vanishes on cohomology for k >= 3.

    On the standard HT landscape the chapter proves this by
    contractibility of Conf_k^<(R): every m_k is Q-exact for k >= 3
    (Proposition prop:m3_vanish).  The engine encodes this as a
    predicate: True for k >= 3 on Heisenberg, affine_sl_2 and
    Virasoro (these are the three families proved in the chapter).

    The per-family flag is stored in `spec.modes` under the key
    ('__D6__', str(k)) as a boolean value; the default is True for
    k >= 3 (chapter theorem) and None for k < 3 (not applicable).
    """
    if k < 3:
        # D_6 is the k >= 3 vanishing statement; lower arities are
        # governed by D_2 (n=2) and are vacuously handled.
        return True
    # Consult override (if the family explicitly records a counter-example)
    key = ("__D6__", str(k))
...
def descent_coefficients(
    ...
    """Compute all five descent coefficients on a single test triple."""
    a, b, c = triple
