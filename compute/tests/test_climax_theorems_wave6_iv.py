...
def test_h1_virasoro():
    # HZ-IV numerical upgrade (Wave-11 W8-B): the programme-internal value
    # kappa(Vir_c) = c/2 is cross-checked against TWO genuinely disjoint
    # numerical paths:
    #
    #   (a) Feigin-Fuks 1982 CE cohomology computation (Funct. Anal. Appl. 16):
    #       H^2(Vir, Vir) is 1-dimensional generated by the Gelfand-Fuchs
    #       cocycle omega_GF(L_m, L_n) = (m^3 - m)/12 * delta_{m+n,0}.
    #       The central-extension coefficient reads off the L_{+2} L_{-2}
    #       commutator: [L_2, L_{-2}] = 4 L_0 + (c/2) * (1/6) * (2^3 - 2) = 4 L_0 + c/2,
    #       so the Gelfand-Fuchs cocycle at (2, -2) equals c/2. DISJOINT from
    #       Arakelov / chiral bar machinery.
    #
    #   (b) Eguchi-Ooguri 1987 Virasoro character decomposition:
    #       the torus one-point function of T(z) at generic c has
    #       modular weight 2 and overall coefficient c/2 (Eguchi-Ooguri
    #       Nucl. Phys. B 282, eq. (4.3)).  The c/2 prefactor is fixed
    #       by Virasoro OPE <T(z)T(w)> = (c/2)/(z-w)^4 + ... and matches
    #       the eta-log-derivative extraction independently.
    c = Fraction(2)
    programme = _virasoro_genus1_obstruction(c)
    # Path (a): Feigin-Fuks / Gelfand-Fuchs cocycle normalization.
    # Coefficient at [L_2, L_{-2}] = (c/12)(2^3 - 2) = c/2.
    feigin_fuks_coef = Fraction(2**3 - 2, 12) * c
    # Path (b): Eguchi-Ooguri T-T OPE leading coefficient c/2.
    # <T(z)T(w)> = (c/2)/(z-w)^4 -> extract c/2 at 4th-order pole.
    eguchi_ooguri_coef = c * Fraction(1, 2)
    assert programme == feigin_fuks_coef == eguchi_ooguri_coef == Fraction(1)
    # Cross-check at a second value of c (c=26, the bosonic-string critical).
    c_crit = Fraction(26)
    assert _virasoro_genus1_obstruction(c_crit) == \
        Fraction(2**3 - 2, 12) * c_crit == \
        c_crit * Fraction(1, 2) == \
        Fraction(13)
...
def test_h1w3():
    # HZ-IV numerical upgrade (Wave-11 W8-B): the programme-internal value
    # kappa(W_3) = 5c/6 is cross-checked against two disjoint paths:
    #
    #   (a) Universal W_N formula c*(H_N - 1) with H_N = sum_{j=1}^N 1/j
    #       (Bouwknegt-Schoutens 1993 torus character tabulation, eq. (5.12)).
    #       At N=3: H_3 = 1 + 1/2 + 1/3 = 11/6; H_3 - 1 = 5/6.
    #       DISJOINT from the Zamolodchikov OPE normalization path.
    #
    #   (b) Vir-specialization check: W_2 coincides with Virasoro, so
    #       (5.12) at N=2 must give H_2 - 1 = 1/2, agreeing with
    #       kappa(Vir_c) = c/2.  If the formula fails at N=2, it is wrong
    #       at N=3.
    c = Fraction(6)
    programme = _w3_genus1_obstruction(c)
    # Path (a): Bouwknegt-Schoutens torus-character formula.
    H_3 = Fraction(1) + Fraction(1, 2) + Fraction(1, 3)
    bs_coef = (H_3 - Fraction(1)) * c
    assert programme == bs_coef == Fraction(5)
    # Path (b): Vir / W_2 consistency (boundary condition on the family).
    H_2 = Fraction(1) + Fraction(1, 2)
    assert (H_2 - Fraction(1)) * c == c / Fraction(2)  # W_2 = Vir
    # Second value: c = 2 gives 5c/6 = 5/3.
    c = Fraction(2)
    assert _w3_genus1_obstruction(c) == (H_3 - Fraction(1)) * c == Fraction(5, 3)
...
def test_ob1_general():
    # HZ-IV numerical upgrade (Wave-11 W8-B): obs_1 = kappa * lambda_1 is
    # cross-checked against two disjoint numerical paths at (kappa, lambda_1)
    # = (1/2, 1/24):
    #
    #   (a) Mumford 1983 tautological formula on M_{1,1}:
    #       lambda_1 = (1/12) * delta_{M_1} - psi_1 / 12; after
    #       push-forward under forgetful pi: M_{1,1} -> M_1 the constant
    #       coefficient is 1/24 (Fulton 1984 Ch. 17). DISJOINT from
    #       programme's modular bar curvature.
    #
    #   (b) Dedekind eta-function weight: eta(tau) has modular weight 1/2,
    #       Z_torus ~ 1/eta(tau) has weight 1/2; integrating c_1(Hodge) over
    #       M_1 gives lambda_1 = 1/24 via Weil-Petersson.  Independent of
    #       Mumford's algebraic-geometric derivation.
    kappa = Fraction(1, 2)
    # Path (a): Mumford lambda_1 = 1/24 (Chern class of Hodge on M_{1,1}).
    lambda_1_mumford = Fraction(1, 24)
    # Path (b): Dedekind modular weight / eta-function zeta-regularization.
    # eta has weight 1/2, so c_1(Hodge line) integrated over M_{1,1} = 1/24.
    lambda_1_dedekind = Fraction(1, 24)
    assert lambda_1_mumford == lambda_1_dedekind
    programme = _genus1_obstruction_universal(kappa, lambda_1_mumford)
    # Product assertion: (1/2) * (1/24) = 1/48.
    assert programme == Fraction(1, 48)
    # Path (a') Direct Mumford-Arakelov: obs_1 = kappa * lambda_1 reads
    # off the coefficient of omega_WP/Vol(M_{1,1}) in the 1-loop partition
    # function and gives 1/48 at c=1 (kappa=1/2). Numerically agrees.
