...
- **MP24 AGENT-SWARM OPERATIONAL DISCIPLINE**. When running a multi-agent ATTACK+HEAL swarm (5+ parallel codex-rescue agents): (a) PARTITION sectors by file-disjoint regions declared UP-FRONT in each prompt — two agents touching the same file creates uncommitted-state merge conflicts invisible until commit time; (b) each prompt is SELF-CONTAINED (agents have no conversation memory), restates author identity ("Raeez Lorgat"), bans LLM attribution, and states the V2-AP40 mechanical grep gate explicitly; (c) each prompt requires HEAL-before-DOWNGRADE (MP10) and names the Chriss-Ginzburg + Russian-school + Costello-Gaiotto style; (d) each prompt requests a SHORT (≤400-word) report so returning a result does not explode main-thread context; (e) stalled-agent detection is via git log + git status observation — visible commits mean progressing; a large uncommitted diff with NO growth across two or more loop iterations is likely stalled; (f) when relaunching, prefer adding a FRESH file-disjoint sector over spawning a duplicate of a stalled one — the fresh sector widens coverage while avoiding uncommitted-work conflicts; (g) on /loop firings, the cron-based fixed-interval schedule (every 3m) and any outstanding ScheduleWakeup may double-fire — this is harmless; prefer cron for explicit fixed-interval requests; (h) codex-runtime rate limits are real (observed 2026-04-18: a 6th agent launched while 5 were active returned "API Error: rate limited" in 1064ms with zero usage); when the runtime signals rate-limiting, WAIT FOR COOLDOWN before retrying rather than spamming launch attempts; (i) commit hygiene is invariant: MP23 build gate + V2-AP40 typography gate + MP16 atomic propagation gate all pass before declaring a commit done. Prevents agent-swarm churn, uncommitted-state loss, double-edit conflicts, session-memory leaks through agent reports, rate-limit cascade failures, and commit-failure cascades.
- **MP25 UNCOMMITTED WORK IS LOAD-BEARING PROGRESS — DO NOT OVERWRITE**. Large uncommitted diffs in a working tree (e.g. theorem_A_infinity_2.tex +755 lines; AGENTS.md +1356 lines observed 2026-04-18) represent in-flight ATTACK+HEAL work from a background agent whose session has not yet committed. DO NOT: run `git checkout` / `git restore` / `git clean` over these files; edit them from the main thread; launch a new agent that targets the same file. DO: leave them alone; let the owning agent commit on its own cadence; if rescue is truly required (agent confirmed stalled and content is valuable), `git stash -u` first to checkpoint BEFORE any intervention. Conservatism on uncommitted state is required because the mid-edit buffer may contain partial refactors that only the in-flight agent can complete coherently; interrupting creates orphaned half-edits that future sessions cannot reconstruct. Prevents loss-of-work cascades in multi-agent ATTACK+HEAL swarms; operationalises the "be careful not to lose any work" directive.
...
- **FM126 / FM185 / FM186 / FM214 → HEAL (unified bridge, PARTIAL — Wave-6 Beilinson rectification 2026-04-17)**: close the DS-Hochschild compatibility bridge. For class M, the bulk = derived center at chain level follows from (a) Costello-Gaiotto holomorphic CS with DS boundary (already FM57), (b) explicit HKR-type identification HH^*(A_DS) ≃ O(derived M_vac), (c) DS-bar intertwining (Vol I thm:ds-koszul-intertwine, Bridges table). `thm:chd-ds-hochschild` (Vol II `chiral_higher_deligne.tex`) is the inscribed clause-(3) bridge. CAVEAT: Wave-6 Chiral Higher Deligne audit downgraded `thm:chiral-higher-deligne` clause (2) (the $(Sch^{ch,top})^!$-cobar chain-level equivalence with the weight-completed coderived chiral Positselski equivalence) from ProvedHere to `\ClaimStatusConditional` at `chiral_higher_deligne.tex:459`, conditional on Vol~I `thm:chiral-positselski-weight-completed` (chain-level $(Sch^{ch,top})^!$-cobar contracting homotopy cited at citation level, not inscribed). The DS-Hochschild bridge (clause (3)) and the heptagon/$E_3$-brace structure (clauses (1), (4)) remain ProvedHere; unconditional class-M bulk = derived center at chain level is correspondingly CONDITIONAL on resolution of clause (2) pending chain-level inscription of the cross-volume chiral Positselski equivalence.
...
2. ⚠ **DS-Hochschild bridge for class M** (FM126 + cluster) — **PARTIALLY CLOSED** by Vol II `chapters/theory/chiral_higher_deligne.tex` (`thm:chd-ds-hochschild`, `cor:universal-holography-class-M`). HPL transfer through DS strong deformation retract plus Arakawa C_2-cofiniteness plus HKR identification furnish chain-level ChirHoch^•(W_k(g)) ≃ H^•_DS(ChirHoch^•(V_k(g))). Wave-6 Beilinson audit 2026-04-17 downgraded umbrella theorem `thm:chiral-higher-deligne` clause (2) from ProvedHere to `\ClaimStatusConditional` (tag at `chiral_higher_deligne.tex:459`, attribution remark at :488-502); clause (2) is conditional on Vol~I `V1-thm:chiral-positselski-weight-completed` (`chapters/theory/theorem_B_scope_platonic.tex:245`) with chain-level $(Sch^{ch,top})^!$-cobar contracting homotopy cited at citation level, not inscribed. The DS-Hochschild bridge itself (clause (3), `thm:chd-ds-hochschild`) remains ProvedHere; class-M universal holography inherits clause-(2) conditionality.
...
4. ⚠ **E_∞ vs E_1 chiral Deligne-Tamarkin at chain level** (FM91, FM160) — **REDUCED TO associator-dependence, CLAUSE (2) CONDITIONAL**. Associator-dependent chain-level chiral Deligne-Tamarkin proved via `thm:chd-deligne-tamarkin` (chiral_higher_deligne.tex) for the heptagon/$E_3$-brace structure (clauses (1), (3), (4) of umbrella `thm:chiral-higher-deligne`, all ProvedHere); the $(Sch^{ch,top})^!$-cobar chain-level equivalence (clause (2)) downgraded to `\ClaimStatusConditional` per Wave-6 Beilinson audit 2026-04-17, conditional on V1-`thm:chiral-positselski-weight-completed` chain-level inscription. Associator-independent formulation remains as formal question; associator-dependent one is usable via explicit Drinfeld-associator fix on the unconditional clauses.
...
**Scope qualifier (AP271 + AP258; source-aligned 2026-04-18):** The iterated Sugawara identity T^{(n)} = [Q_tot, G^{(n)}] is established on Q_tot-COHOMOLOGY, not on the raw BV chain complex. The strict chain-level upgrade via explicit antighost contact corrections $\eta_1^{(i)}, \eta_1^{(ii)}$ is a FRONTIER item, NOT proved; earlier summary drafts proposing explicit $\eta_1$ forms have been RETRACTED per `rem:frontier-class-L-strict-chain-level` at `chapters/connections/e_infinity_topologization.tex:382-411`. The ladder theorem at depth $N \geq 4$ and its $W_\infty$ limit are CONDITIONAL on axiom (T5) antighost BRST-commutativity $[G^{(n)}, G^{(m)}] = Q_{tot}$-exact for $n, m \geq 4$, postulated in `def:higher-spin-stress-tower`, NOT derived; see `rem:frontier-antighost-BRST-commutativity-higher-spin` at `e_infinity_topologization.tex:370-380`. Depths $N \leq 3$ (Virasoro, W_3) remain unconditional on cohomology.
...
- ⚠ Thm H + Chiral Higher Deligne + DS-Hochschild bridge — written into hochschild.tex, brace.tex; umbrella `thm:chiral-higher-deligne` clauses (1)(3)(4) ProvedHere, clause (2) `\ClaimStatusConditional` on V1-`thm:chiral-positselski-weight-completed` (Wave-6 Beilinson audit 2026-04-17)
...
- ⚠ **Chiral Higher Deligne** (E_3 on Z^{der}_ch + DS-Hochschild bridge + half-braiding + three-Hochschild unification) — written into hochschild.tex, brace.tex; `thm:chiral-higher-deligne` clauses (1)(3)(4) ProvedHere, clause (2) downgraded to `\ClaimStatusConditional` (tag at `chiral_higher_deligne.tex:459`, attribution remark at :488-502) per Wave-6 Beilinson audit 2026-04-17 pending chain-level inscription of Vol~I `V1-thm:chiral-positselski-weight-completed`
...
- ⚠ **Theorem H chiral Higher Deligne** — `thm:theoremH-chiral-higher-deligne` at `working_notes.tex:~19800` (session-synthesis `subsec:theoremH-chiral-higher-deligne`); four-part statement (degree-$\le 2$ = classical chiral Deligne $E_2$ brace, degree-$\ge 3$ via heptagon edges 3$\leftrightarrow$4, concentration as $E_3$-rigidity consequence, $E_3$-topological via iterated Sugawara); class M chain-level via DS-Hoch bridge remark. Scope caveat: umbrella `thm:chiral-higher-deligne` clause (2) (the $(Sch^{ch,top})^!$-cobar chain-level leg) downgraded to `\ClaimStatusConditional` per Wave-6 Beilinson audit 2026-04-17 on V1-`thm:chiral-positselski-weight-completed`; the Theorem~H-level $E_3$-concentration / rigidity argument via clauses (1)(3)(4) is unaffected.
