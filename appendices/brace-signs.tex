...
The identity holds because: for each triple of insertion positions $(i, j, k)$ in $a$, the contribution to $a\{b\}\{c\} - a\{b\{c\}\}$ from positions where $b$ is inserted at $i$ and $c$ at $j > i$ (non-nested) matches the contribution to $a\{c\}\{b\} - a\{c\{b\}\}$ from positions where $c$ is at $j$ and $b$ at $i < j$, with the sign $(-1)^{(|b|-1)(|c|-1)}$. The nested terms ($c$ inserted inside $b$, or $b$ inside $c$) cancel on each side independently. This is a straightforward but sign-intensive verification; see~\cite{GetzlerJones1994} for the complete calculation.
\end{proof}
