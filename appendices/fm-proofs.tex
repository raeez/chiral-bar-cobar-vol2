...
\[
D_{\{1,2,3\}} \cong \FM_1(\C) \times \FM_3(\C) \cong \{{\rm pt}\} \times \FM_3(\C) \cong \FM_3(\C),
\]
...
\begin{proposition}[Complete $k=3$ verification]
...
\begin{proposition}[Complete $k=4$ verification]
...
\begin{theorem}[Arnold Cancellation at Codimension-2 Corners---Full Proof]
...
\begin{theorem}[Truncation for LG Cubic]
...
\[
\dim_{\R}\bigl(\FM_k(\C)/(\C \rtimes \R_{>0})\bigr) = 2(k-1) - 3 = 2k - 5,
\]
where we quotient by $2$ real dimensions (translations in $\C$) and $1$ real dimension (dilations in $\R_{>0}$).
